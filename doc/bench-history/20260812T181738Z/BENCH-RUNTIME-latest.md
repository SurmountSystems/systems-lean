...
Generated: **2026-08-12 18:16:21 UTC**
...
- Freestanding is **~20.9x faster** than Boehm GC (free **1.4191** ns/op vs Boehm **29.6849** ns/op; exit **ok**).
- Freestanding is **~12.3x faster** than Lean managed runtime (free **1.4191** ns/op vs lean **17.4417** ns/op; free N=**200000000**, lean N=**50000000**; exit **ok**). Labeled contrast only; not the same C IR.
- Freestanding is **~1.2x slower** than heap_churn (malloc/free) on this mint microbench (free **1.4191** ns/op vs heap **1.2063** ns/op). Honest: freestanding mint median still loses pure malloc/free microchurn under this run's flags.

### Compose class

- Freestanding compose is **~2.3x slower** than compose heap_churn (free-compose **72.5726** ns/op vs gcomp **31.5611** ns/op). Do **not** merge compose ns/op with mint ns/op.

### Headline

| Baseline | Freestanding vs baseline | median ns/op free | median ns/op baseline |
|----------|--------------------------|------------------:|----------------------:|
| Boehm GC | **~20.9x faster** | 1.4191 | 29.6849 |
| Lean managed runtime | **~12.3x faster** | 1.4191 | 17.4417 |
| heap_churn (malloc) | **~1.2x slower** | 1.4191 | 1.2063 |
| compose heap_churn | **~2.3x slower** | 72.5726 | 31.5611 |
...
- Full number tables below. Speed is empirical per job class, N, and machine. Raw ratios (baseline/free): mint heap **0.850**, Boehm **20.918**, lean **12.291**, compose heap **0.435**.

## Verdict (one sentence)

Freestanding is **~20.9x faster** than Boehm and **~12.3x faster** than Lean managed on mint-class this run, with **no GC and no Lean RT** on the freestanding link; vs heap_churn freestanding is **~1.2x slower** (mint) and **~2.3x slower** (compose). PROVABLY seals correctness only.
...
|-----|------|-------------:|-----:|----:|----:|------:|----------------:|-----------:|---------------:|
| R-free (linear freestanding) | ok | 1.4191 | 1.3629 | 1.2263 | 1.4433 | 0.1189 | 0.283819 | 737666067.78 | 3.436 |
| R-gc-shape (malloc/free heap_churn) | ok | 1.2063 | 1.2141 | 1.0452 | 1.3909 | 0.1730 | 0.241267 | 834907231.06 | 2.536 |
| R-boehm (GC_MALLOC/GC_FREE) | ok | 29.6849 | 29.4004 | 28.2791 | 30.2371 | 1.0095 | 5.936983 | 34040299.78 | 48.108 |

| Comparison (mint/consume only) | Value |
|--------------------------------|-------|
| Ratio heap_churn / freestanding ns/op | **0.850** |
| Ratio boehm_gc / freestanding ns/op | **20.918** |
...
| Arm | Exit | median ns/op | mean | min | max | stdev | median wall (s) | mean ops/s | outer real (s) |
|-----|------|-------------:|-----:|----:|----:|------:|----------------:|-----------:|---------------:|
| R-lean (Lean managed runtime) | ok | 17.4417 | 17.3953 | 16.2129 | 18.5312 | 1.1599 | 0.872084 | 57658720.71 | 8.367 |

| Comparison (mint-class labeled) | Value |
|----------------------------------|-------|
| Ratio lean_managed / freestanding ns/op | **12.291** |
...
| Arm | Exit | median ns/op | mean | min | max | stdev | median wall (s) | mean ops/s | outer real (s) |
|-----|------|-------------:|-----:|----:|----:|------:|----------------:|-----------:|---------------:|
| R-free-compose (host_compose freestanding) | ok | 72.5726 | 69.1781 | 56.1375 | 78.8243 | 11.7181 | 0.725726 | 14759716.06 | 7.498 |
| R-gc-shape-compose (malloc node list) | ok | 31.5611 | 30.4666 | 27.3582 | 32.4806 | 2.7310 | 0.315611 | 33008091.46 | 2.768 |

| Comparison (compose only) | Value |
|---------------------------|-------|
| Ratio heap_churn_compose / freestanding_compose ns/op | **0.435** |
...
| consume | 29.5789 |
| post_check | 33.4695 |
| group setup (init+push+edges) | 91.4920 |
| group mint_mark | 60.0526 |
| group check (well_typed+fail_closed+post) | 65.1997 |
| group extract_consume | 73.8605 |
...
| Full transcript | `/tmp/systems-lean-bench-runtime-20260812T181621Z.log` |
