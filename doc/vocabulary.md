...
| **Product-wire freestanding writer (M4 Name B)** | Measured freestanding dialect dual-eq WRITE + INSTALL via Path A Lean-generated host-cc writer (`emit/slake_product_wire_fs_write_tool.c` -> `src/systems/bin/slake-product-wire-fs-write-cc`), not Lake CapableRegenerate ELF. **Done** (`just product-wire-freestanding-write`; `productWireFsWriterFinishedClaimed` true; freestandingDriverComplete Mult-orthogonal; product StillUsesLake remains until M6) |
...
| **Name B full freestanding Mult write (M2)** | Measured Mult package re-emit uses Path A Lean-generated host-cc Mult package writer (`emit/slake_mult_fs_write_tool.c` -> `src/systems/bin/slake-mult-fs-write-cc`), not Lake Mult write ELF. **Done** (`just mult-subset-freestanding-write`; freestandingDriverComplete true; multFsDeepenDriverReady true; product StillUsesLake remains until M6) |
