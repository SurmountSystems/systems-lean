# Self-host host-surface theorem archive (closed)

**Role:** Closed host readiness / theorem dump honesty from the historical map.
**Living tip / acceptance SSoT:** `self-host.md` (thin living index).
**Kind:** product acceptance archive (not residual ledger).

Living pins on DualResidual / SpecProof / LlvmHold / SelfApplyFs win over
land-time theorem dump rows that still say free/complete/llvm false.

Greppable honesty tokens for gates stay on `self-host.md`.

---

## Honesty

Living tip on `self-host.md` wins for current claim flags (complete **true**,
product residual free **true**, PROVABLY **true**, llvm unlock **true**, host
residual remains). Land-time theorem dump rows below may still say free/complete
false at module land; do **not** re-decide living pins from this archive.

- **Land-time / historical:** theorem dump assumed product residual free false
  and freestanding product self-host complete false (SH5 partial is host
  ...
  rebuilding full Slake; land-time `SelfApplyFs` / `LlvmHold` rows encoded
  freestandingProductSelfHostComplete = false). **Living tip complete true.**
- **PROVABLY claimed** (`provablyUnlocked` = true; green product CompCert matrix
  + lake elaborator proof; 2026-08-02)
- SH6 **llvm unlocked with evidence** via `LlvmHold.lean`
  (`llvmHoldReady` true, `llvmUnlocked` **true** + lake blast-radius proof
  2026-08-03; Mult..Graph text ladder + unlock residual done; not full CFG/SSA
  / production backend / Rust-native link; PROVABLY true orthogonal)
- `out/llvm-ir` unit text path measured (unlock residual done; unit package expand Mult..Graph done; compose emit done; optional llvm-as smoke done; Rust-native layout design done; Layout size/align fixture done; ABI parity smoke done; Layout IR tag dual-pin done; Mult-first layout ladder closed; next residual **Superset surface inventory**)
