...
SELF-HOST-EMIT-ERASURE, SLAKE_SELF_HOST_EMIT_EXTRACT_V0, HOST-EMIT-EXTRACT,
SELF-HOST-EMIT-EXTRACT, SLAKE_SELF_HOST_EMIT_TYPES_V0, HOST-EMIT-TYPES,
SELF-HOST-EMIT-TYPES, SLAKE_SELF_HOST_EMIT_PROGRAM_V0, HOST-EMIT-PROGRAM,
SELF-HOST-EMIT-PROGRAM, SLAKE_SELF_HOST_EMIT_GRAPH_V0, HOST-EMIT-GRAPH,
SELF-HOST-EMIT-GRAPH, SLAKE_SELF_HOST_EMIT_COMPOSE_V0, HOST-EMIT-COMPOSE,
SELF-HOST-EMIT-COMPOSE, SLAKE_SELF_HOST_EMIT_PLAN_V0, HOST-EMIT-PLAN,
SELF-HOST-EMIT-PLAN, SLAKE_SELF_HOST_EMIT_APPLY_V0, HOST-EMIT-APPLY,
SELF-HOST-EMIT-APPLY, SLAKE_SELF_HOST_PARITY_MULT_V0, HOST-PARITY-MULT,
...
SLAKE_SELF_HOST_KERNEL_REBUILD_V0, HOST-SELF-HOST-KERNEL-REBUILD,
SELF-HOST-KERNEL-REBUILD, freestanding kernel rebuild evidence,
...
SELF-HOST-PRODUCT-OUT, product Out kernel evidence,
SLAKE_SELF_HOST_PRODUCT_PATH_BOOTSTRAP_V0, HOST-SELF-HOST-PRODUCT-PATH-BOOTSTRAP,
...
SLAKE_SELF_HOST_PRODUCT_WRITER_SURFACE_V0, HOST-SELF-HOST-PRODUCT-WRITER-SURFACE,
SELF-HOST-PRODUCT-WRITER-SURFACE, product writer surface,
productWriterSurfacePartialReady, productWriterSurfaceOk,
SLAKE_SELF_HOST_PRODUCT_WRITER_PATH_PLAN_V0, HOST-SELF-HOST-PRODUCT-WRITER-PATH-PLAN,
...
SLAKE_SELF_HOST_PRODUCT_WRITER_PATH_EXEC_V0, HOST-SELF-HOST-PRODUCT-WRITER-PATH-EXEC,
SELF-HOST-PRODUCT-WRITER-PATH-EXEC, product writer path execution,
productWriterPathExecutionPartialReady, productWriterPathExecutionOk,
loadMultSsot, renderHeader, emitAtRoot, IO.FS.writeFile, just build,
SLAKE_SELF_HOST_PRODUCT_PATH_OWNERSHIP_V0, HOST-SELF-HOST-PRODUCT-PATH-OWNERSHIP,
...
productPathFreestandingOwnershipClaimed, host_emit_mult.ssot.txt,
slake_freestanding.h, slake_freestanding.c,
SLAKE_SELF_HOST_PRODUCT_PATH_PERFORM_V0, HOST-SELF-HOST-PRODUCT-PATH-PERFORM,
...
productPathPerformInstallLakeFreeEntrypointClaimed, FREESTANDING-PERFORM-GAP-READ-SSOT,
...
SLAKE_SELF_HOST_PRODUCT_PATH_PERFORM_STEP_V0, HOST-SELF-HOST-PRODUCT-PATH-PERFORM-STEP,
SELF-HOST-PRODUCT-PATH-PERFORM-STEP, product path perform step,
productPathPerformStepPartialReady, productPathPerformStepOk,
install-freestanding-c-out, Lake-free install entrypoint,
SLAKE_SELF_HOST_PRODUCT_PATH_PERFORM_READ_V0, HOST-SELF-HOST-PRODUCT-PATH-PERFORM-READ,
...
read-product-ssot, slake-read-product-ssot, ProductPathReadSsot,
SLAKE_SELF_HOST_PRODUCT_PATH_PERFORM_COMPOSE_V0, HOST-SELF-HOST-PRODUCT-PATH-PERFORM-COMPOSE,
...
compose-product-plan, slake-compose-product-plan, ProductPathComposePlan,
composePlanApplyBodyAtRoot, WRITER-PATH-STEP-COMPOSE-PLAN-APPLY-BODY,
SLAKE_SELF_HOST_PRODUCT_PATH_PERFORM_WRITE_HC_V0, HOST-SELF-HOST-PRODUCT-PATH-PERFORM-WRITE-HC,
...
write-freestanding-hc, slake-write-freestanding-hc, ProductPathWriteHc,
writeFreestandingHcAtRoot, WRITER-PATH-STEP-WRITE-FREESTANDING-HC,
...
productPathFreestandingCapableFullPerform, regenerate-product-path,
...
productPathOwnershipRegeneratePartialReady, productPathOwnershipRegenerateOk,
...
ownership-regenerate-product-path-authority,
slake-ownership-regenerate, ProductPathOwnershipRegenerate,
...
`RESIDUAL-systems.md`. **Self-host body** is defined (SELF-HOST-BODY). Track 3
**B** freestanding kernel rebuild evidence is **partial**; product kernel
self-application is **partial B2**; product Out kernel evidence is **partial B3**
(Out-first B-bar APIs on `out/freestanding-c/`); product path Lake bootstrap
honesty is **partial B4** (host Lake still writes product wire); product writer
surface substrate is **partial B5** (plan/apply/body + B-bar on product Out);
product writer path **plan** substrate is **partial B6** (ordered freestanding-
capable regenerate steps named); product writer path **execution evidence** is
**partial B7** (host FreestandingEmit + just build perform plan
steps); freestanding product path **ownership inputs** is **partial B8**
(Mult..Out SSOT / emit / Out surfaces named; ownership claimed false); product
path **perform gap** is **partial B9** (Lake still performs read/compose/write;
install step mechanism cp capable; freestanding perform claimed false); first
**perform step** is **partial B10** (Lake-free install entrypoint
`just install-freestanding-c-out`; install entrypoint claimed true; perform
claimed false); **READ-SSOT substrate** is **partial B11** (decomposed dual SSOT
read via ProductPathReadSsot / `just read-product-ssot`; ReadDependsOnLake true;
perform claimed false); **COMPOSE substrate** is **partial B12** (decomposed
plan/apply/body compose via ProductPathComposePlan / `just compose-product-plan`;
ComposeDependsOnLake true; perform claimed false); **WRITE-HC substrate** is
**partial B13** (decomposed freestanding .h/.c write via ProductPathWriteHc /
`just write-freestanding-hc` calling FreestandingEmit.emitAtRoot;
WriteHcDependsOnLake true; perform claimed false); **freestanding-capable gap
measure** is **partial B14 + B26 CapableRead + B27 CapableCompose + B28 CapableWriteHc**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-GAP: install Lake-free true;
CapableRead true after B26 via `just freestanding-capable-read-lake-free`
token+HEADER/BODY no lake; CapableCompose true after B27 via
`just freestanding-capable-compose-lake-free` plan/apply/body token+HEADER/BODY
no lake; CapableWriteHc true after B28 via `just freestanding-capable-write-hc-lake-free`
dual SSOT token+HEADER/BODY + write substrate no lake; full perform false;
`just regenerate-product-path` ordered host pipeline join; complete flag stays false); **freestanding-capable
step contract** is **partial B15**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-STEP-CONTRACT: named freestanding-
capable READ/COMPOSE/WRITE-HC contracts + API surface + authority-not-
FreestandingEmit as product path; READ satisfied true after B16; Compose
satisfied true after B17; WriteHc satisfied true after B18; Full false;
complete stays false);
**freestanding-capable READ** is **partial B16 + B26 CapableRead**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-READ: freestandingCapableReadDualSsot
no FreestandingEmit product authority; HEADER/BODY structural dual SSOT;
ReadSatisfied true; B14 CapableRead true after B26 Lake-free
freestanding-capable-read-lake-free; B16 Lake exe remains diagnostic; complete
stays false);
**freestanding-capable COMPOSE** is **partial B17 + B27 CapableCompose**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-COMPOSE:
freestandingCapableComposePlanApplyBody no FreestandingEmit product authority;
plan/apply/body HEADER/BODY + ordered marker; ComposeSatisfied true; B14
CapableCompose true after B27 Lake-free freestanding-capable-compose-lake-free;
B17 Lake exe remains diagnostic; complete stays false);
**freestanding-capable WRITE-HC** is **partial B18 + B28 CapableWriteHc**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-WRITE-HC:
freestandingCapableWriteFreestandingHc no FreestandingEmit product authority;
dual SSOT HEADER/BODY + template embed + emit .h/.c write; WriteHcSatisfied true;
B14 CapableWriteHc true after B28 Lake-free freestanding-capable-write-hc-lake-free;
B18 Lake exe remains diagnostic; complete stays false);
**freestanding-capable ordered regenerate** is **partial B19**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-REGENERATE:
freestandingCapableOrderedRegenerate READ then COMPOSE then WRITE-HC then Install
Out no FreestandingEmit product authority; Full false; InstallOutOpen false after
B20; complete stays false);
**freestanding-capable Install Out** is **partial B20**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-INSTALL-OUT:
freestandingCapableInstallFreestandingOut no FreestandingEmit product authority;
emit .h/.c to out/freestanding-c/ via Lean IO; InstallOutOpen false; Full false;
complete stays false);
**freestanding-capable Full bar / ownership-gap measure** is **partial B21**
(SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-FULL-BAR: closed step contracts +
InstallOut + ordered regenerate; ownership regenerate WithoutLake true after B30; B14
CapableLakeFree true after B26+B27+B28 (CapableRead + CapableCompose + CapableWriteHc
true); Full false; complete stays false);
**product path ownership regenerate substrate** is **partial B22..B30**
(SELF-HOST-PRODUCT-PATH-OWNERSHIP-REGENERATE: install Lake-free true via
`just ownership-regenerate-install-only`; READ Lake-free true via
`just ownership-regenerate-read-only` (B23 presence+token; distinct from B26
CapableRead freestanding-capable-read-lake-free); COMPOSE Lake-free true via
`just ownership-regenerate-compose-only` (B24); WRITE-HC Lake-free true via
`just ownership-regenerate-write-hc-only` (B25; distinct from B28 CapableWriteHc);
B29 ordered Lake-free join claimed via `just ownership-regenerate-lake-free`
(ties freestanding Capable* lake-free + ownership path pieces + install; no lake);
**partial B30** product path authority + WithoutLake true via
`just ownership-regenerate-product-path-authority`
(`productPathOwnershipRegenerateProductPathAuthorityClaimed` true;
`productPathOwnershipRegenerateWithoutLake` true; FullBar dual-pin true);
ownership claimed false; complete stays false);
**freestanding perform evidence** is **partial B31**
(SELF-HOST-PRODUCT-PATH-PERFORM-EVIDENCE:
`productPathFreestandingPerformEvidenceClaimed` true via
`just freestanding-perform-evidence`; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open; complete stays false);
**freestanding perform official-path gap** is **partial B32**
(SELF-HOST-PRODUCT-PATH-PERFORM-OFFICIAL-PATH:
`productPathFreestandingPerformOfficialPathGapMeasured` true via
`just freestanding-perform-official-path`; official path still Lake FreestandingEmit
/ just build; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open; complete stays false);
**freestanding perform official-path alternate** is **partial B33**
(SELF-HOST-PRODUCT-PATH-PERFORM-OFFICIAL-PATH-ALTERNATE:
`productPathFreestandingPerformOfficialPathAlternateMeasured` true via
`just freestanding-perform-official-path-alternate`; freestanding-capable-regenerate
alternate present + not official + dual-equality blocks retirement; perform claimed
false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open;
complete stays false);
**freestanding perform dual-equality WRITE parity** is **partial B34**
(SELF-HOST-PRODUCT-PATH-PERFORM-DUAL-EQUALITY-WRITE-PARITY:
`productPathFreestandingPerformDualEqualityWriteParityMeasured` true via
`just freestanding-perform-dual-equality-write-parity`; land-time historical
module pins: Capable WRITE structural embed != FreestandingEmit DUAL-SSOT-EQUALITY
and gap open true; living tip after B37: SelfApplyFs SSoT gap open false + Capable
dual-eq live; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open; complete stays false);
**freestanding perform dual-equality WRITE close path** is **partial B35**
(SELF-HOST-PRODUCT-PATH-PERFORM-DUAL-EQUALITY-WRITE-CLOSE-PATH:
`productPathFreestandingPerformDualEqualityWriteClosePathMeasured` true via
`just freestanding-perform-dual-equality-write-close-path`; land-time historical:
ordered close-path steps named; not gap closed true; gap open true on product
module; living tip after B37: SelfApplyFs gap closed + CAPABLE-GAP closed; perform
claimed false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open;
complete stays false);
**freestanding perform dual-equality WRITE API** is **partial B36**
(SELF-HOST-PRODUCT-PATH-PERFORM-DUAL-EQUALITY-WRITE-API:
`productPathFreestandingPerformDualEqualityWriteApiMeasured` true via
`just freestanding-perform-dual-equality-write-api`; real freestanding dual-equality
WRITE API present (Emit* + requireDualSsotEqual + write; not FreestandingEmit
authority); not official; land-time API module may pin gap open; living tip after
B37: SelfApplyFs gap closed + Capable dual-eq live; perform claimed false;
DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open; complete stays
false);
**freestanding perform dual-equality WRITE CAPABLE-GAP** is **partial B37**
(SELF-HOST-PRODUCT-PATH-PERFORM-DUAL-EQUALITY-WRITE-CAPABLE-GAP:
`productPathDualEqualityWriteCapableGapClosed` true +
`productPathCapableWriteDualEqualityLive` true via
`just freestanding-perform-dual-equality-write-capable-gap`; Capable WRITE dual-eq
(Emit* + requireDualSsotEqual); SelfApplyFs gap open false; land-time not official
retire; living tip after B38: official path FreestandingEmit retired;
perform claimed false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE
open; complete stays false);
**freestanding perform RETIRE-OFFICIAL** is **partial B38**
(SELF-HOST-PRODUCT-PATH-PERFORM-RETIRE-OFFICIAL:
`productPathOfficialPathRetireOfficialMeasured` true via
`just freestanding-retire-official`; OfficialRetire short role module; official
just build uses freestanding-capable-regenerate dual-eq WRITE authority;
...
StillUsesLake true; land-time OfficialRetire perform claimed false; living tip
after B39: SelfApplyFs perform claimed true; DependsOnLake true; CAPABLE-GAP closed;
complete stays false);
**freestanding perform claimed** is **partial B39**
(SELF-HOST-PRODUCT-PATH-PERFORM-CLAIMED:
`productPathFreestandingPerformClaimedMeasured` true via
`just freestanding-perform-claimed`; PerformClaimed short role module;
`productPathFreestandingPerformClaimed` true with official dual-eq WRITE evidence;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE closed with evidence; DependsOnLake true;
StillUsesLake true; land-time PerformClaimed ownership false; living tip after B40:
SelfApplyFs ownership claimed true; complete stays false);
**freestanding ownership claimed** is **partial B40**
(SELF-HOST-PRODUCT-PATH-OWNERSHIP-CLAIMED:
`productPathFreestandingOwnershipClaimedMeasured` true via
`just freestanding-ownership-claimed`; OwnershipClaimed short role module;
`productPathFreestandingOwnershipClaimed` true with perform-claimed + official
dual-eq WRITE evidence; FULL-BAR-REQ-OWNERSHIP-CLAIMED-WITH-EVIDENCE closed with
evidence; DependsOnLake true; StillUsesLake true; Full false; complete stays false;
Full step-contract next);
**first B14 CapableRead** is **partial B26** (Lake-free freestanding-capable READ
via `just freestanding-capable-read-lake-free`; productPathFreestandingCapableRead
true; WithoutLake true after B30; Full still false);
**first B14 CapableCompose** is **partial B27** (Lake-free freestanding-capable COMPOSE
via `just freestanding-capable-compose-lake-free`; productPathFreestandingCapableCompose
true);
**first B14 CapableWriteHc** is **partial B28** (Lake-free freestanding-capable WRITE-HC
via `just freestanding-capable-write-hc-lake-free`; productPathFreestandingCapableWriteHc
true; FullBar CapableLakeFree true; WithoutLake true after B30);
**ordered Lake-free ownership regenerate join** is **partial B29**
(`productPathOwnershipRegenerateLakeFreeJoinClaimed` true via
`just ownership-regenerate-lake-free`; process-glue join; B30 product path authority + WithoutLake true).
Dual algorithms into Slake stated in `join-map.md` / `JoinMap.joinAlgUseOk`.
...
HOST-EMIT-LINEAR Linear/ConsumeToken product text; HOST-EMIT-ERASURE Erasure product text; HOST-EMIT-EXTRACT Extract/FAIL_CLOSED product text; HOST-EMIT-TYPES Types/TYPED_IR product text; HOST-EMIT-PROGRAM IR program product text; HOST-EMIT-GRAPH IR graph product text; HOST-EMIT-COMPOSE host compose product text; HOST-EMIT-PLAN emit plan product text; HOST-EMIT-APPLY emit apply product text; Linear / Types / Program /
...
(host SSOT Mult..Apply -> FreestandingEmit -> emit/ + out/freestanding-c under
gates); **SELF-HOST-KERNEL-REBUILD** partial measured Mult..Out regenerate;
**SELF-HOST-KERNEL-SELF-APPLY** partial B2 (named product-path kernel inputs +
automated emit/out match + product wire exercise); **SELF-HOST-PRODUCT-OUT**
partial B3 (Out-first B-bar ownership + APIs on product Out; pure Nix gate +
host pin); **SELF-HOST-PRODUCT-PATH-BOOTSTRAP** partial B4 (host Lake
FreestandingEmit still writes product wire while Out exercises B-bar);
**SELF-HOST-PRODUCT-WRITER-SURFACE** partial B5 (product Out writer
plan/apply/body APIs + ownership + B-bar as writer-path substrate);
**SELF-HOST-PRODUCT-WRITER-PATH-PLAN** partial B6 (ordered freestanding-capable
regenerate plan steps named on host; Lake still bootstrap writer);
**SELF-HOST-PRODUCT-WRITER-PATH-EXEC** partial B7 (host FreestandingEmit + just
just build greppably perform B6 plan steps; Lake bootstrap remains);
**SELF-HOST-PRODUCT-PATH-OWNERSHIP** partial B8 (ownership inputs bar:
Mult..Out dual SSOT + emit .h/.c + Out install surfaces; freestanding ownership
claimed false); **SELF-HOST-PRODUCT-PATH-PERFORM** partial B9 (perform gap:
Lake still read/compose/write; install step mechanism cp capable; freestanding
perform claimed false); **SELF-HOST-PRODUCT-PATH-PERFORM-STEP** partial B10
(Lake-free install entrypoint `just install-freestanding-c-out`; entrypoint
claimed true; perform claimed false; DependsOnLake true for read/compose/write);
**SELF-HOST-PRODUCT-PATH-PERFORM-READ** partial B11 (decomposed dual SSOT read
via ProductPathReadSsot / `just read-product-ssot`; ReadDependsOnLake true;
perform claimed false); **SELF-HOST-PRODUCT-PATH-PERFORM-COMPOSE** partial B12
(decomposed plan/apply/body compose via ProductPathComposePlan /
`just compose-product-plan`; ComposeDependsOnLake true; perform claimed false);
**SELF-HOST-PRODUCT-PATH-PERFORM-WRITE-HC** partial B13 (decomposed freestanding
.h/.c write via ProductPathWriteHc / `just write-freestanding-hc` calling
FreestandingEmit.emitAtRoot; WriteHcDependsOnLake true; perform claimed false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-GAP** partial B14 + B26 + B27 + B28
(freestanding-capable step measure: install Lake-free true; CapableRead true
after B26 freestanding-capable-read-lake-free; CapableCompose true after B27
freestanding-capable-compose-lake-free; CapableWriteHc true after B28
freestanding-capable-write-hc-lake-free; full perform false;
ordered pipeline `just regenerate-product-path`; perform claimed false; complete
still false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-STEP-CONTRACT** partial B15
(freestanding-capable step contract: authority-not-emit; READ satisfied true
after B16; Compose satisfied true after B17; WriteHc satisfied true after B18;
Full false; named API surface + blockers; perform claimed false; complete still
false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-READ** partial B16 + B26
(freestandingCapableReadDualSsot without FreestandingEmit as product authority;
HEADER/BODY structural dual SSOT; ReadSatisfied true; B14 CapableRead true after
B26 Lake-free freestanding-capable-read-lake-free; perform claimed false;
complete still false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-COMPOSE** partial B17 + B27
(freestandingCapableComposePlanApplyBody without FreestandingEmit as product
authority; plan/apply/body HEADER/BODY + ordered marker; ComposeSatisfied true;
B14 CapableCompose true after B27 freestanding-capable-compose-lake-free;
perform claimed false; complete still false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-WRITE-HC** partial B18 + B28
(freestandingCapableWriteFreestandingHc without FreestandingEmit as product
authority; dual SSOT HEADER/BODY + write emit .h/.c; after B37 dual-equality live
via `productPathCapableWriteDualEqualityLive` / Emit* + requireDualSsotEqual
before embed/write; WriteHcSatisfied true; B14 CapableWriteHc true after B28
freestanding-capable-write-hc-lake-free; perform claimed false;
complete still false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-REGENERATE** partial B19
(freestandingCapableOrderedRegenerate READ+COMPOSE+WRITE-HC+INSTALL-OUT; Full
false; InstallOutOpen false after B20; perform claimed false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-INSTALL-OUT** partial B20
(freestandingCapableInstallFreestandingOut; InstallOutOpen false; Full false);
**SELF-HOST-PRODUCT-PATH-FREESTANDING-CAPABLE-FULL-BAR** partial B21
(Full-bar / ownership-gap measure: closed pins true; ownership regenerate
WithoutLake true after B30 (measure of closed dual-pin); B14 CapableLakeFree true
after B26+B27+B28 (Read+Compose+WriteHc true); Full false; perform claimed false;
complete still false);
**SELF-HOST-PRODUCT-PATH-OWNERSHIP-REGENERATE** partial B22..B30
(ownership regenerate substrate: install Lake-free true; READ Lake-free true
(B23 presence+token; distinct from B26 CapableRead); COMPOSE Lake-free true
(B24 presence+token; distinct from B27 CapableCompose); WRITE-HC Lake-free true
(B25); B29 ordered Lake-free join claimed via ownership-regenerate-lake-free;
B30 product path authority + WithoutLake true via
ownership-regenerate-product-path-authority; DependsOnLake true; ownership claimed
false; perform claimed false; complete still false);
**partial B26** first B14 CapableRead true via freestanding-capable-read-lake-free
(token+HEADER/BODY; no lake; freestanding-capable depth).
**partial B27** first B14 CapableCompose true via freestanding-capable-compose-lake-free
(plan/apply/body token+HEADER/BODY; no lake; freestanding-capable depth).
**partial B28** first B14 CapableWriteHc true via freestanding-capable-write-hc-lake-free
(dual SSOT token+HEADER/BODY + write substrate; no lake; freestanding-capable depth);
FullBar CapableLakeFree true; WithoutLake true after B30.
**partial B29** ordered Lake-free ownership regenerate join claimed
(ownership-regenerate-lake-free; process-glue join).
**partial B30** freestanding product path authority + WithoutLake true
(ownership-regenerate-product-path-authority; FullBar dual-pin; Full false).
**partial B31** freestanding perform evidence claimed true
(freestanding-perform-evidence; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B32** freestanding perform official-path gap measured true
(freestanding-perform-official-path; official path still Lake FreestandingEmit /
just build; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B33** freestanding perform official-path alternate measured true
(freestanding-perform-official-path-alternate; freestanding-capable-regenerate
alternate present + not official + dual-equality blocks retirement; perform claimed
false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B34** freestanding perform dual-equality WRITE parity measured true
(freestanding-perform-dual-equality-write-parity; land-time historical: Capable
WRITE structural embed != FreestandingEmit DUAL-SSOT-EQUALITY and module gap open
true; living tip after B37: SelfApplyFs gap open false + Capable dual-eq; perform
claimed false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B35** freestanding perform dual-equality WRITE close path measured true
(freestanding-perform-dual-equality-write-close-path; land-time historical:
ordered close-path steps named; not gap closed / gap open on product module;
living tip after B37: SelfApplyFs gap closed + CAPABLE-GAP closed; perform claimed
false; DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B36** freestanding perform dual-equality WRITE API measured true
(freestanding-perform-dual-equality-write-api; freestandingDualEqualityWriteFreestandingHc
Emit* + requireDualSsotEqual + write; authority not emit; not official; land-time
API module may pin gap open; living tip after B37: SelfApplyFs gap closed + Capable
dual-eq live; perform claimed false; DependsOnLake true;
FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B37** freestanding perform dual-equality WRITE CAPABLE-GAP closed
(freestanding-perform-dual-equality-write-capable-gap; Capable WRITE dual-eq live;
SelfApplyFs gap open false; land-time not official retire; living tip after B38:
official path retired FreestandingEmit; perform claimed false;
DependsOnLake true; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE open).
**partial B38** freestanding perform RETIRE-OFFICIAL
(freestanding-retire-official; OfficialRetire short module; official
just build uses freestanding-capable-regenerate dual-eq WRITE;
productPathOfficialPathStillUsesFreestandingEmit false; DualEqualityBlocks false;
RetireRequired false; StillUsesLake true; perform claimed false;
DependsOnLake true; CAPABLE-GAP stays closed; FULL-BAR-REQ-PERFORM-CLAIMED-WITH-EVIDENCE
open; PERFORM-CLAIMED next).
...
`SystemsLean/EmitExtract.lean` (host-owned Extract + FAIL_CLOSED product C text),
`SystemsLean/EmitTypes.lean` (host-owned Types/TYPED_IR product C text),
`SystemsLean/EmitProgram.lean` (host-owned IR program product C text),
`SystemsLean/EmitGraph.lean` (host-owned IR graph product C text),
`SystemsLean/EmitCompose.lean` (host-owned host compose product C text),
`SystemsLean/EmitPlan.lean` (host-owned emit plan product C text + plan readiness),
`SystemsLean/EmitApply.lean` (host-owned emit apply product C text + tag buffer),
...
living tip free true: residualFreeClaimed true; productResidualRemains false;
hostElaboratorResidualRemains true; host free claimed false; dualResidualReady
free-true shape after product path close; does not forge host free),
...
emitLinearReady + emitErasureReady + emitExtractReady + emitTypesReady +
emitProgramReady + emitGraphReady + emitComposeReady + emitPlanReady +
emitApplyReady && emitBodyReady + freestanding emit stage cite; freestandingProductSelfHostComplete stays false;
residual free stays false),
...
## Bootstrap stages (S0-S4) -- host-assisted Slake

**Greppable:** FIRST-SURFACE, SLAKE_FIRST_SURFACE, SLAKE_FIRST_SURFACE_V0,
SLAKE_FIRST_SURFACE_MULT, HOST-FIRST-SURFACE, firstSurfaceReady,
firstSurfaceUnitReady, firstSurfaceEmitMultOk, slake-first-surface,
just first-surface, FirstSurface, MULT-SUBSET-EMIT, SLAKE_MULT_SUBSET_EMIT,
SLAKE_MULT_SUBSET_EMIT_V0, HOST-MULT-SUBSET-EMIT, multSubsetEmitReady,
multSubsetEmitWroteExpected, slake-mult-subset-emit, just mult-subset-emit,
MultSubsetEmit, MULT-SUBSET-REBUILD, SLAKE_MULT_SUBSET_REBUILD,
...
TYPES-SUBSET-EMIT, SLAKE_TYPES_SUBSET_EMIT, SLAKE_TYPES_SUBSET_EMIT_V0,
HOST-TYPES-SUBSET-EMIT, typesSubsetEmitReady, typesSubsetEmitWroteExpected,
slake-types-subset-emit, just types-subset-emit, TypesSubsetEmit,
slake_types_subset.h, slake_types_subset.c, SLAKE_TYPES_SUBSET_TYPES,
TYPES-SUBSET-REBUILD, SLAKE_TYPES_SUBSET_REBUILD, SLAKE_TYPES_SUBSET_REBUILD_V0,
HOST-TYPES-SUBSET-REBUILD, typesSubsetRebuildReady,
typesSubsetRebuildSelfApplyOk, typesSubsetRebuildWroteExpected,
slake-types-subset-rebuild, just types-subset-rebuild, TypesSubsetRebuild,
PROGRAM-SUBSET-EMIT, SLAKE_PROGRAM_SUBSET_EMIT, SLAKE_PROGRAM_SUBSET_EMIT_V0,
HOST-PROGRAM-SUBSET-EMIT, programSubsetEmitReady, programSubsetEmitWroteExpected,
slake-program-subset-emit, just program-subset-emit, ProgramSubsetEmit,
slake_program_subset.h, slake_program_subset.c, SLAKE_PROGRAM_SUBSET_PROGRAM,
PROGRAM-SUBSET-REBUILD, SLAKE_PROGRAM_SUBSET_REBUILD, SLAKE_PROGRAM_SUBSET_REBUILD_V0,
HOST-PROGRAM-SUBSET-REBUILD, programSubsetRebuildReady,
programSubsetRebuildSelfApplyOk, programSubsetRebuildWroteExpected,
slake-program-subset-rebuild, just program-subset-rebuild, ProgramSubsetRebuild,
EXTRACT-SUBSET-EMIT, SLAKE_EXTRACT_SUBSET_EMIT, SLAKE_EXTRACT_SUBSET_EMIT_V0,
HOST-EXTRACT-SUBSET-EMIT, extractSubsetEmitReady, extractSubsetEmitWroteExpected,
slake-extract-subset-emit, just extract-subset-emit, ExtractSubsetEmit,
slake_extract_subset.h, slake_extract_subset.c, SLAKE_EXTRACT_SUBSET_EXTRACT,
EXTRACT-SUBSET-REBUILD, SLAKE_EXTRACT_SUBSET_REBUILD, SLAKE_EXTRACT_SUBSET_REBUILD_V0,
HOST-EXTRACT-SUBSET-REBUILD, extractSubsetRebuildReady,
extractSubsetRebuildSelfApplyOk, extractSubsetRebuildWroteExpected,
slake-extract-subset-rebuild, just extract-subset-rebuild, ExtractSubsetRebuild,
ERASURE-SUBSET-EMIT, SLAKE_ERASURE_SUBSET_EMIT, SLAKE_ERASURE_SUBSET_EMIT_V0,
HOST-ERASURE-SUBSET-EMIT, erasureSubsetEmitReady, erasureSubsetEmitWroteExpected,
slake-erasure-subset-emit, just erasure-subset-emit, ErasureSubsetEmit,
slake_erasure_subset.h, slake_erasure_subset.c, SLAKE_ERASURE_SUBSET_ERASURE,
ERASURE-SUBSET-REBUILD, SLAKE_ERASURE_SUBSET_REBUILD, SLAKE_ERASURE_SUBSET_REBUILD_V0,
HOST-ERASURE-SUBSET-REBUILD, erasureSubsetRebuildReady,
erasureSubsetRebuildSelfApplyOk, erasureSubsetRebuildWroteExpected,
slake-erasure-subset-rebuild, just erasure-subset-rebuild, ErasureSubsetRebuild,
GRAPH-SUBSET-EMIT, SLAKE_GRAPH_SUBSET_EMIT, SLAKE_GRAPH_SUBSET_EMIT_V0,
HOST-GRAPH-SUBSET-EMIT, graphSubsetEmitReady, graphSubsetEmitWroteExpected,
slake-graph-subset-emit, just graph-subset-emit, GraphSubsetEmit,
slake_graph_subset.h, slake_graph_subset.c, SLAKE_GRAPH_SUBSET_GRAPH,
GRAPH-SUBSET-REBUILD, SLAKE_GRAPH_SUBSET_REBUILD, SLAKE_GRAPH_SUBSET_REBUILD_V0,
HOST-GRAPH-SUBSET-REBUILD, graphSubsetRebuildReady,
graphSubsetRebuildSelfApplyOk, graphSubsetRebuildWroteExpected,
slake-graph-subset-rebuild, just graph-subset-rebuild, GraphSubsetRebuild,
COMPOSE-SUBSET-EMIT, SLAKE_COMPOSE_SUBSET_EMIT, SLAKE_COMPOSE_SUBSET_EMIT_V0,
HOST-COMPOSE-SUBSET-EMIT, composeSubsetEmitReady, composeSubsetEmitWroteExpected,
slake-compose-subset-emit, just compose-subset-emit, ComposeSubsetEmit,
slake_compose_subset.h, slake_compose_subset.c, SLAKE_COMPOSE_SUBSET_COMPOSE,
COMPOSE-SUBSET-REBUILD, SLAKE_COMPOSE_SUBSET_REBUILD, SLAKE_COMPOSE_SUBSET_REBUILD_V0,
HOST-COMPOSE-SUBSET-REBUILD, composeSubsetRebuildReady,
composeSubsetRebuildSelfApplyOk, composeSubsetRebuildWroteExpected,
slake-compose-subset-rebuild, just compose-subset-rebuild, ComposeSubsetRebuild,
mult without-Lake driver design, mult-without-lake-driver-design-2026-08-01,
...
HOST-EMIT-LINEAR, COMPILE-PATH-TYPES, typesFixtureCompilePathReady,
HOST-EMIT-TYPES, COMPILE-PATH-PROGRAM, programFixtureCompilePathReady,
HOST-EMIT-PROGRAM, COMPILE-PATH-EXTRACT, extractRuntimeFsGateReady,
HOST-EMIT-EXTRACT, COMPILE-PATH-ERASURE, erasureFixtureCompilePathReady,
HOST-EMIT-ERASURE, COMPILE-PATH-GRAPH, graphFixtureCompilePathReady,
HOST-EMIT-GRAPH, COMPILE-PATH-COMPOSE, composeFixtureCompilePathReady,
HOST-EMIT-COMPOSE
...
finish), not a blocker. Do not forge StillUsesLake / DependsOnLake false before
S4 elaborator proof.

| Stage | Name | Status | Honest meaning |
|-------|------|--------|----------------|
| **S0** | Host foundation | **done** | Lake elaborates Systems Lean; product wire residual free; claim B writer-path complete true |
...
| **S3** | Rebuild own freestanding subset | **done** | Measured Mult subset self-application (`MultSubsetRebuild` / `just mult-subset-rebuild`); re-emit/re-validate S2 package; Lake still allowed on S3 path |
...
| **M1** | Types subset emit (unit surface growth) | **done** | Host-built Types unit package write (`slake_types_subset.{h,c}`) from COMPILE-PATH-TYPES + HOST-EMIT-TYPES; `TypesSubsetEmit` / `just types-subset-emit`; Lake remains |
| **M1** | Types subset rebuild (self-application) | **done** | Measured Types subset self-application (`TypesSubsetRebuild` / `just types-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M1** | Program subset emit (unit surface growth) | **done** | Host-built Program unit package write (`slake_program_subset.{h,c}`) from COMPILE-PATH-PROGRAM + HOST-EMIT-PROGRAM; `ProgramSubsetEmit` / `just program-subset-emit`; Lake remains |
| **M1** | Program subset rebuild (self-application) | **done** | Measured Program subset self-application (`ProgramSubsetRebuild` / `just program-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M1** | Extract subset emit (unit surface growth) | **done** | Host-built Extract unit package write (`slake_extract_subset.{h,c}`) from COMPILE-PATH-EXTRACT + HOST-EMIT-EXTRACT; `ExtractSubsetEmit` / `just extract-subset-emit`; Lake remains |
| **M1** | Extract subset rebuild (self-application) | **done** | Measured Extract subset self-application (`ExtractSubsetRebuild` / `just extract-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M1** | Erasure subset emit (unit surface growth) | **done** | Host-built Erasure unit package write (`slake_erasure_subset.{h,c}`) from COMPILE-PATH-ERASURE + HOST-EMIT-ERASURE; `ErasureSubsetEmit` / `just erasure-subset-emit`; Lake remains |
| **M1** | Erasure subset rebuild (self-application) | **done** | Measured Erasure subset self-application (`ErasureSubsetRebuild` / `just erasure-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M1** | Graph subset emit (unit surface growth) | **done** | Host-built Graph unit package write (`slake_graph_subset.{h,c}`) from COMPILE-PATH-GRAPH + HOST-EMIT-GRAPH; `GraphSubsetEmit` / `just graph-subset-emit`; Lake remains |
| **M1** | Graph subset rebuild (self-application) | **done** | Measured Graph subset self-application (`GraphSubsetRebuild` / `just graph-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M1** | Compose subset emit (unit surface growth) | **done** | Host-built Compose unit package write (`slake_compose_subset.{h,c}`) from COMPILE-PATH-COMPOSE + HOST-EMIT-COMPOSE; `ComposeSubsetEmit` / `just compose-subset-emit`; Lake remains |
| **M1** | Compose subset rebuild (self-application) | **done** | Measured Compose subset self-application (`ComposeSubsetRebuild` / `just compose-subset-rebuild`); re-emit/re-validate M1 package; Lake still allowed; without-Lake finished later |
| **M2** | Mult without-Lake driver design | **done** (design) | Research note `doc/dev/research/mult-without-lake-driver-design-2026-08-01.md` |
| **M2** | Mult without-Lake prebuilt driver | **done** (partial) | Name A: `just mult-subset-rebuild-without-lake` runs prebuilt `.lake/build/bin/slake-mult-subset-rebuild` (no lake on hot path); `multSubsetRebuildWithoutLakeFinishedClaimed` true; product StillUsesLake/DependsOnLake true |
...
| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/FirstSurface.lean` (`SystemsLean.FirstSurface`) |
| Lake exe | `slake-first-surface` (`FirstSurfaceMain.lean`) |
| just recipe | `just first-surface` |
| Input | `SLAKE_FIRST_SURFACE_MULT` -- same Mult fixture as COMPILE-PATH-MULT / MULT-FIXTURE |
| Output | `firstSurfaceReady` (reuses `multFixtureCompilePathReady`: program + unitCompose + HOST-EMIT-MULT) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (S1):** not full freestanding product-wire regenerate as success;
not S3 self-application; not S4 Lake retire; free/complete living tip unchanged
by this surface; not PROVABLY; not llvm unlock. Inventory design note:
`doc/dev/research/slake-first-compiler-surface-inventory-2026-08-01.md`.

### S2 Mult subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/MultSubsetEmit.lean` (`SystemsLean.MultSubsetEmit`) |
| Lake exe | `slake-mult-subset-emit` (`MultSubsetEmitMain.lean`) |
| just recipe | `just mult-subset-emit` |
| Input | `SLAKE_FIRST_SURFACE_MULT` (S1 FirstSurface Mult unit identity) |
| Output | Mult unit package `emit/slake_mult_subset.h` + `emit/slake_mult_subset.c` (HOST-EMIT-MULT dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `multSubsetEmitReady` + `multSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (S2):** not full freestanding API dialect regenerate as sole
success; not S3 self-application; not S4 Lake retire; free/complete living tip
unchanged; not PROVABLY; not llvm unlock. Subset package is generator output
from Lean EmitMult fragments (no hand-authored Mult product C features).

### S3 Mult subset rebuild / self-application (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/MultSubsetRebuild.lean` (`SystemsLean.MultSubsetRebuild`) |
| Lake exe | `slake-mult-subset-rebuild` (`MultSubsetRebuildMain.lean`) |
| just recipe | `just mult-subset-rebuild` (Lake path); `just mult-subset-rebuild-without-lake` (M2 measured step) |
| Input | S2 Mult subset package identity (`SLAKE_MULT_SUBSET_EMIT_V0` / `multSubsetEmitReady` / `multSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Mult unit package `emit/slake_mult_subset.{h,c}` from S2 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `multSubsetRebuildReady` + `multSubsetRebuildSelfApplyOk` + `multSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; M2 `multSubsetRebuildWithoutLakeFinishedClaimed` true (measured step only) |

**Non-claims (S3):** not S4 Lake retire; free/complete living tip unchanged; not
PROVABLY; not llvm unlock; not full freestanding API dialect regenerate as sole
success. S3 Lake path may still invoke Lake; M2 without-Lake uses prebuilt ELF.
...
### M1 Types subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/TypesSubsetEmit.lean` (`SystemsLean.TypesSubsetEmit`) |
| Lake exe | `slake-types-subset-emit` (`TypesSubsetEmitMain.lean`) |
| just recipe | `just types-subset-emit` |
| Input | `SLAKE_TYPES_SUBSET_TYPES` -- COMPILE-PATH-TYPES / TYPES-FIXTURE (`typesFixtureCompilePathReady`) |
| Output | Types unit package `emit/slake_types_subset.h` + `emit/slake_types_subset.c` (HOST-EMIT-TYPES dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `typesSubsetEmitReady` + `typesSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Types emit):** not full freestanding API dialect regenerate as
sole success; not Types subset rebuild (follow-on); not S4 Lake retire;
free/complete living tip unchanged; not PROVABLY; not llvm unlock; not full
elaborator type checker in C. Subset package is generator output from Lean
EmitTypes fragments (no hand-authored Types product C features). Lake remains
until S4.

### M1 Types subset rebuild / self-application (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/TypesSubsetRebuild.lean` (`SystemsLean.TypesSubsetRebuild`) |
| Lake exe | `slake-types-subset-rebuild` (`TypesSubsetRebuildMain.lean`) |
| just recipe | `just types-subset-rebuild` |
| Input | M1 Types subset package identity (`SLAKE_TYPES_SUBSET_EMIT_V0` / `typesSubsetEmitReady` / `typesSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Types unit package `emit/slake_types_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `typesSubsetRebuildReady` + `typesSubsetRebuildSelfApplyOk` + `typesSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `typesSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Types rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success; not full
type checker in C. Rebuild may still use Lake as host elaborator.

### M1 Program subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ProgramSubsetEmit.lean` (`SystemsLean.ProgramSubsetEmit`) |
| Lake exe | `slake-program-subset-emit` (`ProgramSubsetEmitMain.lean`) |
| just recipe | `just program-subset-emit` |
| Input | `SLAKE_PROGRAM_SUBSET_PROGRAM` -- COMPILE-PATH-PROGRAM / PROGRAM-FIXTURE (`programFixtureCompilePathReady`) |
| Output | Program unit package `emit/slake_program_subset.h` + `emit/slake_program_subset.c` (HOST-EMIT-PROGRAM dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `programSubsetEmitReady` + `programSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Program emit):** not full freestanding API dialect regenerate as
sole success; not Program subset rebuild (follow-on); not S4 Lake retire;
free/complete living tip unchanged; not PROVABLY; not llvm unlock; not CFG/SSA
completeness. Subset package is generator output from Lean EmitProgram fragments
(no hand-authored Program product C features). Lake remains until S4.

### M1 Program subset rebuild / self-application (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ProgramSubsetRebuild.lean` (`SystemsLean.ProgramSubsetRebuild`) |
| Lake exe | `slake-program-subset-rebuild` (`ProgramSubsetRebuildMain.lean`) |
| just recipe | `just program-subset-rebuild` |
| Input | M1 Program subset package identity (`SLAKE_PROGRAM_SUBSET_EMIT_V0` / `programSubsetEmitReady` / `programSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Program unit package `emit/slake_program_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `programSubsetRebuildReady` + `programSubsetRebuildSelfApplyOk` + `programSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `programSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Program rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success; not CFG/SSA.
Rebuild may still use Lake as host elaborator.

### M1 Extract subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ExtractSubsetEmit.lean` (`SystemsLean.ExtractSubsetEmit`) |
| Lake exe | `slake-extract-subset-emit` (`ExtractSubsetEmitMain.lean`) |
| just recipe | `just extract-subset-emit` |
| Input | `SLAKE_EXTRACT_SUBSET_EXTRACT` -- COMPILE-PATH-EXTRACT / EXTRACT-RUNTIME-FS (`extractRuntimeFsGateReady`) |
| Output | Extract unit package `emit/slake_extract_subset.h` + `emit/slake_extract_subset.c` (HOST-EMIT-EXTRACT dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `extractSubsetEmitReady` + `extractSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Extract emit):** not full freestanding API dialect regenerate as
sole success; not S4 Lake retire; free/complete living tip unchanged; not
PROVABLY; not llvm unlock. Subset package is generator output from Lean
EmitExtract fragments (no hand-authored Extract product C features). Lake remains
until S4. Stage tokens FAIL_CLOSED_CHECKER_V1 / RUNTIME-FS stay on the package.

### M1 Extract subset rebuild (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ExtractSubsetRebuild.lean` (`SystemsLean.ExtractSubsetRebuild`) |
| Lake exe | `slake-extract-subset-rebuild` (`ExtractSubsetRebuildMain.lean`) |
| just recipe | `just extract-subset-rebuild` |
| Input | M1 Extract subset package identity (`SLAKE_EXTRACT_SUBSET_EMIT_V0` / `extractSubsetEmitReady` / `extractSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Extract unit package `emit/slake_extract_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `extractSubsetRebuildReady` + `extractSubsetRebuildSelfApplyOk` + `extractSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `extractSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Extract rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success. Rebuild may
still use Lake as host elaborator.

### M1 Erasure subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ErasureSubsetEmit.lean` (`SystemsLean.ErasureSubsetEmit`) |
| Lake exe | `slake-erasure-subset-emit` (`ErasureSubsetEmitMain.lean`) |
| just recipe | `just erasure-subset-emit` |
| Input | `SLAKE_ERASURE_SUBSET_ERASURE` -- COMPILE-PATH-ERASURE / ERASURE-FIXTURE (`erasureFixtureCompilePathReady`) |
| Output | Erasure unit package `emit/slake_erasure_subset.h` + `emit/slake_erasure_subset.c` (HOST-EMIT-ERASURE dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `erasureSubsetEmitReady` + `erasureSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Erasure emit):** not full freestanding API dialect regenerate as
sole success; not S4 Lake retire; free/complete living tip unchanged; not
PROVABLY; not llvm unlock. Subset package is generator output from Lean
EmitErasure fragments (no hand-authored Erasure product C features). Lake remains
until S4. Mult-0 absence honesty (ERASE-RULE-MULT-0 / ERASE-NO-RUNTIME) stays
on the package.

### M1 Erasure subset rebuild (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ErasureSubsetRebuild.lean` (`SystemsLean.ErasureSubsetRebuild`) |
| Lake exe | `slake-erasure-subset-rebuild` (`ErasureSubsetRebuildMain.lean`) |
| just recipe | `just erasure-subset-rebuild` |
| Input | M1 Erasure subset package identity (`SLAKE_ERASURE_SUBSET_EMIT_V0` / `erasureSubsetEmitReady` / `erasureSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Erasure unit package `emit/slake_erasure_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `erasureSubsetRebuildReady` + `erasureSubsetRebuildSelfApplyOk` + `erasureSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `erasureSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Erasure rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success. Rebuild may
still use Lake as host elaborator.

### M1 Graph subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/GraphSubsetEmit.lean` (`SystemsLean.GraphSubsetEmit`) |
| Lake exe | `slake-graph-subset-emit` (`GraphSubsetEmitMain.lean`) |
| just recipe | `just graph-subset-emit` |
| Input | `SLAKE_GRAPH_SUBSET_GRAPH` -- COMPILE-PATH-GRAPH / GRAPH-FIXTURE (`graphFixtureCompilePathReady`) |
| Output | Graph unit package `emit/slake_graph_subset.h` + `emit/slake_graph_subset.c` (HOST-EMIT-GRAPH dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `graphSubsetEmitReady` + `graphSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Graph emit):** not full freestanding API dialect regenerate as
sole success; not S4 Lake retire; free/complete living tip unchanged; not
PROVABLY; not llvm unlock; not full CFG/SSA. Subset package is generator output
from Lean EmitGraph fragments (no hand-authored Graph product C features). Lake
remains until S4.

### M1 Graph subset rebuild (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/GraphSubsetRebuild.lean` (`SystemsLean.GraphSubsetRebuild`) |
| Lake exe | `slake-graph-subset-rebuild` (`GraphSubsetRebuildMain.lean`) |
| just recipe | `just graph-subset-rebuild` |
| Input | M1 Graph subset package identity (`SLAKE_GRAPH_SUBSET_EMIT_V0` / `graphSubsetEmitReady` / `graphSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Graph unit package `emit/slake_graph_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `graphSubsetRebuildReady` + `graphSubsetRebuildSelfApplyOk` + `graphSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `graphSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Graph rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success; not full
CFG/SSA. Rebuild may still use Lake as host elaborator.

### M1 Compose subset emit (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ComposeSubsetEmit.lean` (`SystemsLean.ComposeSubsetEmit`) |
| Lake exe | `slake-compose-subset-emit` (`ComposeSubsetEmitMain.lean`) |
| just recipe | `just compose-subset-emit` |
| Input | `SLAKE_COMPOSE_SUBSET_COMPOSE` -- COMPILE-PATH-COMPOSE / COMPOSE-FIXTURE (`composeFixtureCompilePathReady`) |
| Output | Compose unit package `emit/slake_compose_subset.h` + `emit/slake_compose_subset.c` (HOST-EMIT-COMPOSE dialect; not full `slake_freestanding.{c,h}` rewrite as sole success) |
| Structural ready | `composeSubsetEmitReady` + `composeSubsetEmitWroteExpected` (package identity pin) |
| Lake host | `stillUsesLake` / `dependsOnLake` true |

**Non-claims (M1 Compose emit):** not full freestanding API dialect regenerate as
sole success; not S4 Lake retire; free/complete living tip unchanged; not
PROVABLY; not llvm unlock; not full CFG/SSA; not elaborator MULT-1 freestanding
residual free. Subset package is generator output from Lean EmitCompose fragments
(no hand-authored Compose product C features). Lake remains until S4.

### M1 Compose subset rebuild (done)

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/ComposeSubsetRebuild.lean` (`SystemsLean.ComposeSubsetRebuild`) |
| Lake exe | `slake-compose-subset-rebuild` (`ComposeSubsetRebuildMain.lean`) |
| just recipe | `just compose-subset-rebuild` |
| Input | M1 Compose subset package identity (`SLAKE_COMPOSE_SUBSET_EMIT_V0` / `composeSubsetEmitReady` / `composeSubsetEmitWroteExpected`) |
| Output | Measured self-application: re-emit/re-validate Compose unit package `emit/slake_compose_subset.{h,c}` from M1 inputs (not full freestanding dialect regenerate as sole success) |
| Structural ready | `composeSubsetRebuildReady` + `composeSubsetRebuildSelfApplyOk` + `composeSubsetRebuildWroteExpected` |
| Lake host | `stillUsesLake` / `dependsOnLake` true; `composeSubsetRebuildWithoutLakeFinishedClaimed` false |

**Non-claims (M1 Compose rebuild):** not without-Lake finished (later deepen / S4);
not S4 Lake retire; free/complete living tip unchanged; not PROVABLY; not llvm
unlock; not full freestanding API dialect regenerate as sole success; not full
CFG/SSA; not elaborator MULT-1 freestanding residual free. Rebuild may still use
Lake as host elaborator. M1 unit surfaces for Linear/Types/Program/Extract/Erasure/
Graph/Compose emit+rebuild closed; next product residual is M2 Name B full design
deepen (do not forge freestandingDriverComplete).

### M2 Mult without-Lake driver design (done -- design only)

| Piece | Path / token |
|-------|----------------|
| Research note | `doc/dev/research/mult-without-lake-driver-design-2026-08-01.md` |
| Meaning | Without Lake for the **measured Mult re-emit step** (not product StillUsesLake false) |
| Honesty | StillUsesLake / DependsOnLake stay true until M6 |

**Non-claims (M2 design):** not freestanding Mult driver deepen (Name B); not S4;
not re-open freestandingProductSelfHostComplete; not PROVABLY/llvm.

### M2 Mult without-Lake prebuilt driver (done -- Name A partial)

| Piece | Path / token |
|-------|----------------|
| Measured recipe | `just mult-subset-rebuild-without-lake` |
| Prebuilt binary | `src/systems/.lake/build/bin/slake-mult-subset-rebuild` (bootstrap once: `lake build slake-mult-subset-rebuild`) |
| Host pins | `multSubsetRebuildWithoutLakeFinishedClaimed` true; `justRecipeWithoutLake`; `prebuiltMultRebuildRel`; `multSubsetRebuildWithoutLakeKeepsHostLake` |
| Dual evidence | Lean structural ready + package greps on `emit/slake_mult_subset.{h,c}` after prebuilt run |
| Product Lake | `stillUsesLake` / `dependsOnLake` **true** (host elaborator residual remains until M6) |

**Non-claims (M2 Name A):** not freestanding Mult driver complete; not S4 / M6
product StillUsesLake false; not PROVABLY; not llvm unlock; not re-open
freestandingProductSelfHostComplete; prebuilt Lake ELF is **not** freestanding
self-host. Name B freestanding Mult surface dual-check partial is separate
(`MultFsDeepen`).

### M2 Mult freestanding surface dual-check + Name B full Mult write

| Piece | Path / token |
|-------|----------------|
| Module | `SystemsLean/MultFsDeepen.lean` (`SystemsLean.MultFsDeepen`) |
| Path A tool | `MultFsWriteTool.lean` / `MultFsWriteToolMain.lean` / lake exe `slake-mult-fs-write-tool` (bootstrap: emit tool C) |
| Generated writer C | `emit/slake_mult_fs_write_tool.c` (Lean MultSubsetEmit package bytes embedded) |
| Measured writer bin | `src/systems/bin/slake-mult-fs-write-cc` (host-cc; outside `.lake`; not Lake Mult write ELF) |
| Host Lake write helper | `MultFsWriteMain.lean` / `slake-mult-fs-write` (bootstrap only; not measured writer) |
| Greps recipe | `just mult-subset-freestanding-deepen` (surface dual-check; independent of complete) |
| Write recipe | `just mult-subset-freestanding-write` (host-cc Path A; no lake on hot path; Mult rebuild ELF not the writer) |
| Partial pin | `freestandingDeepenPartial` **true** |
| Host write path pin | `multFsWritePathReady` **true** (MultSubsetEmit freestanding Mult SSOT write) |
| Design complete pin | `freestandingDriverComplete` **true** (Path A host-cc Mult package writer + dual evidence) |
| Ready ladder | `multFsDeepenReady` true; `multFsDeepenDriverReady` true; `multFsWriteNotRebuildDriver` true; `freestandingWriteToolNotLakeBuilt` true |
| Dual evidence | Lean structural ready + theorems/smoke + package greps after host-cc write + freestanding Mult product wire greps |
| Product Lake | `stillUsesLake` / `dependsOnLake` **true** (host elaborator residual remains until M6) |
| Name A additive | `multSubsetRebuildWithoutLakeFinishedClaimed` stays true; prebuilt Mult rebuild path remains valid |
...
HOST-EMIT-PLAN, HOST-EMIT-APPLY, HOST-EMIT-BODY, selfHostBodyReady, emitPlanReady, emitApplyReady, emitBodyReady, emitExtractReady
...
| Erasure product text SSOT | `emit/host_emit_erasure.ssot.txt` + Lean `EmitErasure` (HOST-EMIT-ERASURE) |
| Extract + FAIL_CLOSED product text SSOT | `emit/host_emit_extract.ssot.txt` + Lean `EmitExtract` (HOST-EMIT-EXTRACT) |
| Types product text SSOT | `emit/host_emit_types.ssot.txt` + Lean `EmitTypes` (HOST-EMIT-TYPES) |
| IR program product text SSOT | `emit/host_emit_program.ssot.txt` + Lean `EmitProgram` (HOST-EMIT-PROGRAM) |
| IR graph product text SSOT | `emit/host_emit_graph.ssot.txt` + Lean `EmitGraph` (HOST-EMIT-GRAPH) |
| Host compose product text SSOT | `emit/host_emit_compose.ssot.txt` + Lean `EmitCompose` (HOST-EMIT-COMPOSE) |
| Emit plan product text SSOT | `emit/host_emit_plan.ssot.txt` + Lean `EmitPlan` (HOST-EMIT-PLAN) |
| Emit apply product text SSOT | `emit/host_emit_apply.ssot.txt` + Lean `EmitApply` (HOST-EMIT-APPLY) |
| Emit body scaffolding SSOT | `emit/host_emit_body_fragment.ssot.txt` + Lean `EmitBody` (HOST-EMIT-BODY + HOST-EMIT-SSOT) |
...
2. `just build` copies the release surface to `out/freestanding-c/`.
...
- Greppable ownership tokens on the wire: HOST-EMIT-MULT, HOST-EMIT-LINEAR, HOST-EMIT-ERASURE,
  HOST-EMIT-EXTRACT, HOST-EMIT-TYPES, HOST-EMIT-PROGRAM, HOST-EMIT-GRAPH, HOST-EMIT-COMPOSE,
  HOST-EMIT-PLAN, HOST-EMIT-APPLY, HOST-EMIT-SSOT,
  MULT-0/1/OMEGA, LINEAR-EXACT-ONCE, RUNTIME-FS, not residual free
...
| `just build` / `./src/systems/check.sh` | End-to-end emit write + release copy + hosted probe |
...
`EmitLinear.emitLinearReady` && `EmitErasure.emitErasureReady` &&
`EmitExtract.emitExtractReady` && `EmitTypes.emitTypesReady` &&
`EmitProgram.emitProgramReady` && `EmitGraph.emitGraphReady` &&
`EmitCompose.emitComposeReady` && `EmitPlan.emitPlanReady` &&
`EmitApply.emitApplyReady` && surface cites && freestanding emit stage cite &&
!complete && !residual free && !llvm unlock.
Does **not** re-fold ProductPath / SelfApply readiness theater.
...
## Freestanding product self-host complete (claim B)

**Greppable:** SELF-HOST-FREESTANDING-PRODUCT-COMPLETE,
...
```bash
just freestanding-self-host-complete                       # claim B complete Lake-free
just freestanding-self-host-complete-measure               # complete measure (Lake diagnostic)
just freestanding-step-contract-full                       # Full still green
just freestanding-ownership-claimed                        # B40 still green
just freestanding-perform-claimed                          # B39 still green
just freestanding-retire-official                          # B38 still green
just build                                    # official dual-eq WRITE path
...
| llvm / PROVABLY | Held (LlvmHold) |

---

## Product residual free measurement (Track 4 / claim A)

**Greppable:** SELF-HOST-RESIDUAL-FREE-MEASURE, PRODUCT-RESIDUAL-FREE-MEASURE,
SLAKE_PRODUCT_RESIDUAL_FREE_MEASURE_V0, residualFreeClaimed,
productResidualRemains, DualResidual, out/freestanding-c

**Status (2026-08-01 Step 3):** product residual free **claimed true**
(`DualResidual.residualFreeClaimed` true; `productResidualRemains` false;
host elaborator residual remains). Measurement path + honesty agreement closed
claim A. Free is **not** host elaborator free, **not** proof complete, **not**
PROVABLY, **not** LLVM unlock, **not** Lake gone.

### What product residual free measures

| Side | Free means (this claim) | Gate surface |
|------|-------------------------|--------------|
| **Product** | No managed Lean runtime / GC / forbidden managed residual markers on the freestanding **release** product wire | pure Nix scan of `out/freestanding-c/slake_freestanding.{h,c}` when present (`systems-emit-wire` PRODUCT-RESIDUAL-FREE-MEASURE) |
| **Host elaborator** | Classic Lake residual gone | **Not** this measure; DualResidual keeps host elaborator residual while Lake hosts |

**Forbidden managed residual (fail if present on release source):** classic Lean
C runtime API / headers (`lean_object`, `lean_inc`, `lean_dec`, `lean_alloc_*`,
`lean_initialize`, `lean/lean.h`, `libleanrt`, ...), Boehm/GC markers
(`GC_malloc`, `GC_gcollect`, `GC_INIT`, `boehm_gc`), GMP residual includes,
`pthread_create` / `pthread_join`, and hosted libc includes (`stdio.h`,
`stdlib.h`, `string.h`, `pthread.h`) that freestanding product wire must not pull
as residual. Full list SSoT: `nix/systems-emit-wire/residual-free-measure.nix`.

**Required freestanding honesty (when release present):**
`SLAKE_EMIT_FREESTANDING_C_V0`, `RUNTIME-FS`, `no product GC`, `product residual free`.

**Out of scope for this measure alone:** nm/link of a freestanding object (text
scan only); selling probe green as free; host elaborator residual free; proof
complete; llvm unlock; PROVABLY. (Claim B complete is a separate true bar.)

### Dual residual honesty (HOST-DUAL-RESIDUAL)
...
Flip `residualFreeClaimed` to **true** only when **all** hold (met 2026-08-01):

1. PRODUCT-RESIDUAL-FREE-MEASURE green on release `out/freestanding-c` (forbidden
   managed residual absent; freestanding honesty present).
2. DualResidual deliberately flips product residual free with proved non-claims
...
```bash
just build
just systems-emit-wire   # PRODUCT-RESIDUAL-FREE-MEASURE on release when present
just systems-host        # acceptance + DualResidual free-true pins
just freestanding-self-host-complete  # complete + free living tip; lake when PATH
./src/systems/check.sh
```

### Proved vs not (claim A living tip)

| Proved / claimed | Still false / not claimed |
|------------------|---------------------------|
| `residualFreeClaimed` true (product residual free) | Host elaborator residual free |
| pure Nix measure green (forbidden + honesty) | proof complete / PROVABLY / llvm unlock |
| freestandingProductSelfHostComplete true (claim B) | Lake gone / DependsOnLake false |
| DualResidual host residual remains true | Affine first-class product ABI |
...
runtime" targets the product wire, not "delete Lake tomorrow." Bootstrap S0-S1
use Lake on purpose; S4 retires it only with evidence (see **Bootstrap stages**).
...
- Retired `SLAKE_COMPILE_PATH_V0` shell stamp alone (deleted; static unit walk pure Nix; host CompilePath is not product C)
...
| **SH2** | Host-owned product emit for Mult | Product C text derived from host for Mult contracts; bash NON-SSOT -- **done (partial)** in `EmitMult.lean` + `emit/host_emit_mult.ssot.txt`. Linear + ConsumeToken freestanding text also host-owned: **HOST-EMIT-LINEAR** / `EmitLinear.lean` + `emit/host_emit_linear.ssot.txt` (ownership map `emit/host-owned-emit.md`; no EMIT_LINEAR_V0 residual C stage). Erasure freestanding text host-owned: **HOST-EMIT-ERASURE** / `EmitErasure.lean` + `emit/host_emit_erasure.ssot.txt` (mult-0 absence honesty; no EMIT_ERASURE_V0 residual C stage). Extract + FAIL_CLOSED freestanding text host-owned: **HOST-EMIT-EXTRACT** / `EmitExtract.lean` + `emit/host_emit_extract.ssot.txt` (FAIL_CLOSED_CHECKER_V1 stage tokens stay; no residual EMIT_EXTRACT C stage). Types/TYPED_IR freestanding text host-owned: **HOST-EMIT-TYPES** / `EmitTypes.lean` + `emit/host_emit_types.ssot.txt` (type_tag + slake_ir_node; no EMIT_TYPES_V0 residual C stage). IR program freestanding text host-owned: **HOST-EMIT-PROGRAM** / `EmitProgram.lean` + `emit/host_emit_program.ssot.txt` (slake_ir_program_*; wire id IR_PROGRAM_V0; no EMIT_PROGRAM_V0 residual C stage). IR graph freestanding text host-owned: **HOST-EMIT-GRAPH** / `EmitGraph.lean` + `emit/host_emit_graph.ssot.txt` (slake_ir_graph_*; wire id IR_GRAPH_EDGES_V0; no EMIT_GRAPH_V0 residual C stage). Host compose freestanding text host-owned: **HOST-EMIT-COMPOSE** / `EmitCompose.lean` + `emit/host_emit_compose.ssot.txt` (slake_host_compose_*; wire id HOST_COMPOSE_V0; live-flag honesty not elaborator MULT-1; no EMIT_COMPOSE_V0 residual C stage). Emit plan freestanding text host-owned: **HOST-EMIT-PLAN** / `EmitPlan.lean` + `emit/host_emit_plan.ssot.txt` (slake_emit_plan_*; wire id EMIT_PLAN_V0; readiness inventory only; no residual C stage treadmill). Emit apply freestanding text host-owned: **HOST-EMIT-APPLY** / `EmitApply.lean` + `emit/host_emit_apply.ssot.txt` (slake_emit_apply_*; wire id EMIT_APPLY_V0; fixed tag buffer only; no residual C stage treadmill) |
...
| **Self-host body** | Defined freestanding compile step (acceptance first) | **done (partial)**: plain-English acceptance above (SELF-HOST-BODY); `SelfHostBody.lean` HOST-SELF-HOST-BODY / SELF-HOST-BODY / `SLAKE_SELF_HOST_BODY_V0` (`selfHostBodyReady` = emitMultReady && emitLinearReady && emitErasureReady && emitExtractReady && emitTypesReady && emitProgramReady && emitGraphReady && emitComposeReady && emitPlanReady && emitApplyReady && emitBodyReady && surface && freestanding emit stage cite && free/complete/unlock false; Mult..Body host SSOT + HOST-EMIT-EXTRACT / HOST-EMIT-PLAN / HOST-EMIT-APPLY / HOST-EMIT-BODY; `freestandingProductSelfHostComplete` false; `residualFreeClaimed` false; SELF-HOST-BODY-SMOKE; SELF-HOST-BODY-THEOREM (`selfHostBodyReady_true`, `freestandingProductSelfHostComplete_false`, `residualFreeClaimed_false`)); E2E path under gates via FreestandingEmit + `just build` / check.sh -- **not** residual free; **not** freestanding product self-host complete; **not** full product compiler self-application; **not** llvm unlock |
...
- SH4 ladder host growth (Linear / Types / Program / Emit codegen honesty) is
  ...
  CompilePath COMPILE-PATH-MULT-THEOREM Mult e2e fixture landed
  (multFixtureCompilePathReady_true: grades -> ordered IR -> compose mark+mint ->
  HOST-EMIT-MULT; free/complete/proof/llvm local claims false; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-LINEAR-THEOREM Linear e2e fixture landed
  (linearFixtureCompilePathReady_true: MULT-1 -> ordered IR -> compose mint +
  ConsumeToken mint/consume exact-once -> HOST-EMIT-LINEAR; free/complete/proof/llvm
  local claims false; live-flag not elaborator MULT-1 residual free; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-TYPES-THEOREM Types e2e fixture landed
  (typesFixtureCompilePathReady_true: TYPED_IR kind/mult -> ordered IR -> compose
  mark+mint -> HOST-EMIT-TYPES; free/complete/proof/llvm local claims false; not full
  elaborator type checker; partial only; proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-PROGRAM-THEOREM Program e2e fixture landed
  (programFixtureCompilePathReady_true: multi-node ordered IR + foldWellTyped ->
  compose mark+mint -> HOST-EMIT-PROGRAM; free/complete/proof/llvm local claims false;
  not CFG/SSA; partial only; proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-EXTRACT-THEOREM extract RUNTIME-FS gate landed
  (extractRuntimeFsGateReady_true: Mult + Program ready fixtures accept RUNTIME-FS
  via extractClaimOk / extractOkFs; reject RUNTIME-CLASSIC / EDGE-RUNTIME; raw
  Mult/Program fail extract under FS; free/complete/proof/llvm local claims false;
  partial only; proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-GRAPH-THEOREM Graph e2e fixture landed
  (graphFixtureCompilePathReady_true: multi-node ordered IR + chain edges 0->1,1->2
  -> IrGraph well-typed -> host compose mark+mint with edges -> HOST-EMIT-GRAPH;
  free/complete/proof/llvm local claims false; not CFG/SSA; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-COMPOSE-THEOREM Compose e2e fixture landed
  (composeFixtureCompilePathReady_true: multi-node ordered IR + chain edges
  -> host compose mark+mint (id 9) + ConsumeToken mint/consume exact-once
  -> unitCompileReady + HOST-EMIT-COMPOSE; free/complete/proof/llvm local claims
  false; live-flag not elaborator MULT-1 residual free; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-ERASURE-THEOREM Erasure e2e fixture landed
  (erasureFixtureCompilePathReady_true: MULT-0 erased ordered IR -> host
  markErased mark-only (no mint; Mult=4..Compose=9 stay other fixtures) ->
  unitCompileReady + erased.marked + HOST-EMIT-ERASURE; free/complete/proof/llvm
  local claims false; mult-0 absence honesty not types-in-C; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-PLAN-THEOREM Plan e2e fixture landed
  (planFixtureCompilePathReady_true: multi-node ordered IR -> host compose
  mark+mint (id 10; Mult=4..Compose=9 stay other fixtures) -> planFromCompose /
  planOk inventory (nodeCount 3, runtimeNodes 2, erasedNodes 1) ->
  unitCompileReady + HOST-EMIT-PLAN; free/complete/proof/llvm local claims
  false; readiness inventory not full IR body emit; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-APPLY-THEOREM Apply e2e fixture landed
  (applyFixtureCompilePathReady_true: multi-node ordered IR -> host compose
  mark+mint (id 11; Mult=4..Plan=10 stay other fixtures) -> applyFromCompose /
  applyOk tag-buffer (count 3, tags [2,17,32], count <= applyCap) ->
  unitCompileReady + HOST-EMIT-APPLY; free/complete/proof/llvm local claims
  false; tag-buffer not full IR body emit; partial only;
  proofCompleteClaimed stays false);
  CompilePath COMPILE-PATH-BODY-THEOREM Body e2e fixture landed
  (bodyFixtureCompilePathReady_true: multi-node ordered IR -> host compose
  mark+mint (id 12; Mult=4..Apply=11 stay other fixtures) -> bodyFromCompose /
  bodyOk buffer (r=2 e=1 tagCount 3, EMIT_BODY_V0 / RUNTIME-FS markers,
  HOST-EMIT-SSOT fragment) -> unitCompileReady + HOST-EMIT-BODY;
  free/complete/proof/llvm local claims false; body fragment scaffolding not
  full product module emit; partial only; proofCompleteClaimed stays false);
