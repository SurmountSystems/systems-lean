/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckParityMultTerm named
  closed subset probe (plus-one after the KernelEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ParityMultTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckParityMultTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckKernelProgramTerm, or
  ElabMeetNamedWalkHostModuleCheckKernelEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckParityMultTerm subset:
    prior KernelEmitTerm closed set plus
    SystemsLean.HostModuleCheckParityMultTerm last.
    Membership list is barrel order among that closed set; skip
    Linear, skip IrGraph, skip HostGraphMain. Compile order is
    the KernelEmitTerm compile order, then HostModuleCheckParityMultTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as KernelEmitTerm): ParityMultTerm
    ...
    ParityMultTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckParityMultTerm.
    HostModuleCheckParityMultTerm probe lives here.
  - tryCompileNamedHostModuleCheckParityMultTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckParityMultTerm bad writes a temp
    HostModuleCheckParityMultTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckKernelEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior KernelEmitTerm walker
    tryCompileNamedHostModuleCheckKernelEmitTermSubset, and that this walk
    is not an alias of the KernelEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. Do not plant live
    HostModuleCheckParityMultTerm.lean. Do not skip to
  ...
  Module: SystemsLean.ElabMeetNamedHostModuleCheckParityMultTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckParityMultTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckParityMultTerm named subset.
    Good compiles live Mult through HostModuleCheckKernelEmitTerm plus the
    subset-surface compile closure and HostModuleCheckParityMultTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckParityMultTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior KernelEmitTerm walker
    tryCompileNamedHostModuleCheckKernelEmitTermSubset still works; this
    walker is not an alias of the KernelEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. Do not
    plant live HostModuleCheckParityMultTerm.lean. -/
def elabMeetRunNamedHostModuleCheckParityMultTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckParityMultTerm? <-
    liftIO findLiveHostModuleCheckParityMultTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckParityMultTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckParityMultTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckParityMultTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckParityMultTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckParityMultTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckParityMultTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckParityMultTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckKernelEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckParityMultTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckParityMultTermSubset fakeLake fakeDir
  let notAliasOfKernelEmitTerm :=
    namedClosedHostModuleCheckParityMultTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckParityMultTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckParityMultTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckParityMultTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckParityMultTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckParityMultTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckParityMultTermSubsetProbe

#elabMeetNamedHostModuleCheckParityMultTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckParityMultTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckParityMultTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckParityMultTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckParityMultTermSubset = true := rfl
