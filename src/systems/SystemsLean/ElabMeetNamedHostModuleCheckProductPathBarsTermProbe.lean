/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckProductPathBarsTerm named
  closed subset probe (plus-one after the InventoryCloseTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ProductPathBarsTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckProductPathBarsTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckInventoryCloseTermProbe,
  ...
  Spec (readable):
  - Named HostModuleCheckProductPathBarsTerm subset:
    prior InventoryCloseTerm closed set plus
    SystemsLean.HostModuleCheckProductPathBarsTerm last.
    ...
    the InventoryCloseTerm compile order, then HostModuleCheckProductPathBarsTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as InventoryCloseTerm): ProductPathBarsTerm
    ...
    ProductPathBarsTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckProductPathBarsTerm.
    HostModuleCheckProductPathBarsTerm probe lives here.
  - tryCompileNamedHostModuleCheckProductPathBarsTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckProductPathBarsTerm bad writes a temp
    HostModuleCheckProductPathBarsTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckInventoryCloseTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior InventoryCloseTerm walker
    tryCompileNamedHostModuleCheckInventoryCloseTermSubset, and that this walk
    is not an alias of the InventoryCloseTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckProductPathBarsTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckProductPathBarsTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckProductPathBarsTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckProductPathBarsTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckProductPathBarsTermSubset,
  #elabMeetNamedHostModuleCheckProductPathBarsTermSubsetProbe,
  elabMeetNamedHostModuleCheckProductPathBarsTermSubsetProbe,
  SystemsLean.HostModuleCheckProductPathBarsTerm,
  ElabMeetNamedHostModuleCheckProductPathBarsTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckProductPathBarsTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckProductPathBarsTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckProductPathBarsTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckProductPathBarsTerm named subset.
    Good compiles live Mult through HostModuleCheckInventoryCloseTerm plus the
    subset-surface compile closure and HostModuleCheckProductPathBarsTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckProductPathBarsTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior InventoryCloseTerm walker
    tryCompileNamedHostModuleCheckInventoryCloseTermSubset still works; this
    walker is not an alias of the InventoryCloseTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckProductPathBarsTerm.lean. Do not skip to
    HostModuleCheckProductPathTerm. -/
def elabMeetRunNamedHostModuleCheckProductPathBarsTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckProductPathBarsTerm? <-
    liftIO findLiveHostModuleCheckProductPathBarsTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckProductPathBarsTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckProductPathBarsTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckProductPathBarsTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckProductPathBarsTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckProductPathBarsTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckProductPathBarsTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckProductPathBarsTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckInventoryCloseTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckProductPathBarsTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckProductPathBarsTermSubset fakeLake fakeDir
  let notAliasOfInventoryCloseTerm :=
    namedClosedHostModuleCheckProductPathBarsTermSubsetNames
      ...
      && namedClosedHostModuleCheckProductPathBarsTermSubsetNames.contains
           "SystemsLean.HostModuleCheckProductPathBarsTerm"
      && !namedClosedHostModuleCheckInventoryCloseTermSubsetNames.contains
           "SystemsLean.HostModuleCheckProductPathBarsTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfInventoryCloseTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckProductPathBarsTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckProductPathBarsTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckProductPathBarsTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckProductPathBarsTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckProductPathBarsTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckProductPathBarsTermSubsetProbe

#elabMeetNamedHostModuleCheckProductPathBarsTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckProductPathBarsTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckProductPathBarsTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckProductPathBarsTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckProductPathBarsTermSubset = true := rfl
