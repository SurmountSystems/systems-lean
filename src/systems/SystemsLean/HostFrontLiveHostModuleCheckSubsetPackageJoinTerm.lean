/-
  ...
  src/systems/SystemsLean/HostModuleCheckSubsetPackageJoinTerm.lean with the
  ...
  Kept for the kernel: those five imports, the namespace, four literal
  defs, and the end.
  ...
  One raw string. The product is 331 lines, so the source file is not split.
  Not an LLVM backend.
  ...
  HOST-FRONT-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKSUBSETPACKAGEJOINTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM, liveRel,
  kernelCheckLiveHostModuleCheckSubsetPackageJoinTermSource,
  hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckSubsetPackageJoinTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckSubsetPackageJoinTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 131072
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckSubsetPackageJoinTerm
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckSubsetPackageJoinTerm.lean"
...
def hostFrontLiveHostModuleCheckSubsetPackageJoinTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckSubsetPackageJoinTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckSubsetPackageJoinTermResidualFreeClaimed : Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckSubsetPackageJoinTermProvablyUnlocked : Bool := false
...
/-- Kept-command count: five imports, the namespace, four literal defs,
...
/-- Parse live HostModuleCheckSubsetPackageJoinTerm.lean text.
    Greppable: parseLiveHostModuleCheckSubsetPackageJoinTermSource,
    PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM. -/
def parseLiveHostModuleCheckSubsetPackageJoinTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckSubsetPackageJoinTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckSubsetPackageJoinTermSource,
    PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM. -/
def kernelCheckLiveHostModuleCheckSubsetPackageJoinTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckSubsetPackageJoinTermSource src with
...
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckSubsetPackageJoinTermSource liveHostModuleCheckSubsetPackageJoinTermSource with
...
def liveParseExamplesNotKept : Bool :=
  ...
  | some m =>
    (m.commands.all fun c =>
      ...
      | Cmd.def_ _ _ _ => bodyIsKernelLit c
      | _ => false)
      && (liveHostModuleCheckSubsetPackageJoinTermSource.splitOn "\ntheorem ").length == 4
      && (liveHostModuleCheckSubsetPackageJoinTermSource.splitOn "\nexample").length == 1
      && (liveHostModuleCheckSubsetPackageJoinTermSource.splitOn "\nset_option ").length == 1
...
/-- Wrap module lastSeg is HostModuleCheckSubsetPackageJoinTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckSubsetPackageJoinTerm"
      && lastSeg m.name.raw == "HostModuleCheckSubsetPackageJoinTerm"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckSubsetPackageJoinTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady,
    PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKSUBSETPACKAGEJOINTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM")
    && (liveRel == "HostModuleCheckSubsetPackageJoinTerm.lean")
    && (liveHostModuleCheckSubsetPackageJoinTermRel ==
        "src/systems/SystemsLean/HostModuleCheckSubsetPackageJoinTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckSubsetPackageJoinTermFullHost
    && !hostFrontLiveHostModuleCheckSubsetPackageJoinTermFullBackend
    && !hostFrontLiveHostModuleCheckSubsetPackageJoinTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckSubsetPackageJoinTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckSubsetPackageJoinTermSource liveHostModuleCheckSubsetPackageJoinTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckSubsetPackageJoinTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckSubsetPackageJoinTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckSubsetPackageJoinTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckSubsetPackageJoinTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckSubsetPackageJoinTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckSubsetPackageJoinTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckSubsetPackageJoinTerm.lean != liveHostModuleCheckSubsetPackageJoinTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckSubsetPackageJoinTerm.lean")
  let r := parseLiveHostModuleCheckSubsetPackageJoinTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM reject {reason}")
  | FrontResult.accept m =>
    let k :=
      if m.commands.length > kernelFuel then false
      else HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKSUBSETPACKAGEJOINTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckSubsetPackageJoinTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckSubsetPackageJoinTerm parse false")
    unless hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckSubsetPackageJoinTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckSubsetPackageJoinTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckSubsetPackageJoinTerm source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckSubsetPackageJoinTerm
