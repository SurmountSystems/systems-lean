/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live ParityProgram.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveParityProgramSource. Not occupancy name 50. Not mill 70.
  This wrap is ParityProgram.lean. It is not ParityProgramTheorems.lean.
  Greppable: SYSTEMS_LEAN_HOST, liveParityProgramSource,
  HOST-FRONT-LIVE-PARITY-PROGRAM, liveRel, UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveParityProgramSource
...
/-- Dual-pinned live ParityProgram.lean bytes (must match on-disk file).
    Greppable: liveParityProgramSource, HOST-FRONT-LIVE-PARITY-PROGRAM. -/
def liveParityProgramSource : String := r#"/-
  ...
  ParityProgram):
  - Live in SystemsLean.ParityProgramTheorems (same namespace; long-file peel).
  ...
  - PARITY-PROGRAM-SMOKE behavioral examples live with theorems
  ...
  - HOST-EMIT-PROGRAM / EmitProgram owns freestanding IR program product C text
    SSoT (host_emit_program.ssot.txt + FreestandingEmit embed). ParityProgram does
    not own emit text; this module is path-parity only. No EMIT_PROGRAM_V0 residual
    C stage treadmill. HOST-EMIT-GRAPH / EmitGraph owns freestanding IR graph
    product C text SSoT (host_emit_graph.ssot.txt). HOST-EMIT-COMPOSE / EmitCompose
    owns freestanding host compose product C text SSoT (host_emit_compose.ssot.txt;
    wire id HOST_COMPOSE_V0; live-flag honesty). Not Emit freestanding path
    parity close (ParityEmit).
  - SH5 host compose is SelfApply (not freestanding product self-host complete).
  ...
  productHostComposeConsumeApi_eq, productApiSurfaceOk_true, ParityProgramTheorems
  UNIT_SURFACE host surface. Module: SystemsLean.ParityProgram
  Long-file peel: PARITY-PROGRAM-THEOREM + PARITY-PROGRAM-SMOKE in
  SystemsLean.ParityProgramTheorems (same namespace).
