/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckProgramSubsetRebuildTerm named
  closed subset probe (plus-one after the TypesSubsetRebuildTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ProgramSubsetRebuildTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckProgramSubsetRebuildTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckLinearSubsetRebuildTerm, or
  ElabMeetNamedWalkHostModuleCheckTypesSubsetRebuildTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckProgramSubsetRebuildTerm subset:
    prior TypesSubsetRebuildTerm closed set plus
    SystemsLean.HostModuleCheckProgramSubsetRebuildTerm last.
    ...
    the TypesSubsetRebuildTerm compile order, then HostModuleCheckProgramSubsetRebuildTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as TypesSubsetRebuildTerm): ProgramSubsetRebuildTerm
    ...
    ProgramSubsetRebuildTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckProgramSubsetRebuildTerm.
    HostModuleCheckProgramSubsetRebuildTerm probe lives here.
  - tryCompileNamedHostModuleCheckProgramSubsetRebuildTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckProgramSubsetRebuildTerm bad writes a temp
    HostModuleCheckProgramSubsetRebuildTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckTypesSubsetRebuildTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior TypesSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset, and that this walk
    is not an alias of the TypesSubsetRebuildTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckProgramSubsetRebuildTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckProgramSubsetRebuildTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckProgramSubsetRebuildTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckProgramSubsetRebuildTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckProgramSubsetRebuildTermSubset,
  #elabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe,
  elabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe,
  SystemsLean.HostModuleCheckProgramSubsetRebuildTerm,
  ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckProgramSubsetRebuildTerm named subset.
    Good compiles live Mult through HostModuleCheckTypesSubsetRebuildTerm plus the
    subset-surface compile closure and HostModuleCheckProgramSubsetRebuildTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckProgramSubsetRebuildTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior TypesSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset still works; this
    walker is not an alias of the TypesSubsetRebuildTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckProgramSubsetRebuildTerm.lean. Do not skip to
    HostModuleCheckGraphSubsetRebuildTerm. -/
def elabMeetRunNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckProgramSubsetRebuildTerm? <-
    liftIO findLiveHostModuleCheckProgramSubsetRebuildTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckProgramSubsetRebuildTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckProgramSubsetRebuildTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckProgramSubsetRebuildTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckProgramSubsetRebuildTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckProgramSubsetRebuildTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckProgramSubsetRebuildTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckProgramSubsetRebuildTermSubset fakeLake fakeDir
  let notAliasOfTypesSubsetRebuildTerm :=
    namedClosedHostModuleCheckProgramSubsetRebuildTermSubsetNames
      ...
      && namedClosedHostModuleCheckProgramSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckProgramSubsetRebuildTerm"
      && !namedClosedHostModuleCheckTypesSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckProgramSubsetRebuildTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfTypesSubsetRebuildTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckProgramSubsetRebuildTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckProgramSubsetRebuildTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckProgramSubsetRebuildTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckProgramSubsetRebuildTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe

#elabMeetNamedHostModuleCheckProgramSubsetRebuildTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckProgramSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckProgramSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckProgramSubsetRebuildTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckProgramSubsetRebuildTermSubset = true := rfl
