/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckMultFsWriteToolTerm named-walk
  wrappers (CheckersLater-role walk; LlvmLinearTextTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and LlvmLinearTextTerm walk
  ...
  ElabMeetNamedWalk. Prior LlvmLinearTextTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckLlvmLinearTextTerm. Do not grow that walk.
  ...
  LlvmLinearTextTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckMultFsWriteToolTerm.lean.
  Do not skip to HostModuleCheckFrontMultPackageTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckMultFsWriteToolTerm wrappers live here:
    findLiveHostModuleCheckMultFsWriteToolTermPath,
    namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames,
    namedClosedHostModuleCheckMultFsWriteToolTermCompileOrder,
    barrelListsNamedHostModuleCheckMultFsWriteToolTermSubset,
    tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset,
    tryCompileAfterHostModuleCheckMultFsWriteToolTermDeps.
    Named membership is the prior LlvmLinearTextTerm closed set plus
    HostModuleCheckMultFsWriteToolTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the LlvmLinearTextTerm compile
    order, then HostModuleCheckMultFsWriteToolTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / LlvmLinearTextTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the LlvmLinearTextTerm compile order.
    HostModuleCheckMultFsWriteToolTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckLlvmLinearTextTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckMultFsWriteToolTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckMultFsWriteToolTerm;
  just systems-host.
...
import SystemsLean.ElabMeetNamedWalkHostModuleCheckLlvmLinearTextTerm

namespace SystemsLean.ElabMeet

open Lean Elab Command

/-- Resolve live SystemsLean/HostModuleCheckMultFsWriteToolTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckMultFsWriteToolTermPath. -/
def findLiveHostModuleCheckMultFsWriteToolTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckMultFsWriteToolTerm"

/-- Named HostModuleCheckMultFsWriteToolTerm subset of the live SystemsLean library.
    Membership is the prior LlvmLinearTextTerm closed set plus
    HostModuleCheckMultFsWriteToolTerm last.
    ...
    Greppable: namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames.
    SystemsLean.HostModuleCheckMultFsWriteToolTerm. -/
def namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames : List String :=
  namedClosedHostModuleCheckLlvmLinearTextTermSubsetNames ++
    ["SystemsLean.HostModuleCheckMultFsWriteToolTerm"]

/-- Compile order honors imports: LlvmLinearTextTerm compile order, then
    HostModuleCheckMultFsWriteToolTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckMultFsWriteToolTermCompileOrder. -/
def namedClosedHostModuleCheckMultFsWriteToolTermCompileOrder : List String :=
  namedClosedHostModuleCheckLlvmLinearTextTermCompileOrder ++
    ["SystemsLean.HostModuleCheckMultFsWriteToolTerm"]

/-- True when the barrel lists each HostModuleCheckMultFsWriteToolTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckMultFsWriteToolTermSubset. -/
def barrelListsNamedHostModuleCheckMultFsWriteToolTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames

/-- Compile the named HostModuleCheckMultFsWriteToolTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the LlvmLinearTextTerm
    compile order, then HostModuleCheckMultFsWriteToolTerm last.
    Greppable: tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckMultFsWriteToolTerm. -/
unsafe def tryCompileNamedHostModuleCheckMultFsWriteToolTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames
    namedClosedHostModuleCheckMultFsWriteToolTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckMultFsWriteToolTermSubsetIO]
opaque tryCompileNamedHostModuleCheckMultFsWriteToolTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckMultFsWriteToolTerm subset.
    Greppable: tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset. -/
def tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckMultFsWriteToolTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the LlvmLinearTextTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckMultFsWriteToolTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckMultFsWriteToolTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckLlvmLinearTextTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckMultFsWriteToolTermDepsIO]
opaque tryCompileAfterHostModuleCheckMultFsWriteToolTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckMultFsWriteToolTerm deps.
    Greppable: tryCompileAfterHostModuleCheckMultFsWriteToolTermDeps. -/
def tryCompileAfterHostModuleCheckMultFsWriteToolTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckMultFsWriteToolTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
