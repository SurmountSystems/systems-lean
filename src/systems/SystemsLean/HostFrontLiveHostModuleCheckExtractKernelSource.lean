/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-EXTRACT-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_EXTRACT_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckExtractKernelSource,
  hostFrontLiveHostModuleCheckExtractKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckExtractKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckExtractKernelSource
...
def liveRel : String := "HostModuleCheckExtractKernel.lean"
...
def hostFrontLiveHostModuleCheckExtractKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckExtractKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckExtractKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckExtractKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckExtractKernel.lean bytes.
    Greppable: liveHostModuleCheckExtractKernelSource. -/

def liveHostModuleCheckExtractKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckExtractKernel.lean text.
    Greppable: parseLiveHostModuleCheckExtractKernelSource. -/
def parseLiveHostModuleCheckExtractKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckExtractKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckExtractKernelSource. -/
def kernelCheckLiveHostModuleCheckExtractKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckExtractKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckExtractKernelSource
      liveHostModuleCheckExtractKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthExtractKernelBar"
      && has "checkDepthExtractKernelScope"
      && has "hostModuleCheckExtractKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckExtractKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckExtractKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckExtractKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckExtractKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckExtractKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_EXTRACT_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-EXTRACT-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-KERNEL")
    && (liveRel == "HostModuleCheckExtractKernel.lean")
    && (liveHostModuleCheckExtractKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckExtractKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckExtractKernelFullHost
    && !hostFrontLiveHostModuleCheckExtractKernelFullBackend
    && !hostFrontLiveHostModuleCheckExtractKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckExtractKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckExtractKernelSource
      liveHostModuleCheckExtractKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckExtractKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckExtractKernel.lean")
    ++ bad "rel" (liveHostModuleCheckExtractKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckExtractKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckExtractKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckExtractKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckExtractKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckExtractKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckExtractKernelSource
      liveHostModuleCheckExtractKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckExtractKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckExtractKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckExtractKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckExtractKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckExtractKernel.lean != liveHostModuleCheckExtractKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckExtractKernel.lean")
  let r := parseLiveHostModuleCheckExtractKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckExtractKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckExtractKernel parse false")
    unless hostFrontLiveHostModuleCheckExtractKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckExtractKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckExtractKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckExtractKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckExtractKernelSource
