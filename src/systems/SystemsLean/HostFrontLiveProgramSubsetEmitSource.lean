/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live ProgramSubsetEmit.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveProgramSubsetEmitSource. Not occupancy name 50.
  Not mill 36 remill. Mill stays 69 of 69.
  This wrap is ProgramSubsetEmit.lean. Theorems and examples in that file are
  skipped by the parser. It is not ProgramSubsetEmitMain.lean.
  HostFrontLiveProgramSubsetEmitMain parses ProgramSubsetEmitMain.lean.
  Do not steal that module.
  Unique needles (trailing newline so HostFrontLiveProgramSubsetEmit is not a
  prefix hit on HostFrontLiveProgramSubsetEmitSource):
  HostFrontLiveProgramSubsetEmitSource
  PARSE-LIVE-PROGRAM-SUBSET-EMIT
  HOST-FRONT-LIVE-PROGRAM-SUBSET-EMIT
  PROGRAM-SUBSET-EMIT
  Greppable: SYSTEMS_LEAN_HOST, liveProgramSubsetEmitSource,
  HOST-FRONT-LIVE-PROGRAM-SUBSET-EMIT, liveRel, UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveProgramSubsetEmitSource
...
/-- Dual-pinned live ProgramSubsetEmit.lean bytes (must match on-disk file).
    Greppable: liveProgramSubsetEmitSource, HOST-FRONT-LIVE-PROGRAM-SUBSET-EMIT. -/
def liveProgramSubsetEmitSource : String := r#"/-
...
/-- Live file basename. liveRel is ProgramSubsetEmit.lean. -/
def liveRel : String := "ProgramSubsetEmit.lean"
