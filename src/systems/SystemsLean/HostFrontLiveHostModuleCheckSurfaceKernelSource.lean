/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-SURFACE-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_SURFACE_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-SURFACE-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckSurfaceKernelSource,
  hostFrontLiveHostModuleCheckSurfaceKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckSurfaceKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckSurfaceKernelSource
...
def liveRel : String := "HostModuleCheckSurfaceKernel.lean"
...
def hostFrontLiveHostModuleCheckSurfaceKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckSurfaceKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckSurfaceKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckSurfaceKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckSurfaceKernel.lean bytes.
    Greppable: liveHostModuleCheckSurfaceKernelSource. -/
def liveHostModuleCheckSurfaceKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckSurfaceKernel.lean text.
    Greppable: parseLiveHostModuleCheckSurfaceKernelSource. -/
def parseLiveHostModuleCheckSurfaceKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckSurfaceKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckSurfaceKernelSource. -/
def kernelCheckLiveHostModuleCheckSurfaceKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckSurfaceKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckSurfaceKernelSource
      liveHostModuleCheckSurfaceKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthSurfaceKernelBar"
      && has "checkDepthSurfaceKernelScope"
      && has "hostModuleCheckSurfaceKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckSurfaceKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckSurfaceKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckSurfaceKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckSurfaceKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckSurfaceKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_SURFACE_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-SURFACE-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-SURFACE-KERNEL")
    && (liveRel == "HostModuleCheckSurfaceKernel.lean")
    && (liveHostModuleCheckSurfaceKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckSurfaceKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckSurfaceKernelFullHost
    && !hostFrontLiveHostModuleCheckSurfaceKernelFullBackend
    && !hostFrontLiveHostModuleCheckSurfaceKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckSurfaceKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckSurfaceKernelSource
      liveHostModuleCheckSurfaceKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckSurfaceKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-SURFACE-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckSurfaceKernel.lean")
    ++ bad "rel" (liveHostModuleCheckSurfaceKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckSurfaceKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckSurfaceKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckSurfaceKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckSurfaceKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckSurfaceKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckSurfaceKernelSource
      liveHostModuleCheckSurfaceKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckSurfaceKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckSurfaceKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckSurfaceKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckSurfaceKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckSurfaceKernel.lean != liveHostModuleCheckSurfaceKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckSurfaceKernel.lean")
  let r := parseLiveHostModuleCheckSurfaceKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckSurfaceKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckSurfaceKernel parse false")
    unless hostFrontLiveHostModuleCheckSurfaceKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckSurfaceKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckSurfaceKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckSurfaceKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckSurfaceKernelSource
