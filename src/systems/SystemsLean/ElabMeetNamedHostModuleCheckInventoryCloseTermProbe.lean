/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckInventoryCloseTerm named
  closed subset probe (plus-one after the SelfApplyFsTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. InventoryCloseTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckInventoryCloseTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckInventoryCloseTerm subset:
    prior SelfApplyFsTerm closed set plus
    SystemsLean.HostModuleCheckInventoryCloseTerm last.
    ...
    the SelfApplyFsTerm compile order, then HostModuleCheckInventoryCloseTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as SelfApplyFsTerm): InventoryCloseTerm
    ...
    InventoryCloseTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckInventoryCloseTerm.
    HostModuleCheckInventoryCloseTerm probe lives here.
  - tryCompileNamedHostModuleCheckInventoryCloseTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckInventoryCloseTerm bad writes a temp
    HostModuleCheckInventoryCloseTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckSelfApplyFsTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior SelfApplyFsTerm walker
    tryCompileNamedHostModuleCheckSelfApplyFsTermSubset, and that this walk
    is not an alias of the SelfApplyFsTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckInventoryCloseTerm.lean. Do not skip to
  ...
  SystemsLean.ElabMeetNamedHostModuleCheckInventoryCloseTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckInventoryCloseTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckInventoryCloseTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckInventoryCloseTerm named subset.
    Good compiles live Mult through HostModuleCheckSelfApplyFsTerm plus the
    subset-surface compile closure and HostModuleCheckInventoryCloseTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckInventoryCloseTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior SelfApplyFsTerm walker
    tryCompileNamedHostModuleCheckSelfApplyFsTermSubset still works; this
    walker is not an alias of the SelfApplyFsTerm closed set. Drive is
    good && !bad && isolation.
...
def elabMeetRunNamedHostModuleCheckInventoryCloseTermSubsetProbe : CommandElabM Unit := do
  ...
  let liveHostModuleCheckInventoryCloseTerm? <-
    liftIO findLiveHostModuleCheckInventoryCloseTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckInventoryCloseTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckInventoryCloseTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckInventoryCloseTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckInventoryCloseTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckInventoryCloseTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckInventoryCloseTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckInventoryCloseTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSelfApplyFsTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckInventoryCloseTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckInventoryCloseTermSubset fakeLake fakeDir
  let notAliasOfSelfApplyFsTerm :=
    namedClosedHostModuleCheckInventoryCloseTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckInventoryCloseTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckInventoryCloseTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckInventoryCloseTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckInventoryCloseTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckInventoryCloseTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckInventoryCloseTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckInventoryCloseTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckInventoryCloseTermSubset = true := rfl
