/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckKernelMultTerm.lean.
  ...
  liveRel is HostModuleCheckKernelMultTerm.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckKernelMultTermSource,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-MULT-TERM.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelMultTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelMultTermSource
...
def liveRel : String := "HostModuleCheckKernelMultTerm.lean"

/-- Pinned live HostModuleCheckKernelMultTerm.lean bytes. -/
def liveHostModuleCheckKernelMultTermSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckKernelMultTermSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckKernelMultTermSourceReady : Bool :=
  liveRel == "HostModuleCheckKernelMultTerm.lean"
    && kernelCheckLiveHostModuleCheckKernelMultTermSource liveHostModuleCheckKernelMultTermSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckKernelMultTermSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckKernelMultTerm.lean")
  let k := kernelCheckLiveHostModuleCheckKernelMultTermSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckKernelMultTerm parse false")
  unless hostFrontLiveHostModuleCheckKernelMultTermSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelMultTermSource
