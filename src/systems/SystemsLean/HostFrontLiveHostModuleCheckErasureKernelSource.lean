/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-ERASURE-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_ERASURE_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckErasureKernelSource,
  hostFrontLiveHostModuleCheckErasureKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckErasureKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckErasureKernelSource
...
def liveRel : String := "HostModuleCheckErasureKernel.lean"
...
def hostFrontLiveHostModuleCheckErasureKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckErasureKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckErasureKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckErasureKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckErasureKernel.lean bytes.
    Greppable: liveHostModuleCheckErasureKernelSource. -/

def liveHostModuleCheckErasureKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckErasureKernel.lean text.
    Greppable: parseLiveHostModuleCheckErasureKernelSource. -/
def parseLiveHostModuleCheckErasureKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckErasureKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckErasureKernelSource. -/
def kernelCheckLiveHostModuleCheckErasureKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckErasureKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckErasureKernelSource
      liveHostModuleCheckErasureKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthErasureKernelBar"
      && has "checkDepthErasureKernelScope"
      && has "hostModuleCheckErasureKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckErasureKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckErasureKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckErasureKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckErasureKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckErasureKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_ERASURE_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-ERASURE-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-KERNEL")
    && (liveRel == "HostModuleCheckErasureKernel.lean")
    && (liveHostModuleCheckErasureKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckErasureKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckErasureKernelFullHost
    && !hostFrontLiveHostModuleCheckErasureKernelFullBackend
    && !hostFrontLiveHostModuleCheckErasureKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckErasureKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckErasureKernelSource
      liveHostModuleCheckErasureKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckErasureKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckErasureKernel.lean")
    ++ bad "rel" (liveHostModuleCheckErasureKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckErasureKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckErasureKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckErasureKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckErasureKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckErasureKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckErasureKernelSource
      liveHostModuleCheckErasureKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckErasureKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckErasureKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckErasureKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckErasureKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckErasureKernel.lean != liveHostModuleCheckErasureKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckErasureKernel.lean")
  let r := parseLiveHostModuleCheckErasureKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckErasureKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckErasureKernel parse false")
    unless hostFrontLiveHostModuleCheckErasureKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckErasureKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckErasureKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckErasureKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckErasureKernelSource
