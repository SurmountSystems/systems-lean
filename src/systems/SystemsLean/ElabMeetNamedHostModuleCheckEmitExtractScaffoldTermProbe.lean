/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckEmitExtractScaffoldTerm named
  closed subset probe (plus-one after the EmitErasureScaffoldTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. EmitExtractScaffoldTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckEmitExtractScaffoldTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckEmitComposeScaffoldTerm, or
  ElabMeetNamedWalkHostModuleCheckEmitErasureScaffoldTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckEmitExtractScaffoldTerm subset:
    prior EmitErasureScaffoldTerm closed set plus
    SystemsLean.HostModuleCheckEmitExtractScaffoldTerm last.
    ...
    the EmitErasureScaffoldTerm compile order, then HostModuleCheckEmitExtractScaffoldTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as EmitErasureScaffoldTerm): EmitExtractScaffoldTerm
    ...
    EmitExtractScaffoldTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckEmitExtractScaffoldTerm.
    HostModuleCheckEmitExtractScaffoldTerm probe lives here.
  - tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckEmitExtractScaffoldTerm bad writes a temp
    HostModuleCheckEmitExtractScaffoldTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckEmitErasureScaffoldTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior EmitErasureScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitErasureScaffoldTermSubset, and that this walk
    is not an alias of the EmitErasureScaffoldTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckEmitExtractScaffoldTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckEmitExtractScaffoldTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckEmitExtractScaffoldTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckEmitExtractScaffoldTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitExtractScaffoldTermSubset,
  #elabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe,
  elabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe,
  SystemsLean.HostModuleCheckEmitExtractScaffoldTerm,
  ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckEmitExtractScaffoldTerm named subset.
    Good compiles live Mult through HostModuleCheckEmitErasureScaffoldTerm plus the
    subset-surface compile closure and HostModuleCheckEmitExtractScaffoldTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckEmitExtractScaffoldTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior EmitErasureScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitErasureScaffoldTermSubset still works; this
    walker is not an alias of the EmitErasureScaffoldTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckEmitExtractScaffoldTerm.lean. Do not skip to
    HostModuleCheckEmitBannerScaffoldTerm. -/
def elabMeetRunNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckEmitExtractScaffoldTerm? <-
    liftIO findLiveHostModuleCheckEmitExtractScaffoldTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckEmitExtractScaffoldTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckEmitExtractScaffoldTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckEmitExtractScaffoldTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckEmitExtractScaffoldTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitErasureScaffoldTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset fakeLake fakeDir
  let notAliasOfEmitErasureScaffoldTerm :=
    namedClosedHostModuleCheckEmitExtractScaffoldTermSubsetNames
      ...
      && namedClosedHostModuleCheckEmitExtractScaffoldTermSubsetNames.contains
           "SystemsLean.HostModuleCheckEmitExtractScaffoldTerm"
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfEmitErasureScaffoldTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckEmitExtractScaffoldTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckEmitExtractScaffoldTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitExtractScaffoldTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckEmitExtractScaffoldTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe

#elabMeetNamedHostModuleCheckEmitExtractScaffoldTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckEmitExtractScaffoldTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckEmitExtractScaffoldTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitExtractScaffoldTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckEmitExtractScaffoldTermSubset = true := rfl
