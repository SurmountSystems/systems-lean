/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathGraphTerm named
  closed subset probe (plus-one after the CompilePathProgramTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathGraphTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathGraphTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathTypesTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathProgramTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathGraphTerm subset:
    prior CompilePathProgramTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathGraphTerm last.
    ...
    the CompilePathProgramTerm compile order, then HostModuleCheckCompilePathGraphTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathProgramTerm): CompilePathGraphTerm
    ...
    CompilePathGraphTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathGraphTerm.
    HostModuleCheckCompilePathGraphTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathGraphTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathGraphTerm bad writes a temp
    HostModuleCheckCompilePathGraphTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathProgramTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathProgramTerm walker
    tryCompileNamedHostModuleCheckCompilePathProgramTermSubset, and that this walk
    is not an alias of the CompilePathProgramTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathGraphTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathGraphTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathGraphTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathGraphTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathGraphTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathGraphTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathGraphTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathGraphTerm,
  ElabMeetNamedHostModuleCheckCompilePathGraphTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathGraphTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathGraphTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathGraphTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathGraphTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathProgramTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathGraphTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathGraphTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathProgramTerm walker
    tryCompileNamedHostModuleCheckCompilePathProgramTermSubset still works; this
    walker is not an alias of the CompilePathProgramTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathGraphTerm.lean. Do not skip to
    HostModuleCheckCompilePathComposeTerm. -/
def elabMeetRunNamedHostModuleCheckCompilePathGraphTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathGraphTerm? <-
    liftIO findLiveHostModuleCheckCompilePathGraphTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathGraphTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathGraphTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathGraphTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathGraphTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathGraphTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathGraphTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathGraphTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathProgramTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathGraphTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathGraphTermSubset fakeLake fakeDir
  let notAliasOfCompilePathProgramTerm :=
    namedClosedHostModuleCheckCompilePathGraphTermSubsetNames
      ...
      && namedClosedHostModuleCheckCompilePathGraphTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathGraphTerm"
      && !namedClosedHostModuleCheckCompilePathProgramTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathGraphTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfCompilePathProgramTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathGraphTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathGraphTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathGraphTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathGraphTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckCompilePathGraphTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckCompilePathGraphTermSubsetProbe

#elabMeetNamedHostModuleCheckCompilePathGraphTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathGraphTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathGraphTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathGraphTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathGraphTermSubset = true := rfl
