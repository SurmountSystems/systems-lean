/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-TYPES-TERM-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_TYPES_TERM_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-TYPES-TERM-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckKernelTypesTermKernelSource,
  hostFrontLiveHostModuleCheckKernelTypesTermKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelTypesTermKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelTypesTermKernelSource
...
def liveRel : String := "HostModuleCheckKernelTypesTermKernel.lean"
...
def hostFrontLiveHostModuleCheckKernelTypesTermKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckKernelTypesTermKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckKernelTypesTermKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckKernelTypesTermKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckKernelTypesTermKernel.lean bytes.
    Greppable: liveHostModuleCheckKernelTypesTermKernelSource. -/


def liveHostModuleCheckKernelTypesTermKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckKernelTypesTermKernel.lean text.
    Greppable: parseLiveHostModuleCheckKernelTypesTermKernelSource. -/
def parseLiveHostModuleCheckKernelTypesTermKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckKernelTypesTermKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckKernelTypesTermKernelSource. -/
def kernelCheckLiveHostModuleCheckKernelTypesTermKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckKernelTypesTermKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckKernelTypesTermKernelSource
      liveHostModuleCheckKernelTypesTermKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthKernelTypesTermKernelBar"
      && has "checkDepthKernelTypesTermKernelScope"
      && has "hostModuleCheckKernelTypesTermKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckKernelTypesTermKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckKernelTypesTermKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckKernelTypesTermKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckKernelTypesTermKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckKernelTypesTermKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_TYPES_TERM_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-TYPES-TERM-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-TYPES-TERM-KERNEL")
    && (liveRel == "HostModuleCheckKernelTypesTermKernel.lean")
    && (liveHostModuleCheckKernelTypesTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelTypesTermKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckKernelTypesTermKernelFullHost
    && !hostFrontLiveHostModuleCheckKernelTypesTermKernelFullBackend
    && !hostFrontLiveHostModuleCheckKernelTypesTermKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckKernelTypesTermKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckKernelTypesTermKernelSource
      liveHostModuleCheckKernelTypesTermKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckKernelTypesTermKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-TYPES-TERM-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckKernelTypesTermKernel.lean")
    ++ bad "rel" (liveHostModuleCheckKernelTypesTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelTypesTermKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckKernelTypesTermKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckKernelTypesTermKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckKernelTypesTermKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckKernelTypesTermKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckKernelTypesTermKernelSource
      liveHostModuleCheckKernelTypesTermKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckKernelTypesTermKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckKernelTypesTermKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckKernelTypesTermKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckKernelTypesTermKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckKernelTypesTermKernel.lean != liveHostModuleCheckKernelTypesTermKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckKernelTypesTermKernel.lean")
  let r := parseLiveHostModuleCheckKernelTypesTermKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckKernelTypesTermKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckKernelTypesTermKernel parse false")
    unless hostFrontLiveHostModuleCheckKernelTypesTermKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckKernelTypesTermKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckKernelTypesTermKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckKernelTypesTermKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelTypesTermKernelSource
