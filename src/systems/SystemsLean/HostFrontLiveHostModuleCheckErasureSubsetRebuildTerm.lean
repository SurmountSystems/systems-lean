/-
  ...
  Short role: HostFrontLiveHostModuleCheckErasureSubsetRebuildTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckErasureSubsetRebuildTermSource turns live
    HostModuleCheckErasureSubsetRebuildTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckErasureSubsetRebuildTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckErasureSubsetRebuildTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckErasureSubsetRebuildTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckErasureSubsetRebuildTermSource. Not split.
  - Five dotted imports, namespace SystemsLean.HostModuleCheck,
    five literal defs (bar String, scope String, dual-ok Bool,
    wrote-expected String, skeleton suffix String), and end are kept.
    erasureSubsetRebuildSelfApplyOkTermBody and
    erasureSubsetRebuildReadyTermBody contain a backslash before a
    quote, so those strings split and are not one literal.
    erasureSubsetRebuildWroteExpectedTermBody is one String literal.
    Every other def has a binder or a body that is not one literal
    followed by a command, so it is skip-folded. Three theorems are
    skip-folded. Kept command count is 12. Skipped command count is
    32. Kept plus skipped is 44. kernelFuel is 64. 12 does not pass 64.
    The ready-body quote span is 462 characters. HostTerm string fuel
    is 256, which would drop that opening quote. This checker scans
    strings with fuel 1024. That fuel is not kernelFuel.
  ...
  - Skip-head still sees def checkErasureSubsetRebuildTermDialect and
    def hostModuleCheckErasureSubsetRebuildTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckErasureSubsetRebuildTerm
  PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKERASURESUBSETREBUILDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckErasureSubsetRebuildTermSource,
  kernelCheckLiveHostModuleCheckErasureSubsetRebuildTermSource,
  hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady,
  liveHostModuleCheckErasureSubsetRebuildTermSource,
  liveRel, liveHostModuleCheckErasureSubsetRebuildTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckErasureSubsetRebuildTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckErasureSubsetRebuildTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckErasureSubsetRebuildTerm
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckErasureSubsetRebuildTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckErasureSubsetRebuildTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckErasureSubsetRebuildTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckErasureSubsetRebuildTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckErasureSubsetRebuildTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckErasureSubsetRebuildTermProvablyUnlocked : Bool :=
  false
...
/-- String token fuel. Not kernelFuel. kernelFuel stays 64.
    The ready-body quote span is 462 characters. HostTerm fuel 256
    drops that opening quote. 1024 keeps it. A backslash before a
    quote still ends the string token. -/
def liveStringFuel : Nat := 1024

/-- Kept-command count: five imports, namespace, end, and five
    literal defs (bar String, scope String, dual-ok Bool,
    wrote-expected String, skeleton suffix String).
    erasureSubsetRebuildSelfApplyOkTermBody and
    erasureSubsetRebuildReadyTermBody are not kept.
    erasureSubsetRebuildWroteExpectedTermBody is one String literal.
    Other defs are skip-folded. Three theorems are skip-folded.
    Skipped count is 32. Kept plus skipped is 44. kernelFuel is 64.
    12 does not pass 64. -/
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckErasureSubsetRebuildTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckErasureSubsetRebuildTerm.lean
...
/-- Tokenize like HostFrontLiveHostTerm, with liveStringFuel for quotes.
    Not kernelFuel. kernelFuel stays 64. -/
...
/-- Tokenize after stripCommentsHc. -/
def tokenizeLive (src : String) : List String :=
  tokenizeLiveN (src.length + 8) src.toList []

/-- Dotted ident `SystemsLean . HostModuleCheckErasureSubsetRebuildTerm`. -/
...
/-- Parse live HostModuleCheckErasureSubsetRebuildTerm.lean text.
    Greppable: parseLiveHostModuleCheckErasureSubsetRebuildTermSource,
    PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM. -/
def parseLiveHostModuleCheckErasureSubsetRebuildTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckErasureSubsetRebuildTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckErasureSubsetRebuildTermSource,
    PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM. -/
def kernelCheckLiveHostModuleCheckErasureSubsetRebuildTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckErasureSubsetRebuildTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckErasureSubsetRebuildTermSource
      liveHostModuleCheckErasureSubsetRebuildTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckErasureSubsetRebuildTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckErasureSubsetRebuildTerm"
      && lastSeg m.name.raw == "HostModuleCheckErasureSubsetRebuildTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckErasureSubsetRebuildTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkErasureSubsetRebuildTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckErasureSubsetRebuildTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady,
    PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKERASURESUBSETREBUILDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM")
    && (liveRel == "HostModuleCheckErasureSubsetRebuildTerm.lean")
    && (liveHostModuleCheckErasureSubsetRebuildTermRel ==
        "src/systems/SystemsLean/HostModuleCheckErasureSubsetRebuildTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckErasureSubsetRebuildTermFullHost
    && !hostFrontLiveHostModuleCheckErasureSubsetRebuildTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckErasureSubsetRebuildTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckErasureSubsetRebuildTermSource
        liveHostModuleCheckErasureSubsetRebuildTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckErasureSubsetRebuildTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckErasureSubsetRebuildTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckErasureSubsetRebuildTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckErasureSubsetRebuildTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckErasureSubsetRebuildTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckErasureSubsetRebuildTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckErasureSubsetRebuildTerm.lean != liveHostModuleCheckErasureSubsetRebuildTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckErasureSubsetRebuildTerm.lean")
  let r := parseLiveHostModuleCheckErasureSubsetRebuildTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKERASURESUBSETREBUILDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckErasureSubsetRebuildTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckErasureSubsetRebuildTerm parse false")
    unless hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckErasureSubsetRebuildTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckErasureSubsetRebuildTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckErasureSubsetRebuildTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckErasureSubsetRebuildTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckErasureSubsetRebuildTerm
