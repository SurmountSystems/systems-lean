/-
  ...
  Short role: HostFrontLiveHostModuleCheckFixtureTextsProduct.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckFixtureTextsProductSource turns live
    HostModuleCheckFixtureTextsProduct.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckFixtureTextsProduct
    even without a module line.
  - liveRel is the exact basename HostModuleCheckFixtureTextsProduct.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckFixtureTextsProductSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are liveHostModuleCheckFixtureTextsProductSource0
    ++ liveHostModuleCheckFixtureTextsProductSource1. Two parts only.
  - Real commands: zero imports, namespace SystemsLean.HostModuleCheck,
    eighty defs, and end. That is 82.
  - Kept: the namespace and end. Kept count is 2.
  - Skipped: eighty defs whose bodies are string concatenations, not one
    literal. Skipped count is 80. Kept plus skipped is 82.
  - kernelFuel is 64. Kept count 2 does not pass 64. The eighty
    concatenation defs are not kept, so kernelCheck does not see them.
    Fuel is not raised. kernelCheck is not a constant true.
  - The longest quoted span in the product is 742 characters.
    That is over 256 and under 1024. liveStringFuel is 1024 so
  ...
  - Fixture strings use backslash-quote. takeStringN stops at the first
    quote character. The words def, import, namespace, theorem, structure,
    and end inside those strings stay in the quote span, so they are not
  ...
  HostFrontLiveHostModuleCheckFixtureTextsProduct
  PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT
  HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIXTURETEXTSPRODUCT_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckFixtureTextsProductSource,
  kernelCheckLiveHostModuleCheckFixtureTextsProductSource,
  hostFrontLiveHostModuleCheckFixtureTextsProductReady,
  liveHostModuleCheckFixtureTextsProductSource,
  liveRel, liveHostModuleCheckFixtureTextsProductRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsProduct
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsProductSource
import SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsProductSource01
import SystemsLean.HostKernel

set_option maxRecDepth 131072
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsProduct
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckFixtureTextsProduct.lean"

/-- Full live bytes. First half then second half. -/
def liveHostModuleCheckFixtureTextsProductSource : String :=
  liveHostModuleCheckFixtureTextsProductSource0
    ++ liveHostModuleCheckFixtureTextsProductSource1

/-- Live file relative to repo root. Byte-match path.
    Two embedded parts. Not a third source file. -/
...
def hostFrontLiveHostModuleCheckFixtureTextsProductFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckFixtureTextsProductResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckFixtureTextsProductProvablyUnlocked : Bool :=
  false

/-- Parse fuel (command fold). Eighty skipped defs plus namespace and end. -/
def liveParseFuel : Nat := 512

/-- Skip fuel for un-kernelable def tails. -/
def liveSkipFuel : Nat := 32768

/-- String take fuel. The longest quoted span in the product is 742 characters.
    liveStringFuel stays 1024 so a quote is not cut at liveHostTermParseFuel 256. -/
def liveStringFuel : Nat := 1024

/-- Real command count: zero imports, namespace, eighty defs, and end. -/
def liveRealCmds : Nat := 82

/-- Kept-command count: namespace and end.
    Eighty concatenation defs are skip-folded.
    Skipped count is 80. Kept plus skipped is 82.
    kernelFuel is 64. Kept count 2 does not pass 64.
    Real count 82 is above 64, and those eighty defs are not kept. -/
def liveKeptCmds : Nat := 2

/-- Skipped command count. -/
def liveSkippedCmds : Nat := 80
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckFixtureTextsProduct has dash-dash in the header block comment.
    The live text is 57977 bytes, so this fold's fuel is src.length + 8. -/
...
/-- String-safe comment strip for live HostModuleCheckFixtureTextsProduct.lean
...
/-- Token walk: live text has `def` named `nm` even if the body was skipped.
    The else branch is tail position so a long token list does not grow the stack. -/
def toksHaveDefNamed : Nat -> List String -> String -> Bool
  ...
  | Nat.succ n, "def" :: rest, nm =>
    match parseDefHead rest with
    | some (dname, rest2) =>
      if dname == nm then true else toksHaveDefNamed n rest2 nm
...
/-- Parse live HostModuleCheckFixtureTextsProduct.lean text.
    Greppable: parseLiveHostModuleCheckFixtureTextsProductSource,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT. -/
def parseLiveHostModuleCheckFixtureTextsProductSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckFixtureTextsProduct"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckFixtureTextsProductSource,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT. -/
def kernelCheckLiveHostModuleCheckFixtureTextsProductSource (src : String) : Bool :=
  match parseLiveHostModuleCheckFixtureTextsProductSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckFixtureTextsProductSource
      liveHostModuleCheckFixtureTextsProductSource with
...
/-- No literal def is kept. Eighty concatenation defs are not in this list. -/
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckFixtureTextsProductSource)) 0

/-- Real count, kept count, and skipped count.
    Kept count does not pass 64. Real count is not the kernel fuel gate. -/
def liveCmdBudgetOk : Bool :=
  liveCmdKwCount == liveRealCmds
    && liveKeptCmds + liveSkippedCmds == liveRealCmds
    && liveKeptCmds <= 64

/-- Wrap module lastSeg is HostModuleCheckFixtureTextsProduct
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckFixtureTextsProduct"
      && lastSeg m.name.raw == "HostModuleCheckFixtureTextsProduct"
...
/-- Every concatenation def in the live text, kept or skipped. -/
def liveCoreDefNames : List String := [
  ...
  "hostModuleCheckGoodLlvmHoldText",
  ...
  "hostModuleCheckGoodCompilePathMultText",
  "hostModuleCheckGoodCompilePathLinearText",
  "hostModuleCheckGoodCompilePathTypesText",
  "hostModuleCheckGoodCompilePathProgramText",
  "hostModuleCheckGoodCompilePathGraphText",
  "hostModuleCheckGoodCompilePathComposeText",
  "hostModuleCheckGoodCompilePathPlanText",
  "hostModuleCheckGoodCompilePathApplyText",
  "hostModuleCheckGoodCompilePathBodyText",
  "hostModuleCheckGoodCompilePathErasureText",
  "hostModuleCheckGoodCompilePathExtractText",
  ...
  "hostModuleCheckGoodFirstSurfaceText",
  "hostModuleCheckGoodLinearSubsetEmitText",
  "hostModuleCheckGoodTypesSubsetEmitText",
  "hostModuleCheckGoodEmitPlanText",
  "hostModuleCheckGoodEmitApplyText",
  "hostModuleCheckGoodEmitBodyText"
]

/-- All named defs are present in the token walk. -/
def toksHaveAllDefs : List String -> List String -> Bool
  | _, [] => true
  | toks, nm :: rest =>
    if toksHaveDefNamed liveSkipFuel toks nm then toksHaveAllDefs toks rest
    else false
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckFixtureTextsProductSource
  (src.splitOn needleNamespace).length == 2
    && (src.splitOn needlePerform).length == 2
    && (src.splitOn needleEmitBody).length == 2
    && (src.splitOn needleEnd).length == 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckFixtureTextsProductReady,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT,
    HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckFixtureTextsProductReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIXTURETEXTSPRODUCT_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT")
    && (liveRel == "HostModuleCheckFixtureTextsProduct.lean")
    && (liveHostModuleCheckFixtureTextsProductRel ==
        "src/systems/SystemsLean/HostModuleCheckFixtureTextsProduct.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckFixtureTextsProductFullHost
    && !hostFrontLiveHostModuleCheckFixtureTextsProductResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckFixtureTextsProductProvablyUnlocked
    && kernelCheckLiveHostModuleCheckFixtureTextsProductSource
        liveHostModuleCheckFixtureTextsProductSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckFixtureTextsProductSource "" with
  | FrontResult.reject _ => true
  | FrontResult.accept _ => false

/-! ### Driver (short banners; byte match). Not mill remill. Mill stays 69 of 69. -/

def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckFixtureTextsProductRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckFixtureTextsProductRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckFixtureTextsProductRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckFixtureTextsProductRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckFixtureTextsProductSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckFixtureTextsProduct.lean != liveHostModuleCheckFixtureTextsProductSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckFixtureTextsProduct.lean")
  let r := parseLiveHostModuleCheckFixtureTextsProductSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSPRODUCT ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckFixtureTextsProduct parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckFixtureTextsProduct parse false")
    unless hostFrontLiveHostModuleCheckFixtureTextsProductReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckFixtureTextsProductReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckFixtureTextsProductReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckFixtureTextsProduct source was accepted"
      throw (IO.userError
        "empty HostModuleCheckFixtureTextsProduct source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckFixtureTextsProduct.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsProduct
