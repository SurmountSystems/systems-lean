/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live CompilePathTypes.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveCompilePathTypesSource. Not occupancy name 50. Not mill 70.
  This wrap parses CompilePathTypes.lean only.
  Unique needles use trailing newline so HostFrontLiveCompilePathTypes is not a
  prefix hit on HostFrontLiveCompilePathTypesSource.
  Occupancy leftover is not this wrap.
  Unique needles (trailing newline so HostFrontLiveCompilePathTypes is not a prefix):
  HostFrontLiveCompilePathTypesSource
  PARSE-LIVE-COMPILE-PATH-TYPES
  HOST-FRONT-LIVE-COMPILE-PATH-TYPES
  COMPILE-PATH-TYPES
  Greppable: SYSTEMS_LEAN_HOST, liveCompilePathTypesSource,
  UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveCompilePathTypesSource
...
/-- Dual-pinned live CompilePathTypes.lean bytes (must match on-disk file).
    Greppable: liveCompilePathTypesSource, PARSE-LIVE-COMPILE-PATH-TYPES. -/
def liveCompilePathTypesSource : String := r#"/-
