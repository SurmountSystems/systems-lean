/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckKernelLinearTerm.lean.
  ...
  liveRel is HostModuleCheckKernelLinearTerm.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckKernelLinearTermSource,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermSource
...
def liveRel : String := "HostModuleCheckKernelLinearTerm.lean"

/-- Pinned live HostModuleCheckKernelLinearTerm.lean bytes. -/
def liveHostModuleCheckKernelLinearTermSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckKernelLinearTermSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckKernelLinearTermSourceReady : Bool :=
  liveRel == "HostModuleCheckKernelLinearTerm.lean"
    && kernelCheckLiveHostModuleCheckKernelLinearTermSource liveHostModuleCheckKernelLinearTermSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckKernelLinearTermSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckKernelLinearTerm.lean")
  let k := kernelCheckLiveHostModuleCheckKernelLinearTermSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckKernelLinearTerm parse false")
  unless hostFrontLiveHostModuleCheckKernelLinearTermSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermSource
