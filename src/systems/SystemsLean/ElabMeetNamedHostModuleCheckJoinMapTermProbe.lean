/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckJoinMapTerm named
  closed subset probe (plus-one after the CompilePathTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. JoinMapTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckJoinMapTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckProductOutKernelTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckJoinMapTerm subset:
    prior CompilePathTerm closed set plus
    SystemsLean.HostModuleCheckJoinMapTerm last.
    ...
    the CompilePathTerm compile order, then HostModuleCheckJoinMapTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathTerm): JoinMapTerm
    ...
    JoinMapTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckJoinMapTerm.
    HostModuleCheckJoinMapTerm probe lives here.
  - tryCompileNamedHostModuleCheckJoinMapTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckJoinMapTerm bad writes a temp
    HostModuleCheckJoinMapTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathTerm walker
    tryCompileNamedHostModuleCheckCompilePathTermSubset, and that this walk
    is not an alias of the CompilePathTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckJoinMapTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckJoinMapTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckJoinMapTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckJoinMapTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckJoinMapTermSubset,
  #elabMeetNamedHostModuleCheckJoinMapTermSubsetProbe,
  elabMeetNamedHostModuleCheckJoinMapTermSubsetProbe,
  SystemsLean.HostModuleCheckJoinMapTerm,
  ElabMeetNamedHostModuleCheckJoinMapTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckJoinMapTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckJoinMapTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckJoinMapTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckJoinMapTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathTerm plus the
    subset-surface compile closure and HostModuleCheckJoinMapTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckJoinMapTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathTerm walker
    tryCompileNamedHostModuleCheckCompilePathTermSubset still works; this
    walker is not an alias of the CompilePathTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckJoinMapTerm.lean. Do not skip to
    HostModuleCheckSelfHostTerm. -/
def elabMeetRunNamedHostModuleCheckJoinMapTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckJoinMapTerm? <-
    liftIO findLiveHostModuleCheckJoinMapTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckJoinMapTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckJoinMapTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckJoinMapTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckJoinMapTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckJoinMapTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckJoinMapTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckJoinMapTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckJoinMapTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckJoinMapTermSubset fakeLake fakeDir
  let notAliasOfCompilePathTerm :=
    namedClosedHostModuleCheckJoinMapTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckJoinMapTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckJoinMapTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckJoinMapTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckJoinMapTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckJoinMapTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckJoinMapTermSubsetProbe

#elabMeetNamedHostModuleCheckJoinMapTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckJoinMapTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckJoinMapTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckJoinMapTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckJoinMapTermSubset = true := rfl
