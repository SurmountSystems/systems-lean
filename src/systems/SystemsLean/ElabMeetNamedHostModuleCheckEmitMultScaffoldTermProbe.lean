/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckEmitMultScaffoldTerm named
  closed subset probe (plus-one after the ParityEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. EmitMultScaffoldTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckEmitMultScaffoldTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckParityProgramTerm, or
  ElabMeetNamedWalkHostModuleCheckParityEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckEmitMultScaffoldTerm subset:
    prior ParityEmitTerm closed set plus
    SystemsLean.HostModuleCheckEmitMultScaffoldTerm last.
    Membership list is barrel order among that closed set; skip
    Linear, skip IrGraph, skip HostGraphMain. Compile order is
    the ParityEmitTerm compile order, then HostModuleCheckEmitMultScaffoldTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ParityEmitTerm): EmitMultScaffoldTerm
    ...
    EmitMultScaffoldTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckEmitMultScaffoldTerm.
    HostModuleCheckEmitMultScaffoldTerm probe lives here.
  - tryCompileNamedHostModuleCheckEmitMultScaffoldTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckEmitMultScaffoldTerm bad writes a temp
    HostModuleCheckEmitMultScaffoldTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckParityEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ParityEmitTerm walker
    tryCompileNamedHostModuleCheckParityEmitTermSubset, and that this walk
    is not an alias of the ParityEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. Do not plant live
    HostModuleCheckEmitMultScaffoldTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckEmitMultScaffoldTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckEmitMultScaffoldTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckEmitMultScaffoldTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitMultScaffoldTermSubset,
  #elabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe,
  elabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe,
  SystemsLean.HostModuleCheckEmitMultScaffoldTerm,
  ElabMeetNamedHostModuleCheckEmitMultScaffoldTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckEmitMultScaffoldTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckEmitMultScaffoldTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckEmitMultScaffoldTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckEmitMultScaffoldTerm named subset.
    Good compiles live Mult through HostModuleCheckParityEmitTerm plus the
    subset-surface compile closure and HostModuleCheckEmitMultScaffoldTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckEmitMultScaffoldTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ParityEmitTerm walker
    tryCompileNamedHostModuleCheckParityEmitTermSubset still works; this
    walker is not an alias of the ParityEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. Do not
    plant live HostModuleCheckEmitMultScaffoldTerm.lean. Do not skip to
    HostModuleCheckEmitLinearScaffoldTerm. -/
def elabMeetRunNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckEmitMultScaffoldTerm? <-
    liftIO findLiveHostModuleCheckEmitMultScaffoldTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitMultScaffoldTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckEmitMultScaffoldTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckEmitMultScaffoldTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckEmitMultScaffoldTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckEmitMultScaffoldTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckParityEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckEmitMultScaffoldTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckEmitMultScaffoldTermSubset fakeLake fakeDir
  let notAliasOfParityEmitTerm :=
    namedClosedHostModuleCheckEmitMultScaffoldTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckEmitMultScaffoldTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckEmitMultScaffoldTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitMultScaffoldTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckEmitMultScaffoldTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe

#elabMeetNamedHostModuleCheckEmitMultScaffoldTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckEmitMultScaffoldTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckEmitMultScaffoldTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitMultScaffoldTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckEmitMultScaffoldTermSubset = true := rfl
