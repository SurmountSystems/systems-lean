/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathProgramTerm named
  closed subset probe (plus-one after the CompilePathTypesTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathProgramTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathProgramTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathLinearTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathTypesTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathProgramTerm subset:
    prior CompilePathTypesTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathProgramTerm last.
    ...
    the CompilePathTypesTerm compile order, then HostModuleCheckCompilePathProgramTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathTypesTerm): CompilePathProgramTerm
    ...
    CompilePathProgramTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathProgramTerm.
    HostModuleCheckCompilePathProgramTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathProgramTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathProgramTerm bad writes a temp
    HostModuleCheckCompilePathProgramTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathTypesTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathTypesTerm walker
    tryCompileNamedHostModuleCheckCompilePathTypesTermSubset, and that this walk
    is not an alias of the CompilePathTypesTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathProgramTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathProgramTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathProgramTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathProgramTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathProgramTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathProgramTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathProgramTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathProgramTerm,
  ElabMeetNamedHostModuleCheckCompilePathProgramTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathProgramTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathProgramTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathProgramTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathProgramTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathTypesTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathProgramTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathProgramTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathTypesTerm walker
    tryCompileNamedHostModuleCheckCompilePathTypesTermSubset still works; this
    walker is not an alias of the CompilePathTypesTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathProgramTerm.lean. Do not skip to
    HostModuleCheckCompilePathGraphTerm. -/
elab "#elabMeetNamedHostModuleCheckCompilePathProgramTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathProgramTerm? <-
    liftIO findLiveHostModuleCheckCompilePathProgramTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathProgramTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathProgramTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathProgramTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathProgramTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathProgramTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathProgramTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathProgramTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathTypesTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathProgramTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathProgramTermSubset fakeLake fakeDir
  let notAliasOfCompilePathTypesTerm :=
    namedClosedHostModuleCheckCompilePathProgramTermSubsetNames
      ...
      && namedClosedHostModuleCheckCompilePathProgramTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathProgramTerm"
      && !namedClosedHostModuleCheckCompilePathTypesTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathProgramTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfCompilePathTypesTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathProgramTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathProgramTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathProgramTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathProgramTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathProgramTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathProgramTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathProgramTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathProgramTermSubset = true := rfl
