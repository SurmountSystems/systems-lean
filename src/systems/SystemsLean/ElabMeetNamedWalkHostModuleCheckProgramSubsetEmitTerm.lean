/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckProgramSubsetEmitTerm named-walk
  wrappers (CheckersLater-role walk; TypesSubsetEmitTerm walk is the prior
  ...
  LinearSubsetEmitTerm walk, and TypesSubsetEmitTerm walk stay as
  ...
  ElabMeetNamedWalk. Prior TypesSubsetEmitTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckTypesSubsetEmitTerm. Do not grow that walk.
  ...
  FirstSurfaceTermProbe, LinearSubsetEmitTermProbe, or
  TypesSubsetEmitTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckProgramSubsetEmitTerm.lean.
  Do not skip to HostModuleCheckGraphSubsetEmitTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckProgramSubsetEmitTerm wrappers live here:
    ...
    Named membership is the prior TypesSubsetEmitTerm closed set plus
    HostModuleCheckProgramSubsetEmitTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the TypesSubsetEmitTerm compile
    order, then HostModuleCheckProgramSubsetEmitTerm last. EmitPlanTerm
    ...
    are the TypesSubsetEmitTerm compile order.
    HostModuleCheckProgramSubsetEmitTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckTypesSubsetEmitTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckProgramSubsetEmitTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckProgramSubsetEmitTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckProgramSubsetEmitTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckProgramSubsetEmitTermPath. -/
def findLiveHostModuleCheckProgramSubsetEmitTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckProgramSubsetEmitTerm"

/-- Named HostModuleCheckProgramSubsetEmitTerm subset of the live SystemsLean library.
    Membership is the prior TypesSubsetEmitTerm closed set plus
    HostModuleCheckProgramSubsetEmitTerm last.
    ...
    Greppable: namedClosedHostModuleCheckProgramSubsetEmitTermSubsetNames.
    SystemsLean.HostModuleCheckProgramSubsetEmitTerm. -/
def namedClosedHostModuleCheckProgramSubsetEmitTermSubsetNames : List String :=
  namedClosedHostModuleCheckTypesSubsetEmitTermSubsetNames ++
    ["SystemsLean.HostModuleCheckProgramSubsetEmitTerm"]

/-- Compile order honors imports: TypesSubsetEmitTerm compile order, then
    HostModuleCheckProgramSubsetEmitTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckProgramSubsetEmitTermCompileOrder. -/
def namedClosedHostModuleCheckProgramSubsetEmitTermCompileOrder : List String :=
  namedClosedHostModuleCheckTypesSubsetEmitTermCompileOrder ++
    ["SystemsLean.HostModuleCheckProgramSubsetEmitTerm"]

/-- True when the barrel lists each HostModuleCheckProgramSubsetEmitTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckProgramSubsetEmitTermSubset. -/
def barrelListsNamedHostModuleCheckProgramSubsetEmitTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckProgramSubsetEmitTermSubsetNames

/-- Compile the named HostModuleCheckProgramSubsetEmitTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the TypesSubsetEmitTerm
    compile order, then HostModuleCheckProgramSubsetEmitTerm last.
    Greppable: tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckProgramSubsetEmitTerm. -/
unsafe def tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckProgramSubsetEmitTermSubsetNames
    namedClosedHostModuleCheckProgramSubsetEmitTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubsetIO]
opaque tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckProgramSubsetEmitTerm subset.
    Greppable: tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset. -/
def tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the TypesSubsetEmitTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckProgramSubsetEmitTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckProgramSubsetEmitTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckTypesSubsetEmitTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckProgramSubsetEmitTermDepsIO]
opaque tryCompileAfterHostModuleCheckProgramSubsetEmitTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckProgramSubsetEmitTerm deps.
    Greppable: tryCompileAfterHostModuleCheckProgramSubsetEmitTermDeps. -/
def tryCompileAfterHostModuleCheckProgramSubsetEmitTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckProgramSubsetEmitTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
