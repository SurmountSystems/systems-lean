/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitTypesScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitTypesScaffoldTermSource turns live
    HostModuleCheckEmitTypesScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitTypesScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitTypesScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitTypesScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitTypesScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, five literal defs, and end.
    Kept count is 9.
    The five literal defs are checkDepthEmitTypesScaffoldTermSurfaceBar,
    checkDepthEmitTypesScaffoldTermSurfaceScope,
    hostModuleCheckEmitTypesScaffoldTermSurfaceDualOk,
    hostModuleCheckEmitTypesScaffoldTermSkeletonPrefix, and
  ...
  - The longest quoted span in this file is 60 characters
    (bodyHonestyOk fixture string). That is under 256.
  ...
  HostFrontLiveHostModuleCheckEmitTypesScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITTYPESSCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitTypesScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitTypesScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady,
  liveHostModuleCheckEmitTypesScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitTypesScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitTypesScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitTypesScaffoldTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitTypesScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitTypesScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitTypesScaffoldTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitTypesScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitTypesScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitTypesScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- String take fuel. The longest quoted span in this file is 60 characters.
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitTypesScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitTypesScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitTypesScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitTypesScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitTypesScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitTypesScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitTypesScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitTypesScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitTypesScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitTypesScaffoldTermSource
      liveHostModuleCheckEmitTypesScaffoldTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitTypesScaffoldTermSurfaceBar",
      "checkDepthEmitTypesScaffoldTermSurfaceScope",
      "hostModuleCheckEmitTypesScaffoldTermSurfaceDualOk",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitTypesScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitTypesScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitTypesScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitTypesScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitTypesScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitTypesScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitTypesScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitTypesScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitTypesScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitTypesScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitTypesScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitTypesScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitTypesScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitTypesScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_types_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitTypesScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITTYPESSCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitTypesScaffoldTerm.lean")
    && (liveHostModuleCheckEmitTypesScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitTypesScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitTypesScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitTypesScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitTypesScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitTypesScaffoldTermSource
        liveHostModuleCheckEmitTypesScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitTypesScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitTypesScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitTypesScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitTypesScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitTypesScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitTypesScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitTypesScaffoldTerm.lean != liveHostModuleCheckEmitTypesScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitTypesScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitTypesScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITTYPESSCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitTypesScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitTypesScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitTypesScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitTypesScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitTypesScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitTypesScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
