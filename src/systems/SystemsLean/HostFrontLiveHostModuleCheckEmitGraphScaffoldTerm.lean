/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitGraphScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitGraphScaffoldTermSource turns live
    HostModuleCheckEmitGraphScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitGraphScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitGraphScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitGraphScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitGraphScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, five literal defs, and end.
    Kept count is 9.
    The five literal defs are checkDepthEmitGraphScaffoldTermSurfaceBar,
    checkDepthEmitGraphScaffoldTermSurfaceScope,
    hostModuleCheckEmitGraphScaffoldTermSurfaceDualOk,
    hostModuleCheckEmitGraphScaffoldTermSkeletonPrefix, and
  ...
  HostFrontLiveHostModuleCheckEmitGraphScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitGraphScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitGraphScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady,
  liveHostModuleCheckEmitGraphScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitGraphScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitGraphScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitGraphScaffoldTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitGraphScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitGraphScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitGraphScaffoldTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitGraphScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitGraphScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitGraphScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitGraphScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitGraphScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitGraphScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitGraphScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitGraphScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitGraphScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitGraphScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitGraphScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitGraphScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitGraphScaffoldTermSource
      liveHostModuleCheckEmitGraphScaffoldTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitGraphScaffoldTermSurfaceBar",
      "checkDepthEmitGraphScaffoldTermSurfaceScope",
      "hostModuleCheckEmitGraphScaffoldTermSurfaceDualOk",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitGraphScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitGraphScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitGraphScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitGraphScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitGraphScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitGraphScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitGraphScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitGraphScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitGraphScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitGraphScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitGraphScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitGraphScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitGraphScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitGraphScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_graph_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitGraphScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitGraphScaffoldTerm.lean")
    && (liveHostModuleCheckEmitGraphScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitGraphScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitGraphScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitGraphScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitGraphScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitGraphScaffoldTermSource
        liveHostModuleCheckEmitGraphScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitGraphScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitGraphScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitGraphScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitGraphScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitGraphScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitGraphScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitGraphScaffoldTerm.lean != liveHostModuleCheckEmitGraphScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitGraphScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitGraphScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITGRAPHSCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitGraphScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitGraphScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitGraphScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitGraphScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitGraphScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitGraphScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
