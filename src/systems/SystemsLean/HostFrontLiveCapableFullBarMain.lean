/-
  ...
  Short role: HostFrontLiveCapableFullBarMain. Reuses HostFrontLiveHostTerm
  ...
  prefix hit on HostFrontLiveCapableFullBarMainSource or HostFrontLiveCapableFullBarMainMain.
  ...
  Spec (readable):
  - parseLiveCapableFullBarMainSource turns live CapableFullBarMain.lean
    text into HostTerm.Module.
  - Module name is SystemsLean.CapableFullBarMain even without a module line.
  - kernelCheckLiveCapableFullBarMainSource is HostKernel.kernelCheck of
    that parse.
  ...
  - This live file has no living namespace command. Do not require a
    CapableFullBarMain namespace pin as true.
  - Wrap-assigned module lastSeg is CapableFullBarMain.
  ...
  - Not CapableFullBar.lean wrap. Not CapableReadMain.lean wrap.
  ...
  HostFrontLiveCapableFullBarMain
  ...
  hostFrontLiveCapableFullBarMainReady, liveCapableFullBarMainSource,
  liveCapableFullBarMainRel, UNIT_SURFACE host surface, MULT-0,
  liveParseDoesNotUseMultFixture.
  Module: SystemsLean.HostFrontLiveCapableFullBarMain
  Red/green: dests-skipped until barrel; lake build
  SystemsLean.HostFrontLiveCapableFullBarMain on surmount-1 (queued).
...
/-- Strip comments; keep dash-dash and block-open inside string payloads.
    CapableFullBarMain has no string literals; this strip stays in-string safe.
    HostFrontLiveMult.stripComments is not in-string safe. Backslash
    keeps the next char inside a string. -/
def stripCommentsCfbN (fuel nest : Nat) (lineC inStr : Bool)
  ...
  | c :: rest =>
    ...
    | Nat.succ n =>
      if lineC then
        if c == '\n' then
          stripCommentsCfbN n nest false false ('\n' :: acc) rest
        else
          stripCommentsCfbN n nest true false acc rest
      else if inStr then
        if c == '\\' then
          match rest with
          | d :: rest2 =>
            stripCommentsCfbN n nest false true (d :: c :: acc) rest2
          | [] => (c :: acc).reverse
        else if c == '"' then
          stripCommentsCfbN n nest false false ('"' :: acc) rest
        else
          stripCommentsCfbN n nest false true (c :: acc) rest
      else if nest > 0 then
        match c, rest with
        | '/', '-' :: rest2 =>
          stripCommentsCfbN n (nest + 1) false false acc rest2
        | '-', '/' :: rest2 =>
          stripCommentsCfbN n (nest - 1) false false acc rest2
        | '\n', rest2 =>
          stripCommentsCfbN n nest false false ('\n' :: acc) rest2
        | _, rest2 =>
          stripCommentsCfbN n nest false false acc rest2
      else
        match c, rest with
        | '"', rest2 =>
          stripCommentsCfbN n 0 false true ('"' :: acc) rest2
        | '/', '-' :: rest2 =>
          stripCommentsCfbN n 1 false false acc rest2
        | '-', '-' :: rest2 =>
          stripCommentsCfbN n 0 true false acc rest2
        | _, rest2 =>
          stripCommentsCfbN n 0 false false (c :: acc) rest2

/-- String-safe comment strip for live CapableFullBarMain.lean bytes. -/
def stripCommentsCfb (src : String) : String :=
  String.ofList (stripCommentsCfbN (src.length + 8) 0 false false [] src.toList)
...
def parseDefCapableFullBarMain (fuel : Nat) (dname : String) (rest : List String) :
        ...
        | DefBodyKind.assign =>
          ...
          | some retTy =>
            ...
            | some (body, rest4) =>
              ...
              | t :: _ =>
                if isCmdKw t then
                  let rest5 := skipNonCmd liveCapableFullBarMainSkipFuel rest4
                  if bs.isEmpty then
...
def parseOneCmdCapableFullBarMain (fuel : Nat) (toks : List String) :
  ...
  | "def" :: rest =>
    ...
    | some (dname, rest2) => parseDefCapableFullBarMain fuel dname rest2
    | none => none
  | _ => none
...
def parseLiveCapableFullBarMainSource (src : String) : FrontResult :=
  let toks := tokenizeHostTerm (stripCommentsCfb src)
...
/-- Kernel-check live CapableFullBarMain parse. Not a fixture.
    Not occupancy Term. Greppable: kernelCheckLiveCapableFullBarMainSource,
    PARSE-LIVE-CAPABLE-FULL-BAR-MAIN. -/
...
/-- Live parse command count (dotted import kept; IO main skip-folded).
    ...
    measured cmds=1: the one dotted import SystemsLean.CapableFullBar.
    Do not copy DualEq bound 59 or CapableRead import of CapableRead.
...
def liveParseHasCapableFullBarMainModule : Bool :=
  match liveCapableFullBarMainParsed? with
  | none => false
  | some m => lastSeg m.name.raw == "CapableFullBarMain"

/-- Live parse kept the one dotted import SystemsLean.CapableFullBar. -/
...
def liveParseHasCoreDefs : Bool :=
  ...
  | none => false
  | some _ =>
    let toks := tokenizeHostTerm (stripCommentsCfb liveCapableFullBarMainSource)
    toksHaveDefNamed liveCapableFullBarMainSkipFuel toks "main"
...
def runLiveCapableFullBarMain (root : System.FilePath) : IO Unit := do
  ...
  | FrontResult.accept m =>
    ...
    IO.println s!"GREEN {stageId}: live CapableFullBarMain.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
