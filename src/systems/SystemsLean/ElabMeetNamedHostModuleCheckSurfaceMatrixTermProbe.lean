/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckSurfaceMatrixTerm named
  closed subset probe (plus-one after the SelfHostTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. SurfaceMatrixTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckSurfaceMatrixTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckJoinMapTerm, or
  ElabMeetNamedWalkHostModuleCheckSelfHostTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckSurfaceMatrixTerm subset:
    prior SelfHostTerm closed set plus
    SystemsLean.HostModuleCheckSurfaceMatrixTerm last.
    ...
    the SelfHostTerm compile order, then HostModuleCheckSurfaceMatrixTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as SelfHostTerm): SurfaceMatrixTerm
    ...
    SurfaceMatrixTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckSurfaceMatrixTerm.
    HostModuleCheckSurfaceMatrixTerm probe lives here.
  - tryCompileNamedHostModuleCheckSurfaceMatrixTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckSurfaceMatrixTerm bad writes a temp
    HostModuleCheckSurfaceMatrixTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckSelfHostTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior SelfHostTerm walker
    tryCompileNamedHostModuleCheckSelfHostTermSubset, and that this walk
    is not an alias of the SelfHostTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckSurfaceMatrixTerm.lean. Do not skip to
  ...
  SystemsLean.ElabMeetNamedHostModuleCheckSurfaceMatrixTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckSurfaceMatrixTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckSurfaceMatrixTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckSurfaceMatrixTerm named subset.
    Good compiles live Mult through HostModuleCheckSelfHostTerm plus the
    subset-surface compile closure and HostModuleCheckSurfaceMatrixTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckSurfaceMatrixTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior SelfHostTerm walker
    tryCompileNamedHostModuleCheckSelfHostTermSubset still works; this
    walker is not an alias of the SelfHostTerm closed set. Drive is
    good && !bad && isolation.
...
elab "#elabMeetNamedHostModuleCheckSurfaceMatrixTermSubsetProbe" : command => do
  ...
  let liveHostModuleCheckSurfaceMatrixTerm? <-
    liftIO findLiveHostModuleCheckSurfaceMatrixTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSurfaceMatrixTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckSurfaceMatrixTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckSurfaceMatrixTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckSurfaceMatrixTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckSurfaceMatrixTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckSurfaceMatrixTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckSurfaceMatrixTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSelfHostTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckSurfaceMatrixTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckSurfaceMatrixTermSubset fakeLake fakeDir
  let notAliasOfSelfHostTerm :=
    namedClosedHostModuleCheckSurfaceMatrixTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckSurfaceMatrixTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckSurfaceMatrixTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckSurfaceMatrixTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckSurfaceMatrixTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckSurfaceMatrixTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckSurfaceMatrixTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckSurfaceMatrixTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckSurfaceMatrixTermSubset = true := rfl
