/-
  ...
  src/systems/SystemsLean/HostModuleCheckParityLinearTerm.lean with the
  ...
  HOST-FRONT-LIVE-HOSTMODULECHECKPARITYLINEARTERM,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPARITYLINEARTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM, liveRel,
  kernelCheckLiveHostModuleCheckParityLinearTermSource,
  hostFrontLiveHostModuleCheckParityLinearTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckParityLinearTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckParityLinearTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckParityLinearTerm
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckParityLinearTerm.lean"
...
def hostFrontLiveHostModuleCheckParityLinearTermFullHost :
    Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckParityLinearTermFullBackend :
    Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckParityLinearTermResidualFreeClaimed :
    Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckParityLinearTermProvablyUnlocked :
    Bool := false
...
/-- Parse live HostModuleCheckParityLinearTerm.lean text.
    Greppable: parseLiveHostModuleCheckParityLinearTermSource,
    PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM. -/
def parseLiveHostModuleCheckParityLinearTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name :=
              HostTerm.n "SystemsLean.HostModuleCheckParityLinearTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckParityLinearTermSource,
    PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM. -/
def kernelCheckLiveHostModuleCheckParityLinearTermSource
    (src : String) : Bool :=
  match parseLiveHostModuleCheckParityLinearTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckParityLinearTermSource
      liveHostModuleCheckParityLinearTermSource with
...
/-- Kept String def checkDepthParityLinearTermSurfaceBar is TERM-SURFACE. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthParityLinearTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Kept String def checkDepthParityLinearTermSurfaceScope is
    ParityLinear-only. -/
def liveParseHasScopeLit : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthParityLinearTermSurfaceScope"
          && s == "ParityLinear-only"
      | _ => false
...
/-- Wrap module lastSeg is HostModuleCheckParityLinearTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckParityLinearTerm"
      && lastSeg m.name.raw == "HostModuleCheckParityLinearTerm"
...
def needleBarDef : String := "def checkDepthParityLinearTermSurfaceBar"
...
def needleScope : String := "ParityLinear-only"

/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Two imports. Bar is TERM-SURFACE. Scope is ParityLinear-only. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckParityLinearTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckParityLinearTermReady,
    PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKPARITYLINEARTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckParityLinearTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPARITYLINEARTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPARITYLINEARTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM")
    && (liveRel == "HostModuleCheckParityLinearTerm.lean")
    && (liveHostModuleCheckParityLinearTermRel ==
        "src/systems/SystemsLean/HostModuleCheckParityLinearTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckParityLinearTermFullHost
    && !hostFrontLiveHostModuleCheckParityLinearTermFullBackend
    && !hostFrontLiveHostModuleCheckParityLinearTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckParityLinearTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckParityLinearTermSource
        liveHostModuleCheckParityLinearTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckParityLinearTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckParityLinearTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckParityLinearTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckParityLinearTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckParityLinearTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckParityLinearTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckParityLinearTerm.lean != liveHostModuleCheckParityLinearTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckParityLinearTerm.lean")
  let r := parseLiveHostModuleCheckParityLinearTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKPARITYLINEARTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless hostFrontLiveHostModuleCheckParityLinearTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckParityLinearTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckParityLinearTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckParityLinearTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckParityLinearTerm source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckParityLinearTerm
