/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathMultTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathMultTermSource turns live
    HostModuleCheckCompilePathMultTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathMultTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathMultTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathMultTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathMultTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathMultTermDialect and
    def hostModuleCheckCompilePathMultTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathMultTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHMULTTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathMultTermSource,
  kernelCheckLiveHostModuleCheckCompilePathMultTermSource,
  hostFrontLiveHostModuleCheckCompilePathMultTermReady,
  liveHostModuleCheckCompilePathMultTermSource,
  liveRel, liveHostModuleCheckCompilePathMultTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathMultTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathMultTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathMultTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathMultTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathMultTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathMultTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathMultTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathMultTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathMultTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathMultTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathMultTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathMultTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathMultTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM. -/
def parseLiveHostModuleCheckCompilePathMultTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathMultTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathMultTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathMultTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathMultTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathMultTermSource
      liveHostModuleCheckCompilePathMultTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathMultTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathMultTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathMultTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathMultTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathMultTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathMultTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathMultTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathMultTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHMULTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM")
    && (liveRel == "HostModuleCheckCompilePathMultTerm.lean")
    && (liveHostModuleCheckCompilePathMultTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathMultTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathMultTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathMultTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathMultTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathMultTermSource
        liveHostModuleCheckCompilePathMultTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathMultTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathMultTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathMultTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathMultTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathMultTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathMultTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathMultTerm.lean != liveHostModuleCheckCompilePathMultTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathMultTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathMultTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHMULTTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathMultTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathMultTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathMultTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathMultTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathMultTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathMultTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathMultTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathMultTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
