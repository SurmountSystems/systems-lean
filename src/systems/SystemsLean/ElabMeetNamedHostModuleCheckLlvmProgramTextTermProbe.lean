/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLlvmProgramTextTerm named
  closed subset probe (plus-one after the HostPackageWriteTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LlvmProgramTextTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLlvmProgramTextTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckLlvmProgramTextTerm subset:
    prior HostPackageWriteTerm closed set plus
    SystemsLean.HostModuleCheckLlvmProgramTextTerm last.
    ...
    the HostPackageWriteTerm compile order, then HostModuleCheckLlvmProgramTextTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as HostPackageWriteTerm): LlvmProgramTextTerm
    ...
    LlvmProgramTextTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLlvmProgramTextTerm.
    HostModuleCheckLlvmProgramTextTerm probe lives here.
  - tryCompileNamedHostModuleCheckLlvmProgramTextTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLlvmProgramTextTerm bad writes a temp
    HostModuleCheckLlvmProgramTextTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckHostPackageWriteTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior HostPackageWriteTerm walker
    tryCompileNamedHostModuleCheckHostPackageWriteTermSubset, and that this walk
    is not an alias of the HostPackageWriteTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLlvmProgramTextTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckLlvmProgramTextTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLlvmProgramTextTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckLlvmProgramTextTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmProgramTextTermSubset,
  #elabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetProbe,
  elabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetProbe,
  SystemsLean.HostModuleCheckLlvmProgramTextTerm,
  ElabMeetNamedHostModuleCheckLlvmProgramTextTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLlvmProgramTextTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLlvmProgramTextTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLlvmProgramTextTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLlvmProgramTextTerm named subset.
    Good compiles live Mult through HostModuleCheckHostPackageWriteTerm plus the
    subset-surface compile closure and HostModuleCheckLlvmProgramTextTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLlvmProgramTextTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior HostPackageWriteTerm walker
    tryCompileNamedHostModuleCheckHostPackageWriteTermSubset still works; this
    walker is not an alias of the HostPackageWriteTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLlvmProgramTextTerm.lean. Do not skip to
    HostModuleCheckLlvmGraphTextTerm. -/
def elabMeetRunNamedHostModuleCheckLlvmProgramTextTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLlvmProgramTextTerm? <-
    liftIO findLiveHostModuleCheckLlvmProgramTextTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLlvmProgramTextTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLlvmProgramTextTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLlvmProgramTextTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLlvmProgramTextTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLlvmProgramTextTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckHostPackageWriteTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLlvmProgramTextTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLlvmProgramTextTermSubset fakeLake fakeDir
  let notAliasOfHostPackageWriteTerm :=
    namedClosedHostModuleCheckLlvmProgramTextTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfHostPackageWriteTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLlvmProgramTextTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLlvmProgramTextTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmProgramTextTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLlvmProgramTextTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckLlvmProgramTextTermSubsetProbe

#elabMeetNamedHostModuleCheckLlvmProgramTextTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckLlvmProgramTextTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLlvmProgramTextTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmProgramTextTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLlvmProgramTextTermSubset = true := rfl
