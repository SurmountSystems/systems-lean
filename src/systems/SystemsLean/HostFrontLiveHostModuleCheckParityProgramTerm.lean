/-
  ...
  src/systems/SystemsLean/HostModuleCheckParityProgramTerm.lean with the
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPARITYPROGRAMTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM, liveRel,
  kernelCheckLiveHostModuleCheckParityProgramTermSource,
  hostFrontLiveHostModuleCheckParityProgramTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckParityProgramTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckParityProgramTermSource
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckParityProgramTerm.lean"
...
def hostFrontLiveHostModuleCheckParityProgramTermFullHost :
    Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckParityProgramTermFullBackend :
    Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckParityProgramTermResidualFreeClaimed :
    Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckParityProgramTermProvablyUnlocked :
    Bool := false
...
/-- Parse live HostModuleCheckParityProgramTerm.lean text.
    Greppable: parseLiveHostModuleCheckParityProgramTermSource,
    PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM. -/
def parseLiveHostModuleCheckParityProgramTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name :=
              HostTerm.n "SystemsLean.HostModuleCheckParityProgramTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckParityProgramTermSource,
    PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM. -/
def kernelCheckLiveHostModuleCheckParityProgramTermSource
    (src : String) : Bool :=
  match parseLiveHostModuleCheckParityProgramTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckParityProgramTermSource
      liveHostModuleCheckParityProgramTermSource with
...
/-- Kept String def checkDepthParityProgramTermSurfaceBar is TERM-SURFACE. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthParityProgramTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Kept String def checkDepthParityProgramTermSurfaceScope is
    ParityProgram-only. -/
def liveParseHasScopeLit : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthParityProgramTermSurfaceScope"
          && s == "ParityProgram-only"
      | _ => false
...
/-- Wrap module lastSeg is HostModuleCheckParityProgramTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckParityProgramTerm"
      && lastSeg m.name.raw == "HostModuleCheckParityProgramTerm"
...
def needleBarDef : String := "def checkDepthParityProgramTermSurfaceBar"
...
def needleScope : String := "ParityProgram-only"

/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Two imports. Bar is TERM-SURFACE. Scope is ParityProgram-only. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckParityProgramTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckParityProgramTermReady,
    PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckParityProgramTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPARITYPROGRAMTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM")
    && (liveRel == "HostModuleCheckParityProgramTerm.lean")
    && (liveHostModuleCheckParityProgramTermRel ==
        "src/systems/SystemsLean/HostModuleCheckParityProgramTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckParityProgramTermFullHost
    && !hostFrontLiveHostModuleCheckParityProgramTermFullBackend
    && !hostFrontLiveHostModuleCheckParityProgramTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckParityProgramTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckParityProgramTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckParityProgramTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckParityProgramTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckParityProgramTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckParityProgramTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckParityProgramTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckParityProgramTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckParityProgramTerm.lean != liveHostModuleCheckParityProgramTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckParityProgramTerm.lean")
  let r := parseLiveHostModuleCheckParityProgramTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKPARITYPROGRAMTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless hostFrontLiveHostModuleCheckParityProgramTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckParityProgramTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckParityProgramTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckParityProgramTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckParityProgramTerm source was accepted")
