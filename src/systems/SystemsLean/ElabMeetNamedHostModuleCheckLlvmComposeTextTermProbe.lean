/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLlvmComposeTextTerm named
  closed subset probe (plus-one after the LlvmGraphTextTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LlvmComposeTextTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLlvmComposeTextTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckLlvmComposeTextTerm subset:
    prior LlvmGraphTextTerm closed set plus
    SystemsLean.HostModuleCheckLlvmComposeTextTerm last.
    ...
    the LlvmGraphTextTerm compile order, then HostModuleCheckLlvmComposeTextTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as LlvmGraphTextTerm): LlvmComposeTextTerm
    ...
    LlvmComposeTextTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLlvmComposeTextTerm.
    HostModuleCheckLlvmComposeTextTerm probe lives here.
  - tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLlvmComposeTextTerm bad writes a temp
    HostModuleCheckLlvmComposeTextTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckLlvmGraphTextTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior LlvmGraphTextTerm walker
    tryCompileNamedHostModuleCheckLlvmGraphTextTermSubset, and that this walk
    is not an alias of the LlvmGraphTextTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLlvmComposeTextTerm.lean. Do not skip to
  ...
  SystemsLean.ElabMeetNamedHostModuleCheckLlvmComposeTextTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLlvmComposeTextTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLlvmComposeTextTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLlvmComposeTextTerm named subset.
    Good compiles live Mult through HostModuleCheckLlvmGraphTextTerm plus the
    subset-surface compile closure and HostModuleCheckLlvmComposeTextTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLlvmComposeTextTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior LlvmGraphTextTerm walker
    tryCompileNamedHostModuleCheckLlvmGraphTextTermSubset still works; this
    walker is not an alias of the LlvmGraphTextTerm closed set. Drive is
    good && !bad && isolation.
...
elab "#elabMeetNamedHostModuleCheckLlvmComposeTextTermSubsetProbe" : command => do
  ...
  let liveHostModuleCheckLlvmComposeTextTerm? <-
    liftIO findLiveHostModuleCheckLlvmComposeTextTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLlvmComposeTextTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLlvmComposeTextTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLlvmComposeTextTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLlvmComposeTextTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLlvmComposeTextTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLlvmComposeTextTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLlvmGraphTextTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset fakeLake fakeDir
  let notAliasOfLlvmGraphTextTerm :=
    namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLlvmComposeTextTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLlvmComposeTextTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmComposeTextTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLlvmComposeTextTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckLlvmComposeTextTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLlvmComposeTextTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmComposeTextTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLlvmComposeTextTermSubset = true := rfl
