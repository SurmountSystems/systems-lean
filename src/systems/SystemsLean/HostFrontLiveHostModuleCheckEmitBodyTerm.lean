/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitBodyTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitBodyTermSource turns live
    HostModuleCheckEmitBodyTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitBodyTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitBodyTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitBodyTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitBodyTermSource. Not split.
  - Real commands in this file: two imports, namespace
    ...
    checkDepthEmitBodyTermSurfaceBar,
    checkDepthEmitBodyTermSurfaceScope,
    hostModuleCheckEmitBodyTermSurfaceDualOk, and
    hostModuleCheckEmitBodyTermSkeletonSuffix.
    Twenty-five defs are skip-folded (binders, concatenation, match,
    or application). Two theorems are skip-folded.
    Skipped command count is 27. Kept plus skipped is 35.
  ...
  - Skip-head still sees the literal defs and the skip-folded
    emitBodyFailClosedBodyOk, checkEmitBodyTermDialect, and
    hostModuleCheckEmitBodyTermSurfaceOk defs in the live text.
  ...
  HostFrontLiveHostModuleCheckEmitBodyTerm
  PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITBODYTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITBODYTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitBodyTermSource,
  kernelCheckLiveHostModuleCheckEmitBodyTermSource,
  hostFrontLiveHostModuleCheckEmitBodyTermReady,
  liveHostModuleCheckEmitBodyTermSource,
  liveRel, liveHostModuleCheckEmitBodyTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitBodyTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitBodyTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckEmitBodyTerm
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITBODYTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitBodyTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitBodyTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitBodyTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitBodyTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitBodyTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitBodyTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: two imports, namespace, four literal defs, and end.
    Twenty-five defs and two theorems are skip-folded.
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitBodyTerm has dash-dash inside the header block
...
/-- String-safe comment strip for live HostModuleCheckEmitBodyTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckEmitBodyTerm`. -/
...
/-- Parse live HostModuleCheckEmitBodyTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitBodyTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM. -/
def parseLiveHostModuleCheckEmitBodyTermSource (src : String) : FrontResult :=
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitBodyTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM. -/
def kernelCheckLiveHostModuleCheckEmitBodyTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitBodyTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitBodyTermSource
      liveHostModuleCheckEmitBodyTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckEmitBodyTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitBodyTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitBodyTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckEmitBodyTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitBodyTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitBodyTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks
          "hostModuleCheckEmitBodyTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks
          "hostModuleCheckEmitBodyTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "emitBodyFailClosedBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitBodyTermDialect"
      && toksHaveDefNamed liveSkipFuel toks
...
/-- emitBodyFailClosedBodyOk has a binder, so the def is skip-folded. -/
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitBodyTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitBodyTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITBODYTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitBodyTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITBODYTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITBODYTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM")
    && (liveRel == "HostModuleCheckEmitBodyTerm.lean")
    && (liveHostModuleCheckEmitBodyTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitBodyTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitBodyTermFullHost
    && !hostFrontLiveHostModuleCheckEmitBodyTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitBodyTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitBodyTermSource
        liveHostModuleCheckEmitBodyTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitBodyTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitBodyTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitBodyTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitBodyTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitBodyTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitBodyTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitBodyTerm.lean != liveHostModuleCheckEmitBodyTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitBodyTerm.lean")
  let r := parseLiveHostModuleCheckEmitBodyTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITBODYTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      ...
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitBodyTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitBodyTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitBodyTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitBodyTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitBodyTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitBodyTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitBodyTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckEmitBodyTerm
