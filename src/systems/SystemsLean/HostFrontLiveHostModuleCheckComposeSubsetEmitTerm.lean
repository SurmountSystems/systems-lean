/-
  ...
  Short role: HostFrontLiveHostModuleCheckComposeSubsetEmitTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckComposeSubsetEmitTermSource turns live
    HostModuleCheckComposeSubsetEmitTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckComposeSubsetEmitTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckComposeSubsetEmitTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckComposeSubsetEmitTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckComposeSubsetEmitTermSource. Not split.
  ...
  - Skip-head still sees def checkComposeSubsetEmitTermDialect and
    def hostModuleCheckComposeSubsetEmitTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckComposeSubsetEmitTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPOSESUBSETEMITTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckComposeSubsetEmitTermSource,
  kernelCheckLiveHostModuleCheckComposeSubsetEmitTermSource,
  hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady,
  liveHostModuleCheckComposeSubsetEmitTermSource,
  liveRel, liveHostModuleCheckComposeSubsetEmitTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckComposeSubsetEmitTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckComposeSubsetEmitTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckComposeSubsetEmitTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckComposeSubsetEmitTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckComposeSubsetEmitTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckComposeSubsetEmitTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckComposeSubsetEmitTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckComposeSubsetEmitTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckComposeSubsetEmitTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckComposeSubsetEmitTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckComposeSubsetEmitTerm`. -/
...
/-- Parse live HostModuleCheckComposeSubsetEmitTerm.lean text.
    Greppable: parseLiveHostModuleCheckComposeSubsetEmitTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM. -/
def parseLiveHostModuleCheckComposeSubsetEmitTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckComposeSubsetEmitTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckComposeSubsetEmitTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM. -/
def kernelCheckLiveHostModuleCheckComposeSubsetEmitTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckComposeSubsetEmitTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckComposeSubsetEmitTermSource
      liveHostModuleCheckComposeSubsetEmitTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckComposeSubsetEmitTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckComposeSubsetEmitTerm"
      && lastSeg m.name.raw == "HostModuleCheckComposeSubsetEmitTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckComposeSubsetEmitTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkComposeSubsetEmitTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckComposeSubsetEmitTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPOSESUBSETEMITTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM")
    && (liveRel == "HostModuleCheckComposeSubsetEmitTerm.lean")
    && (liveHostModuleCheckComposeSubsetEmitTermRel ==
        "src/systems/SystemsLean/HostModuleCheckComposeSubsetEmitTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckComposeSubsetEmitTermFullHost
    && !hostFrontLiveHostModuleCheckComposeSubsetEmitTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckComposeSubsetEmitTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckComposeSubsetEmitTermSource
        liveHostModuleCheckComposeSubsetEmitTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckComposeSubsetEmitTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckComposeSubsetEmitTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckComposeSubsetEmitTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckComposeSubsetEmitTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckComposeSubsetEmitTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckComposeSubsetEmitTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckComposeSubsetEmitTerm.lean != liveHostModuleCheckComposeSubsetEmitTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckComposeSubsetEmitTerm.lean")
  let r := parseLiveHostModuleCheckComposeSubsetEmitTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPOSESUBSETEMITTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckComposeSubsetEmitTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckComposeSubsetEmitTerm parse false")
    unless hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckComposeSubsetEmitTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckComposeSubsetEmitTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckComposeSubsetEmitTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckComposeSubsetEmitTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
