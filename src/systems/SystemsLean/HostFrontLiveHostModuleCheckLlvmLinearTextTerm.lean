/-
  ...
  src/systems/SystemsLean/HostModuleCheckLlvmLinearTextTerm.lean with the
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKLLVMLINEARTEXTTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM, liveRel,
  kernelCheckLiveHostModuleCheckLlvmLinearTextTermSource,
  hostFrontLiveHostModuleCheckLlvmLinearTextTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckLlvmLinearTextTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckLlvmLinearTextTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckLlvmLinearTextTerm.lean"
...
def hostFrontLiveHostModuleCheckLlvmLinearTextTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckLlvmLinearTextTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckLlvmLinearTextTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckLlvmLinearTextTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count printed by the closed run (cmds=11).
    From HostModuleCheckLlvmLinearTextTerm.lean. -/
...
/-- Parse live HostModuleCheckLlvmLinearTextTerm.lean text.
    Greppable: parseLiveHostModuleCheckLlvmLinearTextTermSource,
    PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM. -/
def parseLiveHostModuleCheckLlvmLinearTextTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckLlvmLinearTextTerm"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckLlvmLinearTextTermSource,
    PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM. -/
def kernelCheckLiveHostModuleCheckLlvmLinearTextTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckLlvmLinearTextTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckLlvmLinearTextTermSource
      liveHostModuleCheckLlvmLinearTextTermSource with
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthLlvmLinearTextTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Scope literal is LlvmLinearText-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthLlvmLinearTextTermSurfaceScope"
          && s == "LlvmLinearText-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckLlvmLinearTextTermSurfaceDualOk" && b == true
      | _ => false
...
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckLlvmLinearTextTermSkeletonSuffix"
          && s == "end SystemsLean.LlvmLinearText\\n"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckLlvmLinearTextTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckLlvmLinearTextTerm"
      && lastSeg m.name.raw == "HostModuleCheckLlvmLinearTextTerm"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckLlvmLinearTextTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckLlvmLinearTextTermReady,
    PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckLlvmLinearTextTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKLLVMLINEARTEXTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM")
    && (liveRel == "HostModuleCheckLlvmLinearTextTerm.lean")
    && (liveHostModuleCheckLlvmLinearTextTermRel ==
        "src/systems/SystemsLean/HostModuleCheckLlvmLinearTextTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckLlvmLinearTextTermFullHost
    && !hostFrontLiveHostModuleCheckLlvmLinearTextTermFullBackend
    && !hostFrontLiveHostModuleCheckLlvmLinearTextTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckLlvmLinearTextTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckLlvmLinearTextTermSource
        liveHostModuleCheckLlvmLinearTextTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckLlvmLinearTextTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckLlvmLinearTextTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckLlvmLinearTextTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckLlvmLinearTextTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckLlvmLinearTextTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckLlvmLinearTextTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckLlvmLinearTextTerm.lean != liveHostModuleCheckLlvmLinearTextTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckLlvmLinearTextTerm.lean")
  let r := parseLiveHostModuleCheckLlvmLinearTextTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKLLVMLINEARTEXTTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckLlvmLinearTextTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckLlvmLinearTextTerm parse false")
    unless hostFrontLiveHostModuleCheckLlvmLinearTextTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckLlvmLinearTextTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckLlvmLinearTextTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckLlvmLinearTextTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckLlvmLinearTextTerm source was accepted")
