/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathPlanTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathPlanTermSource turns live
    HostModuleCheckCompilePathPlanTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathPlanTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathPlanTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathPlanTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathPlanTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathPlanTermDialect and
    def hostModuleCheckCompilePathPlanTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathPlanTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHPLANTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathPlanTermSource,
  kernelCheckLiveHostModuleCheckCompilePathPlanTermSource,
  hostFrontLiveHostModuleCheckCompilePathPlanTermReady,
  liveHostModuleCheckCompilePathPlanTermSource,
  liveRel, liveHostModuleCheckCompilePathPlanTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathPlanTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathPlanTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathPlanTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathPlanTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathPlanTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathPlanTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathPlanTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathPlanTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathPlanTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathPlanTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathPlanTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathPlanTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathPlanTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM. -/
def parseLiveHostModuleCheckCompilePathPlanTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathPlanTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathPlanTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathPlanTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathPlanTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathPlanTermSource
      liveHostModuleCheckCompilePathPlanTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathPlanTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathPlanTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathPlanTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathPlanTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathPlanTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathPlanTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathPlanTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathPlanTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHPLANTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM")
    && (liveRel == "HostModuleCheckCompilePathPlanTerm.lean")
    && (liveHostModuleCheckCompilePathPlanTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathPlanTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathPlanTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathPlanTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathPlanTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathPlanTermSource
        liveHostModuleCheckCompilePathPlanTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathPlanTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathPlanTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathPlanTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathPlanTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathPlanTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathPlanTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathPlanTerm.lean != liveHostModuleCheckCompilePathPlanTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathPlanTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathPlanTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHPLANTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathPlanTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathPlanTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathPlanTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathPlanTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathPlanTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathPlanTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathPlanTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathPlanTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
