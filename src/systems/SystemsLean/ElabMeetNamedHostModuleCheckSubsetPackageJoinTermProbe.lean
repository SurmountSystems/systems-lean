/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckSubsetPackageJoinTerm named
  closed subset probe (plus-one after the FrontMultPackageTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. SubsetPackageJoinTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckSubsetPackageJoinTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckFrontMultPackageTermProbe,
  ...
  Spec (readable):
  - Named HostModuleCheckSubsetPackageJoinTerm subset:
    prior FrontMultPackageTerm closed set plus
    SystemsLean.HostModuleCheckSubsetPackageJoinTerm last.
    ...
    the FrontMultPackageTerm compile order, then HostModuleCheckSubsetPackageJoinTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as FrontMultPackageTerm): SubsetPackageJoinTerm
    ...
    SubsetPackageJoinTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckSubsetPackageJoinTerm.
    HostModuleCheckSubsetPackageJoinTerm probe lives here.
  - tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckSubsetPackageJoinTerm bad writes a temp
    HostModuleCheckSubsetPackageJoinTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckFrontMultPackageTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior FrontMultPackageTerm walker
    tryCompileNamedHostModuleCheckFrontMultPackageTermSubset, and that this walk
    is not an alias of the FrontMultPackageTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckSubsetPackageJoinTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckSubsetPackageJoinTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckSubsetPackageJoinTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckSubsetPackageJoinTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckSubsetPackageJoinTermSubset,
  #elabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe,
  elabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe,
  SystemsLean.HostModuleCheckSubsetPackageJoinTerm,
  ElabMeetNamedHostModuleCheckSubsetPackageJoinTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckSubsetPackageJoinTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckSubsetPackageJoinTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckSubsetPackageJoinTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckSubsetPackageJoinTerm named subset.
    Good compiles live Mult through HostModuleCheckFrontMultPackageTerm plus the
    subset-surface compile closure and HostModuleCheckSubsetPackageJoinTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckSubsetPackageJoinTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior FrontMultPackageTerm walker
    tryCompileNamedHostModuleCheckFrontMultPackageTermSubset still works; this
    walker is not an alias of the FrontMultPackageTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckSubsetPackageJoinTerm.lean. Do not skip to
    HostModuleCheckLlvmTypesTextTerm. -/
def elabMeetRunNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckSubsetPackageJoinTerm? <-
    liftIO findLiveHostModuleCheckSubsetPackageJoinTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckSubsetPackageJoinTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckSubsetPackageJoinTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckSubsetPackageJoinTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckSubsetPackageJoinTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckFrontMultPackageTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset fakeLake fakeDir
  let notAliasOfFrontMultPackageTerm :=
    namedClosedHostModuleCheckSubsetPackageJoinTermSubsetNames
      != namedClosedHostModuleCheckFrontMultPackageTermSubsetNames
      && namedClosedHostModuleCheckSubsetPackageJoinTermCompileOrder
           != namedClosedHostModuleCheckFrontMultPackageTermCompileOrder
      && namedClosedHostModuleCheckSubsetPackageJoinTermSubsetNames.contains
           "SystemsLean.HostModuleCheckSubsetPackageJoinTerm"
      && !namedClosedHostModuleCheckFrontMultPackageTermSubsetNames.contains
           "SystemsLean.HostModuleCheckSubsetPackageJoinTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfFrontMultPackageTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckSubsetPackageJoinTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckSubsetPackageJoinTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckSubsetPackageJoinTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckSubsetPackageJoinTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe

#elabMeetNamedHostModuleCheckSubsetPackageJoinTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckSubsetPackageJoinTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckSubsetPackageJoinTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckSubsetPackageJoinTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckSubsetPackageJoinTermSubset = true := rfl
