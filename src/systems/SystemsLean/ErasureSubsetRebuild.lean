...
def erasureSubsetRebuildWrite (root : System.FilePath) : IO Unit := do
  ...
  IO.println s!"== {stageId}: Erasure subset rebuild / self-application (M1 follow-on) =="
  IO.println s!"  host: {hostId} / surface: {surfaceId}"
  IO.println s!"  M1 package input id: {erasureSubsetRebuildInputId} (ERASURE-SUBSET-EMIT reuse)"
  IO.println s!"  erasureSubsetRebuildInputReady: {erasureSubsetRebuildInputReady} (erasureSubsetEmitReady)"
  IO.println s!"  erasureSubsetRebuildPackagePinOk: {erasureSubsetRebuildPackagePinOk} (erasureSubsetEmitWroteExpected)"
  IO.println s!"  erasureSubsetRebuildSelfApplyOk: {erasureSubsetRebuildSelfApplyOk}"
  IO.println s!"  erasureSubsetRebuildReady: {erasureSubsetRebuildReady}"
  IO.println s!"  erasureSubsetRebuildWroteExpected: {erasureSubsetRebuildWroteExpected} (package identity pin)"
  IO.println s!"  stillUsesLake: {stillUsesLake} (true until S4)"
  IO.println s!"  dependsOnLake: {dependsOnLake} (host elaborator bootstrap)"
  IO.println s!"  withoutLakeFinished: {erasureSubsetRebuildWithoutLakeFinishedClaimed} (false; deepen later)"
  IO.println s!"  Lake exe: {lakeExeName} / just {justRecipe}"
  IO.println "  honest: rebuild = measured Erasure subset self-application (re-emit/re-validate); not full dialect regenerate"
  ...
  IO.println "  short module name ErasureSubsetRebuild (not ProductPathFreestandingErasureSubsetRebuild)"
  ...
  IO.println s!"GREEN {stageId}: Erasure subset self-application rebuild under {emitDir}/"
