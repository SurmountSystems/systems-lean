/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathTypesTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathTypesTermSource turns live
    HostModuleCheckCompilePathTypesTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathTypesTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathTypesTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathTypesTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathTypesTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathTypesTermDialect and
    def hostModuleCheckCompilePathTypesTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathTypesTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHTYPESTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathTypesTermSource,
  kernelCheckLiveHostModuleCheckCompilePathTypesTermSource,
  hostFrontLiveHostModuleCheckCompilePathTypesTermReady,
  liveHostModuleCheckCompilePathTypesTermSource,
  liveRel, liveHostModuleCheckCompilePathTypesTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathTypesTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathTypesTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckCompilePathTypesTerm
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathTypesTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathTypesTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathTypesTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathTypesTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathTypesTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathTypesTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathTypesTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathTypesTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathTypesTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathTypesTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathTypesTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM. -/
def parseLiveHostModuleCheckCompilePathTypesTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathTypesTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathTypesTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathTypesTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathTypesTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathTypesTermSource
      liveHostModuleCheckCompilePathTypesTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathTypesTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathTypesTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathTypesTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathTypesTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathTypesTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathTypesTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathTypesTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathTypesTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHTYPESTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM")
    && (liveRel == "HostModuleCheckCompilePathTypesTerm.lean")
    && (liveHostModuleCheckCompilePathTypesTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathTypesTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathTypesTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathTypesTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathTypesTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathTypesTermSource
        liveHostModuleCheckCompilePathTypesTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathTypesTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathTypesTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathTypesTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathTypesTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathTypesTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathTypesTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathTypesTerm.lean != liveHostModuleCheckCompilePathTypesTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathTypesTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathTypesTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTYPESTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathTypesTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathTypesTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathTypesTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathTypesTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathTypesTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathTypesTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathTypesTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathTypesTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckCompilePathTypesTerm
