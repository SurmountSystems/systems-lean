/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathComposeTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathComposeTermSource turns live
    HostModuleCheckCompilePathComposeTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathComposeTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathComposeTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathComposeTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathComposeTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathComposeTermDialect and
    def hostModuleCheckCompilePathComposeTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathComposeTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHCOMPOSETERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathComposeTermSource,
  kernelCheckLiveHostModuleCheckCompilePathComposeTermSource,
  hostFrontLiveHostModuleCheckCompilePathComposeTermReady,
  liveHostModuleCheckCompilePathComposeTermSource,
  liveRel, liveHostModuleCheckCompilePathComposeTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathComposeTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathComposeTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathComposeTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathComposeTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathComposeTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathComposeTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathComposeTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathComposeTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathComposeTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathComposeTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathComposeTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathComposeTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathComposeTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM. -/
def parseLiveHostModuleCheckCompilePathComposeTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathComposeTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathComposeTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM. -/
def kernelCheckLiveHostModuleCheckCompilePathComposeTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathComposeTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathComposeTermSource
      liveHostModuleCheckCompilePathComposeTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathComposeTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathComposeTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathComposeTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathComposeTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathComposeTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathComposeTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathComposeTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathComposeTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHCOMPOSETERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM")
    && (liveRel == "HostModuleCheckCompilePathComposeTerm.lean")
    && (liveHostModuleCheckCompilePathComposeTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathComposeTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathComposeTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathComposeTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathComposeTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathComposeTermSource
        liveHostModuleCheckCompilePathComposeTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathComposeTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathComposeTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathComposeTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathComposeTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathComposeTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathComposeTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathComposeTerm.lean != liveHostModuleCheckCompilePathComposeTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathComposeTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathComposeTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHCOMPOSETERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathComposeTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathComposeTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathComposeTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathComposeTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathComposeTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathComposeTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathComposeTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathComposeTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
