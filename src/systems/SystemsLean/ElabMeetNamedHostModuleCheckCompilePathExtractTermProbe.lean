/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathExtractTerm named
  closed subset probe (plus-one after the CompilePathErasureTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathExtractTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathExtractTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathComposeTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathErasureTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathExtractTerm subset:
    prior CompilePathErasureTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathExtractTerm last.
    ...
    the CompilePathErasureTerm compile order, then HostModuleCheckCompilePathExtractTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathErasureTerm): CompilePathExtractTerm
    ...
    CompilePathExtractTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathExtractTerm.
    HostModuleCheckCompilePathExtractTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathExtractTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathExtractTerm bad writes a temp
    HostModuleCheckCompilePathExtractTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathErasureTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathErasureTerm walker
    tryCompileNamedHostModuleCheckCompilePathErasureTermSubset, and that this walk
    is not an alias of the CompilePathErasureTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathExtractTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathExtractTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathExtractTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathExtractTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathExtractTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathExtractTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathExtractTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathExtractTerm,
  ElabMeetNamedHostModuleCheckCompilePathExtractTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathExtractTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathExtractTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathExtractTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathExtractTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathErasureTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathExtractTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathExtractTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathErasureTerm walker
    tryCompileNamedHostModuleCheckCompilePathErasureTermSubset still works; this
    walker is not an alias of the CompilePathErasureTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathExtractTerm.lean. Do not skip to
    HostModuleCheckCompilePathPlanTerm. -/
elab "#elabMeetNamedHostModuleCheckCompilePathExtractTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathExtractTerm? <-
    liftIO findLiveHostModuleCheckCompilePathExtractTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathExtractTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathExtractTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathExtractTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathExtractTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathExtractTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathExtractTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathExtractTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathErasureTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathExtractTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathExtractTermSubset fakeLake fakeDir
  let notAliasOfCompilePathErasureTerm :=
    namedClosedHostModuleCheckCompilePathExtractTermSubsetNames
      != namedClosedHostModuleCheckCompilePathErasureTermSubsetNames
      && namedClosedHostModuleCheckCompilePathExtractTermCompileOrder
           != namedClosedHostModuleCheckCompilePathErasureTermCompileOrder
      && namedClosedHostModuleCheckCompilePathExtractTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathExtractTerm"
      && !namedClosedHostModuleCheckCompilePathErasureTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathExtractTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfCompilePathErasureTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathExtractTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathExtractTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathExtractTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathExtractTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathExtractTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathExtractTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathExtractTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathExtractTermSubset = true := rfl
