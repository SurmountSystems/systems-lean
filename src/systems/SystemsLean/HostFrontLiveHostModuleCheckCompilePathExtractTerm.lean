/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathExtractTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathExtractTermSource turns live
    HostModuleCheckCompilePathExtractTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathExtractTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathExtractTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathExtractTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathExtractTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathExtractTermDialect and
    def hostModuleCheckCompilePathExtractTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathExtractTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHEXTRACTTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathExtractTermSource,
  kernelCheckLiveHostModuleCheckCompilePathExtractTermSource,
  hostFrontLiveHostModuleCheckCompilePathExtractTermReady,
  liveHostModuleCheckCompilePathExtractTermSource,
  liveRel, liveHostModuleCheckCompilePathExtractTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathExtractTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathExtractTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathExtractTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathExtractTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathExtractTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathExtractTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathExtractTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathExtractTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathExtractTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathExtractTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathExtractTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathExtractTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathExtractTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM. -/
def parseLiveHostModuleCheckCompilePathExtractTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathExtractTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathExtractTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathExtractTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathExtractTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathExtractTermSource
      liveHostModuleCheckCompilePathExtractTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathExtractTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathExtractTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathExtractTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathExtractTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathExtractTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathExtractTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathExtractTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathExtractTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHEXTRACTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM")
    && (liveRel == "HostModuleCheckCompilePathExtractTerm.lean")
    && (liveHostModuleCheckCompilePathExtractTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathExtractTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathExtractTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathExtractTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathExtractTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathExtractTermSource
        liveHostModuleCheckCompilePathExtractTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathExtractTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathExtractTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathExtractTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathExtractTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathExtractTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathExtractTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathExtractTerm.lean != liveHostModuleCheckCompilePathExtractTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathExtractTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathExtractTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHEXTRACTTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathExtractTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathExtractTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathExtractTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathExtractTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathExtractTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathExtractTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathExtractTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathExtractTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
