/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitComposeScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitComposeScaffoldTermSource turns live
    HostModuleCheckEmitComposeScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitComposeScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitComposeScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitComposeScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitComposeScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, five literal defs, and end.
    Kept count is 9.
    The five literal defs are checkDepthEmitComposeScaffoldTermSurfaceBar,
    checkDepthEmitComposeScaffoldTermSurfaceScope,
    hostModuleCheckEmitComposeScaffoldTermSurfaceDualOk,
    hostModuleCheckEmitComposeScaffoldTermSkeletonPrefix, and
  ...
  HostFrontLiveHostModuleCheckEmitComposeScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitComposeScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitComposeScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady,
  liveHostModuleCheckEmitComposeScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitComposeScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitComposeScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitComposeScaffoldTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckEmitComposeScaffoldTerm
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitComposeScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitComposeScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitComposeScaffoldTermSource. Not split. -/
def liveHostModuleCheckEmitComposeScaffoldTermRel : String :=
...
def hostFrontLiveHostModuleCheckEmitComposeScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitComposeScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitComposeScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitComposeScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitComposeScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitComposeScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitComposeScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitComposeScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitComposeScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitComposeScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitComposeScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitComposeScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitComposeScaffoldTermSource
      liveHostModuleCheckEmitComposeScaffoldTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitComposeScaffoldTermSurfaceBar",
      "checkDepthEmitComposeScaffoldTermSurfaceScope",
      "hostModuleCheckEmitComposeScaffoldTermSurfaceDualOk",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitComposeScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitComposeScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitComposeScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitComposeScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitComposeScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitComposeScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitComposeScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitComposeScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitComposeScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitComposeScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitComposeScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitComposeScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitComposeScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitComposeScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_compose_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitComposeScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitComposeScaffoldTerm.lean")
    && (liveHostModuleCheckEmitComposeScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitComposeScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitComposeScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitComposeScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitComposeScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitComposeScaffoldTermSource
        liveHostModuleCheckEmitComposeScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitComposeScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitComposeScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitComposeScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitComposeScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitComposeScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitComposeScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitComposeScaffoldTerm.lean != liveHostModuleCheckEmitComposeScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitComposeScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitComposeScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITCOMPOSESCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitComposeScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitComposeScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitComposeScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitComposeScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitComposeScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitComposeScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckEmitComposeScaffoldTerm
