/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-MODS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_MODS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-MODS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckImportModsKernelSource,
  hostFrontLiveHostModuleCheckImportModsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckImportModsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckImportModsKernelSource
...
def liveRel : String := "HostModuleCheckImportModsKernel.lean"
...
def hostFrontLiveHostModuleCheckImportModsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckImportModsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckImportModsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckImportModsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckImportModsKernel.lean bytes.
    Greppable: liveHostModuleCheckImportModsKernelSource. -/


def liveHostModuleCheckImportModsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckImportModsKernel.lean text.
    Greppable: parseLiveHostModuleCheckImportModsKernelSource. -/
def parseLiveHostModuleCheckImportModsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckImportModsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckImportModsKernelSource. -/
def kernelCheckLiveHostModuleCheckImportModsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckImportModsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckImportModsKernelSource
      liveHostModuleCheckImportModsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthImportModsKernelBar"
      && has "checkDepthImportModsKernelScope"
      && has "hostModuleCheckImportModsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckImportModsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckImportModsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckImportModsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckImportModsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckImportModsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_MODS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-MODS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-MODS-KERNEL")
    && (liveRel == "HostModuleCheckImportModsKernel.lean")
    && (liveHostModuleCheckImportModsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportModsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckImportModsKernelFullHost
    && !hostFrontLiveHostModuleCheckImportModsKernelFullBackend
    && !hostFrontLiveHostModuleCheckImportModsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckImportModsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckImportModsKernelSource
      liveHostModuleCheckImportModsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckImportModsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-MODS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckImportModsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckImportModsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportModsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckImportModsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckImportModsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckImportModsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckImportModsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckImportModsKernelSource
      liveHostModuleCheckImportModsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckImportModsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckImportModsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckImportModsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckImportModsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckImportModsKernel.lean != liveHostModuleCheckImportModsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckImportModsKernel.lean")
  let r := parseLiveHostModuleCheckImportModsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckImportModsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckImportModsKernel parse false")
    unless hostFrontLiveHostModuleCheckImportModsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckImportModsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckImportModsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckImportModsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckImportModsKernelSource
