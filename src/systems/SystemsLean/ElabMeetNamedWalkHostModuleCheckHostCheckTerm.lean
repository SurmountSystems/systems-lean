/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckHostCheckTerm named-walk
  wrappers (CheckersLater-role walk; LlvmMultTextTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and LlvmMultTextTerm walk
  ...
  ElabMeetNamedWalk. Prior LlvmMultTextTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckLlvmMultTextTerm. Do not grow that walk.
  ...
  LlvmMultTextTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckHostCheckTerm.lean.
  Do not skip to HostModuleCheckHostGraphTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckHostCheckTerm wrappers live here:
    ...
    Named membership is the prior LlvmMultTextTerm closed set plus
    HostModuleCheckHostCheckTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the LlvmMultTextTerm compile
    order, then HostModuleCheckHostCheckTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / LlvmMultTextTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the LlvmMultTextTerm compile order.
    HostModuleCheckHostCheckTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckLlvmMultTextTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckHostCheckTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckHostCheckTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckHostCheckTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckHostCheckTermPath. -/
def findLiveHostModuleCheckHostCheckTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckHostCheckTerm"

/-- Named HostModuleCheckHostCheckTerm subset of the live SystemsLean library.
    Membership is the prior LlvmMultTextTerm closed set plus
    HostModuleCheckHostCheckTerm last.
    ...
    Greppable: namedClosedHostModuleCheckHostCheckTermSubsetNames.
    SystemsLean.HostModuleCheckHostCheckTerm. -/
def namedClosedHostModuleCheckHostCheckTermSubsetNames : List String :=
  namedClosedHostModuleCheckLlvmMultTextTermSubsetNames ++
    ["SystemsLean.HostModuleCheckHostCheckTerm"]

/-- Compile order honors imports: LlvmMultTextTerm compile order, then
    HostModuleCheckHostCheckTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckHostCheckTermCompileOrder. -/
def namedClosedHostModuleCheckHostCheckTermCompileOrder : List String :=
  namedClosedHostModuleCheckLlvmMultTextTermCompileOrder ++
    ["SystemsLean.HostModuleCheckHostCheckTerm"]

/-- True when the barrel lists each HostModuleCheckHostCheckTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckHostCheckTermSubset. -/
def barrelListsNamedHostModuleCheckHostCheckTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckHostCheckTermSubsetNames

/-- Compile the named HostModuleCheckHostCheckTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the LlvmMultTextTerm
    compile order, then HostModuleCheckHostCheckTerm last.
    Greppable: tryCompileNamedHostModuleCheckHostCheckTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckHostCheckTerm. -/
unsafe def tryCompileNamedHostModuleCheckHostCheckTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckHostCheckTermSubsetNames
    namedClosedHostModuleCheckHostCheckTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckHostCheckTermSubsetIO]
opaque tryCompileNamedHostModuleCheckHostCheckTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckHostCheckTerm subset.
    Greppable: tryCompileNamedHostModuleCheckHostCheckTermSubset. -/
def tryCompileNamedHostModuleCheckHostCheckTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckHostCheckTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the LlvmMultTextTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckHostCheckTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckHostCheckTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckLlvmMultTextTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckHostCheckTermDepsIO]
opaque tryCompileAfterHostModuleCheckHostCheckTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckHostCheckTerm deps.
    Greppable: tryCompileAfterHostModuleCheckHostCheckTermDeps. -/
def tryCompileAfterHostModuleCheckHostCheckTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckHostCheckTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
