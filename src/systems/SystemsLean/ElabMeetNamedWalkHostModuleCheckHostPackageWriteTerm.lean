/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckHostPackageWriteTerm named-walk
  wrappers (CheckersLater-role walk; MultFsDeepenTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and MultFsDeepenTerm walk
  ...
  ElabMeetNamedWalk. Prior MultFsDeepenTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckMultFsDeepenTerm. Do not grow that walk.
  ...
  MultFsDeepenTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckHostPackageWriteTerm.lean.
  Do not skip to HostModuleCheckLlvmProgramTextTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckHostPackageWriteTerm wrappers live here:
    ...
    Named membership is the prior MultFsDeepenTerm closed set plus
    HostModuleCheckHostPackageWriteTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the MultFsDeepenTerm compile
    order, then HostModuleCheckHostPackageWriteTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / MultFsDeepenTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the MultFsDeepenTerm compile order.
    HostModuleCheckHostPackageWriteTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckMultFsDeepenTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckHostPackageWriteTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckHostPackageWriteTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckHostPackageWriteTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckHostPackageWriteTermPath. -/
def findLiveHostModuleCheckHostPackageWriteTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckHostPackageWriteTerm"

/-- Named HostModuleCheckHostPackageWriteTerm subset of the live SystemsLean library.
    Membership is the prior MultFsDeepenTerm closed set plus
    HostModuleCheckHostPackageWriteTerm last.
    ...
    Greppable: namedClosedHostModuleCheckHostPackageWriteTermSubsetNames.
    SystemsLean.HostModuleCheckHostPackageWriteTerm. -/
def namedClosedHostModuleCheckHostPackageWriteTermSubsetNames : List String :=
  namedClosedHostModuleCheckMultFsDeepenTermSubsetNames ++
    ["SystemsLean.HostModuleCheckHostPackageWriteTerm"]

/-- Compile order honors imports: MultFsDeepenTerm compile order, then
    HostModuleCheckHostPackageWriteTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckHostPackageWriteTermCompileOrder. -/
def namedClosedHostModuleCheckHostPackageWriteTermCompileOrder : List String :=
  namedClosedHostModuleCheckMultFsDeepenTermCompileOrder ++
    ["SystemsLean.HostModuleCheckHostPackageWriteTerm"]

/-- True when the barrel lists each HostModuleCheckHostPackageWriteTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckHostPackageWriteTermSubset. -/
def barrelListsNamedHostModuleCheckHostPackageWriteTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckHostPackageWriteTermSubsetNames

/-- Compile the named HostModuleCheckHostPackageWriteTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the MultFsDeepenTerm
    compile order, then HostModuleCheckHostPackageWriteTerm last.
    Greppable: tryCompileNamedHostModuleCheckHostPackageWriteTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckHostPackageWriteTerm. -/
unsafe def tryCompileNamedHostModuleCheckHostPackageWriteTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckHostPackageWriteTermSubsetNames
    namedClosedHostModuleCheckHostPackageWriteTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckHostPackageWriteTermSubsetIO]
opaque tryCompileNamedHostModuleCheckHostPackageWriteTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckHostPackageWriteTerm subset.
    Greppable: tryCompileNamedHostModuleCheckHostPackageWriteTermSubset. -/
def tryCompileNamedHostModuleCheckHostPackageWriteTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckHostPackageWriteTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the MultFsDeepenTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckHostPackageWriteTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckHostPackageWriteTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckMultFsDeepenTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckHostPackageWriteTermDepsIO]
opaque tryCompileAfterHostModuleCheckHostPackageWriteTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckHostPackageWriteTerm deps.
    Greppable: tryCompileAfterHostModuleCheckHostPackageWriteTermDeps. -/
def tryCompileAfterHostModuleCheckHostPackageWriteTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckHostPackageWriteTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
