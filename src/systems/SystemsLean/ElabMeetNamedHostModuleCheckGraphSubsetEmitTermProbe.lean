/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckGraphSubsetEmitTerm named
  closed subset probe (plus-one after the ProgramSubsetEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. GraphSubsetEmitTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckGraphSubsetEmitTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckTypesSubsetEmitTerm, or
  ElabMeetNamedWalkHostModuleCheckProgramSubsetEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckGraphSubsetEmitTerm subset:
    prior ProgramSubsetEmitTerm closed set plus
    SystemsLean.HostModuleCheckGraphSubsetEmitTerm last.
    ...
    the ProgramSubsetEmitTerm compile order, then HostModuleCheckGraphSubsetEmitTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ProgramSubsetEmitTerm): GraphSubsetEmitTerm
    ...
    GraphSubsetEmitTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckGraphSubsetEmitTerm.
    HostModuleCheckGraphSubsetEmitTerm probe lives here.
  - tryCompileNamedHostModuleCheckGraphSubsetEmitTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckGraphSubsetEmitTerm bad writes a temp
    HostModuleCheckGraphSubsetEmitTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckProgramSubsetEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ProgramSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset, and that this walk
    is not an alias of the ProgramSubsetEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckGraphSubsetEmitTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckGraphSubsetEmitTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckGraphSubsetEmitTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckGraphSubsetEmitTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckGraphSubsetEmitTermSubset,
  #elabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe,
  elabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe,
  SystemsLean.HostModuleCheckGraphSubsetEmitTerm,
  ElabMeetNamedHostModuleCheckGraphSubsetEmitTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckGraphSubsetEmitTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckGraphSubsetEmitTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckGraphSubsetEmitTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckGraphSubsetEmitTerm named subset.
    Good compiles live Mult through HostModuleCheckProgramSubsetEmitTerm plus the
    subset-surface compile closure and HostModuleCheckGraphSubsetEmitTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckGraphSubsetEmitTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ProgramSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset still works; this
    walker is not an alias of the ProgramSubsetEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckGraphSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckComposeSubsetEmitTerm. -/
def elabMeetRunNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckGraphSubsetEmitTerm? <-
    liftIO findLiveHostModuleCheckGraphSubsetEmitTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckGraphSubsetEmitTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckGraphSubsetEmitTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckGraphSubsetEmitTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckGraphSubsetEmitTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckGraphSubsetEmitTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckProgramSubsetEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckGraphSubsetEmitTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckGraphSubsetEmitTermSubset fakeLake fakeDir
  let notAliasOfProgramSubsetEmitTerm :=
    namedClosedHostModuleCheckGraphSubsetEmitTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfProgramSubsetEmitTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckGraphSubsetEmitTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckGraphSubsetEmitTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckGraphSubsetEmitTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckGraphSubsetEmitTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe

#elabMeetNamedHostModuleCheckGraphSubsetEmitTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckGraphSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckGraphSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckGraphSubsetEmitTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckGraphSubsetEmitTermSubset = true := rfl
