/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckKernelProgramProof.lean.
  ...
  liveRel is HostModuleCheckKernelProgramProof.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckKernelProgramProofSource,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-PROOF.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelProgramProofSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelProgramProofSource
...
def liveRel : String := "HostModuleCheckKernelProgramProof.lean"

/-- Pinned live HostModuleCheckKernelProgramProof.lean bytes. -/
def liveHostModuleCheckKernelProgramProofSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckKernelProgramProofSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckKernelProgramProofSourceReady : Bool :=
  liveRel == "HostModuleCheckKernelProgramProof.lean"
    && kernelCheckLiveHostModuleCheckKernelProgramProofSource liveHostModuleCheckKernelProgramProofSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckKernelProgramProofSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckKernelProgramProof.lean")
  let k := kernelCheckLiveHostModuleCheckKernelProgramProofSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckKernelProgramProof parse false")
  unless hostFrontLiveHostModuleCheckKernelProgramProofSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelProgramProofSource
