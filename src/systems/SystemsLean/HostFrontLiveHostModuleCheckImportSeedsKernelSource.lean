/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-SEEDS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_SEEDS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-SEEDS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckImportSeedsKernelSource,
  hostFrontLiveHostModuleCheckImportSeedsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckImportSeedsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckImportSeedsKernelSource
...
def liveRel : String := "HostModuleCheckImportSeedsKernel.lean"
...
def hostFrontLiveHostModuleCheckImportSeedsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckImportSeedsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckImportSeedsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckImportSeedsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckImportSeedsKernel.lean bytes.
    Greppable: liveHostModuleCheckImportSeedsKernelSource. -/


def liveHostModuleCheckImportSeedsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckImportSeedsKernel.lean text.
    Greppable: parseLiveHostModuleCheckImportSeedsKernelSource. -/
def parseLiveHostModuleCheckImportSeedsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckImportSeedsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckImportSeedsKernelSource. -/
def kernelCheckLiveHostModuleCheckImportSeedsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckImportSeedsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckImportSeedsKernelSource
      liveHostModuleCheckImportSeedsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthImportSeedsKernelBar"
      && has "checkDepthImportSeedsKernelScope"
      && has "hostModuleCheckImportSeedsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckImportSeedsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckImportSeedsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckImportSeedsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckImportSeedsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckImportSeedsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_SEEDS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-SEEDS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-SEEDS-KERNEL")
    && (liveRel == "HostModuleCheckImportSeedsKernel.lean")
    && (liveHostModuleCheckImportSeedsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportSeedsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckImportSeedsKernelFullHost
    && !hostFrontLiveHostModuleCheckImportSeedsKernelFullBackend
    && !hostFrontLiveHostModuleCheckImportSeedsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckImportSeedsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckImportSeedsKernelSource
      liveHostModuleCheckImportSeedsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckImportSeedsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-SEEDS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckImportSeedsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckImportSeedsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportSeedsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckImportSeedsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckImportSeedsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckImportSeedsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckImportSeedsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckImportSeedsKernelSource
      liveHostModuleCheckImportSeedsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckImportSeedsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckImportSeedsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckImportSeedsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckImportSeedsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckImportSeedsKernel.lean != liveHostModuleCheckImportSeedsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckImportSeedsKernel.lean")
  let r := parseLiveHostModuleCheckImportSeedsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckImportSeedsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckImportSeedsKernel parse false")
    unless hostFrontLiveHostModuleCheckImportSeedsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckImportSeedsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckImportSeedsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckImportSeedsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckImportSeedsKernelSource
