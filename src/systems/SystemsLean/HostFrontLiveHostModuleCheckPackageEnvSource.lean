/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckPackageEnv.lean.
  ...
  liveRel is HostModuleCheckPackageEnv.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckPackageEnvSource,
  PARSE-LIVE-HOST-MODULE-CHECK-PACKAGE-ENV.
  Module: SystemsLean.HostFrontLiveHostModuleCheckPackageEnvSource
  Not FullHost. FullBackend stays false. Not a backend.
  Occupancy stays 49. Mill stays 69 of 69.
  slakeOwnsPackageTypecheck stays false.
  kernelFuel stays 64. This file does not raise it.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveLlvmComposeTextMainSource
import SystemsLean.HostKernel
import SystemsLean.HostFrontLiveHostModuleCheckPackageEnvParse

namespace SystemsLean.HostFrontLiveHostModuleCheckPackageEnvSource

open SystemsLean.HostFront
open SystemsLean.HostKernel
open SystemsLean.HostFrontLiveLlvmComposeTextMain
open SystemsLean.HostTerm
open SystemsLean.HostFrontLiveHostModuleCheckPackageEnvParse

/-- Live basename. Filename only, not a path. -/
def liveRel : String := "HostModuleCheckPackageEnv.lean"

/-- Pinned live HostModuleCheckPackageEnv.lean bytes. -/
def liveHostModuleCheckPackageEnvSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckPackageEnvSource (src : String) : Bool :=
  match parseLiveHostModuleCheckPackageEnvSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckPackageEnvSourceReady : Bool :=
  liveRel == "HostModuleCheckPackageEnv.lean"
    && kernelCheckLiveHostModuleCheckPackageEnvSource liveHostModuleCheckPackageEnvSource

/-- One kept command, for the live log. -/
...
def keptHasImport (cmds : List Cmd) (nm : String) : Bool :=
  cmds.any fun c =>
    match c with
    | Cmd.importModule x => x.raw == nm
    | _ => false

def keptHasDef (cmds : List Cmd) (nm : String) : Bool :=
  cmds.any fun c =>
    match c with
...
def keptHasNamespace (cmds : List Cmd) : Bool :=
  cmds.any fun c =>
    match c with
    | Cmd.namespace x => x.raw == "SystemsLean.HostModuleCheck"
    | _ => false

def keptHasEnd (cmds : List Cmd) : Bool :=
  cmds.any fun c =>
    match c with
    | Cmd.endNamespace x => x.raw == "SystemsLean.HostModuleCheck"
    | _ => false

/-- Imports the product file actually has. Not a shortened list. -/
...
/-- none when every kernel-real command is kept and the list is at most 64. -/
def missingKept (cmds : List Cmd) : Option String :=
  if cmds.length >= 65 then
    some (s!"command count {cmds.length} past kernelFuel 64")
  else
    match requiredImports.find? (fun n => !keptHasImport cmds n) with
    | some n => some ("kept list missing import " ++ n)
    | none =>
      if !keptHasNamespace cmds then
        some "kept list missing namespace SystemsLean.HostModuleCheck"
      else if !keptHasEnd cmds then
        some "kept list missing end SystemsLean.HostModuleCheck"
      else if !keptHasDef cmds "checkDepthPackageElaborateEnvScope" then
        some "kept list missing String def checkDepthPackageElaborateEnvScope"
      else if !keptHasDef cmds "hostModuleCheckPackageElaborateEnvDualOk" then
        some "kept list missing Bool def hostModuleCheckPackageElaborateEnvDualOk"
      else
        none
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckPackageEnvSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckPackageEnv.lean")
  let k := kernelCheckLiveHostModuleCheckPackageEnvSource disk
  IO.println s!"kernelCheck={k}"
  match parseLiveHostModuleCheckPackageEnvSource disk with
  | FrontResult.reject _ =>
    IO.println "parse=reject"
    throw (IO.userError "kernelCheck live HostModuleCheckPackageEnv parse false")
  | FrontResult.accept m =>
    IO.println s!"cmds={m.commands.length}"
    for c in m.commands do
      IO.println s!"kept {cmdTag c}"
    unless k do
      throw (IO.userError "kernelCheck live HostModuleCheckPackageEnv parse false")
    match missingKept m.commands with
    | some why =>
      throw (IO.userError why)
    | none =>
      pure ()
  unless hostFrontLiveHostModuleCheckPackageEnvSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckPackageEnvSource
