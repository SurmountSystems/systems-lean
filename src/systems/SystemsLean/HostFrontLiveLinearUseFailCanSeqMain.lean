/-
  SYSTEMS_LEAN_HOST partial -- thin driver for PARSE-LIVE-LINEAR-USE-FAIL-CAN-SEQ.
  Root for optional lean --run of live LinearUseFail/CanSeq.lean parse +
  kernelCheck.
  Body lives in SystemsLean.HostFrontLiveLinearUseFailCanSeq.
  Unique needles (trailing newline so LinearUseFail wrap is not a prefix):
  HostFrontLiveLinearUseFailCanSeqMain
  PARSE-LIVE-LINEAR-USE-FAIL-CAN-SEQ
  HOST-FRONT-LIVE-LINEAR-USE-FAIL-CAN-SEQ
  SLAKE_HOST_FRONT_LIVE_LINEAR_USE_FAIL_CAN_SEQ_V0
  Greppable: SystemsLean.HostFrontLiveLinearUseFailCanSeqMain,
  parseLiveLinearUseFailCanSeqSource,
  kernelCheckLiveLinearUseFailCanSeqSource,
  SYSTEMS_LEAN_HOST.
  Module: SystemsLean.HostFrontLiveLinearUseFailCanSeqMain
  Not mill 70. Not occupancy name 50. Not FullHost.
  Module must stay ASCII. Not freestanding residual free. Not PROVABLY. SKELETON.
-/

import SystemsLean.HostFrontLiveLinearUseFailCanSeq

/-- lean --run entry: forward argv (repo root). -/
def main (args : List String) : IO UInt32 :=
  SystemsLean.HostFrontLiveLinearUseFailCanSeq.main args
