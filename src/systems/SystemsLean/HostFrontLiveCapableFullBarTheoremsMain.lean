/-
  SYSTEMS_LEAN_HOST partial -- thin driver for
  PARSE-LIVE-CAPABLE-FULL-BAR-THEOREMS.
  Root for optional lean --run of live CapableFullBarTheorems.lean parse plus
  HostKernel.kernelCheck. Not an emit driver. Not an LLVM backend.
  Body lives in SystemsLean.HostFrontLiveCapableFullBarTheorems.
  Greppable: SystemsLean.HostFrontLiveCapableFullBarTheoremsMain,
  HostFrontLiveCapableFullBarTheoremsMain,
  HOST-FRONT-LIVE-CAPABLE-FULL-BAR-THEOREMS,
  SLAKE_HOST_FRONT_LIVE_CAPABLE_FULL_BAR_THEOREMS_V0,
  PARSE-LIVE-CAPABLE-FULL-BAR-THEOREMS, FULL-BAR-THEOREM,
  parseLiveCapableFullBarTheoremsSource,
  kernelCheckLiveCapableFullBarTheoremsSource,
  SYSTEMS_LEAN_HOST.
  Module: SystemsLean.HostFrontLiveCapableFullBarTheoremsMain
  Not FullHost. FullBackend stays false. Not an LLVM backend.
  Occupancy stays 49. Mill stays 69 of 69.
  slakeOwnsPackageTypecheck stays false.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveCapableFullBarTheorems

/-- lean --run entry: forward argv (repo root). -/
def main (args : List String) : IO UInt32 :=
  SystemsLean.HostFrontLiveCapableFullBarTheorems.main args
