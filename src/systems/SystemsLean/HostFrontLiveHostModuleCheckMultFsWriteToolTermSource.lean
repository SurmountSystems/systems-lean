/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckMultFsWriteToolTerm.lean.
  ...
  liveRel is HostModuleCheckMultFsWriteToolTerm.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckMultFsWriteToolTermSource,
  PARSE-LIVE-HOST-MODULE-CHECK-MULT-FS-WRITE-TOOL-TERM.
  Module: SystemsLean.HostFrontLiveHostModuleCheckMultFsWriteToolTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckMultFsWriteToolTermSource
...
def liveRel : String := "HostModuleCheckMultFsWriteToolTerm.lean"

/-- Pinned live HostModuleCheckMultFsWriteToolTerm.lean bytes. -/
def liveHostModuleCheckMultFsWriteToolTermSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckMultFsWriteToolTermSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckMultFsWriteToolTermSourceReady : Bool :=
  liveRel == "HostModuleCheckMultFsWriteToolTerm.lean"
    && kernelCheckLiveHostModuleCheckMultFsWriteToolTermSource liveHostModuleCheckMultFsWriteToolTermSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckMultFsWriteToolTermSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckMultFsWriteToolTerm.lean")
  let k := kernelCheckLiveHostModuleCheckMultFsWriteToolTermSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckMultFsWriteToolTerm parse false")
  unless hostFrontLiveHostModuleCheckMultFsWriteToolTermSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckMultFsWriteToolTermSource
