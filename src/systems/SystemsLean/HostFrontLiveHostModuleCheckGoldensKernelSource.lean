/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-GOLDENS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_GOLDENS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-GOLDENS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckGoldensKernelSource,
  hostFrontLiveHostModuleCheckGoldensKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckGoldensKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckGoldensKernelSource
...
def liveRel : String := "HostModuleCheckGoldensKernel.lean"
...
def hostFrontLiveHostModuleCheckGoldensKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckGoldensKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckGoldensKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckGoldensKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckGoldensKernel.lean bytes.
    Greppable: liveHostModuleCheckGoldensKernelSource. -/

def liveHostModuleCheckGoldensKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckGoldensKernel.lean text.
    Greppable: parseLiveHostModuleCheckGoldensKernelSource. -/
def parseLiveHostModuleCheckGoldensKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckGoldensKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckGoldensKernelSource. -/
def kernelCheckLiveHostModuleCheckGoldensKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckGoldensKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckGoldensKernelSource
      liveHostModuleCheckGoldensKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthGoldensKernelBar"
      && has "checkDepthGoldensKernelScope"
      && has "hostModuleCheckGoldensKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckGoldensKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckGoldensKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckGoldensKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckGoldensKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckGoldensKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_GOLDENS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-GOLDENS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-GOLDENS-KERNEL")
    && (liveRel == "HostModuleCheckGoldensKernel.lean")
    && (liveHostModuleCheckGoldensKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckGoldensKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckGoldensKernelFullHost
    && !hostFrontLiveHostModuleCheckGoldensKernelFullBackend
    && !hostFrontLiveHostModuleCheckGoldensKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckGoldensKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckGoldensKernelSource
      liveHostModuleCheckGoldensKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckGoldensKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-GOLDENS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckGoldensKernel.lean")
    ++ bad "rel" (liveHostModuleCheckGoldensKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckGoldensKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckGoldensKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckGoldensKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckGoldensKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckGoldensKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckGoldensKernelSource
      liveHostModuleCheckGoldensKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckGoldensKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckGoldensKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckGoldensKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckGoldensKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckGoldensKernel.lean != liveHostModuleCheckGoldensKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckGoldensKernel.lean")
  let r := parseLiveHostModuleCheckGoldensKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckGoldensKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckGoldensKernel parse false")
    unless hostFrontLiveHostModuleCheckGoldensKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckGoldensKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckGoldensKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckGoldensKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckGoldensKernelSource
