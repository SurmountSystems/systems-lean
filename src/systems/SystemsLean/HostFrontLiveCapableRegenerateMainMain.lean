/-
  SYSTEMS_LEAN_HOST partial -- thin driver for PARSE-LIVE-CAPABLE-REGENERATE-MAIN.
  Root for optional lean --run of live CapableRegenerateMain.lean parse +
  kernelCheck.
  Body lives in SystemsLean.HostFrontLiveCapableRegenerateMain.
  Unique needles (trailing newline so HostFrontLiveCapableRegenerateMain is not a prefix):
  HostFrontLiveCapableRegenerateMainMain
  PARSE-LIVE-CAPABLE-REGENERATE-MAIN
  HOST-FRONT-LIVE-CAPABLE-REGENERATE-MAIN
  SLAKE_HOST_FRONT_LIVE_CAPABLE_REGENERATE_MAIN_V0
  CAPABLE-REGENERATE-MAIN
  Greppable: SystemsLean.HostFrontLiveCapableRegenerateMainMain,
  parseLiveCapableRegenerateMainSource,
  kernelCheckLiveCapableRegenerateMainSource,
  SYSTEMS_LEAN_HOST.
  Module: SystemsLean.HostFrontLiveCapableRegenerateMainMain
  Not mill remill. Mill stays 69 of 69. Not occupancy name 50. Not FullHost.
  Module must stay ASCII. Not freestanding residual free. Not PROVABLY. SKELETON.
-/

import SystemsLean.HostFrontLiveCapableRegenerateMain

/-- lean --run entry: forward argv (repo root). -/
def main (args : List String) : IO UInt32 :=
  SystemsLean.HostFrontLiveCapableRegenerateMain.main args
