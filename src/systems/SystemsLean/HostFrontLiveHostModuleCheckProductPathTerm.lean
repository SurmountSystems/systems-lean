/-
  ...
  src/systems/SystemsLean/HostModuleCheckProductPathTerm.lean with the
  ...
  HOST-FRONT-LIVE-HOSTMODULECHECKPRODUCTPATHTERM,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPRODUCTPATHTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM, liveRel,
  kernelCheckLiveHostModuleCheckProductPathTermSource,
  hostFrontLiveHostModuleCheckProductPathTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckProductPathTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckProductPathTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckProductPathTerm
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckProductPathTerm.lean"
...
def hostFrontLiveHostModuleCheckProductPathTermFullHost :
    Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckProductPathTermFullBackend :
    Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckProductPathTermResidualFreeClaimed :
    Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckProductPathTermProvablyUnlocked :
    Bool := false
...
/-- Parse live HostModuleCheckProductPathTerm.lean text.
    Greppable: parseLiveHostModuleCheckProductPathTermSource,
    PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM. -/
def parseLiveHostModuleCheckProductPathTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name :=
              HostTerm.n "SystemsLean.HostModuleCheckProductPathTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckProductPathTermSource,
    PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM. -/
def kernelCheckLiveHostModuleCheckProductPathTermSource
    (src : String) : Bool :=
  match parseLiveHostModuleCheckProductPathTermSource src with
...
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckProductPathTermSource
      liveHostModuleCheckProductPathTermSource with
...
/-- Kept String def checkDepthProductPathTermSurfaceBar is TERM-SURFACE. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthProductPathTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Kept String def checkDepthProductPathTermSurfaceScope is
    ProductPath-only. -/
def liveParseHasScopeLit : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthProductPathTermSurfaceScope"
          && s == "ProductPath-only"
      | _ => false

/-- Kept Bool def hostModuleCheckProductPathTermSurfaceDualOk is true.
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckProductPathTermSurfaceDualOk"
          && b == true
      | _ => false

/-- Kept String def productPathReadyTermBody.
...
def liveParseHasSuffixLit : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckProductPathTermSkeletonSuffix"
...
/-- Wrap module lastSeg is HostModuleCheckProductPathTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckProductPathTerm"
      && lastSeg m.name.raw == "HostModuleCheckProductPathTerm"
...
def needleBarDef : String := "def checkDepthProductPathTermSurfaceBar"

/-- Ready-body def name. One String literal. -/
def needleReadyBodyDef : String :=
  "def productPathReadyTermBody"

/-- Suffix def name. One String literal. -/
def needleSuffixDef : String :=
  "def hostModuleCheckProductPathTermSkeletonSuffix"
...
def needleOkDef : String :=
  "def hostModuleCheckProductPathTermSurfaceOk"

/-- Bar string in the live source. -/
def needleBar : String := "TERM-SURFACE"

/-- Scope string in the live source. -/
def needleScope : String := "ProductPath-only"

/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Five imports. Bar is TERM-SURFACE. Scope is ProductPath-only. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckProductPathTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckProductPathTermReady,
    PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKPRODUCTPATHTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckProductPathTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPRODUCTPATHTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPRODUCTPATHTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM")
    && (liveRel == "HostModuleCheckProductPathTerm.lean")
    && (liveHostModuleCheckProductPathTermRel ==
        "src/systems/SystemsLean/HostModuleCheckProductPathTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckProductPathTermFullHost
    && !hostFrontLiveHostModuleCheckProductPathTermFullBackend
    && !hostFrontLiveHostModuleCheckProductPathTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckProductPathTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckProductPathTermSource
        liveHostModuleCheckProductPathTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckProductPathTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckProductPathTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckProductPathTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckProductPathTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckProductPathTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckProductPathTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckProductPathTerm.lean != liveHostModuleCheckProductPathTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckProductPathTerm.lean")
  let r := parseLiveHostModuleCheckProductPathTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM reject {reason}")
  | FrontResult.accept m =>
    let k :=
      if m.commands.length > kernelFuel then false
      else HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKPRODUCTPATHTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckProductPathTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckProductPathTerm parse false")
    unless hostFrontLiveHostModuleCheckProductPathTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckProductPathTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckProductPathTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckProductPathTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckProductPathTerm source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckProductPathTerm
