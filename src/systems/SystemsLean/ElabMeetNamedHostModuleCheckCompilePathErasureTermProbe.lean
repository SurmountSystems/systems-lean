/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathErasureTerm named
  closed subset probe (plus-one after the CompilePathComposeTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathErasureTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathErasureTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathGraphTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathComposeTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathErasureTerm subset:
    prior CompilePathComposeTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathErasureTerm last.
    ...
    the CompilePathComposeTerm compile order, then HostModuleCheckCompilePathErasureTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathComposeTerm): CompilePathErasureTerm
    ...
    CompilePathErasureTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathErasureTerm.
    HostModuleCheckCompilePathErasureTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathErasureTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathErasureTerm bad writes a temp
    HostModuleCheckCompilePathErasureTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathComposeTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathComposeTerm walker
    tryCompileNamedHostModuleCheckCompilePathComposeTermSubset, and that this walk
    is not an alias of the CompilePathComposeTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathErasureTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathErasureTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathErasureTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathErasureTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathErasureTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathErasureTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathErasureTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathErasureTerm,
  ElabMeetNamedHostModuleCheckCompilePathErasureTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathErasureTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathErasureTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathErasureTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathErasureTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathComposeTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathErasureTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathErasureTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathComposeTerm walker
    tryCompileNamedHostModuleCheckCompilePathComposeTermSubset still works; this
    walker is not an alias of the CompilePathComposeTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathErasureTerm.lean. Do not skip to
    HostModuleCheckCompilePathExtractTerm. -/
elab "#elabMeetNamedHostModuleCheckCompilePathErasureTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathErasureTerm? <-
    liftIO findLiveHostModuleCheckCompilePathErasureTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathErasureTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathErasureTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathErasureTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathErasureTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathErasureTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathErasureTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathErasureTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathComposeTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathErasureTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathErasureTermSubset fakeLake fakeDir
  let notAliasOfCompilePathComposeTerm :=
    namedClosedHostModuleCheckCompilePathErasureTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfCompilePathComposeTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathErasureTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathErasureTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathErasureTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathErasureTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathErasureTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathErasureTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathErasureTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathErasureTermSubset = true := rfl
