/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-TERM-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_TERM_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-HOST-TERM-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckHostTermKernelSource,
  hostFrontLiveHostModuleCheckHostTermKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckHostTermKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckHostTermKernelSource
...
def liveRel : String := "HostModuleCheckHostTermKernel.lean"
...
def hostFrontLiveHostModuleCheckHostTermKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckHostTermKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckHostTermKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckHostTermKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckHostTermKernel.lean bytes.
    Greppable: liveHostModuleCheckHostTermKernelSource. -/


def liveHostModuleCheckHostTermKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckHostTermKernel.lean text.
    Greppable: parseLiveHostModuleCheckHostTermKernelSource. -/
def parseLiveHostModuleCheckHostTermKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckHostTermKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckHostTermKernelSource. -/
def kernelCheckLiveHostModuleCheckHostTermKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckHostTermKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckHostTermKernelSource
      liveHostModuleCheckHostTermKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthHostTermKernelBar"
      && has "checkDepthHostTermKernelScope"
      && has "hostModuleCheckHostTermKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckHostTermKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckHostTermKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckHostTermKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckHostTermKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckHostTermKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_TERM_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-TERM-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-TERM-KERNEL")
    && (liveRel == "HostModuleCheckHostTermKernel.lean")
    && (liveHostModuleCheckHostTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostTermKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckHostTermKernelFullHost
    && !hostFrontLiveHostModuleCheckHostTermKernelFullBackend
    && !hostFrontLiveHostModuleCheckHostTermKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckHostTermKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckHostTermKernelSource
      liveHostModuleCheckHostTermKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckHostTermKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-TERM-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckHostTermKernel.lean")
    ++ bad "rel" (liveHostModuleCheckHostTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostTermKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckHostTermKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckHostTermKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckHostTermKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckHostTermKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckHostTermKernelSource
      liveHostModuleCheckHostTermKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckHostTermKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckHostTermKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckHostTermKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckHostTermKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckHostTermKernel.lean != liveHostModuleCheckHostTermKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckHostTermKernel.lean")
  let r := parseLiveHostModuleCheckHostTermKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckHostTermKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckHostTermKernel parse false")
    unless hostFrontLiveHostModuleCheckHostTermKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckHostTermKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckHostTermKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckHostTermKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckHostTermKernelSource
