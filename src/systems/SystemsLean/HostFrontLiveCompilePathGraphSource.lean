/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live CompilePathGraph.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveCompilePathGraphSource. Not occupancy name 50. Not mill 70.
  This wrap parses CompilePathGraph.lean only.
  Unique needles use trailing newline so HostFrontLiveCompilePathGraph is not a
  prefix hit on HostFrontLiveCompilePathGraphSource.
  Occupancy leftover is not this wrap.
  Unique needles (trailing newline so HostFrontLiveCompilePathGraph is not a prefix):
  HostFrontLiveCompilePathGraphSource
  PARSE-LIVE-COMPILE-PATH-GRAPH
  HOST-FRONT-LIVE-COMPILE-PATH-GRAPH
  COMPILE-PATH-GRAPH
  Greppable: SYSTEMS_LEAN_HOST, liveCompilePathGraphSource,
  UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveCompilePathGraphSource
...
/-- Dual-pinned live CompilePathGraph.lean bytes (must match on-disk file).
    Greppable: liveCompilePathGraphSource, PARSE-LIVE-COMPILE-PATH-GRAPH. -/
def liveCompilePathGraphSource : String := r#"/-
