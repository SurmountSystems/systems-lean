/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckErasureSubsetEmitTerm named
  closed subset probe (plus-one after the ComposeSubsetEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ErasureSubsetEmitTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckErasureSubsetEmitTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckGraphSubsetEmitTerm, or
  ElabMeetNamedWalkHostModuleCheckComposeSubsetEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckErasureSubsetEmitTerm subset:
    prior ComposeSubsetEmitTerm closed set plus
    SystemsLean.HostModuleCheckErasureSubsetEmitTerm last.
    ...
    the ComposeSubsetEmitTerm compile order, then HostModuleCheckErasureSubsetEmitTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ComposeSubsetEmitTerm): ErasureSubsetEmitTerm
    ...
    ErasureSubsetEmitTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckErasureSubsetEmitTerm.
    HostModuleCheckErasureSubsetEmitTerm probe lives here.
  - tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckErasureSubsetEmitTerm bad writes a temp
    HostModuleCheckErasureSubsetEmitTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckComposeSubsetEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ComposeSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckComposeSubsetEmitTermSubset, and that this walk
    is not an alias of the ComposeSubsetEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckErasureSubsetEmitTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckErasureSubsetEmitTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetEmitTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckErasureSubsetEmitTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetEmitTermSubset,
  #elabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe,
  elabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe,
  SystemsLean.HostModuleCheckErasureSubsetEmitTerm,
  ElabMeetNamedHostModuleCheckErasureSubsetEmitTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetEmitTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetEmitTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetEmitTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckErasureSubsetEmitTerm named subset.
    Good compiles live Mult through HostModuleCheckComposeSubsetEmitTerm plus the
    subset-surface compile closure and HostModuleCheckErasureSubsetEmitTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckErasureSubsetEmitTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ComposeSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckComposeSubsetEmitTermSubset still works; this
    walker is not an alias of the ComposeSubsetEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckErasureSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckExtractSubsetEmitTerm. -/
def elabMeetRunNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckErasureSubsetEmitTerm? <-
    liftIO findLiveHostModuleCheckErasureSubsetEmitTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckErasureSubsetEmitTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckErasureSubsetEmitTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckErasureSubsetEmitTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckErasureSubsetEmitTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckComposeSubsetEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset fakeLake fakeDir
  let notAliasOfComposeSubsetEmitTerm :=
    namedClosedHostModuleCheckErasureSubsetEmitTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfComposeSubsetEmitTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetEmitTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckErasureSubsetEmitTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetEmitTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckErasureSubsetEmitTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe

#elabMeetNamedHostModuleCheckErasureSubsetEmitTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckErasureSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetEmitTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckErasureSubsetEmitTermSubset = true := rfl
