/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckDualResidualTerm named
  closed subset probe (plus-one after the SpecProofTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. DualResidualTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckDualResidualTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckSurfaceMatrixTerm, or
  ElabMeetNamedWalkHostModuleCheckSpecProofTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckDualResidualTerm subset:
    prior SpecProofTerm closed set plus
    SystemsLean.HostModuleCheckDualResidualTerm last.
    ...
    the SpecProofTerm compile order, then HostModuleCheckDualResidualTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as SpecProofTerm): DualResidualTerm
    ...
    DualResidualTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckDualResidualTerm.
    HostModuleCheckDualResidualTerm probe lives here.
  - tryCompileNamedHostModuleCheckDualResidualTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckDualResidualTerm bad writes a temp
    HostModuleCheckDualResidualTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckSpecProofTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior SpecProofTerm walker
    tryCompileNamedHostModuleCheckSpecProofTermSubset, and that this walk
    is not an alias of the SpecProofTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckDualResidualTerm.lean. Do not skip to
  ...
  SystemsLean.ElabMeetNamedHostModuleCheckDualResidualTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckDualResidualTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckDualResidualTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckDualResidualTerm named subset.
    Good compiles live Mult through HostModuleCheckSpecProofTerm plus the
    subset-surface compile closure and HostModuleCheckDualResidualTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckDualResidualTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior SpecProofTerm walker
    tryCompileNamedHostModuleCheckSpecProofTermSubset still works; this
    walker is not an alias of the SpecProofTerm closed set. Drive is
    good && !bad && isolation.
...
def elabMeetRunNamedHostModuleCheckDualResidualTermSubsetProbe : CommandElabM Unit := do
  ...
  let liveHostModuleCheckDualResidualTerm? <-
    liftIO findLiveHostModuleCheckDualResidualTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckDualResidualTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckDualResidualTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckDualResidualTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckDualResidualTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckDualResidualTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckDualResidualTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckDualResidualTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSpecProofTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckDualResidualTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckDualResidualTermSubset fakeLake fakeDir
  let notAliasOfSpecProofTerm :=
    namedClosedHostModuleCheckDualResidualTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckDualResidualTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckDualResidualTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckDualResidualTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckDualResidualTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckDualResidualTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckDualResidualTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckDualResidualTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckDualResidualTermSubset = true := rfl
