/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckEmitProgramScaffoldTerm named
  closed subset probe (plus-one after the EmitTypesScaffoldTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. EmitProgramScaffoldTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckEmitProgramScaffoldTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckEmitLinearScaffoldTerm, or
  ElabMeetNamedWalkHostModuleCheckEmitTypesScaffoldTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckEmitProgramScaffoldTerm subset:
    prior EmitTypesScaffoldTerm closed set plus
    SystemsLean.HostModuleCheckEmitProgramScaffoldTerm last.
    ...
    the EmitTypesScaffoldTerm compile order, then HostModuleCheckEmitProgramScaffoldTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as EmitTypesScaffoldTerm): EmitProgramScaffoldTerm
    ...
    EmitProgramScaffoldTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckEmitProgramScaffoldTerm.
    HostModuleCheckEmitProgramScaffoldTerm probe lives here.
  - tryCompileNamedHostModuleCheckEmitProgramScaffoldTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckEmitProgramScaffoldTerm bad writes a temp
    HostModuleCheckEmitProgramScaffoldTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckEmitTypesScaffoldTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior EmitTypesScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitTypesScaffoldTermSubset, and that this walk
    is not an alias of the EmitTypesScaffoldTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckEmitProgramScaffoldTerm.lean. Do not skip to
  ...
  findLiveHostModuleCheckEmitProgramScaffoldTermPath,
  ...
  barrelListsNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  tryCompileNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  tryCompileAfterHostModuleCheckEmitProgramScaffoldTermDeps,
  elabMeetDrivesNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitProgramScaffoldTermSubset,
  #elabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe,
  elabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe,
  SystemsLean.HostModuleCheckEmitProgramScaffoldTerm,
  ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckEmitProgramScaffoldTerm named subset.
    Good compiles live Mult through HostModuleCheckEmitTypesScaffoldTerm plus the
    subset-surface compile closure and HostModuleCheckEmitProgramScaffoldTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckEmitProgramScaffoldTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior EmitTypesScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitTypesScaffoldTermSubset still works; this
    walker is not an alias of the EmitTypesScaffoldTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckEmitProgramScaffoldTerm.lean. Do not skip to
    HostModuleCheckEmitGraphScaffoldTerm. -/
def elabMeetRunNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckEmitProgramScaffoldTerm? <-
    liftIO findLiveHostModuleCheckEmitProgramScaffoldTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitProgramScaffoldTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckEmitProgramScaffoldTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckEmitProgramScaffoldTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckEmitProgramScaffoldTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckEmitProgramScaffoldTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitTypesScaffoldTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckEmitProgramScaffoldTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckEmitProgramScaffoldTermSubset fakeLake fakeDir
  let notAliasOfEmitTypesScaffoldTerm :=
    namedClosedHostModuleCheckEmitProgramScaffoldTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfEmitTypesScaffoldTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckEmitProgramScaffoldTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckEmitProgramScaffoldTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitProgramScaffoldTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckEmitProgramScaffoldTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe

#elabMeetNamedHostModuleCheckEmitProgramScaffoldTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckEmitProgramScaffoldTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckEmitProgramScaffoldTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitProgramScaffoldTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckEmitProgramScaffoldTermSubset = true := rfl
