/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathBodyTerm named
  closed subset probe (plus-one after the CompilePathApplyTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathBodyTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathBodyTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathPlanTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathApplyTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathBodyTerm subset:
    prior CompilePathApplyTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathBodyTerm last.
    ...
    the CompilePathApplyTerm compile order, then HostModuleCheckCompilePathBodyTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathApplyTerm): CompilePathBodyTerm
    ...
    CompilePathBodyTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathBodyTerm.
    HostModuleCheckCompilePathBodyTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathBodyTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathBodyTerm bad writes a temp
    HostModuleCheckCompilePathBodyTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathApplyTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathApplyTerm walker
    tryCompileNamedHostModuleCheckCompilePathApplyTermSubset, and that this walk
    is not an alias of the CompilePathApplyTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathBodyTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathBodyTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathBodyTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathBodyTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathBodyTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathBodyTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathBodyTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathBodyTerm,
  ElabMeetNamedHostModuleCheckCompilePathBodyTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathBodyTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathBodyTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathBodyTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathBodyTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathApplyTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathBodyTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathBodyTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathApplyTerm walker
    tryCompileNamedHostModuleCheckCompilePathApplyTermSubset still works; this
    walker is not an alias of the CompilePathApplyTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathBodyTerm.lean. Do not skip to
    HostModuleCheckFirstSurfaceTerm. -/
elab "#elabMeetNamedHostModuleCheckCompilePathBodyTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathBodyTerm? <-
    liftIO findLiveHostModuleCheckCompilePathBodyTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathBodyTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathBodyTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathBodyTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathBodyTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathBodyTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathBodyTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathBodyTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfCompilePathApplyTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathBodyTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathBodyTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathBodyTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathBodyTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathBodyTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathBodyTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathBodyTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathBodyTermSubset = true := rfl
