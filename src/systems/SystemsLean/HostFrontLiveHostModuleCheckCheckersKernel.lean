/-
  ...
  Short role: HostFrontLiveHostModuleCheckCheckersKernel.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCheckersKernelSource turns live
    HostModuleCheckCheckersKernel.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCheckersKernel
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCheckersKernel.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCheckersKernelSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCheckersKernelSource. Not split.
  ...
  - Skip-head still sees def checkDepthCheckersKernelBar and
    def hostModuleCheckCheckersKernelOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCheckersKernel
  PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL
  HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSKERNEL
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCHECKERSKERNEL_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCheckersKernelSource,
  kernelCheckLiveHostModuleCheckCheckersKernelSource,
  hostFrontLiveHostModuleCheckCheckersKernelReady,
  liveHostModuleCheckCheckersKernelSource,
  liveRel, liveHostModuleCheckCheckersKernelRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCheckersKernel
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCheckersKernelSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSKERNEL"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCheckersKernel.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCheckersKernelSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCheckersKernelSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCheckersKernelFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCheckersKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCheckersKernelProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCheckersKernel has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCheckersKernel.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCheckersKernel`. -/
...
/-- Parse live HostModuleCheckCheckersKernel.lean text.
    Greppable: parseLiveHostModuleCheckCheckersKernelSource,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL. -/
def parseLiveHostModuleCheckCheckersKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCheckersKernel"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCheckersKernelSource,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL. -/
def kernelCheckLiveHostModuleCheckCheckersKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCheckersKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCheckersKernelSource
      liveHostModuleCheckCheckersKernelSource with
...
/-- Wrap module lastSeg is HostModuleCheckCheckersKernel
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCheckersKernel"
      && lastSeg m.name.raw == "HostModuleCheckCheckersKernel"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCheckersKernelSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthCheckersKernelBar"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCheckersKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCheckersKernelReady,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL,
    HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSKERNEL.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCheckersKernelReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCHECKERSKERNEL_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSKERNEL")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL")
    && (liveRel == "HostModuleCheckCheckersKernel.lean")
    && (liveHostModuleCheckCheckersKernelRel ==
        "src/systems/SystemsLean/HostModuleCheckCheckersKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCheckersKernelFullHost
    && !hostFrontLiveHostModuleCheckCheckersKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCheckersKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCheckersKernelSource
        liveHostModuleCheckCheckersKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCheckersKernelSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCheckersKernelRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCheckersKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCheckersKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCheckersKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCheckersKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCheckersKernel.lean != liveHostModuleCheckCheckersKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCheckersKernel.lean")
  let r := parseLiveHostModuleCheckCheckersKernelSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCHECKERSKERNEL ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCheckersKernel parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCheckersKernel parse false")
    unless hostFrontLiveHostModuleCheckCheckersKernelReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCheckersKernelReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCheckersKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCheckersKernel source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCheckersKernel source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCheckersKernel.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
