/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-TERM-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_PROGRAM_TERM_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-TERM-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckKernelProgramTermKernelSource,
  hostFrontLiveHostModuleCheckKernelProgramTermKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelProgramTermKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelProgramTermKernelSource
...
def liveRel : String := "HostModuleCheckKernelProgramTermKernel.lean"
...
def hostFrontLiveHostModuleCheckKernelProgramTermKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckKernelProgramTermKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckKernelProgramTermKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckKernelProgramTermKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckKernelProgramTermKernel.lean bytes.
    Greppable: liveHostModuleCheckKernelProgramTermKernelSource. -/


def liveHostModuleCheckKernelProgramTermKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckKernelProgramTermKernel.lean text.
    Greppable: parseLiveHostModuleCheckKernelProgramTermKernelSource. -/
def parseLiveHostModuleCheckKernelProgramTermKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckKernelProgramTermKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckKernelProgramTermKernelSource. -/
def kernelCheckLiveHostModuleCheckKernelProgramTermKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckKernelProgramTermKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckKernelProgramTermKernelSource
      liveHostModuleCheckKernelProgramTermKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthKernelProgramTermKernelBar"
      && has "checkDepthKernelProgramTermKernelScope"
      && has "hostModuleCheckKernelProgramTermKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckKernelProgramTermKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckKernelProgramTermKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckKernelProgramTermKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckKernelProgramTermKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckKernelProgramTermKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_PROGRAM_TERM_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-TERM-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-TERM-KERNEL")
    && (liveRel == "HostModuleCheckKernelProgramTermKernel.lean")
    && (liveHostModuleCheckKernelProgramTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelProgramTermKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckKernelProgramTermKernelFullHost
    && !hostFrontLiveHostModuleCheckKernelProgramTermKernelFullBackend
    && !hostFrontLiveHostModuleCheckKernelProgramTermKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckKernelProgramTermKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckKernelProgramTermKernelSource
      liveHostModuleCheckKernelProgramTermKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckKernelProgramTermKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-PROGRAM-TERM-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckKernelProgramTermKernel.lean")
    ++ bad "rel" (liveHostModuleCheckKernelProgramTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelProgramTermKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckKernelProgramTermKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckKernelProgramTermKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckKernelProgramTermKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckKernelProgramTermKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckKernelProgramTermKernelSource
      liveHostModuleCheckKernelProgramTermKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckKernelProgramTermKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckKernelProgramTermKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckKernelProgramTermKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckKernelProgramTermKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckKernelProgramTermKernel.lean != liveHostModuleCheckKernelProgramTermKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckKernelProgramTermKernel.lean")
  let r := parseLiveHostModuleCheckKernelProgramTermKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckKernelProgramTermKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckKernelProgramTermKernel parse false")
    unless hostFrontLiveHostModuleCheckKernelProgramTermKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckKernelProgramTermKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckKernelProgramTermKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckKernelProgramTermKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelProgramTermKernelSource
