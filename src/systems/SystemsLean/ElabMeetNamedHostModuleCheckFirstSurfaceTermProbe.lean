/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckFirstSurfaceTerm named
  closed subset probe (plus-one after the CompilePathBodyTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. FirstSurfaceTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckFirstSurfaceTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathApplyTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathBodyTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckFirstSurfaceTerm subset:
    prior CompilePathBodyTerm closed set plus
    SystemsLean.HostModuleCheckFirstSurfaceTerm last.
    ...
    the CompilePathBodyTerm compile order, then HostModuleCheckFirstSurfaceTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathBodyTerm): FirstSurfaceTerm
    ...
    FirstSurfaceTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckFirstSurfaceTerm.
    HostModuleCheckFirstSurfaceTerm probe lives here.
  - tryCompileNamedHostModuleCheckFirstSurfaceTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckFirstSurfaceTerm bad writes a temp
    HostModuleCheckFirstSurfaceTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathBodyTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathBodyTerm walker
    tryCompileNamedHostModuleCheckCompilePathBodyTermSubset, and that this walk
    is not an alias of the CompilePathBodyTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckFirstSurfaceTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckFirstSurfaceTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckFirstSurfaceTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckFirstSurfaceTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckFirstSurfaceTermSubset,
  #elabMeetNamedHostModuleCheckFirstSurfaceTermSubsetProbe,
  elabMeetNamedHostModuleCheckFirstSurfaceTermSubsetProbe,
  SystemsLean.HostModuleCheckFirstSurfaceTerm,
  ElabMeetNamedHostModuleCheckFirstSurfaceTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckFirstSurfaceTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckFirstSurfaceTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckFirstSurfaceTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckFirstSurfaceTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathBodyTerm plus the
    subset-surface compile closure and HostModuleCheckFirstSurfaceTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckFirstSurfaceTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathBodyTerm walker
    tryCompileNamedHostModuleCheckCompilePathBodyTermSubset still works; this
    walker is not an alias of the CompilePathBodyTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckFirstSurfaceTerm.lean. Do not skip to
    HostModuleCheckLinearSubsetEmitTerm. -/
def elabMeetRunNamedHostModuleCheckFirstSurfaceTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckFirstSurfaceTerm? <-
    liftIO findLiveHostModuleCheckFirstSurfaceTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckFirstSurfaceTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckFirstSurfaceTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckFirstSurfaceTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckFirstSurfaceTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckFirstSurfaceTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckFirstSurfaceTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckFirstSurfaceTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathBodyTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckFirstSurfaceTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckFirstSurfaceTermSubset fakeLake fakeDir
  let notAliasOfCompilePathBodyTerm :=
    namedClosedHostModuleCheckFirstSurfaceTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckFirstSurfaceTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckFirstSurfaceTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckFirstSurfaceTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckFirstSurfaceTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckFirstSurfaceTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckFirstSurfaceTermSubsetProbe

#elabMeetNamedHostModuleCheckFirstSurfaceTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckFirstSurfaceTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckFirstSurfaceTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckFirstSurfaceTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckFirstSurfaceTermSubset = true := rfl
