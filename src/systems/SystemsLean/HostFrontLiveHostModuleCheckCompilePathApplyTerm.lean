/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathApplyTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathApplyTermSource turns live
    HostModuleCheckCompilePathApplyTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathApplyTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathApplyTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathApplyTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathApplyTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathApplyTermDialect and
    def hostModuleCheckCompilePathApplyTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathApplyTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHAPPLYTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathApplyTermSource,
  kernelCheckLiveHostModuleCheckCompilePathApplyTermSource,
  hostFrontLiveHostModuleCheckCompilePathApplyTermReady,
  liveHostModuleCheckCompilePathApplyTermSource,
  liveRel, liveHostModuleCheckCompilePathApplyTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathApplyTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathApplyTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathApplyTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathApplyTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathApplyTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathApplyTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathApplyTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathApplyTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathApplyTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathApplyTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathApplyTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathApplyTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathApplyTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM. -/
def parseLiveHostModuleCheckCompilePathApplyTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathApplyTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathApplyTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathApplyTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathApplyTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathApplyTermSource
      liveHostModuleCheckCompilePathApplyTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathApplyTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathApplyTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathApplyTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathApplyTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathApplyTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathApplyTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathApplyTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathApplyTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHAPPLYTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM")
    && (liveRel == "HostModuleCheckCompilePathApplyTerm.lean")
    && (liveHostModuleCheckCompilePathApplyTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathApplyTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathApplyTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathApplyTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathApplyTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathApplyTermSource
        liveHostModuleCheckCompilePathApplyTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathApplyTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathApplyTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathApplyTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathApplyTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathApplyTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathApplyTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathApplyTerm.lean != liveHostModuleCheckCompilePathApplyTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathApplyTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathApplyTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHAPPLYTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathApplyTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathApplyTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathApplyTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathApplyTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathApplyTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathApplyTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathApplyTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathApplyTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
