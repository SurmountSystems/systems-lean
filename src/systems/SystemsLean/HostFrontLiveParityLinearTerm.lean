...
def cmdBodyKnownPlt (kn : List String) : Cmd -> Bool
  | Cmd.def_ _ _ body =>
      termKnownN liveHostTermParseFuel kn body
        ...
        && termNoAppN liveParityLinearTermParseFuel body
...
/-- Live parse has core ParityLinearTerm defs (kept or skipped head).
  ...
  | some m =>
    let hasCmd (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    let toks := tokenizeHostTerm (stripComments liveParityLinearTermSource)
    let has (nm : String) : Bool :=
      hasCmd nm || toksHaveDefNamed liveParityLinearTermSkipFuel toks nm
