/-
  ...
  Short role: HostFrontLiveHostModuleCheckFirstSurfaceTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckFirstSurfaceTermSource turns live
    HostModuleCheckFirstSurfaceTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckFirstSurfaceTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckFirstSurfaceTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckFirstSurfaceTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckFirstSurfaceTermSource. Not split.
  ...
  - Kept: five imports, the namespace, four literal defs, and end.
    Kept count is 11.
    The four literal defs are checkDepthFirstSurfaceTermSurfaceBar,
    checkDepthFirstSurfaceTermSurfaceScope,
    hostModuleCheckFirstSurfaceTermSurfaceDualOk, and
  ...
  - The longest quote span needs 342 take steps, over the 256 take
    in HostFrontLiveHostTerm.liveHostTermParseFuel.
    liveStringFuel is 1024 so takeStringN does not cut that span.
  ...
  HostFrontLiveHostModuleCheckFirstSurfaceTerm
  PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM
  HOST-FRONT-LIVE-HOSTMODULECHECKFIRSTSURFACETERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIRSTSURFACETERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckFirstSurfaceTermSource,
  kernelCheckLiveHostModuleCheckFirstSurfaceTermSource,
  hostFrontLiveHostModuleCheckFirstSurfaceTermReady,
  liveHostModuleCheckFirstSurfaceTermSource,
  liveRel, liveHostModuleCheckFirstSurfaceTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckFirstSurfaceTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckFirstSurfaceTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKFIRSTSURFACETERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckFirstSurfaceTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckFirstSurfaceTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckFirstSurfaceTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckFirstSurfaceTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckFirstSurfaceTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckFirstSurfaceTermProvablyUnlocked : Bool :=
  false
...
/-- String take fuel. Longest quote span needs 342 steps, over 256.
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckFirstSurfaceTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckFirstSurfaceTerm.lean
...
/-- Parse live HostModuleCheckFirstSurfaceTerm.lean text.
    Greppable: parseLiveHostModuleCheckFirstSurfaceTermSource,
    PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM. -/
def parseLiveHostModuleCheckFirstSurfaceTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckFirstSurfaceTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckFirstSurfaceTermSource,
    PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM. -/
def kernelCheckLiveHostModuleCheckFirstSurfaceTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckFirstSurfaceTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckFirstSurfaceTermSource
      liveHostModuleCheckFirstSurfaceTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthFirstSurfaceTermSurfaceBar",
      "checkDepthFirstSurfaceTermSurfaceScope",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckFirstSurfaceTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckFirstSurfaceTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckFirstSurfaceTerm"
      && lastSeg m.name.raw == "HostModuleCheckFirstSurfaceTerm"
...
def liveCoreDefNames : List String := [
  ...
  "checkDepthFirstSurfaceTermSurfaceScope",
  "hostModuleCheckFirstSurfaceTermSurfaceDualOk",
  "firstSurfaceReadyTermBody",
  "firstSurfaceEmitBodyOk",
  "firstSurfaceResidualFreeBodyOk",
  "firstSurfaceCompleteBodyOk",
  "firstSurfaceReadyBodyOk",
...
/-- Theorem names present in the live text. Proofs are skip-folded. -/
def liveCoreTheoremNames : List String := [
  "hostModuleCheckFirstSurfaceTermSurfaceOk_true",
  "hostModuleCheckFirstSurfaceReadyMultilineOk_true",
  "hostModuleCheck_firstSurface_term_surface_ids_eq"
]
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckFirstSurfaceTermSource)
    liveCoreDefNames.all (fun nm => toksHaveDefNamed liveSkipFuel toks nm)
      && liveCoreTheoremNames.all
        (fun nm => toksHaveTheoremNamed liveSkipFuel toks nm)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckFirstSurfaceTermSource
  liveNeedles.all fun nd => (src.splitOn nd).length == 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckFirstSurfaceTermReady,
    PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKFIRSTSURFACETERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckFirstSurfaceTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIRSTSURFACETERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKFIRSTSURFACETERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM")
    && (liveRel == "HostModuleCheckFirstSurfaceTerm.lean")
    && (liveHostModuleCheckFirstSurfaceTermRel ==
        "src/systems/SystemsLean/HostModuleCheckFirstSurfaceTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckFirstSurfaceTermFullHost
    && !hostFrontLiveHostModuleCheckFirstSurfaceTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckFirstSurfaceTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckFirstSurfaceTermSource
        liveHostModuleCheckFirstSurfaceTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckFirstSurfaceTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckFirstSurfaceTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckFirstSurfaceTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckFirstSurfaceTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckFirstSurfaceTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckFirstSurfaceTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckFirstSurfaceTerm.lean != liveHostModuleCheckFirstSurfaceTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckFirstSurfaceTerm.lean")
  let r := parseLiveHostModuleCheckFirstSurfaceTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKFIRSTSURFACETERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckFirstSurfaceTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckFirstSurfaceTerm parse false")
    unless hostFrontLiveHostModuleCheckFirstSurfaceTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckFirstSurfaceTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckFirstSurfaceTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckFirstSurfaceTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckFirstSurfaceTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckFirstSurfaceTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
