/-
  ...
  src/systems/SystemsLean/HostModuleCheckSmoke.lean with the
  ...
  The live file has one import. That import stays inside the
  embedded source bytes. This checker does not import it.
  Checker imports: HostFrontLiveHostTerm, the Source module, and HostKernel.
  Namespace SystemsLean.HostModuleCheck.
  Kept for the kernel: that one import, the namespace, and the end.
  There is no column-0 def. example is not kept. set_option is not kept.
  There is no open. Theorems are not kept.
  A product Bool that appears inside an example is not a kept atom.
  ...
  Kept count is 3, at most kernelFuel 64. kernelFuel is not raised.
  liveStringFuel is 8192. That is not kernelFuel.
  One raw string. The product is 384 lines, so the source file is not split.
  Not an LLVM backend.
  ...
  HOST-FRONT-LIVE-HOSTMODULECHECKSMOKE,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKSMOKE_V0,
  PARSE-LIVE-HOSTMODULECHECKSMOKE, liveRel,
  kernelCheckLiveHostModuleCheckSmokeSource,
  hostFrontLiveHostModuleCheckSmokeReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckSmoke
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckSmokeSource
import SystemsLean.HostKernel

set_option maxRecDepth 131072
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckSmoke
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckSmoke.lean"
...
def hostFrontLiveHostModuleCheckSmokeFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckSmokeFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckSmokeResidualFreeClaimed : Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckSmokeProvablyUnlocked : Bool := false
...
/-- String-literal scan. Not kernelFuel. kernelFuel stays 64.
    Example bodies hold long String literals. Those strings are not
    kept commands. 8192 covers the longest quote. -/
def liveStringFuel : Nat := 8192

/-- Kept-command count: one import, the namespace, and the end.
    example, set_option, open, and theorem are not kept.
    There is no column-0 def. kernelFuel is 64. -/
...
/-- Parse live HostModuleCheckSmoke.lean text.
    Greppable: parseLiveHostModuleCheckSmokeSource,
    PARSE-LIVE-HOSTMODULECHECKSMOKE. -/
def parseLiveHostModuleCheckSmokeSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckSmoke"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckSmokeSource,
    PARSE-LIVE-HOSTMODULECHECKSMOKE. -/
def kernelCheckLiveHostModuleCheckSmokeSource (src : String) : Bool :=
  match parseLiveHostModuleCheckSmokeSource src with
...
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckSmokeSource liveHostModuleCheckSmokeSource with
...
/-- One import, SystemsLean.HostModuleCheck.
    The product module is not an import of this checker. -/
def liveParseImportsOk : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    importNames m.commands == ["SystemsLean.HostModuleCheck"]
...
/-- After the one import: namespace, then end.
    No kept def sits between them. An empty middle is allowed. -/
...
/-- Examples and set_option are in the live text and are not kept.
    Kept commands are only the import, the namespace, and the end.
    There is no column-0 def. A Bool inside an example is not a kept atom
    and is not this checker's kernelCheck. -/
def liveParseExamplesNotKept : Bool :=
  ...
  | some m =>
    (m.commands.all fun c =>
      ...
      | _ => false)
      && (liveHostModuleCheckSmokeSource.splitOn "\nexample").length > 2
      && (liveHostModuleCheckSmokeSource.splitOn "\nset_option ").length == 2
...
/-- Wrap module lastSeg is HostModuleCheckSmoke
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckSmoke"
      && lastSeg m.name.raw == "HostModuleCheckSmoke"

/-- Import needle, column 0, once. -/
...
/-- Example lines are present. They are not kept commands. -/
def needleExample : String := "\nexample"

/-- Each scaffold needle occurs once. Examples occur more than once.
    splitOn, not a prefix check. One import. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckSmokeSource
  (src.splitOn needleImport).length == 2
    && (src.splitOn needleNamespace).length == 2
    && (src.splitOn needleEnd).length == 2
    && (src.splitOn needleExample).length > 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckSmokeReady,
    PARSE-LIVE-HOSTMODULECHECKSMOKE,
    HOST-FRONT-LIVE-HOSTMODULECHECKSMOKE.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckSmokeReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKSMOKE_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKSMOKE")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKSMOKE")
    && (liveRel == "HostModuleCheckSmoke.lean")
    && (liveHostModuleCheckSmokeRel ==
        "src/systems/SystemsLean/HostModuleCheckSmoke.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckSmokeFullHost
    && !hostFrontLiveHostModuleCheckSmokeFullBackend
    && !hostFrontLiveHostModuleCheckSmokeResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckSmokeProvablyUnlocked
    && kernelCheckLiveHostModuleCheckSmokeSource liveHostModuleCheckSmokeSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckSmokeSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKSMOKE =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckSmokeRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckSmokeRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckSmokeRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckSmokeRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckSmokeSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckSmoke.lean != liveHostModuleCheckSmokeSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckSmoke.lean")
  let r := parseLiveHostModuleCheckSmokeSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKSMOKE reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKSMOKE reject {reason}")
  | FrontResult.accept m =>
    let k :=
      if m.commands.length > kernelFuel then false
      else HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKSMOKE ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckSmoke parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckSmoke parse false")
    unless hostFrontLiveHostModuleCheckSmokeReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckSmokeReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckSmokeReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckSmoke source was accepted"
      throw (IO.userError "empty HostModuleCheckSmoke source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckSmoke
