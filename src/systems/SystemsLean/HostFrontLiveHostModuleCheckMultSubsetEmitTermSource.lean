/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckMultSubsetEmitTerm.lean.
  ...
  liveRel is HostModuleCheckMultSubsetEmitTerm.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckMultSubsetEmitTermSource,
  PARSE-LIVE-HOST-MODULE-CHECK-MULT-SUBSET-EMIT-TERM.
  Module: SystemsLean.HostFrontLiveHostModuleCheckMultSubsetEmitTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckMultSubsetEmitTermSource
...
def liveRel : String := "HostModuleCheckMultSubsetEmitTerm.lean"

/-- Pinned live HostModuleCheckMultSubsetEmitTerm.lean bytes. -/
def liveHostModuleCheckMultSubsetEmitTermSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckMultSubsetEmitTermSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckMultSubsetEmitTermSourceReady : Bool :=
  liveRel == "HostModuleCheckMultSubsetEmitTerm.lean"
    && kernelCheckLiveHostModuleCheckMultSubsetEmitTermSource liveHostModuleCheckMultSubsetEmitTermSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckMultSubsetEmitTermSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckMultSubsetEmitTerm.lean")
  let k := kernelCheckLiveHostModuleCheckMultSubsetEmitTermSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckMultSubsetEmitTerm parse false")
  unless hostFrontLiveHostModuleCheckMultSubsetEmitTermSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckMultSubsetEmitTermSource
