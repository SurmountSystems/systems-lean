/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckMultSubsetEmitTerm named
  closed subset probe (plus-one after the ExtractSubsetEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. MultSubsetEmitTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckMultSubsetEmitTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckErasureSubsetEmitTerm, or
  ElabMeetNamedWalkHostModuleCheckExtractSubsetEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckMultSubsetEmitTerm subset:
    prior ExtractSubsetEmitTerm closed set plus
    SystemsLean.HostModuleCheckMultSubsetEmitTerm last.
    ...
    the ExtractSubsetEmitTerm compile order, then HostModuleCheckMultSubsetEmitTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ExtractSubsetEmitTerm): MultSubsetEmitTerm
    ...
    MultSubsetEmitTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckMultSubsetEmitTerm.
    HostModuleCheckMultSubsetEmitTerm probe lives here.
  - tryCompileNamedHostModuleCheckMultSubsetEmitTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckMultSubsetEmitTerm bad writes a temp
    HostModuleCheckMultSubsetEmitTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckExtractSubsetEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ExtractSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset, and that this walk
    is not an alias of the ExtractSubsetEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckMultSubsetEmitTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckMultSubsetEmitTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckMultSubsetEmitTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckMultSubsetEmitTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckMultSubsetEmitTermSubset,
  #elabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetProbe,
  elabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetProbe,
  SystemsLean.HostModuleCheckMultSubsetEmitTerm,
  ElabMeetNamedHostModuleCheckMultSubsetEmitTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckMultSubsetEmitTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckMultSubsetEmitTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckMultSubsetEmitTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckMultSubsetEmitTerm named subset.
    Good compiles live Mult through HostModuleCheckExtractSubsetEmitTerm plus the
    subset-surface compile closure and HostModuleCheckMultSubsetEmitTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckMultSubsetEmitTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ExtractSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset still works; this
    walker is not an alias of the ExtractSubsetEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckMultSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckMultSubsetRebuildTerm. -/
def elabMeetRunNamedHostModuleCheckMultSubsetEmitTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckMultSubsetEmitTerm? <-
    liftIO findLiveHostModuleCheckMultSubsetEmitTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckMultSubsetEmitTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckMultSubsetEmitTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckMultSubsetEmitTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckMultSubsetEmitTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckMultSubsetEmitTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckMultSubsetEmitTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckMultSubsetEmitTermSubset fakeLake fakeDir
  let notAliasOfExtractSubsetEmitTerm :=
    namedClosedHostModuleCheckMultSubsetEmitTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfExtractSubsetEmitTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckMultSubsetEmitTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckMultSubsetEmitTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckMultSubsetEmitTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckMultSubsetEmitTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckMultSubsetEmitTermSubsetProbe

#elabMeetNamedHostModuleCheckMultSubsetEmitTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckMultSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckMultSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckMultSubsetEmitTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckMultSubsetEmitTermSubset = true := rfl
