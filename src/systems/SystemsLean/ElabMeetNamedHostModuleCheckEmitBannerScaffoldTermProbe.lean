/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckEmitBannerScaffoldTerm named
  closed subset probe (plus-one after the EmitExtractScaffoldTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. EmitBannerScaffoldTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckEmitBannerScaffoldTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckEmitErasureScaffoldTerm, or
  ElabMeetNamedWalkHostModuleCheckEmitExtractScaffoldTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckEmitBannerScaffoldTerm subset:
    prior EmitExtractScaffoldTerm closed set plus
    SystemsLean.HostModuleCheckEmitBannerScaffoldTerm last.
    ...
    the EmitExtractScaffoldTerm compile order, then HostModuleCheckEmitBannerScaffoldTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as EmitExtractScaffoldTerm): EmitBannerScaffoldTerm
    ...
    EmitBannerScaffoldTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckEmitBannerScaffoldTerm.
    HostModuleCheckEmitBannerScaffoldTerm probe lives here.
  - tryCompileNamedHostModuleCheckEmitBannerScaffoldTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckEmitBannerScaffoldTerm bad writes a temp
    HostModuleCheckEmitBannerScaffoldTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckEmitExtractScaffoldTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior EmitExtractScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset, and that this walk
    is not an alias of the EmitExtractScaffoldTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckEmitBannerScaffoldTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckEmitBannerScaffoldTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckEmitBannerScaffoldTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckEmitBannerScaffoldTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitBannerScaffoldTermSubset,
  #elabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe,
  elabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe,
  SystemsLean.HostModuleCheckEmitBannerScaffoldTerm,
  ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckEmitBannerScaffoldTerm named subset.
    Good compiles live Mult through HostModuleCheckEmitExtractScaffoldTerm plus the
    subset-surface compile closure and HostModuleCheckEmitBannerScaffoldTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckEmitBannerScaffoldTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior EmitExtractScaffoldTerm walker
    tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset still works; this
    walker is not an alias of the EmitExtractScaffoldTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckEmitBannerScaffoldTerm.lean. Do not skip to
    HostModuleCheckKernelSelfApplyTerm. -/
def elabMeetRunNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckEmitBannerScaffoldTerm? <-
    liftIO findLiveHostModuleCheckEmitBannerScaffoldTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitBannerScaffoldTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckEmitBannerScaffoldTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckEmitBannerScaffoldTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckEmitBannerScaffoldTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckEmitBannerScaffoldTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckEmitExtractScaffoldTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckEmitBannerScaffoldTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckEmitBannerScaffoldTermSubset fakeLake fakeDir
  let notAliasOfEmitExtractScaffoldTerm :=
    namedClosedHostModuleCheckEmitBannerScaffoldTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfEmitExtractScaffoldTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckEmitBannerScaffoldTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckEmitBannerScaffoldTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitBannerScaffoldTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckEmitBannerScaffoldTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe

#elabMeetNamedHostModuleCheckEmitBannerScaffoldTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckEmitBannerScaffoldTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckEmitBannerScaffoldTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckEmitBannerScaffoldTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckEmitBannerScaffoldTermSubset = true := rfl
