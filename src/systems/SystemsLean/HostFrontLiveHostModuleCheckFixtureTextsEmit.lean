/-
  ...
  Short role: HostFrontLiveHostModuleCheckFixtureTextsEmit.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckFixtureTextsEmitSource turns live
    HostModuleCheckFixtureTextsEmit.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckFixtureTextsEmit
    even without a module line.
  - liveRel is the exact basename HostModuleCheckFixtureTextsEmit.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckFixtureTextsEmitSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckFixtureTextsEmitSource. Not split.
  - Real commands in this file: namespace SystemsLean.HostModuleCheck,
    thirty-five defs, and end. No imports. No theorems. That is 37.
  ...
  - Skipped: thirty-five append defs. Skipped count is 35.
    Kept plus skipped is 37.
    kernelFuel is 64. 37 does not pass 64. Fuel is not raised.
    Defs are not dropped to squeeze under 64.
  - The longest source line is 111 characters. liveStringFuel is 1024.
  ...
  HostFrontLiveHostModuleCheckFixtureTextsEmit
  PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT
  HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIXTURETEXTSEMIT_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckFixtureTextsEmitSource,
  kernelCheckLiveHostModuleCheckFixtureTextsEmitSource,
  hostFrontLiveHostModuleCheckFixtureTextsEmitReady,
  liveHostModuleCheckFixtureTextsEmitSource,
  liveRel, liveHostModuleCheckFixtureTextsEmitRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsEmit
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsEmitSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsEmit
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckFixtureTextsEmit.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckFixtureTextsEmitSource is the one embedded copy
    from HostFrontLiveHostModuleCheckFixtureTextsEmitSource. Not split. -/
...
def hostFrontLiveHostModuleCheckFixtureTextsEmitFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckFixtureTextsEmitResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckFixtureTextsEmitProvablyUnlocked : Bool :=
  false
...
/-- String take fuel. Longest source line is 111, under 1024. -/
def liveStringFuel : Nat := 1024

/-- Real command count: namespace, thirty-five defs, and end. -/
def liveRealCmds : Nat := 37

/-- Kept-command count: namespace and end.
    Thirty-five string-append defs are skip-folded.
    Skipped count is 35. Kept plus skipped is 37.
    kernelFuel is 64. 37 does not pass 64. -/
def liveKeptCmds : Nat := 2

/-- Skipped command count. -/
def liveSkippedCmds : Nat := 35
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckFixtureTextsEmit has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckFixtureTextsEmit.lean
...
/-- Parse live HostModuleCheckFixtureTextsEmit.lean text.
    Greppable: parseLiveHostModuleCheckFixtureTextsEmitSource,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT. -/
def parseLiveHostModuleCheckFixtureTextsEmitSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckFixtureTextsEmit"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckFixtureTextsEmitSource,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT. -/
def kernelCheckLiveHostModuleCheckFixtureTextsEmitSource (src : String) : Bool :=
  match parseLiveHostModuleCheckFixtureTextsEmitSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckFixtureTextsEmitSource
      liveHostModuleCheckFixtureTextsEmitSource with
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckFixtureTextsEmitSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckFixtureTextsEmit
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckFixtureTextsEmit"
      && lastSeg m.name.raw == "HostModuleCheckFixtureTextsEmit"
...
def liveCoreDefNames : List String := [
  ...
  "hostModuleCheckGoodEmitMultText",
  "hostModuleCheckGoodEmitLinearText",
  "hostModuleCheckGoodEmitTypesText",
  "hostModuleCheckGoodEmitProgramText",
  "hostModuleCheckGoodEmitGraphText",
  "hostModuleCheckGoodEmitComposeText",
  "hostModuleCheckGoodEmitErasureText",
  "hostModuleCheckGoodEmitExtractText",
  "hostModuleCheckGoodEmitBannerText",
...
/-- Skip-head: the thirty-five append defs are in the live text. -/
def liveParseHasCoreDefs : Bool :=
  match liveParsed? with
  | none => false
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckFixtureTextsEmitSource)
...
def liveNeedles : List String := [
  ...
  "def hostModuleCheckGoodProductPathOwnershipInputsText : String :=\n",
  "def hostModuleCheckGoodProductPathOwnershipInputsTheoremsText : String :=\n",
  "end SystemsLean.HostModuleCheck\n"
]

/-- Each needle occurs once. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckFixtureTextsEmitSource
  liveNeedles.all fun nd => (src.splitOn nd).length == 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckFixtureTextsEmitReady,
    PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT,
    HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckFixtureTextsEmitReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKFIXTURETEXTSEMIT_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT")
    && (liveRel == "HostModuleCheckFixtureTextsEmit.lean")
    && (liveHostModuleCheckFixtureTextsEmitRel ==
        "src/systems/SystemsLean/HostModuleCheckFixtureTextsEmit.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckFixtureTextsEmitFullHost
    && !hostFrontLiveHostModuleCheckFixtureTextsEmitResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckFixtureTextsEmitProvablyUnlocked
    && kernelCheckLiveHostModuleCheckFixtureTextsEmitSource
        liveHostModuleCheckFixtureTextsEmitSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckFixtureTextsEmitSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckFixtureTextsEmitRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckFixtureTextsEmitRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckFixtureTextsEmitRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckFixtureTextsEmitRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckFixtureTextsEmitSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckFixtureTextsEmit.lean != liveHostModuleCheckFixtureTextsEmitSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckFixtureTextsEmit.lean")
  let r := parseLiveHostModuleCheckFixtureTextsEmitSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKFIXTURETEXTSEMIT ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckFixtureTextsEmit parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckFixtureTextsEmit parse false")
    unless hostFrontLiveHostModuleCheckFixtureTextsEmitReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckFixtureTextsEmitReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckFixtureTextsEmitReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckFixtureTextsEmit source was accepted"
      throw (IO.userError
        "empty HostModuleCheckFixtureTextsEmit source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckFixtureTextsEmit.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckFixtureTextsEmit
