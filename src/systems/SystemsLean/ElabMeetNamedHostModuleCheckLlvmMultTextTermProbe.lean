/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLlvmMultTextTerm named
  closed subset probe (plus-one after the HostFrontTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LlvmMultTextTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLlvmMultTextTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckLlvmMultTextTerm subset:
    prior HostFrontTerm closed set plus
    SystemsLean.HostModuleCheckLlvmMultTextTerm last.
    ...
    the HostFrontTerm compile order, then HostModuleCheckLlvmMultTextTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as HostFrontTerm): LlvmMultTextTerm
    ...
    LlvmMultTextTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLlvmMultTextTerm.
    HostModuleCheckLlvmMultTextTerm probe lives here.
  - tryCompileNamedHostModuleCheckLlvmMultTextTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLlvmMultTextTerm bad writes a temp
    HostModuleCheckLlvmMultTextTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckHostFrontTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior HostFrontTerm walker
    tryCompileNamedHostModuleCheckHostFrontTermSubset, and that this walk
    is not an alias of the HostFrontTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLlvmMultTextTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckLlvmMultTextTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLlvmMultTextTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckLlvmMultTextTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmMultTextTermSubset,
  #elabMeetNamedHostModuleCheckLlvmMultTextTermSubsetProbe,
  elabMeetNamedHostModuleCheckLlvmMultTextTermSubsetProbe,
  SystemsLean.HostModuleCheckLlvmMultTextTerm,
  ElabMeetNamedHostModuleCheckLlvmMultTextTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLlvmMultTextTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLlvmMultTextTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLlvmMultTextTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLlvmMultTextTerm named subset.
    Good compiles live Mult through HostModuleCheckHostFrontTerm plus the
    subset-surface compile closure and HostModuleCheckLlvmMultTextTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLlvmMultTextTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior HostFrontTerm walker
    tryCompileNamedHostModuleCheckHostFrontTermSubset still works; this
    walker is not an alias of the HostFrontTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLlvmMultTextTerm.lean. Do not skip to
    HostModuleCheckHostCheckTerm. -/
def elabMeetRunNamedHostModuleCheckLlvmMultTextTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLlvmMultTextTerm? <-
    liftIO findLiveHostModuleCheckLlvmMultTextTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLlvmMultTextTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLlvmMultTextTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLlvmMultTextTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLlvmMultTextTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLlvmMultTextTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLlvmMultTextTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLlvmMultTextTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckHostFrontTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLlvmMultTextTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLlvmMultTextTermSubset fakeLake fakeDir
  let notAliasOfHostFrontTerm :=
    namedClosedHostModuleCheckLlvmMultTextTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLlvmMultTextTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLlvmMultTextTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmMultTextTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLlvmMultTextTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckLlvmMultTextTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckLlvmMultTextTermSubsetProbe

#elabMeetNamedHostModuleCheckLlvmMultTextTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckLlvmMultTextTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLlvmMultTextTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmMultTextTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLlvmMultTextTermSubset = true := rfl
