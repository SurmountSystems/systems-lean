/-
  ...
  This wrap is HostModuleCheckKernelLinearTermKernel.lean.
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_LINEAR_TERM_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckKernelLinearTermKernelSource,
  hostFrontLiveHostModuleCheckKernelLinearTermKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermKernelSource
...
/-- Greppable parse id. -/
def parseId : String :=
  "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL"

/-- Live file basename. Greppable: liveRel. Must be the bare product name. -/
def liveRel : String := "HostModuleCheckKernelLinearTermKernel.lean"

/-- Live file relative to repo root. -/
def liveHostModuleCheckKernelLinearTermKernelRel : String :=
...
def hostFrontLiveHostModuleCheckKernelLinearTermKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckKernelLinearTermKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckKernelLinearTermKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckKernelLinearTermKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckKernelLinearTermKernel.lean bytes.
    Greppable: liveHostModuleCheckKernelLinearTermKernelSource. -/


def liveHostModuleCheckKernelLinearTermKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckKernelLinearTermKernel.lean text.
    Greppable: parseLiveHostModuleCheckKernelLinearTermKernelSource. -/
def parseLiveHostModuleCheckKernelLinearTermKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckKernelLinearTermKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckKernelLinearTermKernelSource. -/
def kernelCheckLiveHostModuleCheckKernelLinearTermKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckKernelLinearTermKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckKernelLinearTermKernelSource
      liveHostModuleCheckKernelLinearTermKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthKernelLinearTermKernelBar"
      && has "checkDepthKernelLinearTermKernelScope"
      && has "hostModuleCheckKernelLinearTermKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckKernelLinearTermKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckKernelLinearTermKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckKernelLinearTermKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckKernelLinearTermKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckKernelLinearTermKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_KERNEL_LINEAR_TERM_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL")
    && (liveRel == "HostModuleCheckKernelLinearTermKernel.lean")
    && (liveHostModuleCheckKernelLinearTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelLinearTermKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckKernelLinearTermKernelFullHost
    && !hostFrontLiveHostModuleCheckKernelLinearTermKernelFullBackend
    && !hostFrontLiveHostModuleCheckKernelLinearTermKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckKernelLinearTermKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckKernelLinearTermKernelSource
      liveHostModuleCheckKernelLinearTermKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckKernelLinearTermKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-LINEAR-TERM-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckKernelLinearTermKernel.lean")
    ++ bad "rel" (liveHostModuleCheckKernelLinearTermKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckKernelLinearTermKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckKernelLinearTermKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckKernelLinearTermKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckKernelLinearTermKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckKernelLinearTermKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckKernelLinearTermKernelSource
      liveHostModuleCheckKernelLinearTermKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  IO.println s!"  host={hostId} file={liveHostModuleCheckKernelLinearTermKernelRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckKernelLinearTermKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckKernelLinearTermKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckKernelLinearTermKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckKernelLinearTermKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckKernelLinearTermKernel.lean != liveHostModuleCheckKernelLinearTermKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckKernelLinearTermKernel.lean")
  let r := parseLiveHostModuleCheckKernelLinearTermKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckKernelLinearTermKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckKernelLinearTermKernel parse false")
    unless hostFrontLiveHostModuleCheckKernelLinearTermKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckKernelLinearTermKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckKernelLinearTermKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckKernelLinearTermKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelLinearTermKernelSource
