/-
  ...
  src/systems/SystemsLean/HostModuleCheckKernelProgramTerm.lean with the
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKKERNELPROGRAMTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM, liveRel,
  kernelCheckLiveHostModuleCheckKernelProgramTermSource,
  hostFrontLiveHostModuleCheckKernelProgramTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelProgramTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckKernelProgramTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckKernelProgramTerm.lean"
...
def hostFrontLiveHostModuleCheckKernelProgramTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckKernelProgramTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckKernelProgramTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckKernelProgramTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count printed by the closed run (cmds=8).
    From HostModuleCheckKernelProgramTerm.lean. -/
...
/-- Parse live HostModuleCheckKernelProgramTerm.lean text.
    Greppable: parseLiveHostModuleCheckKernelProgramTermSource,
    PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM. -/
def parseLiveHostModuleCheckKernelProgramTermSource (src : String) :
    FrontResult :=
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckKernelProgramTermSource,
    PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM. -/
def kernelCheckLiveHostModuleCheckKernelProgramTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckKernelProgramTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckKernelProgramTermSource
      liveHostModuleCheckKernelProgramTermSource with
...
/-- Two imports. Not zero.
    Surface and HostModuleCheckEmitPlanTerm. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthKernelProgramTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Scope literal is KernelProgram-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthKernelProgramTermSurfaceScope"
          && s == "KernelProgram-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckKernelProgramTermSurfaceDualOk" && b == true
      | _ => false
...
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckKernelProgramTermSkeletonSuffix"
          && s == "end SystemsLean.KernelProgram\\n"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckKernelProgramTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckKernelProgramTerm"
      && lastSeg m.name.raw == "HostModuleCheckKernelProgramTerm"
...
/-- stageId body checker needle. Body is a match, not a kernel atom. -/
...
/-- programKernelOk body needle. Match result, not a kernel atom. -/
def needleCheckBody : String :=
  "body == \"programKernelReady\""
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckKernelProgramTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckKernelProgramTermReady,
    PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckKernelProgramTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKKERNELPROGRAMTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM")
    && (liveRel == "HostModuleCheckKernelProgramTerm.lean")
    && (liveHostModuleCheckKernelProgramTermRel ==
        "src/systems/SystemsLean/HostModuleCheckKernelProgramTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckKernelProgramTermFullHost
    && !hostFrontLiveHostModuleCheckKernelProgramTermFullBackend
    && !hostFrontLiveHostModuleCheckKernelProgramTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckKernelProgramTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckKernelProgramTermSource
        liveHostModuleCheckKernelProgramTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckKernelProgramTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckKernelProgramTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckKernelProgramTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckKernelProgramTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckKernelProgramTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckKernelProgramTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckKernelProgramTerm.lean != liveHostModuleCheckKernelProgramTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckKernelProgramTerm.lean")
  let r := parseLiveHostModuleCheckKernelProgramTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKKERNELPROGRAMTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless hostFrontLiveHostModuleCheckKernelProgramTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckKernelProgramTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckKernelProgramTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckKernelProgramTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckKernelProgramTerm source was accepted")
