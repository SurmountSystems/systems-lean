/-
  ...
  Short role: HostFrontLiveHostModuleCheckPackageEnvFixturesLater.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckPackageEnvFixturesLaterSource turns live
    HostModuleCheckPackageEnvFixturesLater.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckPackageEnvFixturesLater
    even without a module line.
  - Imports are this product's fourteen SystemsLean.HostModuleCheck* imports.
  - kernelCheckLiveHostModuleCheckPackageEnvFixturesLaterSource is
  ...
  Greppable: SYSTEMS_LEAN_HOST, HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEENVFIXTURESLATER_V0,
  PARSE-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER,
  parseLiveHostModuleCheckPackageEnvFixturesLaterSource,
  kernelCheckLiveHostModuleCheckPackageEnvFixturesLaterSource,
  hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady,
  liveHostModuleCheckPackageEnvFixturesLaterSource, liveRel,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckPackageEnvFixturesLater
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER"

/-- Honesty: this parser is not the HostTerm Mult fixture. -/
def liveParseDoesNotUseMultFixture : Bool := true

/-- Honesty: FullHost stays false. -/
def hostFrontLiveHostModuleCheckPackageEnvFixturesLaterFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckPackageEnvFixturesLaterFullBackend : Bool :=
  false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckPackageEnvFixturesLaterResidualFreeClaimed :
    Bool := false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckPackageEnvFixturesLaterProvablyUnlocked :
    Bool := false
...
/-- Kept commands: fourteen imports, the namespace, and end.
    The 78 String concatenation defs are not single kernel literals.
    kernelFuel is 64. Sixteen commands is under that cap. -/
def liveKeptCmds : Nat := 16

/-- Dotted ident `SystemsLean . HostModuleCheckPackageEnvFixturesLater`. -/
...
/-- Parse live HostModuleCheckPackageEnvFixturesLater.lean text.
    Greppable: parseLiveHostModuleCheckPackageEnvFixturesLaterSource,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER. -/
def parseLiveHostModuleCheckPackageEnvFixturesLaterSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name :=
              HostTerm.n "SystemsLean.HostModuleCheckPackageEnvFixturesLater"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckPackageEnvFixturesLaterSource. -/
def kernelCheckLiveHostModuleCheckPackageEnvFixturesLaterSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckPackageEnvFixturesLaterSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckPackageEnvFixturesLaterSource
      liveHostModuleCheckPackageEnvFixturesLaterSource with
...
def liveParseModuleNameOk : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckPackageEnvFixturesLater"
      && lastSeg m.name.raw == "HostModuleCheckPackageEnvFixturesLater"

/-- The fourteen import lines. Each must occur once. -/
...
/-- S17 good text def. Not a single kernel literal. -/
def needleS17 : String :=
  "def hostModuleCheckGoodPackageEnvS17Text : String :=\n"

/-- HostCheck bad text def. Not a single kernel literal. -/
def needleHostCheck : String :=
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckPackageEnvFixturesLaterSource
  needleImports.all (fun n => (src.splitOn n).length == 2)
    && (src.splitOn needleNamespace).length == 2
    && (src.splitOn needleS17).length == 2
    && (src.splitOn needleHostCheck).length == 2
    && (src.splitOn needlePartial).length == 2
    && (src.splitOn needleEnd).length == 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEENVFIXTURESLATER_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPACKAGEENVFIXTURESLATER")
    && (liveRel == "HostModuleCheckPackageEnvFixturesLater.lean")
    && (liveHostModuleCheckPackageEnvFixturesLaterRel ==
        "src/systems/SystemsLean/" ++ liveRel)
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckPackageEnvFixturesLaterFullHost
    && !hostFrontLiveHostModuleCheckPackageEnvFixturesLaterFullBackend
    && !hostFrontLiveHostModuleCheckPackageEnvFixturesLaterResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckPackageEnvFixturesLaterProvablyUnlocked
    && kernelCheckLiveHostModuleCheckPackageEnvFixturesLaterSource
        liveHostModuleCheckPackageEnvFixturesLaterSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckPackageEnvFixturesLaterSource "" with
  | FrontResult.reject _ => true
  | FrontResult.accept _ => false

/-! ### Driver (short banners; dual-pin file equality). Not a mill row. -/

def runLiveHostModuleCheckPackageEnvFixturesLater (root : System.FilePath) :
    IO Unit := do
  IO.println s!"== {stageId}: {parseId} =="
  IO.println s!"liveRel={liveRel}"
  IO.println s!"  host={hostId} file={liveHostModuleCheckPackageEnvFixturesLaterRel}"
  let path := root / liveHostModuleCheckPackageEnvFixturesLaterRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckPackageEnvFixturesLaterRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckPackageEnvFixturesLaterRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckPackageEnvFixturesLaterSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckPackageEnvFixturesLater.lean != liveHostModuleCheckPackageEnvFixturesLaterSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckPackageEnvFixturesLater.lean")
  let r := parseLiveHostModuleCheckPackageEnvFixturesLaterSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckPackageEnvFixturesLater parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckPackageEnvFixturesLater parse false")
    unless hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckPackageEnvFixturesLaterReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty source must reject"
      throw (IO.userError "empty source must reject")
    IO.println s!"GREEN {stageId}: live HostModuleCheckPackageEnvFixturesLater.lean parse kernelCheck; host tools stay 69 of 69"
...
def main (args : List String) : IO UInt32 := do
  let root : System.FilePath :=
    ...
    | [] => "."
  try
    runLiveHostModuleCheckPackageEnvFixturesLater root
    pure 0
  catch e =>
    IO.eprintln s!"{e}"
    pure 1
