/-
  ...
  src/systems/SystemsLean/HostModuleCheckKernelLinearProof.lean with the
  ...
  Two imports: SystemsLean.HostModuleCheckSurface and
  SystemsLean.HostModuleCheckMultProof.
  Namespace SystemsLean.HostModuleCheck.
  Kept for the kernel: those imports, the namespace, literal String
  and Bool defs, and end. Copied front-term kept count is 7.
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKKERNELLINEARPROOF_V0,
  PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF, liveRel,
  kernelCheckLiveHostModuleCheckKernelLinearProofSource,
  hostFrontLiveHostModuleCheckKernelLinearProofReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelLinearProof
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckKernelLinearProofSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKKERNELLINEARPROOF"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckKernelLinearProof.lean"
...
def hostFrontLiveHostModuleCheckKernelLinearProofFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckKernelLinearProofFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckKernelLinearProofResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckKernelLinearProofProvablyUnlocked : Bool :=
  false
...
/-- Copied front-term kept-command count (cmds=7).
    Not a hand count of HostModuleCheckKernelLinearProof.lean.
...
/-- Parse live HostModuleCheckKernelLinearProof.lean text.
    Greppable: parseLiveHostModuleCheckKernelLinearProofSource,
    PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF. -/
def parseLiveHostModuleCheckKernelLinearProofSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckKernelLinearProof"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckKernelLinearProofSource,
    PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF. -/
def kernelCheckLiveHostModuleCheckKernelLinearProofSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckKernelLinearProofSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckKernelLinearProofSource
      liveHostModuleCheckKernelLinearProofSource with
...
/-- Kept String literal defs. This product has no skeleton-suffix literal. -/
def countStringLits : List Cmd -> Nat
  | [] => 0
  | Cmd.def_ _ (some HostType.string) (Term.litString _) :: rest =>
    Nat.succ (countStringLits rest)
  | _ :: rest => countStringLits rest

/-- Two imports. Not zero.
    Surface and HostModuleCheckMultProof. -/
...
/-- Proof-surface bar is the string literal PROOF-SURFACE. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthKernelLinearProofSurfaceBar"
          && s == "PROOF-SURFACE"
      | _ => false

/-- Scope literal is KernelLinearTheorems-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthKernelLinearProofSurfaceScope"
          && s == "KernelLinearTheorems-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckKernelLinearProofSurfaceDualOk" && b == true
      | _ => false

/-- No skeleton-suffix def in this product. The kept String literals are
    the proof bar and the scope. Two of them, and the scope text matches. -/
def liveParseHasSuffix : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    countStringLits m.commands == 2
      && m.commands.any fun c =>
        match c with
        | Cmd.def_ x (some HostType.string) (Term.litString s) =>
          x.raw == "checkDepthKernelLinearProofSurfaceScope"
...
/-- Wrap module lastSeg is HostModuleCheckKernelLinearProof
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckKernelLinearProof"
      && lastSeg m.name.raw == "HostModuleCheckKernelLinearProof"
...
/-- Import SystemsLean.HostModuleCheckMultProof. Not dropped. -/
def needleImportMultProof : String :=
  "import SystemsLean.HostModuleCheckMultProof\n"

/-- Namespace needle with a trailing newline. -/
def needleNamespace : String := "namespace SystemsLean.HostModuleCheck\n"

/-- Proof-surface bar def needle. -/
...
/-- Statement checker needle. Body is a match, not a kernel atom. -/
...
/-- stageId_eq target text. Present in the live source, not a kernel atom. -/
def needleEmptyStruct : String :=
  "stageId = \"SLAKE_SELF_HOST_KERNEL_LINEAR_V0\""

/-- Proof body must be rfl. Match result, not a kernel atom. -/
def needleCheckBody : String :=
  "body == \"rfl\""

/-- Ill-typed proof reason. Not a kernel atom. -/
def needleExtractBody : String :=
  "some reasonIllTypedProof"
...
/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Two imports. The live file does not have zero imports. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckKernelLinearProofSource
  (src.splitOn needleImportSurface).length == 2
    && (src.splitOn needleImportMultProof).length == 2
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckKernelLinearProofReady,
    PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF,
    HOST-FRONT-LIVE-HOSTMODULECHECKKERNELLINEARPROOF.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckKernelLinearProofReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKKERNELLINEARPROOF_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKKERNELLINEARPROOF")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF")
    && (liveRel == "HostModuleCheckKernelLinearProof.lean")
    && (liveHostModuleCheckKernelLinearProofRel ==
        "src/systems/SystemsLean/HostModuleCheckKernelLinearProof.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckKernelLinearProofFullHost
    && !hostFrontLiveHostModuleCheckKernelLinearProofFullBackend
    && !hostFrontLiveHostModuleCheckKernelLinearProofResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckKernelLinearProofProvablyUnlocked
    && kernelCheckLiveHostModuleCheckKernelLinearProofSource
        liveHostModuleCheckKernelLinearProofSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckKernelLinearProofSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckKernelLinearProofRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckKernelLinearProofRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckKernelLinearProofRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckKernelLinearProofRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckKernelLinearProofSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckKernelLinearProof.lean != liveHostModuleCheckKernelLinearProofSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckKernelLinearProof.lean")
  let r := parseLiveHostModuleCheckKernelLinearProofSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKKERNELLINEARPROOF ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckKernelLinearProof parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckKernelLinearProof parse false")
    unless hostFrontLiveHostModuleCheckKernelLinearProofReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckKernelLinearProofReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckKernelLinearProofReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckKernelLinearProof source was accepted"
      throw (IO.userError "empty HostModuleCheckKernelLinearProof source was accepted")
