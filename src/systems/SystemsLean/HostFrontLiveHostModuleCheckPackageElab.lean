/-
  ...
  src/systems/SystemsLean/HostModuleCheckPackageElab.lean with the
  ...
  The live file has thirteen imports. Those imports stay inside the
  ...
  and Bool defs, and end. The driver printed cmds=23, so liveKeptCmds is 23.
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEELAB_V0,
  PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB, liveRel,
  kernelCheckLiveHostModuleCheckPackageElabSource,
  hostFrontLiveHostModuleCheckPackageElabReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckPackageElab
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckPackageElabSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEELAB"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckPackageElab.lean"
...
def hostFrontLiveHostModuleCheckPackageElabFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckPackageElabFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckPackageElabResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckPackageElabProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count printed by the closed run (cmds=23).
    From HostModuleCheckPackageElab.lean. -/
def liveKeptCmds : Nat := 23
...
/-- Parse live HostModuleCheckPackageElab.lean text.
    Greppable: parseLiveHostModuleCheckPackageElabSource,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB. -/
def parseLiveHostModuleCheckPackageElabSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckPackageElab"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckPackageElabSource,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB. -/
def kernelCheckLiveHostModuleCheckPackageElabSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckPackageElabSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckPackageElabSource
      liveHostModuleCheckPackageElabSource with
...
/-- Package elaborate bar is the string literal PACKAGE-ELABORATE.
    TERM-SURFACE inside the embedded text is inventory, not this bar. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthPackageElaborateBar"
          && s == "PACKAGE-ELABORATE"
      | _ => false

/-- Scope literal is MultExtractFoundation-only. Not Mult-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthPackageElaborateScope"
          && s == "MultExtractFoundation-only"
      | _ => false

/-- First-band dual-ok is the Bool literal true.
    Not hostModuleCheckTermSurfaceDualOk. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckPackageElaborateFirstBandDualOk"
          && b == true
      | _ => false

/-- Package-band incomplete reason is one string literal.
    String-join bodies are not kept. -/
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "reasonPackageBandIncomplete"
          && s == "PACKAGE-BAND-INCOMPLETE"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckPackageElab
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckPackageElab"
      && lastSeg m.name.raw == "HostModuleCheckPackageElab"
...
/-- Thirteen import lines. Each must occur once. -/
...
/-- Foundation band body needle. Not a kernel atom. -/
def needleCheckBody : String :=
  "packageInventoryHas content \"packageBand: Mult..Extract-foundation\""

/-- Package-band incomplete reject. Not a kernel atom. -/
def needleExtractBody : String :=
  "some reasonPackageBandIncomplete"

/-- First-band gate needle. Not bare dualOk alone. -/
...
/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Thirteen imports. The live file does not have zero imports. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckPackageElabSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckPackageElabReady,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB,
    HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEELAB.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckPackageElabReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEELAB_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEELAB")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB")
    && (liveRel == "HostModuleCheckPackageElab.lean")
    && (liveHostModuleCheckPackageElabRel ==
        "src/systems/SystemsLean/HostModuleCheckPackageElab.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckPackageElabFullHost
    && !hostFrontLiveHostModuleCheckPackageElabFullBackend
    && !hostFrontLiveHostModuleCheckPackageElabResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckPackageElabProvablyUnlocked
    && kernelCheckLiveHostModuleCheckPackageElabSource
        liveHostModuleCheckPackageElabSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckPackageElabSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckPackageElabRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckPackageElabRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckPackageElabRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckPackageElabRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckPackageElabSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckPackageElab.lean != liveHostModuleCheckPackageElabSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckPackageElab.lean")
  let r := parseLiveHostModuleCheckPackageElabSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKPACKAGEELAB ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckPackageElab parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckPackageElab parse false")
    unless hostFrontLiveHostModuleCheckPackageElabReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckPackageElabReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckPackageElabReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckPackageElab source was accepted"
      throw (IO.userError "empty HostModuleCheckPackageElab source was accepted")
