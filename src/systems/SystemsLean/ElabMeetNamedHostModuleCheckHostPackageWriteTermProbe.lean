/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckHostPackageWriteTerm named
  closed subset probe (plus-one after the MultFsDeepenTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. HostPackageWriteTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckHostPackageWriteTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckMultFsDeepenTermProbe,
  ...
  ElabMeetNamedWalkHostModuleCheckMultFsDeepenTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckHostPackageWriteTerm subset:
    prior MultFsDeepenTerm closed set plus
    SystemsLean.HostModuleCheckHostPackageWriteTerm last.
    ...
    the MultFsDeepenTerm compile order, then HostModuleCheckHostPackageWriteTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as MultFsDeepenTerm): HostPackageWriteTerm
    ...
    HostPackageWriteTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckHostPackageWriteTerm.
    HostModuleCheckHostPackageWriteTerm probe lives here.
  - tryCompileNamedHostModuleCheckHostPackageWriteTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckHostPackageWriteTerm bad writes a temp
    HostModuleCheckHostPackageWriteTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckMultFsDeepenTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior MultFsDeepenTerm walker
    tryCompileNamedHostModuleCheckMultFsDeepenTermSubset, and that this walk
    is not an alias of the MultFsDeepenTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckHostPackageWriteTerm.lean. Do not skip to
  ...
  findLiveHostModuleCheckHostPackageWriteTermPath,
  namedClosedHostModuleCheckHostPackageWriteTermSubsetNames,
  namedClosedHostModuleCheckHostPackageWriteTermCompileOrder,
  barrelListsNamedHostModuleCheckHostPackageWriteTermSubset,
  tryCompileNamedHostModuleCheckHostPackageWriteTermSubset,
  tryCompileAfterHostModuleCheckHostPackageWriteTermDeps,
  elabMeetDrivesNamedHostModuleCheckHostPackageWriteTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckHostPackageWriteTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckHostPackageWriteTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckHostPackageWriteTermSubset,
  #elabMeetNamedHostModuleCheckHostPackageWriteTermSubsetProbe,
  elabMeetNamedHostModuleCheckHostPackageWriteTermSubsetProbe,
  SystemsLean.HostModuleCheckHostPackageWriteTerm,
  ElabMeetNamedHostModuleCheckHostPackageWriteTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckHostPackageWriteTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckHostPackageWriteTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckHostPackageWriteTermProbe;
  just systems-host.
...
import SystemsLean.ElabMeetNamedWalkHostModuleCheckMultFsDeepenTerm
...
/-- Record accept / reject for the HostModuleCheckHostPackageWriteTerm named subset.
    Good compiles live Mult through HostModuleCheckMultFsDeepenTerm plus the
    subset-surface compile closure and HostModuleCheckHostPackageWriteTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckHostPackageWriteTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior MultFsDeepenTerm walker
    tryCompileNamedHostModuleCheckMultFsDeepenTermSubset still works; this
    walker is not an alias of the MultFsDeepenTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckHostPackageWriteTerm.lean. Do not skip to
    HostModuleCheckLlvmProgramTextTerm. -/
elab "#elabMeetNamedHostModuleCheckHostPackageWriteTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckHostPackageWriteTerm? <-
    liftIO findLiveHostModuleCheckHostPackageWriteTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckHostPackageWriteTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckHostPackageWriteTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckHostPackageWriteTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckHostPackageWriteTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckHostPackageWriteTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckHostPackageWriteTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckHostPackageWriteTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckMultFsDeepenTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckHostPackageWriteTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckHostPackageWriteTermSubset fakeLake fakeDir
  let notAliasOfMultFsDeepenTerm :=
    namedClosedHostModuleCheckHostPackageWriteTermSubsetNames
      != namedClosedHostModuleCheckMultFsDeepenTermSubsetNames
      && namedClosedHostModuleCheckHostPackageWriteTermCompileOrder
           != namedClosedHostModuleCheckMultFsDeepenTermCompileOrder
      && namedClosedHostModuleCheckHostPackageWriteTermSubsetNames.contains
           "SystemsLean.HostModuleCheckHostPackageWriteTerm"
      && !namedClosedHostModuleCheckMultFsDeepenTermSubsetNames.contains
           "SystemsLean.HostModuleCheckHostPackageWriteTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfMultFsDeepenTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckHostPackageWriteTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckHostPackageWriteTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckHostPackageWriteTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckHostPackageWriteTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckHostPackageWriteTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckHostPackageWriteTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckHostPackageWriteTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckHostPackageWriteTermSubset = true := rfl
