/-
  ...
  src/systems/SystemsLean/HostModuleCheckLlvmProgramTextTerm.lean with the
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKLLVMPROGRAMTEXTTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM, liveRel,
  kernelCheckLiveHostModuleCheckLlvmProgramTextTermSource,
  hostFrontLiveHostModuleCheckLlvmProgramTextTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckLlvmProgramTextTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckLlvmProgramTextTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckLlvmProgramTextTerm.lean"
...
def hostFrontLiveHostModuleCheckLlvmProgramTextTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckLlvmProgramTextTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckLlvmProgramTextTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckLlvmProgramTextTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count printed by the closed run (cmds=11).
    From HostModuleCheckLlvmProgramTextTerm.lean. -/
...
/-- Parse live HostModuleCheckLlvmProgramTextTerm.lean text.
    Greppable: parseLiveHostModuleCheckLlvmProgramTextTermSource,
    PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM. -/
def parseLiveHostModuleCheckLlvmProgramTextTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckLlvmProgramTextTerm"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckLlvmProgramTextTermSource,
    PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM. -/
def kernelCheckLiveHostModuleCheckLlvmProgramTextTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckLlvmProgramTextTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckLlvmProgramTextTermSource
      liveHostModuleCheckLlvmProgramTextTermSource with
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthLlvmProgramTextTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Scope literal is LlvmProgramText-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthLlvmProgramTextTermSurfaceScope"
          && s == "LlvmProgramText-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckLlvmProgramTextTermSurfaceDualOk" && b == true
      | _ => false
...
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckLlvmProgramTextTermSkeletonSuffix"
          && s == "end SystemsLean.LlvmProgramText\\n"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckLlvmProgramTextTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckLlvmProgramTextTerm"
      && lastSeg m.name.raw == "HostModuleCheckLlvmProgramTextTerm"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckLlvmProgramTextTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckLlvmProgramTextTermReady,
    PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckLlvmProgramTextTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKLLVMPROGRAMTEXTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM")
    && (liveRel == "HostModuleCheckLlvmProgramTextTerm.lean")
    && (liveHostModuleCheckLlvmProgramTextTermRel ==
        "src/systems/SystemsLean/HostModuleCheckLlvmProgramTextTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckLlvmProgramTextTermFullHost
    && !hostFrontLiveHostModuleCheckLlvmProgramTextTermFullBackend
    && !hostFrontLiveHostModuleCheckLlvmProgramTextTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckLlvmProgramTextTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckLlvmProgramTextTermSource
        liveHostModuleCheckLlvmProgramTextTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckLlvmProgramTextTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckLlvmProgramTextTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckLlvmProgramTextTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckLlvmProgramTextTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckLlvmProgramTextTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckLlvmProgramTextTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckLlvmProgramTextTerm.lean != liveHostModuleCheckLlvmProgramTextTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckLlvmProgramTextTerm.lean")
  let r := parseLiveHostModuleCheckLlvmProgramTextTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKLLVMPROGRAMTEXTTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckLlvmProgramTextTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckLlvmProgramTextTerm parse false")
    unless hostFrontLiveHostModuleCheckLlvmProgramTextTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckLlvmProgramTextTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckLlvmProgramTextTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckLlvmProgramTextTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckLlvmProgramTextTerm source was accepted")
