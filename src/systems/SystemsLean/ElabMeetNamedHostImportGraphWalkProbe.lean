/-
  SYSTEMS_LEAN_HOST partial -- twenty-seventh HostImportGraphWalk named
  closed subset probe.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. Twenty-seventh wrappers live in
  ElabMeetNamedWalkHostImportGraphWalkLater. Shared helpers stay in
  ElabMeetNamedWalk. Do not put this probe back on ElabMeetSubset,
  Tail, Later, ElabMeetCompile, ElabMeetNamedWalkHostTerm,
  ElabMeetNamedWalkHostPackageWrite, ElabMeetNamedHostPackageRootsProbe,
  ElabMeetNamedHostImportGraphSeedsProbe, ElabMeetNamedHostImportGraphModelProbe,
  ElabMeetNamedHostImportGraphModsProbe,
  ElabMeetNamedHostImportGraphModsLaterProbe,
  ElabMeetNamedHostImportGraphLoadOkLaterProbe, or
  ElabMeetNamedHostImportGraphWalkLaterProbe.
  Reuses tryCompileNamedMemberListIO (no cloned walker).

  Spec (readable):
  - Named HostImportGraphWalk subset (twenty-seven named members):
    prior twenty-six plus SystemsLean.HostImportGraphWalk last.
    Membership list is barrel order among the twenty-seven; skip
    Linear, skip IrGraph, skip HostGraphMain. Compile order is
    the twenty-sixth compile order, then HostImportGraphWalk last.
    IrGraph and HostCompose are compile dependencies only.
    Twenty-seven-name walker is a wrapper in
    ElabMeetNamedWalkHostImportGraphWalkLater.
    Twenty-seventh HostImportGraphWalk probe lives here.
  - tryCompileNamedHostImportGraphWalkSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    Twenty-seven-name HostImportGraphWalk bad writes a temp
    HostImportGraphWalk copy and compiles it after Mult and the
    HostFront / HostGraph / SubsetEmit / HostPackageWrite /
    HostPackageWriteTheorems / HostPackageRoots /
    HostImportGraphSeeds / HostImportGraphModel / HostImportGraphMods /
    HostImportGraphModsLater / HostImportGraphLoadOkLater /
    HostImportGraphWalkLater oleans.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior twenty-six-module walker
    tryCompileNamedHostImportGraphWalkLaterSubset, and that this walk
    is not an alias of the twenty-sixth. Linear skipped. IrGraph
    skipped as a grow-tip Name. Do not plant live
    HostImportGraphWalk.lean.
  - The drive is good && !bad && isolation.
  - When SLAKE_PACKAGE_TYPECHECK=1 (library walk), plant drive Bools
    via elabMeetPlantNamedSubsetDrive so this probe kernel-compiles
    as a library. Lake unset-env still runs the live nested compile.
    Do not drop this module from the 747 list.
  - slakeOwnsPackageTypecheck stays false. FullHost stays false.

  Greppable: SYSTEMS_LEAN_HOST, HOST-ELAB-MEET, SLAKE_ELAB_MEET,
  tryCompileOnDiskModule, tryCompileOnDiskModuleIO,
  findLiveHostImportGraphWalkPath,
  namedClosedHostImportGraphWalkSubsetNames,
  namedClosedHostImportGraphWalkCompileOrder,
  barrelListsNamedHostImportGraphWalkSubset,
  tryCompileNamedHostImportGraphWalkSubset,
  tryCompileAfterHostImportGraphWalkDeps,
  elabMeetDrivesNamedHostImportGraphWalkSubset,
  elabMeetAcceptsGoodNamedHostImportGraphWalkSubset,
  elabMeetRejectsBadNamedHostImportGraphWalkSubset,
  elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset,
  elabMeetRunNamedHostImportGraphWalkSubsetProbe,
  elabMeetPlantNamedSubsetDrive, slakePackageTypecheckWalk,
  SLAKE_PACKAGE_TYPECHECK,
  #elabMeetNamedHostImportGraphWalkSubsetProbe,
  elabMeetNamedHostImportGraphWalkSubsetProbe,
  SystemsLean.HostImportGraphWalk,
  ElabMeetNamedHostImportGraphWalkProbe,
  SystemsLean.ElabMeetNamedHostImportGraphWalkProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostImportGraphWalkProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostImportGraphWalkProbe;
  just systems-host.
  Module must stay ASCII.
  Not freestanding residual free. Not PROVABLY. Not llvm unlock.
  Not FullHost. Not FullHostElaborateRemains true. Not Lake gone.
  slakeOwnsPackageTypecheck stays false.
-/

import Lean
import SystemsLean.ElabMeetLive
import SystemsLean.ElabMeetNamedWalk
import SystemsLean.ElabMeetNamedWalkHostPackageWrite
import SystemsLean.ElabMeetNamedWalkHostImportGraphSeeds
import SystemsLean.ElabMeetNamedWalkHostImportGraphWalkLater

namespace SystemsLean.ElabMeet

open Lean Elab Command

/-- Record accept / reject for the HostImportGraphWalk named subset.
    Good compiles live Mult through HostImportGraphWalkLater plus the
    subset-emit compile closure and HostImportGraphWalk after
    reading the real lakefile. Bad compiles a temp
    HostImportGraphWalk copy with a planted type error after Mult
    and the HostFront / HostGraph / SubsetEmit / HostPackageWrite /
    HostPackageWriteTheorems / HostPackageRoots /
    HostImportGraphSeeds / HostImportGraphModel / HostImportGraphMods /
    HostImportGraphModsLater / HostImportGraphLoadOkLater /
    HostImportGraphWalkLater oleans.
    Isolation: leftover temp good.lean and leftover fake lean_lib
    ElabMeetRichLib stay false; the prior twenty-six-module walker
    tryCompileNamedHostImportGraphWalkLaterSubset still works; this
    walker is not an alias of the twenty-sixth. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. Do not
    plant live HostImportGraphWalk.lean. -/
def elabMeetRunNamedHostImportGraphWalkSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostImportGraphWalk? <- liftIO findLiveHostImportGraphWalkPath
  let oleanDir <- liftIO IO.FS.createTempDir
  let badDir <- liftIO IO.FS.createTempDir
  let isoDir <- liftIO IO.FS.createTempDir
  let fakeDir <- liftIO IO.FS.createTempDir
  let good <-
    match liveLake? with
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostImportGraphWalkSubset liveLake oleanDir
  let badCompiled <-
    match liveHostImportGraphWalk? with
    | none =>
      pure true
    | some liveHostImportGraphWalk => do
      let liveText <- liftIO (IO.FS.readFile liveHostImportGraphWalk)
      let badPath :=
        badDir / "ElabMeetNamedHostImportGraphWalkSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostImportGraphWalkDeps
        badPath badDir `ElabMeetNamedHostImportGraphWalkSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
      tryCompileLiveModule liveMult isoDir
  let thmIsoDir <- liftIO IO.FS.createTempDir
  let liveThmStillOk <-
    match liveThm? with
    | none => pure false
    | some liveThm =>
      tryCompileLiveTheorems liveThm thmIsoDir
  let subsetIsoDir <- liftIO IO.FS.createTempDir
  let priorSubsetStillOk <-
    match liveLake? with
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostImportGraphWalkLaterSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostImportGraphWalkSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostImportGraphWalkSubset fakeLake fakeDir
  let notAliasOfTwentySixth :=
    namedClosedHostImportGraphWalkSubsetNames
      != namedClosedHostImportGraphWalkLaterSubsetNames
      && namedClosedHostImportGraphWalkCompileOrder
           != namedClosedHostImportGraphWalkLaterCompileOrder
      && namedClosedHostImportGraphWalkSubsetNames.contains
           "SystemsLean.HostImportGraphWalk"
      && !namedClosedHostImportGraphWalkLaterSubsetNames.contains
           "SystemsLean.HostImportGraphWalk"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfTwentySixth
  try
    liftIO (IO.FS.removeDirAll oleanDir)
  catch _ =>
    pure ()
  try
    liftIO (IO.FS.removeDirAll badDir)
  catch _ =>
    pure ()
  try
    liftIO (IO.FS.removeDirAll isoDir)
  catch _ =>
    pure ()
  try
    liftIO (IO.FS.removeDirAll thmIsoDir)
  catch _ =>
    pure ()
  try
    liftIO (IO.FS.removeDirAll subsetIsoDir)
  catch _ =>
    pure ()
  try
    liftIO (IO.FS.removeDirAll fakeDir)
  catch _ =>
    pure ()
  let gStx <- if good then `(true) else `(false)
  let rStx <- if !badCompiled then `(true) else `(false)
  let iStx <- if isolation then `(true) else `(false)
  let dStx <-
    if good && !badCompiled && isolation then
      `(true)
    else
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostImportGraphWalkSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostImportGraphWalkSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset
  let dN := mkIdent `elabMeetDrivesNamedHostImportGraphWalkSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostImportGraphWalkSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    elabMeetPlantNamedSubsetDrive
      `elabMeetAcceptsGoodNamedHostImportGraphWalkSubset
      `elabMeetRejectsBadNamedHostImportGraphWalkSubset
      `elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset
      `elabMeetDrivesNamedHostImportGraphWalkSubset
  else
    elabMeetRunNamedHostImportGraphWalkSubsetProbe

#elabMeetNamedHostImportGraphWalkSubsetProbe

example : elabMeetAcceptsGoodNamedHostImportGraphWalkSubset = true := rfl
example : elabMeetRejectsBadNamedHostImportGraphWalkSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset = true := rfl
example : elabMeetDrivesNamedHostImportGraphWalkSubset = true := rfl

/-- Drive fold for the HostImportGraphWalk closed subset. Greppable: elabMeetDrivesNamedHostImportGraphWalkSubset. -/
theorem elabMeetDrivesNamedHostImportGraphWalkSubset_true :
    elabMeetDrivesNamedHostImportGraphWalkSubset = true := by
  native_decide

theorem elabMeetAcceptsGoodNamedHostImportGraphWalkSubset_true :
    elabMeetAcceptsGoodNamedHostImportGraphWalkSubset = true := by
  native_decide

theorem elabMeetRejectsBadNamedHostImportGraphWalkSubset_true :
    elabMeetRejectsBadNamedHostImportGraphWalkSubset = true := by
  native_decide

theorem elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset_true :
    elabMeetRejectsOldWalkAsNamedHostImportGraphWalkSubset = true := by
  native_decide

end SystemsLean.ElabMeet
