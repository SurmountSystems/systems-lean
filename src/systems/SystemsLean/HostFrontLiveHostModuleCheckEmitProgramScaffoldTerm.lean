/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitProgramScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitProgramScaffoldTermSource turns live
    HostModuleCheckEmitProgramScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitProgramScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitProgramScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitProgramScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitProgramScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, five literal defs, and end.
    Kept count is 9.
    The five literal defs are checkDepthEmitProgramScaffoldTermSurfaceBar,
    checkDepthEmitProgramScaffoldTermSurfaceScope,
    hostModuleCheckEmitProgramScaffoldTermSurfaceDualOk,
    hostModuleCheckEmitProgramScaffoldTermSkeletonPrefix, and
  ...
  HostFrontLiveHostModuleCheckEmitProgramScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitProgramScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitProgramScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady,
  liveHostModuleCheckEmitProgramScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitProgramScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitProgramScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitProgramScaffoldTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitProgramScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitProgramScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitProgramScaffoldTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitProgramScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitProgramScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitProgramScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitProgramScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitProgramScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitProgramScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitProgramScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitProgramScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitProgramScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitProgramScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitProgramScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitProgramScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitProgramScaffoldTermSource
      liveHostModuleCheckEmitProgramScaffoldTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitProgramScaffoldTermSurfaceBar",
      "checkDepthEmitProgramScaffoldTermSurfaceScope",
      "hostModuleCheckEmitProgramScaffoldTermSurfaceDualOk",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitProgramScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitProgramScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitProgramScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitProgramScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitProgramScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitProgramScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitProgramScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitProgramScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitProgramScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitProgramScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitProgramScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitProgramScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitProgramScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitProgramScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_program_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitProgramScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitProgramScaffoldTerm.lean")
    && (liveHostModuleCheckEmitProgramScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitProgramScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitProgramScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitProgramScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitProgramScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitProgramScaffoldTermSource
        liveHostModuleCheckEmitProgramScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitProgramScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitProgramScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitProgramScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitProgramScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitProgramScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitProgramScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitProgramScaffoldTerm.lean != liveHostModuleCheckEmitProgramScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitProgramScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitProgramScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITPROGRAMSCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitProgramScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitProgramScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitProgramScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitProgramScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitProgramScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitProgramScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
