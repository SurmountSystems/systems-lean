/-
  ...
  Short role: HostFrontLiveHostModuleCheckCheckersLater.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCheckersLaterSource turns live
    HostModuleCheckCheckersLater.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCheckersLater
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCheckersLater.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCheckersLaterSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCheckersLaterSource. Not split.
  - Three dotted imports, namespace SystemsLean.HostModuleCheck,
    and end are kept. Every def has a binder and an application
    body, so each is skip-folded. No String or Bool literal def is kept.
    Kept command count is 5. kernelFuel is 64. 5 does not pass 64.
  - Skip-head still sees def checkFreestandingEmitSurface and
    def checkSelfApplyTheoremsSurface in the live text.
  ...
  HostFrontLiveHostModuleCheckCheckersLater
  PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER
  HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSLATER
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCHECKERSLATER_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCheckersLaterSource,
  kernelCheckLiveHostModuleCheckCheckersLaterSource,
  hostFrontLiveHostModuleCheckCheckersLaterReady,
  liveHostModuleCheckCheckersLaterSource,
  liveRel, liveHostModuleCheckCheckersLaterRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCheckersLater
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCheckersLaterSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSLATER"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCheckersLater.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCheckersLaterSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCheckersLaterSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCheckersLaterFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCheckersLaterResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCheckersLaterProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: three imports, namespace, and end.
    Every def has a binder, so it is not a kernel literal and is
    skip-folded. kernelFuel is 64. 5 does not pass 64. -/
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCheckersLater has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCheckersLater.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCheckersLater`. -/
...
/-- Parse live HostModuleCheckCheckersLater.lean text.
    Greppable: parseLiveHostModuleCheckCheckersLaterSource,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER. -/
def parseLiveHostModuleCheckCheckersLaterSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCheckersLater"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCheckersLaterSource,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER. -/
def kernelCheckLiveHostModuleCheckCheckersLaterSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCheckersLaterSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCheckersLaterSource
      liveHostModuleCheckCheckersLaterSource with
...
/-- Wrap module lastSeg is HostModuleCheckCheckersLater
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCheckersLater"
      && lastSeg m.name.raw == "HostModuleCheckCheckersLater"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCheckersLaterSource)
    toksHaveDefNamed liveSkipFuel toks "checkFreestandingEmitSurface"
      && toksHaveDefNamed liveSkipFuel toks
        "checkSelfApplyTheoremsSurface"
...
def needleCheckFreestanding : String :=
  "def checkFreestandingEmitSurface (surf : RealModuleSurface) : ModuleCheckResult :=\n"

/-- Last surface def. The signature line ends before the result type. -/
def needleCheckSelfApplyTheorems : String :=
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCheckersLaterSource
  (src.splitOn needleImportFixtures).length == 2
    ...
    && (src.splitOn needleCheckFreestanding).length > 1
    && (src.splitOn needleCheckSelfApplyTheorems).length > 1
    && (src.splitOn needleEnd).length > 1

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCheckersLaterReady,
    PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER,
    HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSLATER.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCheckersLaterReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCHECKERSLATER_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCHECKERSLATER")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER")
    && (liveRel == "HostModuleCheckCheckersLater.lean")
    && (liveHostModuleCheckCheckersLaterRel ==
        "src/systems/SystemsLean/HostModuleCheckCheckersLater.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCheckersLaterFullHost
    && !hostFrontLiveHostModuleCheckCheckersLaterResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCheckersLaterProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCheckersLaterSource
        liveHostModuleCheckCheckersLaterSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCheckersLaterSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCheckersLaterRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCheckersLaterRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCheckersLaterRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCheckersLaterRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCheckersLaterSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCheckersLater.lean != liveHostModuleCheckCheckersLaterSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCheckersLater.lean")
  let r := parseLiveHostModuleCheckCheckersLaterSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCHECKERSLATER ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCheckersLater parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCheckersLater parse false")
    unless hostFrontLiveHostModuleCheckCheckersLaterReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCheckersLaterReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCheckersLaterReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCheckersLater source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCheckersLater source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCheckersLater.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
