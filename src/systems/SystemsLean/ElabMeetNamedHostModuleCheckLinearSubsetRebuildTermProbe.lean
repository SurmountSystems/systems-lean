/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLinearSubsetRebuildTerm named
  closed subset probe (plus-one after the MultSubsetRebuildTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LinearSubsetRebuildTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLinearSubsetRebuildTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckMultSubsetEmitTerm, or
  ElabMeetNamedWalkHostModuleCheckMultSubsetRebuildTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckLinearSubsetRebuildTerm subset:
    prior MultSubsetRebuildTerm closed set plus
    SystemsLean.HostModuleCheckLinearSubsetRebuildTerm last.
    ...
    the MultSubsetRebuildTerm compile order, then HostModuleCheckLinearSubsetRebuildTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as MultSubsetRebuildTerm): LinearSubsetRebuildTerm
    ...
    LinearSubsetRebuildTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLinearSubsetRebuildTerm.
    HostModuleCheckLinearSubsetRebuildTerm probe lives here.
  - tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLinearSubsetRebuildTerm bad writes a temp
    HostModuleCheckLinearSubsetRebuildTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckMultSubsetRebuildTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior MultSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckMultSubsetRebuildTermSubset, and that this walk
    is not an alias of the MultSubsetRebuildTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLinearSubsetRebuildTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckLinearSubsetRebuildTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetRebuildTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckLinearSubsetRebuildTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetRebuildTermSubset,
  #elabMeetNamedHostModuleCheckLinearSubsetRebuildTermSubsetProbe,
  elabMeetNamedHostModuleCheckLinearSubsetRebuildTermSubsetProbe,
  SystemsLean.HostModuleCheckLinearSubsetRebuildTerm,
  ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLinearSubsetRebuildTerm named subset.
    Good compiles live Mult through HostModuleCheckMultSubsetRebuildTerm plus the
    subset-surface compile closure and HostModuleCheckLinearSubsetRebuildTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLinearSubsetRebuildTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior MultSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckMultSubsetRebuildTermSubset still works; this
    walker is not an alias of the MultSubsetRebuildTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLinearSubsetRebuildTerm.lean. Do not skip to
    HostModuleCheckTypesSubsetRebuildTerm. -/
elab "#elabMeetNamedHostModuleCheckLinearSubsetRebuildTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLinearSubsetRebuildTerm? <-
    liftIO findLiveHostModuleCheckLinearSubsetRebuildTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLinearSubsetRebuildTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLinearSubsetRebuildTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLinearSubsetRebuildTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLinearSubsetRebuildTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLinearSubsetRebuildTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckMultSubsetRebuildTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset fakeLake fakeDir
  let notAliasOfMultSubsetRebuildTerm :=
    namedClosedHostModuleCheckLinearSubsetRebuildTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfMultSubsetRebuildTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetRebuildTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLinearSubsetRebuildTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetRebuildTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLinearSubsetRebuildTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLinearSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetRebuildTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLinearSubsetRebuildTermSubset = true := rfl
