/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLlvmComposeTextTerm named-walk
  wrappers (CheckersLater-role walk; LlvmGraphTextTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and LlvmGraphTextTerm walk
  ...
  ElabMeetNamedWalk. Prior LlvmGraphTextTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckLlvmGraphTextTerm. Do not grow that walk.
  ...
  LlvmGraphTextTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckLlvmComposeTextTerm.lean.
  Do not skip to HostModuleCheckSelfApplyFsTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckLlvmComposeTextTerm wrappers live here:
    findLiveHostModuleCheckLlvmComposeTextTermPath,
    namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames,
    namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder,
    barrelListsNamedHostModuleCheckLlvmComposeTextTermSubset,
    tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset,
    tryCompileAfterHostModuleCheckLlvmComposeTextTermDeps.
    Named membership is the prior LlvmGraphTextTerm closed set plus
    HostModuleCheckLlvmComposeTextTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the LlvmGraphTextTerm compile
    order, then HostModuleCheckLlvmComposeTextTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / LlvmGraphTextTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the LlvmGraphTextTerm compile order.
    HostModuleCheckLlvmComposeTextTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckLlvmGraphTextTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckLlvmComposeTextTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckLlvmComposeTextTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckLlvmComposeTextTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckLlvmComposeTextTermPath. -/
def findLiveHostModuleCheckLlvmComposeTextTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckLlvmComposeTextTerm"

/-- Named HostModuleCheckLlvmComposeTextTerm subset of the live SystemsLean library.
    Membership is the prior LlvmGraphTextTerm closed set plus
    HostModuleCheckLlvmComposeTextTerm last.
    ...
    Greppable: namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames.
    SystemsLean.HostModuleCheckLlvmComposeTextTerm. -/
def namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames : List String :=
  namedClosedHostModuleCheckLlvmGraphTextTermSubsetNames ++
    ["SystemsLean.HostModuleCheckLlvmComposeTextTerm"]

/-- Compile order honors imports: LlvmGraphTextTerm compile order, then
    HostModuleCheckLlvmComposeTextTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder. -/
def namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder : List String :=
  namedClosedHostModuleCheckLlvmGraphTextTermCompileOrder ++
    ["SystemsLean.HostModuleCheckLlvmComposeTextTerm"]

/-- True when the barrel lists each HostModuleCheckLlvmComposeTextTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckLlvmComposeTextTermSubset. -/
def barrelListsNamedHostModuleCheckLlvmComposeTextTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames

/-- Compile the named HostModuleCheckLlvmComposeTextTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the LlvmGraphTextTerm
    compile order, then HostModuleCheckLlvmComposeTextTerm last.
    Greppable: tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckLlvmComposeTextTerm. -/
unsafe def tryCompileNamedHostModuleCheckLlvmComposeTextTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames
    namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckLlvmComposeTextTermSubsetIO]
opaque tryCompileNamedHostModuleCheckLlvmComposeTextTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckLlvmComposeTextTerm subset.
    Greppable: tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset. -/
def tryCompileNamedHostModuleCheckLlvmComposeTextTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckLlvmComposeTextTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the LlvmGraphTextTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckLlvmComposeTextTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckLlvmComposeTextTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckLlvmGraphTextTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckLlvmComposeTextTermDepsIO]
opaque tryCompileAfterHostModuleCheckLlvmComposeTextTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckLlvmComposeTextTerm deps.
    Greppable: tryCompileAfterHostModuleCheckLlvmComposeTextTermDeps. -/
def tryCompileAfterHostModuleCheckLlvmComposeTextTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckLlvmComposeTextTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
