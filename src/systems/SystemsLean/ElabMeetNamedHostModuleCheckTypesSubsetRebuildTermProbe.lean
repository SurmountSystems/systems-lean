/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckTypesSubsetRebuildTerm named
  closed subset probe (plus-one after the LinearSubsetRebuildTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. TypesSubsetRebuildTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckTypesSubsetRebuildTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckMultSubsetRebuildTerm, or
  ElabMeetNamedWalkHostModuleCheckLinearSubsetRebuildTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckTypesSubsetRebuildTerm subset:
    prior LinearSubsetRebuildTerm closed set plus
    SystemsLean.HostModuleCheckTypesSubsetRebuildTerm last.
    ...
    the LinearSubsetRebuildTerm compile order, then HostModuleCheckTypesSubsetRebuildTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as LinearSubsetRebuildTerm): TypesSubsetRebuildTerm
    ...
    TypesSubsetRebuildTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckTypesSubsetRebuildTerm.
    HostModuleCheckTypesSubsetRebuildTerm probe lives here.
  - tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckTypesSubsetRebuildTerm bad writes a temp
    HostModuleCheckTypesSubsetRebuildTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckLinearSubsetRebuildTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior LinearSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset, and that this walk
    is not an alias of the LinearSubsetRebuildTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckTypesSubsetRebuildTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckTypesSubsetRebuildTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckTypesSubsetRebuildTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckTypesSubsetRebuildTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckTypesSubsetRebuildTermSubset,
  #elabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe,
  elabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe,
  SystemsLean.HostModuleCheckTypesSubsetRebuildTerm,
  ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckTypesSubsetRebuildTerm named subset.
    Good compiles live Mult through HostModuleCheckLinearSubsetRebuildTerm plus the
    subset-surface compile closure and HostModuleCheckTypesSubsetRebuildTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckTypesSubsetRebuildTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior LinearSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset still works; this
    walker is not an alias of the LinearSubsetRebuildTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckTypesSubsetRebuildTerm.lean. Do not skip to
    HostModuleCheckProgramSubsetRebuildTerm. -/
def elabMeetRunNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckTypesSubsetRebuildTerm? <-
    liftIO findLiveHostModuleCheckTypesSubsetRebuildTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckTypesSubsetRebuildTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckTypesSubsetRebuildTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckTypesSubsetRebuildTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckTypesSubsetRebuildTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLinearSubsetRebuildTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckTypesSubsetRebuildTermSubset fakeLake fakeDir
  let notAliasOfLinearSubsetRebuildTerm :=
    namedClosedHostModuleCheckTypesSubsetRebuildTermSubsetNames
      ...
      && namedClosedHostModuleCheckTypesSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckTypesSubsetRebuildTerm"
      && !namedClosedHostModuleCheckLinearSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckTypesSubsetRebuildTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfLinearSubsetRebuildTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckTypesSubsetRebuildTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckTypesSubsetRebuildTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckTypesSubsetRebuildTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckTypesSubsetRebuildTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe

#elabMeetNamedHostModuleCheckTypesSubsetRebuildTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckTypesSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckTypesSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckTypesSubsetRebuildTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckTypesSubsetRebuildTermSubset = true := rfl
