/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathLinearTerm.
  ...
  Not HostFrontLiveHostModuleCheckCompilePathExtractTerm.lean.
  Not HostModuleCheckCompilePathGraphTerm.lean.
  Not a fifth-4 file. Not an ElabMeet file.

  Spec (readable):
  - parseLiveHostModuleCheckCompilePathLinearTermSource turns live
    HostModuleCheckCompilePathLinearTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathLinearTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathLinearTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathLinearTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathLinearTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathLinearTermDialect and
    def hostModuleCheckCompilePathLinearTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathLinearTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHLINEARTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathLinearTermSource,
  kernelCheckLiveHostModuleCheckCompilePathLinearTermSource,
  hostFrontLiveHostModuleCheckCompilePathLinearTermReady,
  liveHostModuleCheckCompilePathLinearTermSource,
  liveRel, liveHostModuleCheckCompilePathLinearTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathLinearTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathLinearTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathLinearTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathLinearTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathLinearTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathLinearTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathLinearTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathLinearTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathLinearTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathLinearTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathLinearTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathLinearTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathLinearTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM. -/
def parseLiveHostModuleCheckCompilePathLinearTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathLinearTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathLinearTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathLinearTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathLinearTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathLinearTermSource
      liveHostModuleCheckCompilePathLinearTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathLinearTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathLinearTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathLinearTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathLinearTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathLinearTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathLinearTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathLinearTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathLinearTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHLINEARTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM")
    && (liveRel == "HostModuleCheckCompilePathLinearTerm.lean")
    && (liveHostModuleCheckCompilePathLinearTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathLinearTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathLinearTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathLinearTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathLinearTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathLinearTermSource
        liveHostModuleCheckCompilePathLinearTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathLinearTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathLinearTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathLinearTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathLinearTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathLinearTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathLinearTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathLinearTerm.lean != liveHostModuleCheckCompilePathLinearTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathLinearTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathLinearTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHLINEARTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathLinearTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathLinearTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathLinearTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathLinearTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathLinearTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathLinearTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathLinearTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathLinearTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
