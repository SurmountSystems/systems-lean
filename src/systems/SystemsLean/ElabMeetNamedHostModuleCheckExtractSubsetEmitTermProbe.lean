/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckExtractSubsetEmitTerm named
  closed subset probe (plus-one after the ErasureSubsetEmitTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ExtractSubsetEmitTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckExtractSubsetEmitTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckComposeSubsetEmitTerm, or
  ElabMeetNamedWalkHostModuleCheckErasureSubsetEmitTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckExtractSubsetEmitTerm subset:
    prior ErasureSubsetEmitTerm closed set plus
    SystemsLean.HostModuleCheckExtractSubsetEmitTerm last.
    ...
    the ErasureSubsetEmitTerm compile order, then HostModuleCheckExtractSubsetEmitTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ErasureSubsetEmitTerm): ExtractSubsetEmitTerm
    ...
    ExtractSubsetEmitTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckExtractSubsetEmitTerm.
    HostModuleCheckExtractSubsetEmitTerm probe lives here.
  - tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckExtractSubsetEmitTerm bad writes a temp
    HostModuleCheckExtractSubsetEmitTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckErasureSubsetEmitTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ErasureSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset, and that this walk
    is not an alias of the ErasureSubsetEmitTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckExtractSubsetEmitTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckExtractSubsetEmitTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckExtractSubsetEmitTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckExtractSubsetEmitTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckExtractSubsetEmitTermSubset,
  #elabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe,
  elabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe,
  SystemsLean.HostModuleCheckExtractSubsetEmitTerm,
  ElabMeetNamedHostModuleCheckExtractSubsetEmitTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckExtractSubsetEmitTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckExtractSubsetEmitTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckExtractSubsetEmitTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckExtractSubsetEmitTerm named subset.
    Good compiles live Mult through HostModuleCheckErasureSubsetEmitTerm plus the
    subset-surface compile closure and HostModuleCheckExtractSubsetEmitTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckExtractSubsetEmitTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ErasureSubsetEmitTerm walker
    tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset still works; this
    walker is not an alias of the ErasureSubsetEmitTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckExtractSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckMultSubsetEmitTerm. -/
def elabMeetRunNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckExtractSubsetEmitTerm? <-
    liftIO findLiveHostModuleCheckExtractSubsetEmitTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckExtractSubsetEmitTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckExtractSubsetEmitTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckExtractSubsetEmitTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckExtractSubsetEmitTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckErasureSubsetEmitTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckExtractSubsetEmitTermSubset fakeLake fakeDir
  let notAliasOfErasureSubsetEmitTerm :=
    namedClosedHostModuleCheckExtractSubsetEmitTermSubsetNames
      != namedClosedHostModuleCheckErasureSubsetEmitTermSubsetNames
      && namedClosedHostModuleCheckExtractSubsetEmitTermCompileOrder
           != namedClosedHostModuleCheckErasureSubsetEmitTermCompileOrder
      && namedClosedHostModuleCheckExtractSubsetEmitTermSubsetNames.contains
           "SystemsLean.HostModuleCheckExtractSubsetEmitTerm"
      && !namedClosedHostModuleCheckErasureSubsetEmitTermSubsetNames.contains
           "SystemsLean.HostModuleCheckExtractSubsetEmitTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfErasureSubsetEmitTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckExtractSubsetEmitTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckExtractSubsetEmitTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckExtractSubsetEmitTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckExtractSubsetEmitTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe

#elabMeetNamedHostModuleCheckExtractSubsetEmitTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckExtractSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckExtractSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckExtractSubsetEmitTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckExtractSubsetEmitTermSubset = true := rfl
