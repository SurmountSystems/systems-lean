/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckErasureSubsetRebuildTerm named
  closed subset probe (plus-one after the ComposeSubsetRebuildTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ErasureSubsetRebuildTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckErasureSubsetRebuildTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermProbe,
  ...
  Spec (readable):
  - Named HostModuleCheckErasureSubsetRebuildTerm subset:
    prior ComposeSubsetRebuildTerm closed set plus
    SystemsLean.HostModuleCheckErasureSubsetRebuildTerm last.
    ...
    the ComposeSubsetRebuildTerm compile order, then HostModuleCheckErasureSubsetRebuildTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ComposeSubsetRebuildTerm): ErasureSubsetRebuildTerm
    ...
    ErasureSubsetRebuildTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckErasureSubsetRebuildTerm.
    HostModuleCheckErasureSubsetRebuildTerm probe lives here.
  - tryCompileNamedHostModuleCheckErasureSubsetRebuildTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckErasureSubsetRebuildTerm bad writes a temp
    HostModuleCheckErasureSubsetRebuildTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckComposeSubsetRebuildTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ComposeSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset, and that this walk
    is not an alias of the ComposeSubsetRebuildTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckErasureSubsetRebuildTerm.lean. Do not skip to
  ...
  findLiveHostModuleCheckErasureSubsetRebuildTermPath,
  ...
  barrelListsNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  tryCompileNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  tryCompileAfterHostModuleCheckErasureSubsetRebuildTermDeps,
  elabMeetDrivesNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetRebuildTermSubset,
  #elabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe,
  elabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe,
  SystemsLean.HostModuleCheckErasureSubsetRebuildTerm,
  ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermProbe;
  just systems-host.
...
import SystemsLean.ElabMeetNamedWalkHostModuleCheckComposeSubsetRebuildTerm
...
/-- Record accept / reject for the HostModuleCheckErasureSubsetRebuildTerm named subset.
    Good compiles live Mult through HostModuleCheckComposeSubsetRebuildTerm plus the
    subset-surface compile closure and HostModuleCheckErasureSubsetRebuildTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckErasureSubsetRebuildTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ComposeSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset still works; this
    walker is not an alias of the ComposeSubsetRebuildTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckErasureSubsetRebuildTerm.lean. Do not skip to
    HostModuleCheckExtractSubsetRebuildTerm. -/
def elabMeetRunNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckErasureSubsetRebuildTerm? <-
    liftIO findLiveHostModuleCheckErasureSubsetRebuildTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckErasureSubsetRebuildTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckErasureSubsetRebuildTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckErasureSubsetRebuildTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckErasureSubsetRebuildTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckErasureSubsetRebuildTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckErasureSubsetRebuildTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckErasureSubsetRebuildTermSubset fakeLake fakeDir
  let notAliasOfComposeSubsetRebuildTerm :=
    namedClosedHostModuleCheckErasureSubsetRebuildTermSubsetNames
      ...
      && namedClosedHostModuleCheckErasureSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckErasureSubsetRebuildTerm"
      && !namedClosedHostModuleCheckComposeSubsetRebuildTermSubsetNames.contains
           "SystemsLean.HostModuleCheckErasureSubsetRebuildTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfComposeSubsetRebuildTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetRebuildTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckErasureSubsetRebuildTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetRebuildTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckErasureSubsetRebuildTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe

#elabMeetNamedHostModuleCheckErasureSubsetRebuildTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckErasureSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckErasureSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckErasureSubsetRebuildTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckErasureSubsetRebuildTermSubset = true := rfl
