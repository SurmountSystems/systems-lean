/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathTypesTerm named
  closed subset probe (plus-one after the CompilePathLinearTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathTypesTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathTypesTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathMultTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathLinearTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathTypesTerm subset:
    prior CompilePathLinearTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathTypesTerm last.
    ...
    the CompilePathLinearTerm compile order, then HostModuleCheckCompilePathTypesTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathLinearTerm): CompilePathTypesTerm
    ...
    CompilePathTypesTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathTypesTerm.
    HostModuleCheckCompilePathTypesTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathTypesTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathTypesTerm bad writes a temp
    HostModuleCheckCompilePathTypesTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathLinearTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathLinearTerm walker
    tryCompileNamedHostModuleCheckCompilePathLinearTermSubset, and that this walk
    is not an alias of the CompilePathLinearTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathTypesTerm.lean. Do not skip to
  ...
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathTypesTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathTypesTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathTypesTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathTypesTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathLinearTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathTypesTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathTypesTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathLinearTerm walker
    tryCompileNamedHostModuleCheckCompilePathLinearTermSubset still works; this
    walker is not an alias of the CompilePathLinearTerm closed set. Drive is
    good && !bad && isolation.
...
def elabMeetRunNamedHostModuleCheckCompilePathTypesTermSubsetProbe : CommandElabM Unit := do
  ...
  let liveHostModuleCheckCompilePathTypesTerm? <-
    liftIO findLiveHostModuleCheckCompilePathTypesTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathTypesTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathTypesTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathTypesTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathTypesTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathTypesTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathTypesTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathTypesTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathLinearTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathTypesTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathTypesTermSubset fakeLake fakeDir
  let notAliasOfCompilePathLinearTerm :=
    namedClosedHostModuleCheckCompilePathTypesTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfCompilePathLinearTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathTypesTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathTypesTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathTypesTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathTypesTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathTypesTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathTypesTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathTypesTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathTypesTermSubset = true := rfl
