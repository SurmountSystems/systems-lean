/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-FRONT-THEOREMS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_FRONT_THEOREMS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-FRONT-THEOREMS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckFrontTheoremsKernelSource,
  hostFrontLiveHostModuleCheckFrontTheoremsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckFrontTheoremsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckFrontTheoremsKernelSource
...
def liveRel : String := "HostModuleCheckFrontTheoremsKernel.lean"
...
def hostFrontLiveHostModuleCheckFrontTheoremsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckFrontTheoremsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckFrontTheoremsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckFrontTheoremsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckFrontTheoremsKernel.lean bytes.
    Greppable: liveHostModuleCheckFrontTheoremsKernelSource. -/
def liveHostModuleCheckFrontTheoremsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckFrontTheoremsKernel.lean text.
    Greppable: parseLiveHostModuleCheckFrontTheoremsKernelSource. -/
def parseLiveHostModuleCheckFrontTheoremsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckFrontTheoremsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckFrontTheoremsKernelSource. -/
def kernelCheckLiveHostModuleCheckFrontTheoremsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckFrontTheoremsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckFrontTheoremsKernelSource
      liveHostModuleCheckFrontTheoremsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthFrontTheoremsKernelBar"
      && has "checkDepthFrontTheoremsKernelScope"
      && has "hostModuleCheckFrontTheoremsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckFrontTheoremsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckFrontTheoremsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckFrontTheoremsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckFrontTheoremsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckFrontTheoremsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_FRONT_THEOREMS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-FRONT-THEOREMS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-FRONT-THEOREMS-KERNEL")
    && (liveRel == "HostModuleCheckFrontTheoremsKernel.lean")
    && (liveHostModuleCheckFrontTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckFrontTheoremsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckFrontTheoremsKernelFullHost
    && !hostFrontLiveHostModuleCheckFrontTheoremsKernelFullBackend
    && !hostFrontLiveHostModuleCheckFrontTheoremsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckFrontTheoremsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckFrontTheoremsKernelSource
      liveHostModuleCheckFrontTheoremsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckFrontTheoremsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-FRONT-THEOREMS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckFrontTheoremsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckFrontTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckFrontTheoremsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckFrontTheoremsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckFrontTheoremsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckFrontTheoremsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckFrontTheoremsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckFrontTheoremsKernelSource
      liveHostModuleCheckFrontTheoremsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckFrontTheoremsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckFrontTheoremsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckFrontTheoremsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckFrontTheoremsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckFrontTheoremsKernel.lean != liveHostModuleCheckFrontTheoremsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckFrontTheoremsKernel.lean")
  let r := parseLiveHostModuleCheckFrontTheoremsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckFrontTheoremsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckFrontTheoremsKernel parse false")
    unless hostFrontLiveHostModuleCheckFrontTheoremsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckFrontTheoremsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckFrontTheoremsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckFrontTheoremsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckFrontTheoremsKernelSource
