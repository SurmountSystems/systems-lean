/-
  ...
  Short role: HostFrontLiveHostModuleCheckExtractTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckExtractTermSource turns live
    HostModuleCheckExtractTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckExtractTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckExtractTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckExtractTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckExtractTermSource. Not split.
  ...
  - Kept: two imports, the namespace, four literal defs, and end.
    Kept count is 8.
    The four literal defs are checkDepthExtractTermSurfaceBar,
    checkDepthExtractTermSurfaceScope,
    hostModuleCheckExtractTermSurfaceDualOk, and
  ...
  - The longest quoted span in this file is 160 characters
    (extractCheckFailClosedBodyOk, the checkFailClosed body string).
  ...
  - Fixture strings use backslash-quote. takeStringN stops at the first
    quote character. A fixture line that opens with the word def keeps
    that word inside the opening quote span, so those fixture lines are
    not extra command keywords.
  ...
  HostFrontLiveHostModuleCheckExtractTerm
  PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEXTRACTTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEXTRACTTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckExtractTermSource,
  kernelCheckLiveHostModuleCheckExtractTermSource,
  hostFrontLiveHostModuleCheckExtractTermReady,
  liveHostModuleCheckExtractTermSource,
  liveRel, liveHostModuleCheckExtractTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckExtractTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckExtractTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEXTRACTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckExtractTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckExtractTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckExtractTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckExtractTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckExtractTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckExtractTermProvablyUnlocked : Bool :=
  false
...
/-- String take fuel. The longest quoted span in this file is 160 characters.
    That is under 256. liveStringFuel stays 1024 so takeStringN does not use
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckExtractTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckExtractTerm.lean
...
/-- Parse live HostModuleCheckExtractTerm.lean text.
    Greppable: parseLiveHostModuleCheckExtractTermSource,
    PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM. -/
def parseLiveHostModuleCheckExtractTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckExtractTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckExtractTermSource,
    PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM. -/
def kernelCheckLiveHostModuleCheckExtractTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckExtractTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckExtractTermSource
      liveHostModuleCheckExtractTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthExtractTermSurfaceBar",
      "checkDepthExtractTermSurfaceScope",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckExtractTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckExtractTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckExtractTerm"
      && lastSeg m.name.raw == "HostModuleCheckExtractTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckExtractTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthExtractTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthExtractTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "extractBodyAfterAssign"
      && toksHaveDefNamed liveSkipFuel toks "extractDefBodyCollapsed"
      && toksHaveDefNamed liveSkipFuel toks "extractDefArmRhsList"
      && toksHaveDefNamed liveSkipFuel toks "extractArmListHas"
      && toksHaveDefNamed liveSkipFuel toks "extractRuntimeClaimNameArmsOk"
      && toksHaveDefNamed liveSkipFuel toks "extractIsFreestandingGoalArmsOk"
      && toksHaveDefNamed liveSkipFuel toks "extractOfRuntimeTagArmsOk"
      && toksHaveDefNamed liveSkipFuel toks "extractIsValidRuntimeTagBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "extractCheckFailClosedBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "extractExtractOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "extractExtractOkFromTagsBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkExtractTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineExtractWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodExtractTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractCheckFailClosedText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsFreestandingGoalText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractExtractOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsValidRuntimeTagText"
      && toksHaveDefNamed liveSkipFuel toks "extractTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractCheckFailClosedL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsFreestandingGoalL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractExtractOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsValidRuntimeTagL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractCheckFailClosedDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsFreestandingGoalDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractExtractOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadExtractIsValidRuntimeTagDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckExtractTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckExtractTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_extract_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckExtractTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckExtractTermReady,
    PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEXTRACTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckExtractTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEXTRACTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEXTRACTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM")
    && (liveRel == "HostModuleCheckExtractTerm.lean")
    && (liveHostModuleCheckExtractTermRel ==
        "src/systems/SystemsLean/HostModuleCheckExtractTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckExtractTermFullHost
    && !hostFrontLiveHostModuleCheckExtractTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckExtractTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckExtractTermSource
        liveHostModuleCheckExtractTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckExtractTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckExtractTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckExtractTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckExtractTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckExtractTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckExtractTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckExtractTerm.lean != liveHostModuleCheckExtractTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckExtractTerm.lean")
  let r := parseLiveHostModuleCheckExtractTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEXTRACTTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckExtractTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckExtractTerm parse false")
    unless hostFrontLiveHostModuleCheckExtractTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckExtractTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckExtractTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckExtractTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckExtractTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckExtractTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
