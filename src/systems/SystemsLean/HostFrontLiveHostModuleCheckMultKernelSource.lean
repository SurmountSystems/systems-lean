/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckMultKernel.lean.
  ...
  liveRel is HostModuleCheckMultKernel.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckMultKernelSource,
  PARSE-LIVE-HOST-MODULE-CHECK-MULT-KERNEL.
  Module: SystemsLean.HostFrontLiveHostModuleCheckMultKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckMultKernelSource
...
def liveRel : String := "HostModuleCheckMultKernel.lean"

/-- Pinned live HostModuleCheckMultKernel.lean bytes. -/
def liveHostModuleCheckMultKernelSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckMultKernelSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckMultKernelSourceReady : Bool :=
  liveRel == "HostModuleCheckMultKernel.lean"
    && kernelCheckLiveHostModuleCheckMultKernelSource liveHostModuleCheckMultKernelSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckMultKernelSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckMultKernel.lean")
  let k := kernelCheckLiveHostModuleCheckMultKernelSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckMultKernel parse false")
  unless hostFrontLiveHostModuleCheckMultKernelSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckMultKernelSource
