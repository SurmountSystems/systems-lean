/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathMultTerm named
  closed subset probe (plus-one after the DualResidualTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathMultTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathMultTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckSpecProofTerm, or
  ElabMeetNamedWalkHostModuleCheckDualResidualTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathMultTerm subset:
    prior DualResidualTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathMultTerm last.
    ...
    the DualResidualTerm compile order, then HostModuleCheckCompilePathMultTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as DualResidualTerm): CompilePathMultTerm
    ...
    CompilePathMultTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathMultTerm.
    HostModuleCheckCompilePathMultTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathMultTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathMultTerm bad writes a temp
    HostModuleCheckCompilePathMultTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckDualResidualTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior DualResidualTerm walker
    tryCompileNamedHostModuleCheckDualResidualTermSubset, and that this walk
    is not an alias of the DualResidualTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathMultTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathMultTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathMultTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathMultTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathMultTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathMultTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathMultTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathMultTerm,
  ElabMeetNamedHostModuleCheckCompilePathMultTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathMultTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathMultTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathMultTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathMultTerm named subset.
    Good compiles live Mult through HostModuleCheckDualResidualTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathMultTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathMultTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior DualResidualTerm walker
    tryCompileNamedHostModuleCheckDualResidualTermSubset still works; this
    walker is not an alias of the DualResidualTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathMultTerm.lean. Do not skip to
    HostModuleCheckCompilePathLinearTerm. -/
def elabMeetRunNamedHostModuleCheckCompilePathMultTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathMultTerm? <-
    liftIO findLiveHostModuleCheckCompilePathMultTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathMultTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathMultTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathMultTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathMultTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathMultTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathMultTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathMultTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathMultTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathMultTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathMultTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathMultTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckCompilePathMultTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckCompilePathMultTermSubsetProbe

#elabMeetNamedHostModuleCheckCompilePathMultTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathMultTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathMultTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathMultTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathMultTermSubset = true := rfl
