/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-ERASURE-THEOREMS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_ERASURE_THEOREMS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-THEOREMS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckErasureTheoremsKernelSource,
  hostFrontLiveHostModuleCheckErasureTheoremsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckErasureTheoremsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckErasureTheoremsKernelSource
...
def liveRel : String := "HostModuleCheckErasureTheoremsKernel.lean"
...
def hostFrontLiveHostModuleCheckErasureTheoremsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckErasureTheoremsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckErasureTheoremsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckErasureTheoremsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckErasureTheoremsKernel.lean bytes.
    Greppable: liveHostModuleCheckErasureTheoremsKernelSource. -/

def liveHostModuleCheckErasureTheoremsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckErasureTheoremsKernel.lean text.
    Greppable: parseLiveHostModuleCheckErasureTheoremsKernelSource. -/
def parseLiveHostModuleCheckErasureTheoremsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckErasureTheoremsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckErasureTheoremsKernelSource. -/
def kernelCheckLiveHostModuleCheckErasureTheoremsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckErasureTheoremsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckErasureTheoremsKernelSource
      liveHostModuleCheckErasureTheoremsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthErasureTheoremsKernelBar"
      && has "checkDepthErasureTheoremsKernelScope"
      && has "hostModuleCheckErasureTheoremsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckErasureTheoremsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckErasureTheoremsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckErasureTheoremsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckErasureTheoremsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckErasureTheoremsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_ERASURE_THEOREMS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-ERASURE-THEOREMS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-THEOREMS-KERNEL")
    && (liveRel == "HostModuleCheckErasureTheoremsKernel.lean")
    && (liveHostModuleCheckErasureTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckErasureTheoremsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckErasureTheoremsKernelFullHost
    && !hostFrontLiveHostModuleCheckErasureTheoremsKernelFullBackend
    && !hostFrontLiveHostModuleCheckErasureTheoremsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckErasureTheoremsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckErasureTheoremsKernelSource
      liveHostModuleCheckErasureTheoremsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckErasureTheoremsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-ERASURE-THEOREMS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckErasureTheoremsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckErasureTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckErasureTheoremsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckErasureTheoremsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckErasureTheoremsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckErasureTheoremsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckErasureTheoremsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckErasureTheoremsKernelSource
      liveHostModuleCheckErasureTheoremsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckErasureTheoremsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckErasureTheoremsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckErasureTheoremsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckErasureTheoremsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckErasureTheoremsKernel.lean != liveHostModuleCheckErasureTheoremsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckErasureTheoremsKernel.lean")
  let r := parseLiveHostModuleCheckErasureTheoremsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckErasureTheoremsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckErasureTheoremsKernel parse false")
    unless hostFrontLiveHostModuleCheckErasureTheoremsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckErasureTheoremsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckErasureTheoremsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckErasureTheoremsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckErasureTheoremsKernelSource
