/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathBodyTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathBodyTermSource turns live
    HostModuleCheckCompilePathBodyTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathBodyTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathBodyTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathBodyTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathBodyTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathBodyTermDialect and
    def hostModuleCheckCompilePathBodyTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathBodyTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHBODYTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathBodyTermSource,
  kernelCheckLiveHostModuleCheckCompilePathBodyTermSource,
  hostFrontLiveHostModuleCheckCompilePathBodyTermReady,
  liveHostModuleCheckCompilePathBodyTermSource,
  liveRel, liveHostModuleCheckCompilePathBodyTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathBodyTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathBodyTermSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckCompilePathBodyTerm
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathBodyTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathBodyTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathBodyTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathBodyTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathBodyTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathBodyTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathBodyTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathBodyTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathBodyTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathBodyTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathBodyTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM. -/
def parseLiveHostModuleCheckCompilePathBodyTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathBodyTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathBodyTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathBodyTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathBodyTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathBodyTermSource
      liveHostModuleCheckCompilePathBodyTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathBodyTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathBodyTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathBodyTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathBodyTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathBodyTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathBodyTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathBodyTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathBodyTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHBODYTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM")
    && (liveRel == "HostModuleCheckCompilePathBodyTerm.lean")
    && (liveHostModuleCheckCompilePathBodyTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathBodyTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathBodyTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathBodyTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathBodyTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathBodyTermSource
        liveHostModuleCheckCompilePathBodyTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathBodyTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathBodyTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathBodyTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathBodyTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathBodyTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathBodyTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathBodyTerm.lean != liveHostModuleCheckCompilePathBodyTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathBodyTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathBodyTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHBODYTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathBodyTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathBodyTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathBodyTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathBodyTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathBodyTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathBodyTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathBodyTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathBodyTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
...
end SystemsLean.HostFrontLiveHostModuleCheckCompilePathBodyTerm
