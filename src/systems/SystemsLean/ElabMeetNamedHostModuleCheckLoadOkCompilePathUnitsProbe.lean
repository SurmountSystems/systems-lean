/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLoadOkCompilePathUnits named
  closed subset probe (plus-one after the SelfHostBodyTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LoadOkCompilePathUnits wrappers live in
  ElabMeetNamedWalkHostModuleCheckLoadOkCompilePathUnits. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckProbeWireTermProbe,
  ElabMeetNamedHostModuleCheckSelfHostBodyTermProbe,
  ...
  ElabMeetNamedWalkHostModuleCheckProbeWireTerm, or
  ElabMeetNamedWalkHostModuleCheckSelfHostBodyTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckLoadOkCompilePathUnits subset:
    prior SelfHostBodyTerm closed set plus
    SystemsLean.HostModuleCheckLoadOkCompilePathUnits last.
    ...
    the SelfHostBodyTerm compile order, then HostModuleCheckLoadOkCompilePathUnits last.
    First import of the product module is HostModuleCheckCompilePathMultTerm.
    ...
    Linear skipped as a named plus-one. LoadOkCompilePathUnits walker is a
    wrapper in ElabMeetNamedWalkHostModuleCheckLoadOkCompilePathUnits.
    HostModuleCheckLoadOkCompilePathUnits probe lives here.
  - tryCompileNamedHostModuleCheckLoadOkCompilePathUnitsSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLoadOkCompilePathUnits bad writes a temp
    HostModuleCheckLoadOkCompilePathUnits copy and compiles it after Mult and the
    ...
    HostModuleCheckSelfHostBodyTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior SelfHostBodyTerm walker
    tryCompileNamedHostModuleCheckSelfHostBodyTermSubset, and that this walk
    is not an alias of the SelfHostBodyTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLoadOkCompilePathUnits.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckLoadOkCompilePathUnitsSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLoadOkCompilePathUnitsSubset,
  elabMeetRejectsBadNamedHostModuleCheckLoadOkCompilePathUnitsSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLoadOkCompilePathUnitsSubset,
  #elabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe,
  elabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe,
  SystemsLean.HostModuleCheckLoadOkCompilePathUnits,
  ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsProbe;
  just systems-host.
...
import SystemsLean.ElabMeetNamedWalkHostModuleCheckLoadOkCompilePathUnits

namespace SystemsLean.ElabMeet

open Lean Elab Command

/-- Record accept / reject for the HostModuleCheckLoadOkCompilePathUnits named subset.
    Good compiles live Mult through HostModuleCheckSelfHostBodyTerm plus the
    subset-surface compile closure and HostModuleCheckLoadOkCompilePathUnits after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLoadOkCompilePathUnits copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior SelfHostBodyTerm walker
    tryCompileNamedHostModuleCheckSelfHostBodyTermSubset still works; this
    walker is not an alias of the SelfHostBodyTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLoadOkCompilePathUnits.lean. Do not skip to
    HostModuleCheckLoadOkLaterTerm. -/
def elabMeetRunNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLoadOkCompilePathUnits? <-
    liftIO findLiveHostModuleCheckLoadOkCompilePathUnitsPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLoadOkCompilePathUnitsSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLoadOkCompilePathUnits? with
    | none =>
      pure true
    | some liveHostModuleCheckLoadOkCompilePathUnits => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLoadOkCompilePathUnits)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLoadOkCompilePathUnitsDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSelfHostBodyTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLoadOkCompilePathUnitsSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLoadOkCompilePathUnitsSubset fakeLake fakeDir
  let notAliasOfSelfHostBodyTerm :=
    namedClosedHostModuleCheckLoadOkCompilePathUnitsSubsetNames
      != namedClosedHostModuleCheckSelfHostBodyTermSubsetNames
      && namedClosedHostModuleCheckLoadOkCompilePathUnitsCompileOrder
           != namedClosedHostModuleCheckSelfHostBodyTermCompileOrder
      && namedClosedHostModuleCheckLoadOkCompilePathUnitsSubsetNames.contains
           "SystemsLean.HostModuleCheckLoadOkCompilePathUnits"
      && !namedClosedHostModuleCheckSelfHostBodyTermSubsetNames.contains
           "SystemsLean.HostModuleCheckLoadOkCompilePathUnits"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfSelfHostBodyTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLoadOkCompilePathUnitsSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLoadOkCompilePathUnitsSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLoadOkCompilePathUnitsSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLoadOkCompilePathUnitsSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe

#elabMeetNamedHostModuleCheckLoadOkCompilePathUnitsSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckLoadOkCompilePathUnitsSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLoadOkCompilePathUnitsSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLoadOkCompilePathUnitsSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLoadOkCompilePathUnitsSubset = true := rfl
