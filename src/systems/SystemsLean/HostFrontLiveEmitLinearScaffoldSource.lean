/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live EmitLinearScaffold.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveEmitLinearScaffoldSource. Not occupancy name 50. Not mill 70.
  This wrap is EmitLinearScaffold.lean. It is not EmitLinear.lean.
  Greppable: SYSTEMS_LEAN_HOST, liveEmitLinearScaffoldSource,
  HOST-FRONT-LIVE-EMIT-LINEAR-SCAFFOLD, liveRel, UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveEmitLinearScaffoldSource
  Not FullHost. Not freestanding residual free. Not PROVABLY.
  Not full product module emit. FullBackend stays false in the wrap.
  Module must stay ASCII.
-/

namespace SystemsLean.HostFrontLiveEmitLinearScaffold

/-- Dual-pinned live EmitLinearScaffold.lean bytes (must match on-disk file).
    Greppable: liveEmitLinearScaffoldSource, HOST-FRONT-LIVE-EMIT-LINEAR-SCAFFOLD. -/
def liveEmitLinearScaffoldSource : String := r#"/-
  SYSTEMS_LEAN_HOST partial -- freestanding Linear product C scaffolding.
  Side: classic Lean elaborator under src/systems/ (not freestanding C runtime).
  Owns HOST-EMIT-LINEAR product-text fragments (linearHeaderFragment /
  linearBodyFragment), stage/map ids, ownership honesty, and emitLinearReady
  used by SystemsLean.EmitLinear. Same namespace SystemsLean.EmitLinear so
  call sites stay unqualified. Long-file peel from EmitLinear.lean.

  Spec (readable):
  - stageId / hostEmitLinearId / selfHostEmitLinearId: greppable stage map.
  - linearHeaderFragment / linearBodyFragment: freestanding C scaffolding SSoT
    text (LINEAR_C_HEADER / LINEAR_C_BODY map; FreestandingEmit embeds durable
    SSOT file).
  - emitLinearReady: surface + header/body piece honesty (cheap; not residual free).
  ...
  - Durable file host_emit_linear.ssot.txt remains HOST-EMIT-LINEAR dialect
  ...
  SELF-HOST-EMIT-LINEAR, linearHeaderFragment, linearBodyFragment, emitLinearReady,
  emitLinearOk, ownershipHonestyOk, headerHonestyOk, bodyHonestyOk, emitLinearSurfaceOk,
  EMIT-LINEAR-SMOKE, HOST-EMIT-LINEAR-SMOKE, EmitLinearScaffold,
  LINEAR-EXACT-ONCE, CONSUME_TOKEN_HOST_V0, JOIN-ALG, slake_linear_consume,
  slake_consume_token_consume, NON-SSOT, RUNTIME-FS,
  UNIT_SURFACE host surface.
  Module: SystemsLean.EmitLinearScaffold
  Red/green: just systems-host; lake build SystemsLean.EmitLinear.
  Module must stay ASCII.
...
/-! ### Host-owned freestanding Linear product C (HOST-EMIT-LINEAR)
    Greppable: HOST-EMIT-LINEAR, SELF-HOST-EMIT-LINEAR, SLAKE_SELF_HOST_EMIT_LINEAR_V0,
    linearHeaderFragment, linearBodyFragment, emitLinearReady.
    maxHeartbeats raised: linear bodies need room for decide honesty. -/
