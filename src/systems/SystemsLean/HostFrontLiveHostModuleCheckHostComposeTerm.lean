/-
  ...
  src/systems/SystemsLean/HostModuleCheckHostComposeTerm.lean with the
  ...
  defs (term-surface bar, scope, dualOk), the skeleton suffix string,
  and end. A count of 8.
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKHOSTCOMPOSETERM_V0,
  PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM, liveRel,
  kernelCheckLiveHostModuleCheckHostComposeTermSource,
  hostFrontLiveHostModuleCheckHostComposeTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckHostComposeTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckHostComposeTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckHostComposeTerm.lean"
...
def hostFrontLiveHostModuleCheckHostComposeTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckHostComposeTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckHostComposeTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckHostComposeTermProvablyUnlocked : Bool :=
  false
...
/-- Kept commands: two imports, namespace, three literal defs,
    skeleton suffix, and end. A count of 8.
    From HostModuleCheckHostComposeTerm.lean. -/
...
/-- Parse live HostModuleCheckHostComposeTerm.lean text.
    Greppable: parseLiveHostModuleCheckHostComposeTermSource,
    PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM. -/
def parseLiveHostModuleCheckHostComposeTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckHostComposeTerm"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckHostComposeTermSource,
    PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM. -/
def kernelCheckLiveHostModuleCheckHostComposeTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckHostComposeTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckHostComposeTermSource
      liveHostModuleCheckHostComposeTermSource with
...
/-- Two imports. Not zero.
    SystemsLean.HostModuleCheckSurface and HostModuleCheckMultTerm. -/
...
/-- Live parse command count. Two imports, namespace, three literal
    defs, skeleton suffix, end. Count is 8. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthHostComposeTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Scope literal is HostCompose-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthHostComposeTermSurfaceScope"
          && s == "HostCompose-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckHostComposeTermSurfaceDualOk" && b == true
      | _ => false
...
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckHostComposeTermSkeletonSuffix"
          && s == "end SystemsLean.HostCompose\\n"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckHostComposeTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckHostComposeTerm"
      && lastSeg m.name.raw == "HostModuleCheckHostComposeTerm"
...
/-- Import SystemsLean.HostModuleCheckMultTerm. Not dropped. -/
...
/-- Empty-body checker needle. Body is a match, not a kernel atom. -/
...
/-- Good empty structure text, nested := kept. -/
def needleEmptyStruct : String :=
  "body == \"{ graph := IrGraph.empty linear := LinearHost.empty erased := Erasure.unmarked }\""

/-- checkFailClosed body needle. -/
def needleCheckBody : String :=
  "body == \"hostIsWellTyped hc && multPreScan hc\""

/-- extractOkFs body needle. -/
def needleExtractBody : String :=
  "body == \"extractOk hc RuntimeClaim.runtimeFs\""
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckHostComposeTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckHostComposeTermReady,
    PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckHostComposeTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKHOSTCOMPOSETERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM")
    && (liveRel == "HostModuleCheckHostComposeTerm.lean")
    && (liveHostModuleCheckHostComposeTermRel ==
        "src/systems/SystemsLean/HostModuleCheckHostComposeTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckHostComposeTermFullHost
    && !hostFrontLiveHostModuleCheckHostComposeTermFullBackend
    && !hostFrontLiveHostModuleCheckHostComposeTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckHostComposeTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckHostComposeTermSource
        liveHostModuleCheckHostComposeTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckHostComposeTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckHostComposeTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckHostComposeTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckHostComposeTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckHostComposeTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckHostComposeTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckHostComposeTerm.lean != liveHostModuleCheckHostComposeTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckHostComposeTerm.lean")
  let r := parseLiveHostModuleCheckHostComposeTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKHOSTCOMPOSETERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckHostComposeTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckHostComposeTerm parse false")
    unless hostFrontLiveHostModuleCheckHostComposeTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckHostComposeTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckHostComposeTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckHostComposeTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckHostComposeTerm source was accepted")
