/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckComposeSubsetRebuildTerm named
  closed subset probe (plus-one after the GraphSubsetRebuildTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. ComposeSubsetRebuildTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckComposeSubsetRebuildTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckProgramSubsetRebuildTerm, or
  ElabMeetNamedWalkHostModuleCheckGraphSubsetRebuildTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckComposeSubsetRebuildTerm subset:
    prior GraphSubsetRebuildTerm closed set plus
    SystemsLean.HostModuleCheckComposeSubsetRebuildTerm last.
    ...
    the GraphSubsetRebuildTerm compile order, then HostModuleCheckComposeSubsetRebuildTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as GraphSubsetRebuildTerm): ComposeSubsetRebuildTerm
    ...
    ComposeSubsetRebuildTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckComposeSubsetRebuildTerm.
    HostModuleCheckComposeSubsetRebuildTerm probe lives here.
  - tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckComposeSubsetRebuildTerm bad writes a temp
    HostModuleCheckComposeSubsetRebuildTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckGraphSubsetRebuildTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior GraphSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckGraphSubsetRebuildTermSubset, and that this walk
    is not an alias of the GraphSubsetRebuildTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckComposeSubsetRebuildTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckComposeSubsetRebuildTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckComposeSubsetRebuildTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckComposeSubsetRebuildTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckComposeSubsetRebuildTermSubset,
  #elabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe,
  elabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe,
  SystemsLean.HostModuleCheckComposeSubsetRebuildTerm,
  ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckComposeSubsetRebuildTerm named subset.
    Good compiles live Mult through HostModuleCheckGraphSubsetRebuildTerm plus the
    subset-surface compile closure and HostModuleCheckComposeSubsetRebuildTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckComposeSubsetRebuildTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior GraphSubsetRebuildTerm walker
    tryCompileNamedHostModuleCheckGraphSubsetRebuildTermSubset still works; this
    walker is not an alias of the GraphSubsetRebuildTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckComposeSubsetRebuildTerm.lean. Do not skip to
    HostModuleCheckErasureSubsetRebuildTerm. -/
def elabMeetRunNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckComposeSubsetRebuildTerm? <-
    liftIO findLiveHostModuleCheckComposeSubsetRebuildTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckComposeSubsetRebuildTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckComposeSubsetRebuildTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckComposeSubsetRebuildTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckComposeSubsetRebuildTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckGraphSubsetRebuildTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckComposeSubsetRebuildTermSubset fakeLake fakeDir
  let notAliasOfGraphSubsetRebuildTerm :=
    namedClosedHostModuleCheckComposeSubsetRebuildTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfGraphSubsetRebuildTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckComposeSubsetRebuildTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckComposeSubsetRebuildTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckComposeSubsetRebuildTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckComposeSubsetRebuildTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe

#elabMeetNamedHostModuleCheckComposeSubsetRebuildTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckComposeSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckComposeSubsetRebuildTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckComposeSubsetRebuildTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckComposeSubsetRebuildTermSubset = true := rfl
