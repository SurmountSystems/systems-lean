/-
  ...
  src/systems/SystemsLean/HostModuleCheckFrontKernel.lean
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-FRONT-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_FRONT_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-FRONT-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckFrontKernelSource,
  hostFrontLiveHostModuleCheckFrontKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckFrontKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckFrontKernelSource
...
def liveRel : String := "HostModuleCheckFrontKernel.lean"
...
def hostFrontLiveHostModuleCheckFrontKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckFrontKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckFrontKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckFrontKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckFrontKernel.lean bytes.
    Greppable: liveHostModuleCheckFrontKernelSource. -/

def liveHostModuleCheckFrontKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckFrontKernel.lean text.
    Greppable: parseLiveHostModuleCheckFrontKernelSource. -/
def parseLiveHostModuleCheckFrontKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckFrontKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckFrontKernelSource. -/
def kernelCheckLiveHostModuleCheckFrontKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckFrontKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckFrontKernelSource
      liveHostModuleCheckFrontKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthFrontKernelBar"
      && has "checkDepthFrontKernelScope"
      && has "hostModuleCheckFrontKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckFrontKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckFrontKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckFrontKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckFrontKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckFrontKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_FRONT_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-FRONT-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-FRONT-KERNEL")
    && (liveRel == "HostModuleCheckFrontKernel.lean")
    && (liveHostModuleCheckFrontKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckFrontKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckFrontKernelFullHost
    && !hostFrontLiveHostModuleCheckFrontKernelFullBackend
    && !hostFrontLiveHostModuleCheckFrontKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckFrontKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckFrontKernelSource
      liveHostModuleCheckFrontKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckFrontKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-FRONT-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckFrontKernel.lean")
    ++ bad "rel" (liveHostModuleCheckFrontKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckFrontKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckFrontKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckFrontKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckFrontKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckFrontKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckFrontKernelSource
      liveHostModuleCheckFrontKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckFrontKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckFrontKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckFrontKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckFrontKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckFrontKernel.lean != liveHostModuleCheckFrontKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckFrontKernel.lean")
  let r := parseLiveHostModuleCheckFrontKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckFrontKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckFrontKernel parse false")
    unless hostFrontLiveHostModuleCheckFrontKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckFrontKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckFrontKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckFrontKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckFrontKernelSource
