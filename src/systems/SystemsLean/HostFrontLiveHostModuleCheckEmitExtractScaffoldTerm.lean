/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitExtractScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitExtractScaffoldTermSource turns live
    HostModuleCheckEmitExtractScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitExtractScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitExtractScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitExtractScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitExtractScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, five literal defs, and end.
    Kept count is 9.
    The five literal defs are checkDepthEmitExtractScaffoldTermSurfaceBar,
    checkDepthEmitExtractScaffoldTermSurfaceScope,
    hostModuleCheckEmitExtractScaffoldTermSurfaceDualOk,
    hostModuleCheckEmitExtractScaffoldTermSkeletonPrefix, and
  ...
  HostFrontLiveHostModuleCheckEmitExtractScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitExtractScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitExtractScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady,
  liveHostModuleCheckEmitExtractScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitExtractScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitExtractScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitExtractScaffoldTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitExtractScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitExtractScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitExtractScaffoldTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitExtractScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitExtractScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitExtractScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitExtractScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitExtractScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitExtractScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitExtractScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitExtractScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitExtractScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitExtractScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitExtractScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitExtractScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitExtractScaffoldTermSource
      liveHostModuleCheckEmitExtractScaffoldTermSource with
...
def liveParseDefsOk : Bool :=
  ...
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitExtractScaffoldTermSurfaceBar",
      "checkDepthEmitExtractScaffoldTermSurfaceScope",
      "hostModuleCheckEmitExtractScaffoldTermSurfaceDualOk",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitExtractScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitExtractScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitExtractScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitExtractScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitExtractScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitExtractScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitExtractScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitExtractScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitExtractScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitExtractScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitExtractScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitExtractScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitExtractScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitExtractScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_extract_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitExtractScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitExtractScaffoldTerm.lean")
    && (liveHostModuleCheckEmitExtractScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitExtractScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitExtractScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitExtractScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitExtractScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitExtractScaffoldTermSource
        liveHostModuleCheckEmitExtractScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitExtractScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitExtractScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitExtractScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitExtractScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitExtractScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitExtractScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitExtractScaffoldTerm.lean != liveHostModuleCheckEmitExtractScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitExtractScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitExtractScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITEXTRACTSCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitExtractScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitExtractScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitExtractScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitExtractScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitExtractScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitExtractScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
