/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-GRAPH-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_GRAPH_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-HOST-GRAPH-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckHostGraphKernelSource,
  hostFrontLiveHostModuleCheckHostGraphKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckHostGraphKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckHostGraphKernelSource
...
def liveRel : String := "HostModuleCheckHostGraphKernel.lean"
...
def hostFrontLiveHostModuleCheckHostGraphKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckHostGraphKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckHostGraphKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckHostGraphKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckHostGraphKernel.lean bytes.
    Greppable: liveHostModuleCheckHostGraphKernelSource. -/


def liveHostModuleCheckHostGraphKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckHostGraphKernel.lean text.
    Greppable: parseLiveHostModuleCheckHostGraphKernelSource. -/
def parseLiveHostModuleCheckHostGraphKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckHostGraphKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckHostGraphKernelSource. -/
def kernelCheckLiveHostModuleCheckHostGraphKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckHostGraphKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckHostGraphKernelSource
      liveHostModuleCheckHostGraphKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthHostGraphKernelBar"
      && has "checkDepthHostGraphKernelScope"
      && has "hostModuleCheckHostGraphKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckHostGraphKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckHostGraphKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckHostGraphKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckHostGraphKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckHostGraphKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_GRAPH_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-GRAPH-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-GRAPH-KERNEL")
    && (liveRel == "HostModuleCheckHostGraphKernel.lean")
    && (liveHostModuleCheckHostGraphKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostGraphKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckHostGraphKernelFullHost
    && !hostFrontLiveHostModuleCheckHostGraphKernelFullBackend
    && !hostFrontLiveHostModuleCheckHostGraphKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckHostGraphKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckHostGraphKernelSource
      liveHostModuleCheckHostGraphKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckHostGraphKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-GRAPH-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckHostGraphKernel.lean")
    ++ bad "rel" (liveHostModuleCheckHostGraphKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostGraphKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckHostGraphKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckHostGraphKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckHostGraphKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckHostGraphKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckHostGraphKernelSource
      liveHostModuleCheckHostGraphKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckHostGraphKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckHostGraphKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckHostGraphKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckHostGraphKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckHostGraphKernel.lean != liveHostModuleCheckHostGraphKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckHostGraphKernel.lean")
  let r := parseLiveHostModuleCheckHostGraphKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckHostGraphKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckHostGraphKernel parse false")
    unless hostFrontLiveHostModuleCheckHostGraphKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckHostGraphKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckHostGraphKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckHostGraphKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckHostGraphKernelSource
