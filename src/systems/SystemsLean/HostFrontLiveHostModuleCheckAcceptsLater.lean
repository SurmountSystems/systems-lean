/-
  ...
  Short role: HostFrontLiveHostModuleCheckAcceptsLater.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckAcceptsLaterSource turns live
    HostModuleCheckAcceptsLater.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckAcceptsLater
    even without a module line.
  - liveRel is the exact basename HostModuleCheckAcceptsLater.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckAcceptsLaterSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - Fourteen dotted imports, namespace SystemsLean.HostModuleCheck, and
    end are kept. Fifty-nine def bodies are applications, not kernel
    literals, so they are skip-folded. Kept command count is 16.
    kernelFuel is 64. 16 does not pass 64.
  - Skip-head still sees def hostModuleCheckBadMissingInductive and
    def hostModuleCheckBadExtractIsValidRuntimeTag in the live text.
  ...
  HostFrontLiveHostModuleCheckAcceptsLater
  PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER
  HOST-FRONT-LIVE-HOSTMODULECHECKACCEPTSLATER
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKACCEPTSLATER_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckAcceptsLaterSource,
  kernelCheckLiveHostModuleCheckAcceptsLaterSource,
  hostFrontLiveHostModuleCheckAcceptsLaterReady,
  liveHostModuleCheckAcceptsLaterSource,
  liveRel, liveHostModuleCheckAcceptsLaterRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckAcceptsLater
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckAcceptsLaterSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKACCEPTSLATER"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckAcceptsLater.lean"
...
def hostFrontLiveHostModuleCheckAcceptsLaterFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckAcceptsLaterResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckAcceptsLaterProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: fourteen imports, namespace, and end.
    Fifty-nine checkRealModule defs are not kernel literals.
    kernelFuel is 64. -/
def liveKeptCmds : Nat := 16
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckAcceptsLater has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckAcceptsLater.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckAcceptsLater`. -/
...
/-- Parse live HostModuleCheckAcceptsLater.lean text.
    Greppable: parseLiveHostModuleCheckAcceptsLaterSource,
    PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER. -/
def parseLiveHostModuleCheckAcceptsLaterSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckAcceptsLater"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckAcceptsLaterSource,
    PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER. -/
def kernelCheckLiveHostModuleCheckAcceptsLaterSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckAcceptsLaterSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckAcceptsLaterSource
      liveHostModuleCheckAcceptsLaterSource with
...
/-- Wrap module lastSeg is HostModuleCheckAcceptsLater
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckAcceptsLater"
      && lastSeg m.name.raw == "HostModuleCheckAcceptsLater"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckAcceptsLaterSource)
    toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadMissingInductive"
      && toksHaveDefNamed liveSkipFuel toks
        "hostModuleCheckBadExtractIsValidRuntimeTag"
...
def needleImportExtract : String :=
  "import SystemsLean.HostModuleCheckExtractTerm\n"

/-- Namespace needle with a trailing newline. -/
def needleNamespace : String := "namespace SystemsLean.HostModuleCheck\n"

/-- First fail-closed def. Body is not a kernel literal. -/
def needleMissingInductive : String :=
  "def hostModuleCheckBadMissingInductive : ModuleCheckResult :=\n"

/-- Last Extract def. Body is not a kernel literal. -/
def needleValidRuntimeTag : String :=
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckAcceptsLaterSource
  (src.splitOn needleImportFixtures).length == 2
    && (src.splitOn needleImportExtract).length == 2
    && (src.splitOn needleNamespace).length > 1
    && (src.splitOn needleMissingInductive).length > 1
    && (src.splitOn needleValidRuntimeTag).length > 1
    && (src.splitOn needleEnd).length > 1

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckAcceptsLaterReady,
    PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER,
    HOST-FRONT-LIVE-HOSTMODULECHECKACCEPTSLATER.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckAcceptsLaterReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKACCEPTSLATER_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKACCEPTSLATER")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER")
    && (liveRel == "HostModuleCheckAcceptsLater.lean")
    && (liveHostModuleCheckAcceptsLaterRel ==
        "src/systems/SystemsLean/HostModuleCheckAcceptsLater.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckAcceptsLaterFullHost
    && !hostFrontLiveHostModuleCheckAcceptsLaterResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckAcceptsLaterProvablyUnlocked
    && kernelCheckLiveHostModuleCheckAcceptsLaterSource
        liveHostModuleCheckAcceptsLaterSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckAcceptsLaterSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckAcceptsLaterRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckAcceptsLaterRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckAcceptsLaterRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckAcceptsLaterRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckAcceptsLaterSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckAcceptsLater.lean != liveHostModuleCheckAcceptsLaterSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckAcceptsLater.lean")
  let r := parseLiveHostModuleCheckAcceptsLaterSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKACCEPTSLATER ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckAcceptsLater parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckAcceptsLater parse false")
    unless hostFrontLiveHostModuleCheckAcceptsLaterReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckAcceptsLaterReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckAcceptsLaterReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckAcceptsLater source was accepted"
      throw (IO.userError
        "empty HostModuleCheckAcceptsLater source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckAcceptsLater.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
