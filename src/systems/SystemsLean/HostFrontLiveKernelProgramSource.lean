/-
  SYSTEMS_LEAN_HOST partial -- dual-pin live KernelProgram.lean bytes.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveKernelProgramSource. Not occupancy name 50. Not mill 70.
  Greppable: SYSTEMS_LEAN_HOST, PARSE-LIVE-KERNELPROGRAM, liveKernelProgramSource,
  HOST-FRONT-LIVE-KERNELPROGRAM, UNIT_SURFACE host surface.
  Module: SystemsLean.HostFrontLiveKernelProgramSource
...
/-- Dual-pinned live KernelProgram.lean bytes (must match on-disk file).
    Greppable: liveKernelProgramSource, PARSE-LIVE-KERNELPROGRAM. -/
def liveKernelProgramSource : String := r#"/-
  ...
  KernelProgram):
  - Live in SystemsLean.KernelProgramTheorems (same namespace; long-file peel).
  ...
  - KERNEL-PROGRAM-SMOKE behavioral examples live with theorems
  ...
  - Program end-to-end compile path (multi-node IR -> compose -> HOST-EMIT-PROGRAM)
    is CompilePath.programFixtureCompilePathReady (COMPILE-PATH-PROGRAM /
    PROGRAM-FIXTURE); this module stays IR kernel + graph/compose honesty only
    (import cycle: KernelProgram imports CompilePath).
  ...
  Long-file peel: KERNEL-PROGRAM-THEOREM + KERNEL-PROGRAM-SMOKE in
  SystemsLean.KernelProgramTheorems (same namespace). Core dialect stays here.
...
/- Theorems and KERNEL-PROGRAM-SMOKE live in SystemsLean.KernelProgramTheorems
   (same namespace SystemsLean.KernelProgram; long-file peel). Parent keeps
   dialect only -- do not import KernelProgramTheorems here (import cycle). -/
