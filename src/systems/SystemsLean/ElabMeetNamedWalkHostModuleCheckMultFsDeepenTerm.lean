/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckMultFsDeepenTerm named-walk
  wrappers (CheckersLater-role walk; LlvmTypesTextTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and LlvmTypesTextTerm walk
  ...
  ElabMeetNamedWalk. Prior LlvmTypesTextTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckLlvmTypesTextTerm. Do not grow that walk.
  ...
  LlvmTypesTextTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckMultFsDeepenTerm.lean.
  Do not skip to HostModuleCheckHostPackageWriteTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckMultFsDeepenTerm wrappers live here:
    ...
    Named membership is the prior LlvmTypesTextTerm closed set plus
    HostModuleCheckMultFsDeepenTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the LlvmTypesTextTerm compile
    order, then HostModuleCheckMultFsDeepenTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / LlvmTypesTextTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the LlvmTypesTextTerm compile order.
    HostModuleCheckMultFsDeepenTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckLlvmTypesTextTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckMultFsDeepenTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckMultFsDeepenTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckMultFsDeepenTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckMultFsDeepenTermPath. -/
def findLiveHostModuleCheckMultFsDeepenTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckMultFsDeepenTerm"

/-- Named HostModuleCheckMultFsDeepenTerm subset of the live SystemsLean library.
    Membership is the prior LlvmTypesTextTerm closed set plus
    HostModuleCheckMultFsDeepenTerm last.
    ...
    Greppable: namedClosedHostModuleCheckMultFsDeepenTermSubsetNames.
    SystemsLean.HostModuleCheckMultFsDeepenTerm. -/
def namedClosedHostModuleCheckMultFsDeepenTermSubsetNames : List String :=
  namedClosedHostModuleCheckLlvmTypesTextTermSubsetNames ++
    ["SystemsLean.HostModuleCheckMultFsDeepenTerm"]

/-- Compile order honors imports: LlvmTypesTextTerm compile order, then
    HostModuleCheckMultFsDeepenTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckMultFsDeepenTermCompileOrder. -/
def namedClosedHostModuleCheckMultFsDeepenTermCompileOrder : List String :=
  namedClosedHostModuleCheckLlvmTypesTextTermCompileOrder ++
    ["SystemsLean.HostModuleCheckMultFsDeepenTerm"]

/-- True when the barrel lists each HostModuleCheckMultFsDeepenTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckMultFsDeepenTermSubset. -/
def barrelListsNamedHostModuleCheckMultFsDeepenTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckMultFsDeepenTermSubsetNames

/-- Compile the named HostModuleCheckMultFsDeepenTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the LlvmTypesTextTerm
    compile order, then HostModuleCheckMultFsDeepenTerm last.
    Greppable: tryCompileNamedHostModuleCheckMultFsDeepenTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckMultFsDeepenTerm. -/
unsafe def tryCompileNamedHostModuleCheckMultFsDeepenTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckMultFsDeepenTermSubsetNames
    namedClosedHostModuleCheckMultFsDeepenTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckMultFsDeepenTermSubsetIO]
opaque tryCompileNamedHostModuleCheckMultFsDeepenTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckMultFsDeepenTerm subset.
    Greppable: tryCompileNamedHostModuleCheckMultFsDeepenTermSubset. -/
def tryCompileNamedHostModuleCheckMultFsDeepenTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckMultFsDeepenTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the LlvmTypesTextTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckMultFsDeepenTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckMultFsDeepenTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckLlvmTypesTextTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckMultFsDeepenTermDepsIO]
opaque tryCompileAfterHostModuleCheckMultFsDeepenTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckMultFsDeepenTerm deps.
    Greppable: tryCompileAfterHostModuleCheckMultFsDeepenTermDeps. -/
def tryCompileAfterHostModuleCheckMultFsDeepenTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckMultFsDeepenTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
