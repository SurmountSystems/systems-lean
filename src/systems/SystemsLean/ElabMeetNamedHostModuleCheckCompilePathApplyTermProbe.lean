/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckCompilePathApplyTerm named
  closed subset probe (plus-one after the CompilePathPlanTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. CompilePathApplyTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckCompilePathApplyTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathExtractTerm, or
  ElabMeetNamedWalkHostModuleCheckCompilePathPlanTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckCompilePathApplyTerm subset:
    prior CompilePathPlanTerm closed set plus
    SystemsLean.HostModuleCheckCompilePathApplyTerm last.
    ...
    the CompilePathPlanTerm compile order, then HostModuleCheckCompilePathApplyTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as CompilePathPlanTerm): CompilePathApplyTerm
    ...
    CompilePathApplyTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckCompilePathApplyTerm.
    HostModuleCheckCompilePathApplyTerm probe lives here.
  - tryCompileNamedHostModuleCheckCompilePathApplyTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckCompilePathApplyTerm bad writes a temp
    HostModuleCheckCompilePathApplyTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckCompilePathPlanTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior CompilePathPlanTerm walker
    tryCompileNamedHostModuleCheckCompilePathPlanTermSubset, and that this walk
    is not an alias of the CompilePathPlanTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckCompilePathApplyTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckCompilePathApplyTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckCompilePathApplyTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckCompilePathApplyTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathApplyTermSubset,
  #elabMeetNamedHostModuleCheckCompilePathApplyTermSubsetProbe,
  elabMeetNamedHostModuleCheckCompilePathApplyTermSubsetProbe,
  SystemsLean.HostModuleCheckCompilePathApplyTerm,
  ElabMeetNamedHostModuleCheckCompilePathApplyTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckCompilePathApplyTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckCompilePathApplyTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckCompilePathApplyTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckCompilePathApplyTerm named subset.
    Good compiles live Mult through HostModuleCheckCompilePathPlanTerm plus the
    subset-surface compile closure and HostModuleCheckCompilePathApplyTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckCompilePathApplyTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior CompilePathPlanTerm walker
    tryCompileNamedHostModuleCheckCompilePathPlanTermSubset still works; this
    walker is not an alias of the CompilePathPlanTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckCompilePathApplyTerm.lean. Do not skip to
    HostModuleCheckCompilePathBodyTerm. -/
def elabMeetRunNamedHostModuleCheckCompilePathApplyTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckCompilePathApplyTerm? <-
    liftIO findLiveHostModuleCheckCompilePathApplyTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathApplyTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckCompilePathApplyTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckCompilePathApplyTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckCompilePathApplyTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckCompilePathApplyTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckCompilePathApplyTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckCompilePathApplyTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckCompilePathPlanTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckCompilePathApplyTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckCompilePathApplyTermSubset fakeLake fakeDir
  let notAliasOfCompilePathPlanTerm :=
    namedClosedHostModuleCheckCompilePathApplyTermSubsetNames
      != namedClosedHostModuleCheckCompilePathPlanTermSubsetNames
      && namedClosedHostModuleCheckCompilePathApplyTermCompileOrder
           != namedClosedHostModuleCheckCompilePathPlanTermCompileOrder
      && namedClosedHostModuleCheckCompilePathApplyTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathApplyTerm"
      && !namedClosedHostModuleCheckCompilePathPlanTermSubsetNames.contains
           "SystemsLean.HostModuleCheckCompilePathApplyTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfCompilePathPlanTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckCompilePathApplyTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckCompilePathApplyTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathApplyTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckCompilePathApplyTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckCompilePathApplyTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckCompilePathApplyTermSubsetProbe

#elabMeetNamedHostModuleCheckCompilePathApplyTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckCompilePathApplyTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckCompilePathApplyTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckCompilePathApplyTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckCompilePathApplyTermSubset = true := rfl
