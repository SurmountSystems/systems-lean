/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckHostCheckTerm named
  closed subset probe (plus-one after the LlvmMultTextTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. HostCheckTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckHostCheckTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckLlvmMultTextTermProbe,
  ...
  Spec (readable):
  - Named HostModuleCheckHostCheckTerm subset:
    prior LlvmMultTextTerm closed set plus
    SystemsLean.HostModuleCheckHostCheckTerm last.
    ...
    the LlvmMultTextTerm compile order, then HostModuleCheckHostCheckTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as LlvmMultTextTerm): HostCheckTerm
    ...
    HostCheckTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckHostCheckTerm.
    HostModuleCheckHostCheckTerm probe lives here.
  - tryCompileNamedHostModuleCheckHostCheckTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckHostCheckTerm bad writes a temp
    HostModuleCheckHostCheckTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckLlvmMultTextTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior LlvmMultTextTerm walker
    tryCompileNamedHostModuleCheckLlvmMultTextTermSubset, and that this walk
    is not an alias of the LlvmMultTextTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckHostCheckTerm.lean. Do not skip to
  ...
  findLiveHostModuleCheckHostCheckTermPath,
  namedClosedHostModuleCheckHostCheckTermSubsetNames,
  namedClosedHostModuleCheckHostCheckTermCompileOrder,
  barrelListsNamedHostModuleCheckHostCheckTermSubset,
  tryCompileNamedHostModuleCheckHostCheckTermSubset,
  tryCompileAfterHostModuleCheckHostCheckTermDeps,
  elabMeetDrivesNamedHostModuleCheckHostCheckTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckHostCheckTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckHostCheckTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckHostCheckTermSubset,
  #elabMeetNamedHostModuleCheckHostCheckTermSubsetProbe,
  elabMeetNamedHostModuleCheckHostCheckTermSubsetProbe,
  SystemsLean.HostModuleCheckHostCheckTerm,
  ElabMeetNamedHostModuleCheckHostCheckTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckHostCheckTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckHostCheckTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckHostCheckTermProbe;
  just systems-host.
...
import SystemsLean.ElabMeetNamedWalkHostModuleCheckLlvmMultTextTerm
import SystemsLean.ElabMeetNamedWalkHostModuleCheckHostCheckTerm

namespace SystemsLean.ElabMeet

open Lean Elab Command

/-- Record accept / reject for the HostModuleCheckHostCheckTerm named subset.
    Good compiles live Mult through HostModuleCheckLlvmMultTextTerm plus the
    subset-surface compile closure and HostModuleCheckHostCheckTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckHostCheckTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior LlvmMultTextTerm walker
    tryCompileNamedHostModuleCheckLlvmMultTextTermSubset still works; this
    walker is not an alias of the LlvmMultTextTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckHostCheckTerm.lean. Do not skip to
    HostModuleCheckHostGraphTerm. -/
def elabMeetRunNamedHostModuleCheckHostCheckTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckHostCheckTerm? <-
    liftIO findLiveHostModuleCheckHostCheckTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckHostCheckTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckHostCheckTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckHostCheckTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckHostCheckTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckHostCheckTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckHostCheckTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckHostCheckTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckHostCheckTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckHostCheckTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckHostCheckTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckHostCheckTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckHostCheckTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckHostCheckTermSubsetProbe

#elabMeetNamedHostModuleCheckHostCheckTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckHostCheckTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckHostCheckTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckHostCheckTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckHostCheckTermSubset = true := rfl
