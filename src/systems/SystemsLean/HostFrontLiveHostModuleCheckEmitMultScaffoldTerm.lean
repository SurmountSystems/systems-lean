/-
  ...
  Short role: HostFrontLiveHostModuleCheckEmitMultScaffoldTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckEmitMultScaffoldTermSource turns live
    HostModuleCheckEmitMultScaffoldTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckEmitMultScaffoldTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckEmitMultScaffoldTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckEmitMultScaffoldTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckEmitMultScaffoldTermSource. Not split.
  ...
  - Kept: two imports, the namespace, four literal defs, and end.
    Kept count is 8.
    The four literal defs are checkDepthEmitMultScaffoldTermSurfaceBar,
    checkDepthEmitMultScaffoldTermSurfaceScope,
    hostModuleCheckEmitMultScaffoldTermSurfaceDualOk, and
    hostModuleCheckEmitMultScaffoldTermSkeletonSuffix.
    The skeleton prefix is two string literals joined by concatenation,
    so it is not kept.
  - Skipped: twenty-five defs that have binders or are not one literal,
    plus two theorems. Skipped count is 27.
  ...
  HostFrontLiveHostModuleCheckEmitMultScaffoldTerm
  PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITMULTSCAFFOLDTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckEmitMultScaffoldTermSource,
  kernelCheckLiveHostModuleCheckEmitMultScaffoldTermSource,
  hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady,
  liveHostModuleCheckEmitMultScaffoldTermSource,
  liveRel, liveHostModuleCheckEmitMultScaffoldTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckEmitMultScaffoldTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckEmitMultScaffoldTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckEmitMultScaffoldTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckEmitMultScaffoldTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckEmitMultScaffoldTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckEmitMultScaffoldTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckEmitMultScaffoldTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckEmitMultScaffoldTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: two imports, namespace, four literal defs, and end.
    Twenty-five non-literal defs and two theorems are skip-folded.
    The skeleton prefix is concatenated, so it is not a kept literal.
    Skipped count is 27. Kept plus skipped is 35.
    kernelFuel is 64. 35 does not pass 64. -/
def liveKeptCmds : Nat := 8

/-- Skipped command count. -/
def liveSkippedCmds : Nat := 27
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckEmitMultScaffoldTerm has dash-dash in the header block comment. -/
...
/-- String-safe comment strip for live HostModuleCheckEmitMultScaffoldTerm.lean
...
/-- Parse live HostModuleCheckEmitMultScaffoldTerm.lean text.
    Greppable: parseLiveHostModuleCheckEmitMultScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM. -/
def parseLiveHostModuleCheckEmitMultScaffoldTermSource (src : String) : FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckEmitMultScaffoldTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckEmitMultScaffoldTermSource,
    PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM. -/
def kernelCheckLiveHostModuleCheckEmitMultScaffoldTermSource (src : String) : Bool :=
  match parseLiveHostModuleCheckEmitMultScaffoldTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckEmitMultScaffoldTermSource
      liveHostModuleCheckEmitMultScaffoldTermSource with
...
/-- Four literal defs kept. Binders and non-literals are not in this list.
    The skeleton prefix is concatenated, so it is not in this list. -/
def liveParseDefsOk : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    cmdDefNames m.commands == [
      "checkDepthEmitMultScaffoldTermSurfaceBar",
      "checkDepthEmitMultScaffoldTermSurfaceScope",
...
def liveCmdKwCount : Nat :=
  countCmdKw liveSkipFuel
    (tokenizeLive (stripCommentsHc liveHostModuleCheckEmitMultScaffoldTermSource)) 0
...
/-- Wrap module lastSeg is HostModuleCheckEmitMultScaffoldTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckEmitMultScaffoldTerm"
      && lastSeg m.name.raw == "HostModuleCheckEmitMultScaffoldTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeLive
      (stripCommentsHc liveHostModuleCheckEmitMultScaffoldTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkDepthEmitMultScaffoldTermSurfaceBar"
      && toksHaveDefNamed liveSkipFuel toks "checkDepthEmitMultScaffoldTermSurfaceScope"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermSurfaceDualOk"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldStageIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldSelfHostIdBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldReadyBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldOkBodyOk"
      && toksHaveDefNamed liveSkipFuel toks "checkEmitMultScaffoldTermDialect"
      && toksHaveDefNamed liveSkipFuel toks "refineEmitMultScaffoldWithTermSurface"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermSkeletonPrefix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermSkeletonSuffix"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermGoodMid"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckGoodEmitMultScaffoldTermText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldStageIdText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldReadyText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldOkText"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldContractText"
      && toksHaveDefNamed liveSkipFuel toks "emitMultScaffoldTermSurfaceFrom"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldStageIdL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldReadyL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldOkL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldContractL0Accept"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermDialectOk"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldStageIdDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldReadyDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldOkDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckBadEmitMultScaffoldContractDialectReject"
      && toksHaveDefNamed liveSkipFuel toks "hostModuleCheckEmitMultScaffoldTermSurfaceOk"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheckEmitMultScaffoldTermSurfaceOk_true"
      && toksHaveTheoremNamed liveSkipFuel toks
        "hostModuleCheck_emit_mult_scaffold_term_surface_ids_eq"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckEmitMultScaffoldTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady,
    PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKEMITMULTSCAFFOLDTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM")
    && (liveRel == "HostModuleCheckEmitMultScaffoldTerm.lean")
    && (liveHostModuleCheckEmitMultScaffoldTermRel ==
        "src/systems/SystemsLean/HostModuleCheckEmitMultScaffoldTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckEmitMultScaffoldTermFullHost
    && !hostFrontLiveHostModuleCheckEmitMultScaffoldTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckEmitMultScaffoldTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckEmitMultScaffoldTermSource
        liveHostModuleCheckEmitMultScaffoldTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckEmitMultScaffoldTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckEmitMultScaffoldTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckEmitMultScaffoldTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckEmitMultScaffoldTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckEmitMultScaffoldTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckEmitMultScaffoldTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckEmitMultScaffoldTerm.lean != liveHostModuleCheckEmitMultScaffoldTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckEmitMultScaffoldTerm.lean")
  let r := parseLiveHostModuleCheckEmitMultScaffoldTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKEMITMULTSCAFFOLDTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k} real={liveCmdKwCount} kept={liveKeptCmds} skipped={liveSkippedCmds}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckEmitMultScaffoldTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckEmitMultScaffoldTerm parse false")
    unless hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckEmitMultScaffoldTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckEmitMultScaffoldTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckEmitMultScaffoldTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckEmitMultScaffoldTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
