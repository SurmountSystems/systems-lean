/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLlvmTypesTextTerm named
  closed subset probe (plus-one after the SubsetPackageJoinTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LlvmTypesTextTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLlvmTypesTextTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckLlvmTypesTextTerm subset:
    prior SubsetPackageJoinTerm closed set plus
    SystemsLean.HostModuleCheckLlvmTypesTextTerm last.
    ...
    the SubsetPackageJoinTerm compile order, then HostModuleCheckLlvmTypesTextTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as SubsetPackageJoinTerm): LlvmTypesTextTerm
    ...
    LlvmTypesTextTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLlvmTypesTextTerm.
    HostModuleCheckLlvmTypesTextTerm probe lives here.
  - tryCompileNamedHostModuleCheckLlvmTypesTextTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLlvmTypesTextTerm bad writes a temp
    HostModuleCheckLlvmTypesTextTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckSubsetPackageJoinTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior SubsetPackageJoinTerm walker
    tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset, and that this walk
    is not an alias of the SubsetPackageJoinTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLlvmTypesTextTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckLlvmTypesTextTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLlvmTypesTextTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckLlvmTypesTextTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmTypesTextTermSubset,
  #elabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetProbe,
  elabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetProbe,
  SystemsLean.HostModuleCheckLlvmTypesTextTerm,
  ElabMeetNamedHostModuleCheckLlvmTypesTextTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLlvmTypesTextTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLlvmTypesTextTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLlvmTypesTextTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLlvmTypesTextTerm named subset.
    Good compiles live Mult through HostModuleCheckSubsetPackageJoinTerm plus the
    subset-surface compile closure and HostModuleCheckLlvmTypesTextTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLlvmTypesTextTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior SubsetPackageJoinTerm walker
    tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset still works; this
    walker is not an alias of the SubsetPackageJoinTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLlvmTypesTextTerm.lean. Do not skip to
    HostModuleCheckMultFsDeepenTerm. -/
def elabMeetRunNamedHostModuleCheckLlvmTypesTextTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLlvmTypesTextTerm? <-
    liftIO findLiveHostModuleCheckLlvmTypesTextTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLlvmTypesTextTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLlvmTypesTextTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLlvmTypesTextTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLlvmTypesTextTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLlvmTypesTextTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSubsetPackageJoinTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLlvmTypesTextTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLlvmTypesTextTermSubset fakeLake fakeDir
  let notAliasOfSubsetPackageJoinTerm :=
    namedClosedHostModuleCheckLlvmTypesTextTermSubsetNames
  ...
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      ...
      && notAliasOfSubsetPackageJoinTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLlvmTypesTextTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLlvmTypesTextTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmTypesTextTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLlvmTypesTextTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckLlvmTypesTextTermSubsetProbe

#elabMeetNamedHostModuleCheckLlvmTypesTextTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckLlvmTypesTextTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLlvmTypesTextTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLlvmTypesTextTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLlvmTypesTextTermSubset = true := rfl
