/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathGraphTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathGraphTermSource turns live
    HostModuleCheckCompilePathGraphTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathGraphTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathGraphTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathGraphTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathGraphTermSource. Not split.
  ...
  - Skip-head still sees def checkCompilePathGraphTermDialect and
    def hostModuleCheckCompilePathGraphTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathGraphTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHGRAPHTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathGraphTermSource,
  kernelCheckLiveHostModuleCheckCompilePathGraphTermSource,
  hostFrontLiveHostModuleCheckCompilePathGraphTermReady,
  liveHostModuleCheckCompilePathGraphTermSource,
  liveRel, liveHostModuleCheckCompilePathGraphTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathGraphTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathGraphTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathGraphTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathGraphTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathGraphTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathGraphTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathGraphTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathGraphTermProvablyUnlocked : Bool :=
  false
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathGraphTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathGraphTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathGraphTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathGraphTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathGraphTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM. -/
def parseLiveHostModuleCheckCompilePathGraphTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathGraphTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathGraphTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathGraphTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathGraphTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathGraphTermSource
      liveHostModuleCheckCompilePathGraphTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathGraphTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathGraphTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathGraphTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathGraphTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathGraphTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathGraphTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathGraphTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathGraphTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHGRAPHTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM")
    && (liveRel == "HostModuleCheckCompilePathGraphTerm.lean")
    && (liveHostModuleCheckCompilePathGraphTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathGraphTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathGraphTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathGraphTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathGraphTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathGraphTermSource
        liveHostModuleCheckCompilePathGraphTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathGraphTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathGraphTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathGraphTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathGraphTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathGraphTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathGraphTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathGraphTerm.lean != liveHostModuleCheckCompilePathGraphTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathGraphTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathGraphTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHGRAPHTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathGraphTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathGraphTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathGraphTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathGraphTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathGraphTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathGraphTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathGraphTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathGraphTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
