...
/-- Live parse has core ParityMultTerm defs that survive skip-un-kernelable.
    Greppable: checkParityMultTermDialect, hostModuleCheckParityMultTermSurfaceOk,
    hostModuleCheckGoodParityMultTermText. -/
...
def runLiveParityMultTerm (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveParityMultTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckParityMultTerm.lean != liveParityMultTermSource"
  ...
  | FrontResult.accept m =>
    ...
    IO.println s!"GREEN {stageId}: live HostModuleCheckParityMultTerm.lean parse kernelCheck; not mill 70"
