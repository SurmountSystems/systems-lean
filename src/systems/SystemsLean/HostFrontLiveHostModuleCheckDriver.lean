/-
  ...
  Short role: HostFrontLiveHostModuleCheckDriver.
  ...
  Not HostModuleCheckDriverIO.lean.
  Not HostKernel.lean.
  Not a fifth-4 file. Not an ElabMeet file.

  Spec (readable):
  - parseLiveHostModuleCheckDriverSource turns live
    HostModuleCheckDriver.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckDriver
    even without a module line.
  - liveRel is the exact basename HostModuleCheckDriver.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckDriverSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are liveHostModuleCheckDriverSource0 ++
    liveHostModuleCheckDriverSource1. Two parts only.
  - Twenty-five dotted imports, namespace SystemsLean.HostModuleCheck,
    and end are kept. Two set_option lines are skipped. Two defs
    (runModuleCheck and main) have binders and do bodies, so they are
    skip-folded. Kept command count is 27. kernelFuel is 64.
    27 does not pass 64.
  - Skip-head still sees def runModuleCheck and def main in the live text.
  ...
  HostFrontLiveHostModuleCheckDriver
  PARSE-LIVE-HOSTMODULECHECKDRIVER
  HOST-FRONT-LIVE-HOSTMODULECHECKDRIVER
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKDRIVER_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckDriverSource,
  kernelCheckLiveHostModuleCheckDriverSource,
  hostFrontLiveHostModuleCheckDriverReady,
  liveHostModuleCheckDriverSource,
  liveRel, liveHostModuleCheckDriverRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckDriver
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckDriverSource
import SystemsLean.HostFrontLiveHostModuleCheckDriverSource01
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKDRIVER"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKDRIVER"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckDriver.lean"

/-- Full live bytes. First half then second half. -/
def liveHostModuleCheckDriverSource : String :=
  liveHostModuleCheckDriverSource0 ++ liveHostModuleCheckDriverSource1
...
def hostFrontLiveHostModuleCheckDriverFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckDriverResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckDriverProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: twenty-five imports, namespace, and end.
    Two set_option lines are skipped. Two defs have binders and do
    bodies, so they are skip-folded. kernelFuel is 64. 27 does not
    pass 64. -/
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckDriver has dash-dash in the header block comment
    and in line comments. No backslash escapes in that file. -/
...
/-- String-safe comment strip for live HostModuleCheckDriver.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckDriver`. -/
...
/-- Parse live HostModuleCheckDriver.lean text.
    Greppable: parseLiveHostModuleCheckDriverSource,
    PARSE-LIVE-HOSTMODULECHECKDRIVER. -/
def parseLiveHostModuleCheckDriverSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckDriver"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckDriverSource,
    PARSE-LIVE-HOSTMODULECHECKDRIVER. -/
def kernelCheckLiveHostModuleCheckDriverSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckDriverSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckDriverSource
      liveHostModuleCheckDriverSource with
...
/-- Wrap module lastSeg is HostModuleCheckDriver
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckDriver"
      && lastSeg m.name.raw == "HostModuleCheckDriver"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckDriverSource)
    toksHaveDefNamed liveSkipFuel toks "runModuleCheck"
      && toksHaveDefNamed liveSkipFuel toks "main"
...
def needleImportParityEmit : String :=
  "import SystemsLean.HostModuleCheckParityEmitTerm\n"

/-- Namespace needle with a trailing newline. -/
def needleNamespace : String := "namespace SystemsLean.HostModuleCheck\n"

/-- Driver entry. Body is not a kernel literal. -/
def needleRunModuleCheck : String :=
  "def runModuleCheck (root : System.FilePath) : IO Unit := do\n"

/-- CLI entry. Body is not a kernel literal. -/
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckDriverSource
  (src.splitOn needleImportSeeds).length == 2
    && (src.splitOn needleImportParityEmit).length == 2
    && (src.splitOn needleNamespace).length == 2
    && (src.splitOn needleRunModuleCheck).length == 2
    && (src.splitOn needleMain).length == 2
    && (src.splitOn needleEnd).length == 2

/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckDriverReady,
    PARSE-LIVE-HOSTMODULECHECKDRIVER,
    HOST-FRONT-LIVE-HOSTMODULECHECKDRIVER.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckDriverReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKDRIVER_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKDRIVER")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKDRIVER")
    && (liveRel == "HostModuleCheckDriver.lean")
    && (liveHostModuleCheckDriverRel ==
        "src/systems/SystemsLean/HostModuleCheckDriver.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckDriverFullHost
    && !hostFrontLiveHostModuleCheckDriverResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckDriverProvablyUnlocked
    && kernelCheckLiveHostModuleCheckDriverSource
        liveHostModuleCheckDriverSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckDriverSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKDRIVER =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckDriverRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckDriverRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckDriverRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckDriverRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckDriverSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckDriver.lean != liveHostModuleCheckDriverSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckDriver.lean")
  let r := parseLiveHostModuleCheckDriverSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKDRIVER reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKDRIVER reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKDRIVER ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckDriver parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckDriver parse false")
    unless hostFrontLiveHostModuleCheckDriverReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckDriverReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckDriverReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckDriver source was accepted"
      throw (IO.userError
        "empty HostModuleCheckDriver source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckDriver.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
