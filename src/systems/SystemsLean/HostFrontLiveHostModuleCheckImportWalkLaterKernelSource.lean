/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-WALK-LATER-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_WALK_LATER_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-WALK-LATER-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckImportWalkLaterKernelSource,
  hostFrontLiveHostModuleCheckImportWalkLaterKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckImportWalkLaterKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckImportWalkLaterKernelSource
...
def liveRel : String := "HostModuleCheckImportWalkLaterKernel.lean"
...
def hostFrontLiveHostModuleCheckImportWalkLaterKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckImportWalkLaterKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckImportWalkLaterKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckImportWalkLaterKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckImportWalkLaterKernel.lean bytes.
    Greppable: liveHostModuleCheckImportWalkLaterKernelSource. -/


def liveHostModuleCheckImportWalkLaterKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckImportWalkLaterKernel.lean text.
    Greppable: parseLiveHostModuleCheckImportWalkLaterKernelSource. -/
def parseLiveHostModuleCheckImportWalkLaterKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckImportWalkLaterKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckImportWalkLaterKernelSource. -/
def kernelCheckLiveHostModuleCheckImportWalkLaterKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckImportWalkLaterKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckImportWalkLaterKernelSource
      liveHostModuleCheckImportWalkLaterKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthImportWalkLaterKernelBar"
      && has "checkDepthImportWalkLaterKernelScope"
      && has "hostModuleCheckImportWalkLaterKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckImportWalkLaterKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckImportWalkLaterKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckImportWalkLaterKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckImportWalkLaterKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckImportWalkLaterKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_IMPORT_WALK_LATER_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-IMPORT-WALK-LATER-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-WALK-LATER-KERNEL")
    && (liveRel == "HostModuleCheckImportWalkLaterKernel.lean")
    && (liveHostModuleCheckImportWalkLaterKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportWalkLaterKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckImportWalkLaterKernelFullHost
    && !hostFrontLiveHostModuleCheckImportWalkLaterKernelFullBackend
    && !hostFrontLiveHostModuleCheckImportWalkLaterKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckImportWalkLaterKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckImportWalkLaterKernelSource
      liveHostModuleCheckImportWalkLaterKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckImportWalkLaterKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-IMPORT-WALK-LATER-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckImportWalkLaterKernel.lean")
    ++ bad "rel" (liveHostModuleCheckImportWalkLaterKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckImportWalkLaterKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckImportWalkLaterKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckImportWalkLaterKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckImportWalkLaterKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckImportWalkLaterKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckImportWalkLaterKernelSource
      liveHostModuleCheckImportWalkLaterKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckImportWalkLaterKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckImportWalkLaterKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckImportWalkLaterKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckImportWalkLaterKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckImportWalkLaterKernel.lean != liveHostModuleCheckImportWalkLaterKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckImportWalkLaterKernel.lean")
  let r := parseLiveHostModuleCheckImportWalkLaterKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckImportWalkLaterKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckImportWalkLaterKernel parse false")
    unless hostFrontLiveHostModuleCheckImportWalkLaterKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckImportWalkLaterKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckImportWalkLaterKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckImportWalkLaterKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckImportWalkLaterKernelSource
