/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckHostGraphTerm named
  closed subset probe (plus-one after the HostCheckTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. HostGraphTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckHostGraphTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckHostGraphTerm subset:
    prior HostCheckTerm closed set plus
    SystemsLean.HostModuleCheckHostGraphTerm last.
    ...
    the HostCheckTerm compile order, then HostModuleCheckHostGraphTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as HostCheckTerm): HostGraphTerm
    ...
    HostGraphTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckHostGraphTerm.
    HostModuleCheckHostGraphTerm probe lives here.
  - tryCompileNamedHostModuleCheckHostGraphTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckHostGraphTerm bad writes a temp
    HostModuleCheckHostGraphTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckHostCheckTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior HostCheckTerm walker
    tryCompileNamedHostModuleCheckHostCheckTermSubset, and that this walk
    is not an alias of the HostCheckTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckHostGraphTerm.lean. Do not skip to
  ...
  findLiveHostModuleCheckHostGraphTermPath,
  namedClosedHostModuleCheckHostGraphTermSubsetNames,
  namedClosedHostModuleCheckHostGraphTermCompileOrder,
  barrelListsNamedHostModuleCheckHostGraphTermSubset,
  tryCompileNamedHostModuleCheckHostGraphTermSubset,
  tryCompileAfterHostModuleCheckHostGraphTermDeps,
  elabMeetDrivesNamedHostModuleCheckHostGraphTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckHostGraphTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckHostGraphTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckHostGraphTermSubset,
  #elabMeetNamedHostModuleCheckHostGraphTermSubsetProbe,
  elabMeetNamedHostModuleCheckHostGraphTermSubsetProbe,
  SystemsLean.HostModuleCheckHostGraphTerm,
  ElabMeetNamedHostModuleCheckHostGraphTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckHostGraphTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckHostGraphTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckHostGraphTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckHostGraphTerm named subset.
    Good compiles live Mult through HostModuleCheckHostCheckTerm plus the
    subset-surface compile closure and HostModuleCheckHostGraphTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckHostGraphTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior HostCheckTerm walker
    tryCompileNamedHostModuleCheckHostCheckTermSubset still works; this
    walker is not an alias of the HostCheckTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckHostGraphTerm.lean. Do not skip to
    HostModuleCheckLlvmLinearTextTerm. -/
def elabMeetRunNamedHostModuleCheckHostGraphTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckHostGraphTerm? <-
    liftIO findLiveHostModuleCheckHostGraphTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckHostGraphTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckHostGraphTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckHostGraphTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckHostGraphTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckHostGraphTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckHostGraphTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckHostGraphTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckHostGraphTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckHostGraphTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckHostGraphTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckHostGraphTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckHostGraphTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckHostGraphTermSubsetProbe

#elabMeetNamedHostModuleCheckHostGraphTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckHostGraphTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckHostGraphTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckHostGraphTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckHostGraphTermSubset = true := rfl
