/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckSelfApplyFsTerm named-walk
  wrappers (CheckersLater-role walk; LlvmComposeTextTerm walk is the prior
  ...
  GraphSubsetRebuildTerm walk, and LlvmComposeTextTerm walk
  ...
  ElabMeetNamedWalk. Prior LlvmComposeTextTerm wrappers stay in
  ElabMeetNamedWalkHostModuleCheckLlvmComposeTextTerm. Do not grow that walk.
  ...
  LlvmComposeTextTermProbe.
  Do not grow ElabMeetCompile.
  Do not plant live HostModuleCheckSelfApplyFsTerm.lean.
  Do not skip to HostModuleCheckInventoryCloseTerm.
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - HostModuleCheckSelfApplyFsTerm wrappers live here:
    ...
    Named membership is the prior LlvmComposeTextTerm closed set plus
    HostModuleCheckSelfApplyFsTerm last. Barrel order among that
    closed set; skip Linear, skip IrGraph, skip HostGraphMain,
    skip HostCompose. Compile order is the LlvmComposeTextTerm compile
    order, then HostModuleCheckSelfApplyFsTerm last. EmitPlanTerm
    ...
    GraphSubsetRebuildTerm / LlvmComposeTextTerm order), not a
    named plus-one.
    IrGraph and HostCompose are compile dependencies only. After-deps
    are the LlvmComposeTextTerm compile order.
    HostModuleCheckSelfApplyFsTerm imports HostModuleCheckSurface and
    HostModuleCheckEmitPlanTerm (same dispatcher-split honesty as
    HostModuleCheckLlvmComposeTextTerm; EmitPlanTerm is compile-dep only).
  ...
  Module: SystemsLean.ElabMeetNamedWalkHostModuleCheckSelfApplyFsTerm
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedWalkHostModuleCheckSelfApplyFsTerm;
  just systems-host.
...
/-- Resolve live SystemsLean/HostModuleCheckSelfApplyFsTerm.lean.
    Wrapper over findLiveModulePath.
    Greppable: findLiveHostModuleCheckSelfApplyFsTermPath. -/
def findLiveHostModuleCheckSelfApplyFsTermPath : IO (Option System.FilePath) :=
  findLiveModulePath "SystemsLean.HostModuleCheckSelfApplyFsTerm"

/-- Named HostModuleCheckSelfApplyFsTerm subset of the live SystemsLean library.
    Membership is the prior LlvmComposeTextTerm closed set plus
    HostModuleCheckSelfApplyFsTerm last.
    ...
    Greppable: namedClosedHostModuleCheckSelfApplyFsTermSubsetNames.
    SystemsLean.HostModuleCheckSelfApplyFsTerm. -/
def namedClosedHostModuleCheckSelfApplyFsTermSubsetNames : List String :=
  namedClosedHostModuleCheckLlvmComposeTextTermSubsetNames ++
    ["SystemsLean.HostModuleCheckSelfApplyFsTerm"]

/-- Compile order honors imports: LlvmComposeTextTerm compile order, then
    HostModuleCheckSelfApplyFsTerm last. IrGraph and HostCompose are
    ...
    Greppable: namedClosedHostModuleCheckSelfApplyFsTermCompileOrder. -/
def namedClosedHostModuleCheckSelfApplyFsTermCompileOrder : List String :=
  namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder ++
    ["SystemsLean.HostModuleCheckSelfApplyFsTerm"]

/-- True when the barrel lists each HostModuleCheckSelfApplyFsTerm subset member.
    Wrapper over barrelListsNamedMembers.
    Greppable: barrelListsNamedHostModuleCheckSelfApplyFsTermSubset. -/
def barrelListsNamedHostModuleCheckSelfApplyFsTermSubset (text : String) : Bool :=
  barrelListsNamedMembers text namedClosedHostModuleCheckSelfApplyFsTermSubsetNames

/-- Compile the named HostModuleCheckSelfApplyFsTerm subset after reading
    the live lakefile. Wrapper over tryCompileNamedMemberListIO.
    Compile order honors imports: Mult first, then the LlvmComposeTextTerm
    compile order, then HostModuleCheckSelfApplyFsTerm last.
    Greppable: tryCompileNamedHostModuleCheckSelfApplyFsTermSubset.
    tryCompileOnDiskModule. SystemsLean.HostModuleCheckSelfApplyFsTerm. -/
unsafe def tryCompileNamedHostModuleCheckSelfApplyFsTermSubsetIO
    (srcPath oleanDir : System.FilePath) : IO Bool := do
  tryCompileNamedMemberListIO srcPath oleanDir
    namedClosedHostModuleCheckSelfApplyFsTermSubsetNames
    namedClosedHostModuleCheckSelfApplyFsTermCompileOrder

@[implemented_by tryCompileNamedHostModuleCheckSelfApplyFsTermSubsetIO]
opaque tryCompileNamedHostModuleCheckSelfApplyFsTermSubsetIOSafe
    (srcPath oleanDir : System.FilePath) : IO Bool

/-- CommandElab wrapper: compile the HostModuleCheckSelfApplyFsTerm subset.
    Greppable: tryCompileNamedHostModuleCheckSelfApplyFsTermSubset. -/
def tryCompileNamedHostModuleCheckSelfApplyFsTermSubset
    (srcPath oleanDir : System.FilePath) : CommandElabM Bool := do
  try
    liftIO
      (tryCompileNamedHostModuleCheckSelfApplyFsTermSubsetIOSafe srcPath oleanDir)
  catch _ =>
    pure false

/-- Compile live Mult first, then the LlvmComposeTextTerm compile order
    ...
    Greppable: tryCompileAfterHostModuleCheckSelfApplyFsTermDeps.
    tryCompileOnDiskModule. -/
unsafe def tryCompileAfterHostModuleCheckSelfApplyFsTermDepsIO
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool :=
  tryCompileAfterNamedDepsIO srcPath oleanDir thmMod
    namedClosedHostModuleCheckLlvmComposeTextTermCompileOrder

@[implemented_by tryCompileAfterHostModuleCheckSelfApplyFsTermDepsIO]
opaque tryCompileAfterHostModuleCheckSelfApplyFsTermDepsIOSafe
    (srcPath oleanDir : System.FilePath) (thmMod : Name) : IO Bool

/-- CommandElab wrapper: compile after HostModuleCheckSelfApplyFsTerm deps.
    Greppable: tryCompileAfterHostModuleCheckSelfApplyFsTermDeps. -/
def tryCompileAfterHostModuleCheckSelfApplyFsTermDeps
    (srcPath oleanDir : System.FilePath) (thmMod : Name) :
    CommandElabM Bool := do
  try
    liftIO
      (tryCompileAfterHostModuleCheckSelfApplyFsTermDepsIOSafe srcPath oleanDir thmMod)
  catch _ =>
    pure false
