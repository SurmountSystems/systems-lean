/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-GRAPH-THEOREMS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_GRAPH_THEOREMS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-GRAPH-THEOREMS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckGraphTheoremsKernelSource,
  hostFrontLiveHostModuleCheckGraphTheoremsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckGraphTheoremsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckGraphTheoremsKernelSource
...
def liveRel : String := "HostModuleCheckGraphTheoremsKernel.lean"
...
def hostFrontLiveHostModuleCheckGraphTheoremsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckGraphTheoremsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckGraphTheoremsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckGraphTheoremsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckGraphTheoremsKernel.lean bytes.
    Greppable: liveHostModuleCheckGraphTheoremsKernelSource. -/

def liveHostModuleCheckGraphTheoremsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckGraphTheoremsKernel.lean text.
    Greppable: parseLiveHostModuleCheckGraphTheoremsKernelSource. -/
def parseLiveHostModuleCheckGraphTheoremsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckGraphTheoremsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckGraphTheoremsKernelSource. -/
def kernelCheckLiveHostModuleCheckGraphTheoremsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckGraphTheoremsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckGraphTheoremsKernelSource
      liveHostModuleCheckGraphTheoremsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthGraphTheoremsKernelBar"
      && has "checkDepthGraphTheoremsKernelScope"
      && has "hostModuleCheckGraphTheoremsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckGraphTheoremsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckGraphTheoremsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckGraphTheoremsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckGraphTheoremsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckGraphTheoremsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_GRAPH_THEOREMS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-GRAPH-THEOREMS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-GRAPH-THEOREMS-KERNEL")
    && (liveRel == "HostModuleCheckGraphTheoremsKernel.lean")
    && (liveHostModuleCheckGraphTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckGraphTheoremsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckGraphTheoremsKernelFullHost
    && !hostFrontLiveHostModuleCheckGraphTheoremsKernelFullBackend
    && !hostFrontLiveHostModuleCheckGraphTheoremsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckGraphTheoremsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckGraphTheoremsKernelSource
      liveHostModuleCheckGraphTheoremsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckGraphTheoremsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-GRAPH-THEOREMS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckGraphTheoremsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckGraphTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckGraphTheoremsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckGraphTheoremsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckGraphTheoremsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckGraphTheoremsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckGraphTheoremsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckGraphTheoremsKernelSource
      liveHostModuleCheckGraphTheoremsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckGraphTheoremsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckGraphTheoremsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckGraphTheoremsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckGraphTheoremsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckGraphTheoremsKernel.lean != liveHostModuleCheckGraphTheoremsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckGraphTheoremsKernel.lean")
  let r := parseLiveHostModuleCheckGraphTheoremsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckGraphTheoremsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckGraphTheoremsKernel parse false")
    unless hostFrontLiveHostModuleCheckGraphTheoremsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckGraphTheoremsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckGraphTheoremsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckGraphTheoremsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckGraphTheoremsKernelSource
