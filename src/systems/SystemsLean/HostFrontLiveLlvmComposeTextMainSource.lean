/-
  SYSTEMS_LEAN_HOST partial -- parse live LlvmComposeTextMain.lean.
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Short role: HostFrontLiveLlvmComposeTextMainSource. Reuses HostFrontLiveHostTerm
  skip-un-kernelable fold (Types-style atom bodies). Not HostFront G1.
  Not HostTerm.multFixtureModule. Not KernelMult-only.
  This wrap parses LlvmComposeTextMain.lean only.
  It is not LlvmComposeText.lean. Do not retarget the neighbor.
  liveRel is defined once in this file. The bare name is LlvmComposeTextMain.lean.
  ...
  liveRel, liveLlvmComposeTextMainRel, UNIT_SURFACE host surface,
...
def liveRel : String := "LlvmComposeTextMain.lean"

/-- Dual-pinned live LlvmComposeTextMain.lean bytes.
    The checker accepts this Main source. The forwarder only calls
    SystemsLean.LlvmComposeText.main. Do not reimplement the neighbor.
    Greppable: liveLlvmComposeTextMainSource. -/
...
def liveLlvmComposeTextMainRel : String := liveRel
...
/-- Kernel-check live LlvmComposeTextMain parse. Not a fixture.
    ...
    Greppable: kernelCheckLiveLlvmComposeTextMainSource,
    PARSE-LIVE-LLVM-COMPOSE-TEXT-MAIN. -/
...
/-- Live parse command count (dotted import kept; IO main skip-folded).
    The one dotted import is SystemsLean.LlvmComposeText.
    Not a LlvmComposeText.lean library wrap. -/
...
def needleImport : String := "import SystemsLean.LlvmComposeText\n"

/-- Forward line needle. Trailing newline. -/
def needleForward : String := "  SystemsLean.LlvmComposeText.main args\n"
...
def needleModuleCite : String := "  Module: SystemsLean.LlvmComposeTextMain\n"

/-- Each needle occurs in the pinned live source. splitOn, not a prefix check. -/
def liveNeedlesOk : Bool :=
  let src := liveLlvmComposeTextMainSource
...
def hostFrontLiveLlvmComposeTextMainReady : Bool :=
  (stageId == "SLAKE_HOST_FRONT_LIVE_LLVM_COMPOSE_TEXT_MAIN_V0")
    ...
    && (liveRel == "LlvmComposeTextMain.lean")
    && (liveLlvmComposeTextMainRel == "LlvmComposeTextMain.lean")
    && (liveLlvmComposeTextMainRel.splitOn "/").length == 1
...
def runLiveLlvmComposeTextMain (root : System.FilePath) : IO Unit := do
  ...
  | FrontResult.accept m =>
    ...
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty LlvmComposeTextMain source was accepted"
      throw (IO.userError "empty LlvmComposeTextMain source was accepted")
