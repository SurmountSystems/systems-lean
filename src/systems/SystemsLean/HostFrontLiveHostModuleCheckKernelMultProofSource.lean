/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckKernelMultProof.lean.
  ...
  liveRel is HostModuleCheckKernelMultProof.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckKernelMultProofSource,
  PARSE-LIVE-HOST-MODULE-CHECK-KERNEL-MULT-PROOF.
  Module: SystemsLean.HostFrontLiveHostModuleCheckKernelMultProofSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckKernelMultProofSource
...
def liveRel : String := "HostModuleCheckKernelMultProof.lean"

/-- Pinned live HostModuleCheckKernelMultProof.lean bytes. -/
def liveHostModuleCheckKernelMultProofSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckKernelMultProofSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckKernelMultProofSourceReady : Bool :=
  liveRel == "HostModuleCheckKernelMultProof.lean"
    && kernelCheckLiveHostModuleCheckKernelMultProofSource liveHostModuleCheckKernelMultProofSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckKernelMultProofSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckKernelMultProof.lean")
  let k := kernelCheckLiveHostModuleCheckKernelMultProofSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckKernelMultProof parse false")
  unless hostFrontLiveHostModuleCheckKernelMultProofSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckKernelMultProofSource
