/-
  ...
  src/systems/SystemsLean/HostModuleCheckMultProof.lean with the
  ...
  HOST-FRONT-LIVE-HOSTMODULECHECKMULTPROOF,
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKMULTPROOF_V0,
  PARSE-LIVE-HOSTMODULECHECKMULTPROOF, liveRel,
  kernelCheckLiveHostModuleCheckMultProofSource,
  hostFrontLiveHostModuleCheckMultProofReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckMultProof
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckMultProofSource
import SystemsLean.HostKernel

set_option maxRecDepth 32768
set_option maxHeartbeats 100000000

namespace SystemsLean.HostFrontLiveHostModuleCheckMultProof
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKMULTPROOF"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKMULTPROOF"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckMultProof.lean"
...
def hostFrontLiveHostModuleCheckMultProofFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckMultProofFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckMultProofResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckMultProofProvablyUnlocked : Bool :=
  false
...
/-- Copied front-term kept-command count (cmds=7).
    Not a hand count of HostModuleCheckMultProof.lean.
...
/-- Parse live HostModuleCheckMultProof.lean text.
    Greppable: parseLiveHostModuleCheckMultProofSource,
    PARSE-LIVE-HOSTMODULECHECKMULTPROOF. -/
def parseLiveHostModuleCheckMultProofSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckMultProof"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckMultProofSource,
    PARSE-LIVE-HOSTMODULECHECKMULTPROOF. -/
def kernelCheckLiveHostModuleCheckMultProofSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckMultProofSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckMultProofSource
      liveHostModuleCheckMultProofSource with
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthProofSurfaceBar"
          && s == "PROOF-SURFACE"
      | _ => false

/-- Scope literal is MultTheorems-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthProofSurfaceScope"
          && s == "MultTheorems-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckProofSurfaceDualOk" && b == true
      | _ => false

/-- No skeleton-suffix def in this product. The kept String literals are
    the ill-typed reason, the proof bar, and the scope. Three of them,
    and the scope text matches. -/
def liveParseHasSuffix : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    countStringLits m.commands == 3
      && m.commands.any fun c =>
        match c with
        | Cmd.def_ x (some HostType.string) (Term.litString s) =>
          x.raw == "checkDepthProofSurfaceScope"
...
/-- Wrap module lastSeg is HostModuleCheckMultProof
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckMultProof"
      && lastSeg m.name.raw == "HostModuleCheckMultProof"
...
/-- ofNat?_zero statement target. Present in the live source, not a kernel atom. -/
def needleEmptyStruct : String :=
  "ofNat? 0 = some Mult.mult0"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckMultProofSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckMultProofReady,
    PARSE-LIVE-HOSTMODULECHECKMULTPROOF,
    HOST-FRONT-LIVE-HOSTMODULECHECKMULTPROOF.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckMultProofReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKMULTPROOF_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKMULTPROOF")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKMULTPROOF")
    && (liveRel == "HostModuleCheckMultProof.lean")
    && (liveHostModuleCheckMultProofRel ==
        "src/systems/SystemsLean/HostModuleCheckMultProof.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckMultProofFullHost
    && !hostFrontLiveHostModuleCheckMultProofFullBackend
    && !hostFrontLiveHostModuleCheckMultProofResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckMultProofProvablyUnlocked
    && kernelCheckLiveHostModuleCheckMultProofSource
        liveHostModuleCheckMultProofSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckMultProofSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKMULTPROOF =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckMultProofRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckMultProofRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckMultProofRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckMultProofRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckMultProofSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckMultProof.lean != liveHostModuleCheckMultProofSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckMultProof.lean")
  let r := parseLiveHostModuleCheckMultProofSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKMULTPROOF reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKMULTPROOF reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKMULTPROOF ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckMultProof parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckMultProof parse false")
    unless hostFrontLiveHostModuleCheckMultProofReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckMultProofReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckMultProofReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckMultProof source was accepted"
      throw (IO.userError "empty HostModuleCheckMultProof source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckMultProof
