...
def graphSubsetEmitWrite (root : System.FilePath) : IO Unit := do
  ...
  IO.println s!"== {stageId}: Graph subset freestanding emit (ideal ladder M1) =="
  IO.println s!"  host: {hostId} / surface: {surfaceId}"
  IO.println s!"  Graph unit input id: {graphSubsetEmitInputId} (COMPILE-PATH-GRAPH reuse)"
  IO.println s!"  graphSubsetEmitInputReady: {graphSubsetEmitInputReady} (graphFixtureCompilePathReady)"
  IO.println s!"  graphSubsetEmitGraphDialectOk: {graphSubsetEmitGraphDialectOk} (HOST-EMIT-GRAPH)"
  IO.println s!"  graphSubsetEmitPackageOk: {graphSubsetEmitPackageOk}"
  IO.println s!"  graphSubsetEmitReady: {graphSubsetEmitReady}"
  IO.println s!"  graphSubsetEmitWroteExpected: {graphSubsetEmitWroteExpected} (package identity pin)"
  ...
  IO.println "  honest: M1 = Graph subset unit package write; not full product-wire regenerate"
  IO.println "  honest: Lake host remains; free/complete living tip unchanged; not PROVABLY; not llvm"
  IO.println "  honest: not Graph subset rebuild; not S4 Lake retire; not full CFG/SSA"
  IO.println "  short module name GraphSubsetEmit (not ProductPathFreestandingGraphSubset)"
  ...
  IO.println s!"GREEN {stageId}: Graph subset freestanding C wrote under {emitDir}/"
