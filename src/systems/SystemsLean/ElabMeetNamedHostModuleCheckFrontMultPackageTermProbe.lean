/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckFrontMultPackageTerm named
  closed subset probe (plus-one after the MultFsWriteToolTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. FrontMultPackageTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckFrontMultPackageTerm. Shared helpers stay in
  ...
  ElabMeetNamedHostModuleCheckMultFsWriteToolTermProbe,
  ...
  ElabMeetNamedWalkHostModuleCheckMultFsWriteToolTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckFrontMultPackageTerm subset:
    prior MultFsWriteToolTerm closed set plus
    SystemsLean.HostModuleCheckFrontMultPackageTerm last.
    ...
    the MultFsWriteToolTerm compile order, then HostModuleCheckFrontMultPackageTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as MultFsWriteToolTerm): FrontMultPackageTerm
    ...
    FrontMultPackageTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckFrontMultPackageTerm.
    HostModuleCheckFrontMultPackageTerm probe lives here.
  - tryCompileNamedHostModuleCheckFrontMultPackageTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckFrontMultPackageTerm bad writes a temp
    HostModuleCheckFrontMultPackageTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckMultFsWriteToolTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior MultFsWriteToolTerm walker
    tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset, and that this walk
    is not an alias of the MultFsWriteToolTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckFrontMultPackageTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckFrontMultPackageTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckFrontMultPackageTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckFrontMultPackageTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckFrontMultPackageTermSubset,
  #elabMeetNamedHostModuleCheckFrontMultPackageTermSubsetProbe,
  elabMeetNamedHostModuleCheckFrontMultPackageTermSubsetProbe,
  SystemsLean.HostModuleCheckFrontMultPackageTerm,
  ElabMeetNamedHostModuleCheckFrontMultPackageTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckFrontMultPackageTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckFrontMultPackageTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckFrontMultPackageTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckFrontMultPackageTerm named subset.
    Good compiles live Mult through HostModuleCheckMultFsWriteToolTerm plus the
    subset-surface compile closure and HostModuleCheckFrontMultPackageTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckFrontMultPackageTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior MultFsWriteToolTerm walker
    tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset still works; this
    walker is not an alias of the MultFsWriteToolTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckFrontMultPackageTerm.lean. Do not skip to
    HostModuleCheckSubsetPackageJoinTerm. -/
elab "#elabMeetNamedHostModuleCheckFrontMultPackageTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckFrontMultPackageTerm? <-
    liftIO findLiveHostModuleCheckFrontMultPackageTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckFrontMultPackageTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckFrontMultPackageTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckFrontMultPackageTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckFrontMultPackageTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckFrontMultPackageTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckFrontMultPackageTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckFrontMultPackageTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckMultFsWriteToolTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckFrontMultPackageTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckFrontMultPackageTermSubset fakeLake fakeDir
  let notAliasOfMultFsWriteToolTerm :=
    namedClosedHostModuleCheckFrontMultPackageTermSubsetNames
      ...
      && namedClosedHostModuleCheckFrontMultPackageTermSubsetNames.contains
           "SystemsLean.HostModuleCheckFrontMultPackageTerm"
      && !namedClosedHostModuleCheckMultFsWriteToolTermSubsetNames.contains
           "SystemsLean.HostModuleCheckFrontMultPackageTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfMultFsWriteToolTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckFrontMultPackageTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckFrontMultPackageTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckFrontMultPackageTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckFrontMultPackageTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckFrontMultPackageTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckFrontMultPackageTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckFrontMultPackageTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckFrontMultPackageTermSubset = true := rfl
