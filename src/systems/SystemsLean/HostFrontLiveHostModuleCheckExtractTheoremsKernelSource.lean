/-
  ...
  This wrap is HostModuleCheckExtractTheoremsKernel.lean.
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_EXTRACT_THEOREMS_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckExtractTheoremsKernelSource,
  hostFrontLiveHostModuleCheckExtractTheoremsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckExtractTheoremsKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckExtractTheoremsKernelSource
...
/-- Greppable parse id. -/
def parseId : String :=
  "PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL"

/-- Live file basename. Greppable: liveRel. Must be the bare product name. -/
def liveRel : String := "HostModuleCheckExtractTheoremsKernel.lean"

/-- Live file relative to repo root. -/
def liveHostModuleCheckExtractTheoremsKernelRel : String :=
...
def hostFrontLiveHostModuleCheckExtractTheoremsKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckExtractTheoremsKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckExtractTheoremsKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckExtractTheoremsKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckExtractTheoremsKernel.lean bytes.
    Greppable: liveHostModuleCheckExtractTheoremsKernelSource. -/

def liveHostModuleCheckExtractTheoremsKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckExtractTheoremsKernel.lean text.
    Greppable: parseLiveHostModuleCheckExtractTheoremsKernelSource. -/
def parseLiveHostModuleCheckExtractTheoremsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckExtractTheoremsKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckExtractTheoremsKernelSource. -/
def kernelCheckLiveHostModuleCheckExtractTheoremsKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckExtractTheoremsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckExtractTheoremsKernelSource
      liveHostModuleCheckExtractTheoremsKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthExtractTheoremsKernelBar"
      && has "checkDepthExtractTheoremsKernelScope"
      && has "hostModuleCheckExtractTheoremsKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckExtractTheoremsKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckExtractTheoremsKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckExtractTheoremsKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckExtractTheoremsKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckExtractTheoremsKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_EXTRACT_THEOREMS_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL")
    && (liveRel == "HostModuleCheckExtractTheoremsKernel.lean")
    && (liveHostModuleCheckExtractTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckExtractTheoremsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckExtractTheoremsKernelFullHost
    && !hostFrontLiveHostModuleCheckExtractTheoremsKernelFullBackend
    && !hostFrontLiveHostModuleCheckExtractTheoremsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckExtractTheoremsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckExtractTheoremsKernelSource
      liveHostModuleCheckExtractTheoremsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckExtractTheoremsKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-EXTRACT-THEOREMS-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckExtractTheoremsKernel.lean")
    ++ bad "rel" (liveHostModuleCheckExtractTheoremsKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckExtractTheoremsKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckExtractTheoremsKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckExtractTheoremsKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckExtractTheoremsKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckExtractTheoremsKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckExtractTheoremsKernelSource
      liveHostModuleCheckExtractTheoremsKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  IO.println s!"  host={hostId} file={liveHostModuleCheckExtractTheoremsKernelRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckExtractTheoremsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckExtractTheoremsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckExtractTheoremsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckExtractTheoremsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckExtractTheoremsKernel.lean != liveHostModuleCheckExtractTheoremsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckExtractTheoremsKernel.lean")
  let r := parseLiveHostModuleCheckExtractTheoremsKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckExtractTheoremsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckExtractTheoremsKernel parse false")
    unless hostFrontLiveHostModuleCheckExtractTheoremsKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckExtractTheoremsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckExtractTheoremsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckExtractTheoremsKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckExtractTheoremsKernelSource
