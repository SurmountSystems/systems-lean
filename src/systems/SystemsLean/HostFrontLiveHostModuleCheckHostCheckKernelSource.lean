/-
  ...
  src/systems/SystemsLean/HostModuleCheckHostCheckKernel.lean
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-CHECK-KERNEL,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_CHECK_KERNEL_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-HOST-CHECK-KERNEL, liveRel,
  kernelCheckLiveHostModuleCheckHostCheckKernelSource,
  hostFrontLiveHostModuleCheckHostCheckKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckHostCheckKernelSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckHostCheckKernelSource
...
def liveRel : String := "HostModuleCheckHostCheckKernel.lean"
...
def hostFrontLiveHostModuleCheckHostCheckKernelFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckHostCheckKernelFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckHostCheckKernelResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckHostCheckKernelProvablyUnlocked : Bool :=
  false
...
/-- On-disk HostModuleCheckHostCheckKernel.lean bytes.
    Greppable: liveHostModuleCheckHostCheckKernelSource. -/


def liveHostModuleCheckHostCheckKernelSource : String := r#"/-
...
/-- Parse live HostModuleCheckHostCheckKernel.lean text.
    Greppable: parseLiveHostModuleCheckHostCheckKernelSource. -/
def parseLiveHostModuleCheckHostCheckKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckHostCheckKernel"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckHostCheckKernelSource. -/
def kernelCheckLiveHostModuleCheckHostCheckKernelSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckHostCheckKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckHostCheckKernelSource
      liveHostModuleCheckHostCheckKernelSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthHostCheckKernelBar"
      && has "checkDepthHostCheckKernelScope"
      && has "hostModuleCheckHostCheckKernelDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckHostCheckKernel"
...
def liveParseKwCountsOk : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckHostCheckKernelSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckHostCheckKernelSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckHostCheckKernelReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckHostCheckKernelReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_HOST_CHECK_KERNEL_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-HOST-CHECK-KERNEL")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-CHECK-KERNEL")
    && (liveRel == "HostModuleCheckHostCheckKernel.lean")
    && (liveHostModuleCheckHostCheckKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostCheckKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckHostCheckKernelFullHost
    && !hostFrontLiveHostModuleCheckHostCheckKernelFullBackend
    && !hostFrontLiveHostModuleCheckHostCheckKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckHostCheckKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckHostCheckKernelSource
      liveHostModuleCheckHostCheckKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckHostCheckKernelSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-HOST-CHECK-KERNEL")
    ++ bad "liveRel" (liveRel == "HostModuleCheckHostCheckKernel.lean")
    ++ bad "rel" (liveHostModuleCheckHostCheckKernelRel ==
      "src/systems/SystemsLean/HostModuleCheckHostCheckKernel.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckHostCheckKernelFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckHostCheckKernelFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckHostCheckKernelResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckHostCheckKernelProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckHostCheckKernelSource
      liveHostModuleCheckHostCheckKernelSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckHostCheckKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckHostCheckKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckHostCheckKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckHostCheckKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckHostCheckKernel.lean != liveHostModuleCheckHostCheckKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckHostCheckKernel.lean")
  let r := parseLiveHostModuleCheckHostCheckKernelSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckHostCheckKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckHostCheckKernel parse false")
    unless hostFrontLiveHostModuleCheckHostCheckKernelReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckHostCheckKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckHostCheckKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckHostCheckKernel source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckHostCheckKernelSource
