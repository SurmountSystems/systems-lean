/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckSelfHostBodyTerm named
  closed subset probe (plus-one after the ProbeWireTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. SelfHostBodyTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckSelfHostBodyTerm. Shared helpers stay in
  ...
  Spec (readable):
  - Named HostModuleCheckSelfHostBodyTerm subset:
    prior ProbeWireTerm closed set plus
    SystemsLean.HostModuleCheckSelfHostBodyTerm last.
    ...
    the ProbeWireTerm compile order, then HostModuleCheckSelfHostBodyTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as ProbeWireTerm): SelfHostBodyTerm
    ...
    SelfHostBodyTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckSelfHostBodyTerm.
    HostModuleCheckSelfHostBodyTerm probe lives here.
  - tryCompileNamedHostModuleCheckSelfHostBodyTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckSelfHostBodyTerm bad writes a temp
    HostModuleCheckSelfHostBodyTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckProbeWireTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior ProbeWireTerm walker
    tryCompileNamedHostModuleCheckProbeWireTermSubset, and that this walk
    is not an alias of the ProbeWireTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckSelfHostBodyTerm.lean. Do not skip to
  ...
  elabMeetDrivesNamedHostModuleCheckSelfHostBodyTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckSelfHostBodyTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckSelfHostBodyTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckSelfHostBodyTermSubset,
  #elabMeetNamedHostModuleCheckSelfHostBodyTermSubsetProbe,
  elabMeetNamedHostModuleCheckSelfHostBodyTermSubsetProbe,
  SystemsLean.HostModuleCheckSelfHostBodyTerm,
  ElabMeetNamedHostModuleCheckSelfHostBodyTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckSelfHostBodyTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckSelfHostBodyTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckSelfHostBodyTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckSelfHostBodyTerm named subset.
    Good compiles live Mult through HostModuleCheckProbeWireTerm plus the
    subset-surface compile closure and HostModuleCheckSelfHostBodyTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckSelfHostBodyTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior ProbeWireTerm walker
    tryCompileNamedHostModuleCheckProbeWireTermSubset still works; this
    walker is not an alias of the ProbeWireTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckSelfHostBodyTerm.lean. Do not skip to
    HostModuleCheckLoadOkLaterTerm. -/
def elabMeetRunNamedHostModuleCheckSelfHostBodyTermSubsetProbe : CommandElabM Unit := do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckSelfHostBodyTerm? <-
    liftIO findLiveHostModuleCheckSelfHostBodyTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckSelfHostBodyTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckSelfHostBodyTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckSelfHostBodyTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckSelfHostBodyTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckSelfHostBodyTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckSelfHostBodyTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckSelfHostBodyTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckProbeWireTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckSelfHostBodyTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckSelfHostBodyTermSubset fakeLake fakeDir
  let notAliasOfProbeWireTerm :=
    namedClosedHostModuleCheckSelfHostBodyTermSubsetNames
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckSelfHostBodyTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckSelfHostBodyTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckSelfHostBodyTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckSelfHostBodyTermSubset
  elabCommand (<- `(def $gN : Bool := $gStx))
  elabCommand (<- `(def $rN : Bool := $rStx))
  elabCommand (<- `(def $iN : Bool := $iStx))
  elabCommand (<- `(def $dN : Bool := $dStx))

elab "#elabMeetNamedHostModuleCheckSelfHostBodyTermSubsetProbe" : command => do
  if (<- liftIO slakePackageTypecheckWalk) then
    ...
    elabMeetRunNamedHostModuleCheckSelfHostBodyTermSubsetProbe

#elabMeetNamedHostModuleCheckSelfHostBodyTermSubsetProbe

example : elabMeetAcceptsGoodNamedHostModuleCheckSelfHostBodyTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckSelfHostBodyTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckSelfHostBodyTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckSelfHostBodyTermSubset = true := rfl
