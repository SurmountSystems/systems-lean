/-
  ...
  src/systems/SystemsLean/HostModuleCheckPackageWriteTheoremsKernel.lean with the
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL_V0,
  PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL, liveRel,
  kernelCheckLiveHostModuleCheckPackageWriteTheoremsKernelSource,
  hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckPackageWriteTheoremsKernel
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckPackageWriteTheoremsKernelSource
...
/-- Live file basename. -/
def liveRel : String := "HostModuleCheckPackageWriteTheoremsKernel.lean"
...
def hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelFullHost :
    Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelFullBackend :
    Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelResidualFreeClaimed :
    Bool := false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelProvablyUnlocked :
    Bool := false
...
/-- Parse live HostModuleCheckPackageWriteTheoremsKernel.lean text.
    Greppable: parseLiveHostModuleCheckPackageWriteTheoremsKernelSource,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL. -/
def parseLiveHostModuleCheckPackageWriteTheoremsKernelSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name :=
              HostTerm.n "SystemsLean.HostModuleCheckPackageWriteTheoremsKernel"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckPackageWriteTheoremsKernelSource,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL. -/
def kernelCheckLiveHostModuleCheckPackageWriteTheoremsKernelSource
    (src : String) : Bool :=
  match parseLiveHostModuleCheckPackageWriteTheoremsKernelSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckPackageWriteTheoremsKernelSource
      liveHostModuleCheckPackageWriteTheoremsKernelSource with
...
/-- Two imports. HostKernel then HostFrontLivePackageWriteTheorems.
...
/-- Kept String def checkDepthPackageWriteTheoremsKernelBar is KERNEL-CHECK. -/
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthPackageWriteTheoremsKernelBar"
          && s == "KERNEL-CHECK"
      | _ => false

/-- Kept String def checkDepthPackageWriteTheoremsKernelScope is
...
def liveParseHasScopeLit : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthPackageWriteTheoremsKernelScope"
          && s == "HostPackageWriteTheorems-only"
      | _ => false
...
/-- Wrap module lastSeg is HostModuleCheckPackageWriteTheoremsKernel
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckPackageWriteTheoremsKernel"
      && lastSeg m.name.raw == "HostModuleCheckPackageWriteTheoremsKernel"

/-- Import HostKernel, once, with a trailing newline. -/
def needleImportKernel : String := "import SystemsLean.HostKernel\n"

/-- Import HostFrontLivePackageWriteTheorems, once, with a trailing newline. -/
def needleImportWriteTheorems : String :=
...
def needleBarDef : String := "def checkDepthPackageWriteTheoremsKernelBar"

/-- Combined ok def name. Not this checker's kernelCheck. -/
def needleOkDef : String :=
  "def hostModuleCheckPackageWriteTheoremsKernelOk"

/-- Bar string in the live source. -/
def needleBar : String := "KERNEL-CHECK"

/-- Scope string in the live source. -/
def needleScope : String := "HostPackageWriteTheorems-only"

/-- Each needle occurs in the pinned live source. splitOn, not a prefix check.
    Two imports. Bar is KERNEL-CHECK. Scope is HostPackageWriteTheorems-only. -/
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckPackageWriteTheoremsKernelSource
  (src.splitOn needleImportKernel).length == 2
    && (src.splitOn needleImportWriteTheorems).length == 2
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady,
    PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL,
    HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL")
    && (liveRel == "HostModuleCheckPackageWriteTheoremsKernel.lean")
    && (liveHostModuleCheckPackageWriteTheoremsKernelRel ==
        "src/systems/SystemsLean/HostModuleCheckPackageWriteTheoremsKernel.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelFullHost
    && !hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelFullBackend
    && !hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelProvablyUnlocked
    && kernelCheckLiveHostModuleCheckPackageWriteTheoremsKernelSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckPackageWriteTheoremsKernelSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckPackageWriteTheoremsKernelRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckPackageWriteTheoremsKernelRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckPackageWriteTheoremsKernelRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckPackageWriteTheoremsKernelRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckPackageWriteTheoremsKernelSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckPackageWriteTheoremsKernel.lean != liveHostModuleCheckPackageWriteTheoremsKernelSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckPackageWriteTheoremsKernel.lean")
  let r := parseLiveHostModuleCheckPackageWriteTheoremsKernelSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKPACKAGEWRITETHEOREMSKERNEL ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckPackageWriteTheoremsKernel parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckPackageWriteTheoremsKernel parse false")
    unless hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckPackageWriteTheoremsKernelReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckPackageWriteTheoremsKernel source was accepted"
      throw (IO.userError "empty HostModuleCheckPackageWriteTheoremsKernel source was accepted")
