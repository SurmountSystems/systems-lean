/-
  ...
  Short role: HostFrontLiveHostModuleCheckCompilePathTerm.
  ...
  Spec (readable):
  - parseLiveHostModuleCheckCompilePathTermSource turns live
    HostModuleCheckCompilePathTerm.lean text into HostTerm.Module.
  - Module name is SystemsLean.HostModuleCheckCompilePathTerm
    even without a module line.
  - liveRel is the exact basename HostModuleCheckCompilePathTerm.lean.
    Disk reads use src/systems/SystemsLean/ plus that basename.
  - kernelCheckLiveHostModuleCheckCompilePathTermSource is
    HostKernel.kernelCheck of that parse. The reject arm returns false.
  - The embedded bytes are one part,
    liveHostModuleCheckCompilePathTermSource. Not split.
  - Two dotted imports, namespace SystemsLean.HostModuleCheck,
    ...
    is skip-folded. Two theorems are skip-folded. Kept command count
    is 8. Skipped command count is 27. Kept plus skipped is 35.
    kernelFuel is 64. 8 does not pass 64.
  ...
  - Skip-head still sees def checkCompilePathTermDialect and
    def hostModuleCheckCompilePathTermSurfaceOk in the live text.
  ...
  HostFrontLiveHostModuleCheckCompilePathTerm
  PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM
  HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTERM
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHTERM_V0

  Greppable: SYSTEMS_LEAN_HOST,
  parseLiveHostModuleCheckCompilePathTermSource,
  kernelCheckLiveHostModuleCheckCompilePathTermSource,
  hostFrontLiveHostModuleCheckCompilePathTermReady,
  liveHostModuleCheckCompilePathTermSource,
  liveRel, liveHostModuleCheckCompilePathTermRel, stripCommentsHc,
  UNIT_SURFACE host surface, MULT-0.
  Module: SystemsLean.HostFrontLiveHostModuleCheckCompilePathTerm
  Not package typecheck GREEN. Not FullHost. Not Lake-gone.
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckCompilePathTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckCompilePathTerm.lean"

/-- Live file relative to repo root. Dual-pin path.
    liveHostModuleCheckCompilePathTermSource is the one embedded copy
    from HostFrontLiveHostModuleCheckCompilePathTermSource. Not split. -/
...
def hostFrontLiveHostModuleCheckCompilePathTermFullHost : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckCompilePathTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: not PROVABLY. -/
def hostFrontLiveHostModuleCheckCompilePathTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count: two imports, namespace, two String literals,
    ...
    Other defs are skip-folded. Two theorems are skip-folded.
    Skipped count is 27. Kept plus skipped is 35. kernelFuel is 64.
    8 does not pass 64. -/
...
/-- String-safe comment fold from HostFrontLiveHostCheck.stripCommentsHc.
    ...
    HostModuleCheckCompilePathTerm has dash-dash in the header block
...
/-- String-safe comment strip for live HostModuleCheckCompilePathTerm.lean
...
/-- Dotted ident `SystemsLean . HostModuleCheckCompilePathTerm`. -/
...
/-- Parse live HostModuleCheckCompilePathTerm.lean text.
    Greppable: parseLiveHostModuleCheckCompilePathTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM. -/
def parseLiveHostModuleCheckCompilePathTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckCompilePathTerm"
            commands := cmds }
...
/-- Kernel-check the live parse. Not a fixture. Not a constant true.
    ...
    Greppable: kernelCheckLiveHostModuleCheckCompilePathTermSource,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM. -/
def kernelCheckLiveHostModuleCheckCompilePathTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckCompilePathTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckCompilePathTermSource
      liveHostModuleCheckCompilePathTermSource with
...
/-- Wrap module lastSeg is HostModuleCheckCompilePathTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckCompilePathTerm"
      && lastSeg m.name.raw == "HostModuleCheckCompilePathTerm"
...
def liveParseHasCoreDefs : Bool :=
  ...
  | some _ =>
    let toks := tokenizeHostTerm
      (stripCommentsHc liveHostModuleCheckCompilePathTermSource)
    toksHaveDefNamed liveSkipFuel toks "checkCompilePathTermDialect"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckCompilePathTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckCompilePathTermReady,
    PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckCompilePathTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKCOMPILEPATHTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKCOMPILEPATHTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM")
    && (liveRel == "HostModuleCheckCompilePathTerm.lean")
    && (liveHostModuleCheckCompilePathTermRel ==
        "src/systems/SystemsLean/HostModuleCheckCompilePathTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckCompilePathTermFullHost
    && !hostFrontLiveHostModuleCheckCompilePathTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckCompilePathTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckCompilePathTermSource
        liveHostModuleCheckCompilePathTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckCompilePathTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckCompilePathTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckCompilePathTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckCompilePathTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckCompilePathTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckCompilePathTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckCompilePathTerm.lean != liveHostModuleCheckCompilePathTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckCompilePathTerm.lean")
  let r := parseLiveHostModuleCheckCompilePathTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM reject {reason}"
    throw (IO.userError
      s!"PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKCOMPILEPATHTERM ACCEPT liveRel={liveRel} cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckCompilePathTerm parse false"
      throw (IO.userError
        "kernelCheck live HostModuleCheckCompilePathTerm parse false")
    unless hostFrontLiveHostModuleCheckCompilePathTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckCompilePathTermReady false"
      throw (IO.userError
        "hostFrontLiveHostModuleCheckCompilePathTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckCompilePathTerm source was accepted"
      throw (IO.userError
        "empty HostModuleCheckCompilePathTerm source was accepted")
    IO.println s!"GREEN {stageId}: live HostModuleCheckCompilePathTerm.lean parse kernelCheck; not mill remill; mill stays 69 of 69"
