/-
  SYSTEMS_LEAN_HOST partial -- HostModuleCheckLinearSubsetEmitTerm named
  closed subset probe (plus-one after the FirstSurfaceTerm closed set).
  Side: classic Lean elaborator under src/systems/ (not freestanding C).
  Role module extracted so ElabMeetSubset stays under about 880.
  Same namespace SystemsLean.ElabMeet. LinearSubsetEmitTerm wrappers live in
  ElabMeetNamedWalkHostModuleCheckLinearSubsetEmitTerm. Shared helpers stay in
  ...
  ElabMeetNamedWalkHostModuleCheckCompilePathBodyTerm, or
  ElabMeetNamedWalkHostModuleCheckFirstSurfaceTerm.
  Reuses tryCompileNamedMemberListIO (no cloned walker).
  Rec-depth 8192 keep on later ElabMeet fold; this extract does not fold.

  Spec (readable):
  - Named HostModuleCheckLinearSubsetEmitTerm subset:
    prior FirstSurfaceTerm closed set plus
    SystemsLean.HostModuleCheckLinearSubsetEmitTerm last.
    ...
    the FirstSurfaceTerm compile order, then HostModuleCheckLinearSubsetEmitTerm last.
    EmitPlanTerm is a compile dependency only, not a named plus-one.
    Dispatcher-split honesty (same as FirstSurfaceTerm): LinearSubsetEmitTerm
    ...
    LinearSubsetEmitTerm walker is a wrapper in
    ElabMeetNamedWalkHostModuleCheckLinearSubsetEmitTerm.
    HostModuleCheckLinearSubsetEmitTerm probe lives here.
  - tryCompileNamedHostModuleCheckLinearSubsetEmitTermSubset is false unless the
    path is the live lakefile, the parsed library is
    SystemsLean, and the barrel lists the subset.
    HostModuleCheckLinearSubsetEmitTerm bad writes a temp
    HostModuleCheckLinearSubsetEmitTerm copy and compiles it after Mult and the
    ...
    HostModuleCheckFirstSurfaceTerm oleans, plus EmitPlanTerm and
    MultTerm compile deps.
    Isolation names leftover temp good.lean, leftover fake
    lean_lib ElabMeetRichLib, the prior FirstSurfaceTerm walker
    tryCompileNamedHostModuleCheckFirstSurfaceTermSubset, and that this walk
    is not an alias of the FirstSurfaceTerm closed set. Linear skipped. IrGraph
    skipped as a grow-tip Name. HostCompose skipped. Do not plant live
    HostModuleCheckLinearSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckTypesSubsetEmitTerm. This Name is
    HostModuleCheckLinearSubsetEmitTerm, not SystemsLean.Linear.
  ...
  elabMeetDrivesNamedHostModuleCheckLinearSubsetEmitTermSubset,
  elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetEmitTermSubset,
  elabMeetRejectsBadNamedHostModuleCheckLinearSubsetEmitTermSubset,
  elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetEmitTermSubset,
  #elabMeetNamedHostModuleCheckLinearSubsetEmitTermSubsetProbe,
  elabMeetNamedHostModuleCheckLinearSubsetEmitTermSubsetProbe,
  SystemsLean.HostModuleCheckLinearSubsetEmitTerm,
  ElabMeetNamedHostModuleCheckLinearSubsetEmitTermProbe,
  SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetEmitTermProbe,
  UNIT_SURFACE host surface, MULT-0, MULT-1, MULT-OMEGA.
  Module: SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetEmitTermProbe
  Short role name (not ProductPathFreestanding*).
  Red/green: lake build SystemsLean.ElabMeetNamedHostModuleCheckLinearSubsetEmitTermProbe;
  just systems-host.
...
/-- Record accept / reject for the HostModuleCheckLinearSubsetEmitTerm named subset.
    Good compiles live Mult through HostModuleCheckFirstSurfaceTerm plus the
    subset-surface compile closure and HostModuleCheckLinearSubsetEmitTerm after
    reading the real lakefile. Bad compiles a temp
    HostModuleCheckLinearSubsetEmitTerm copy with a planted type error after Mult
    ...
    ElabMeetRichLib stay false; the prior FirstSurfaceTerm walker
    tryCompileNamedHostModuleCheckFirstSurfaceTermSubset still works; this
    walker is not an alias of the FirstSurfaceTerm closed set. Drive is
    good && !bad && isolation.
    Linear skipped. IrGraph skipped as a grow-tip Name. HostCompose skipped.
    Do not plant live HostModuleCheckLinearSubsetEmitTerm.lean. Do not skip to
    HostModuleCheckTypesSubsetEmitTerm. -/
elab "#elabMeetNamedHostModuleCheckLinearSubsetEmitTermSubsetProbe" : command => do
  let liveLake? <- liftIO findLiveLakefilePath
  let liveMult? <- liftIO findLiveMultPath
  let liveThm? <- liftIO findLiveMultTheoremsPath
  let liveHostModuleCheckLinearSubsetEmitTerm? <-
    liftIO findLiveHostModuleCheckLinearSubsetEmitTermPath
  ...
  let good <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckLinearSubsetEmitTermSubset liveLake oleanDir
  let badCompiled <-
    match liveHostModuleCheckLinearSubsetEmitTerm? with
    | none =>
      pure true
    | some liveHostModuleCheckLinearSubsetEmitTerm => do
      let liveText <- liftIO (IO.FS.readFile liveHostModuleCheckLinearSubsetEmitTerm)
      let badPath :=
        badDir / "ElabMeetNamedHostModuleCheckLinearSubsetEmitTermSubsetBad.lean"
      let planted := "\ndef plantedTypeError : Nat := true\n"
      liftIO (IO.FS.writeFile badPath (liveText ++ planted))
      tryCompileAfterHostModuleCheckLinearSubsetEmitTermDeps
        badPath badDir `ElabMeetNamedHostModuleCheckLinearSubsetEmitTermSubsetBad
  let liveMultStillOk <-
    match liveMult? with
    | none => pure false
    | some liveMult =>
  ...
  let priorSubsetStillOk <-
    ...
    | none => pure false
    | some liveLake =>
      tryCompileNamedHostModuleCheckFirstSurfaceTermSubset liveLake subsetIsoDir
  let goodIso := isoDir / "good.lean"
  liftIO (IO.FS.writeFile goodIso "def n : Nat := 0")
  let thisOnTempSnippet <-
    tryCompileNamedHostModuleCheckLinearSubsetEmitTermSubset goodIso isoDir
  let fakeLake := fakeDir / "lakefile.lean"
  liftIO (IO.FS.writeFile fakeLake leftoverFakePackageLakefileText)
  let thisOnFakePackage <-
    tryCompileNamedHostModuleCheckLinearSubsetEmitTermSubset fakeLake fakeDir
  let notAliasOfFirstSurfaceTerm :=
    namedClosedHostModuleCheckLinearSubsetEmitTermSubsetNames
      ...
      && namedClosedHostModuleCheckLinearSubsetEmitTermSubsetNames.contains
           "SystemsLean.HostModuleCheckLinearSubsetEmitTerm"
      && !namedClosedHostModuleCheckFirstSurfaceTermSubsetNames.contains
           "SystemsLean.HostModuleCheckLinearSubsetEmitTerm"
  let isolation :=
    liveMultStillOk && liveThmStillOk && priorSubsetStillOk
      && !thisOnTempSnippet && !thisOnFakePackage
      && notAliasOfFirstSurfaceTerm
  try
  ...
  let dStx <-
    if good && !badCompiled && isolation then
      ...
      `(false)
  let gN := mkIdent `elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetEmitTermSubset
  let rN := mkIdent `elabMeetRejectsBadNamedHostModuleCheckLinearSubsetEmitTermSubset
  let iN := mkIdent `elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetEmitTermSubset
  let dN := mkIdent `elabMeetDrivesNamedHostModuleCheckLinearSubsetEmitTermSubset
...
example : elabMeetAcceptsGoodNamedHostModuleCheckLinearSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsBadNamedHostModuleCheckLinearSubsetEmitTermSubset = true := rfl
example : elabMeetRejectsOldWalkAsNamedHostModuleCheckLinearSubsetEmitTermSubset = true := rfl
example : elabMeetDrivesNamedHostModuleCheckLinearSubsetEmitTermSubset = true := rfl
