/-
  SYSTEMS_LEAN_HOST partial -- live OccupancyTheorems.lean.
  Pinned text only. This module does not import SystemsLean.OccupancyProbe.
  The probe name stays inside the string. Not a compile of the probe.
  liveRel is OccupancyTheorems.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveOccupancyTheoremsSource,
  PARSE-LIVE-OCCUPANCY-THEOREMS.
  Module: SystemsLean.HostFrontLiveOccupancyTheoremsSource
...
namespace SystemsLean.HostFrontLiveOccupancyTheoremsSource
...
def liveRel : String := "OccupancyTheorems.lean"

/-- Pinned live OccupancyTheorems.lean bytes. -/
def liveOccupancyTheoremsSource : String := r#"/-
...
/-- Accepted parse calls HostKernel.kernelCheck. Not a constant true.
    Theorems skip-fold. The probe is not compiled. -/
def kernelCheckLiveOccupancyTheoremsSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveOccupancyTheoremsSourceReady : Bool :=
  liveRel == "OccupancyTheorems.lean"
    && kernelCheckLiveOccupancyTheoremsSource liveOccupancyTheoremsSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveOccupancyTheoremsSource then
    throw (IO.userError "dual-pin mismatch OccupancyTheorems.lean")
  let k := kernelCheckLiveOccupancyTheoremsSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live OccupancyTheorems parse false")
  unless hostFrontLiveOccupancyTheoremsSourceReady do
...
end SystemsLean.HostFrontLiveOccupancyTheoremsSource
