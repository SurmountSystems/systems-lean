/-
  ...
  HOST-FRONT-LIVE-HOST-MODULE-CHECK-JOIN-MAP-TERM,
  SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_JOIN_MAP_TERM_V0,
  PARSE-LIVE-HOST-MODULE-CHECK-JOIN-MAP-TERM, liveRel,
  kernelCheckLiveHostModuleCheckJoinMapTermSource,
  hostFrontLiveHostModuleCheckJoinMapTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckJoinMapTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckJoinMapTermSource
...
def liveRel : String := "HostModuleCheckJoinMapTerm.lean"
...
def hostFrontLiveHostModuleCheckJoinMapTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckJoinMapTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckJoinMapTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. -/
def hostFrontLiveHostModuleCheckJoinMapTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count from the lean --run log.
    Two imports, the namespace, four literal defs, and end. -/
...
/-- On-disk HostModuleCheckJoinMapTerm.lean bytes.
    Greppable: liveHostModuleCheckJoinMapTermSource. -/

def liveHostModuleCheckJoinMapTermSource : String := r#"/-
...
/-- Parse live HostModuleCheckJoinMapTerm.lean text.
    Greppable: parseLiveHostModuleCheckJoinMapTermSource. -/
def parseLiveHostModuleCheckJoinMapTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckJoinMapTerm"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture.
    Greppable: kernelCheckLiveHostModuleCheckJoinMapTermSource. -/
def kernelCheckLiveHostModuleCheckJoinMapTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckJoinMapTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckJoinMapTermSource
      liveHostModuleCheckJoinMapTermSource with
  ...
  | some m =>
    let has (nm : String) : Bool :=
      m.commands.any fun c =>
        ...
        | _ => false
    has "checkDepthJoinMapTermSurfaceBar"
      && has "checkDepthJoinMapTermSurfaceScope"
      && has "hostModuleCheckJoinMapTermSurfaceDualOk"
...
def liveParseHasModuleName : Bool :=
  ...
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckJoinMapTerm"
...
/-- Keyword counts: two theorem commands, no example, no set_option. -/
def liveParseKwCountsOk : Bool :=
  match liveParsed? with
  | none => false
  | some _ =>
    let toks := tokenizeHostTerm
      (stripComments liveHostModuleCheckJoinMapTermSource)
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckJoinMapTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckJoinMapTermReady.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckJoinMapTermReady : Bool :=
  (stageId ==
    "SLAKE_HOST_FRONT_LIVE_HOST_MODULE_CHECK_JOIN_MAP_TERM_V0")
    && (hostId ==
      "HOST-FRONT-LIVE-HOST-MODULE-CHECK-JOIN-MAP-TERM")
    && (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-JOIN-MAP-TERM")
    && (liveRel == "HostModuleCheckJoinMapTerm.lean")
    && (liveHostModuleCheckJoinMapTermRel ==
      "src/systems/SystemsLean/HostModuleCheckJoinMapTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckJoinMapTermFullHost
    && !hostFrontLiveHostModuleCheckJoinMapTermFullBackend
    && !hostFrontLiveHostModuleCheckJoinMapTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckJoinMapTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckJoinMapTermSource
      liveHostModuleCheckJoinMapTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckJoinMapTermSource "" with
...
def readyFailures : List String :=
  ...
  bad "stageId" (stageId ==
    ...
    ++ bad "parseId" (parseId ==
      "PARSE-LIVE-HOST-MODULE-CHECK-JOIN-MAP-TERM")
    ++ bad "liveRel" (liveRel == "HostModuleCheckJoinMapTerm.lean")
    ++ bad "rel" (liveHostModuleCheckJoinMapTermRel ==
      "src/systems/SystemsLean/HostModuleCheckJoinMapTerm.lean")
    ++ bad "notMult" liveParseDoesNotUseMultFixture
    ++ bad "fullHost" (!hostFrontLiveHostModuleCheckJoinMapTermFullHost)
    ++ bad "fullBackend"
      (!hostFrontLiveHostModuleCheckJoinMapTermFullBackend)
    ++ bad "residual"
      (!hostFrontLiveHostModuleCheckJoinMapTermResidualFreeClaimed)
    ++ bad "provably"
      (!hostFrontLiveHostModuleCheckJoinMapTermProvablyUnlocked)
    ++ bad "kernel" (kernelCheckLiveHostModuleCheckJoinMapTermSource
      liveHostModuleCheckJoinMapTermSource)
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  let path := root / liveHostModuleCheckJoinMapTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckJoinMapTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckJoinMapTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckJoinMapTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckJoinMapTerm.lean != liveHostModuleCheckJoinMapTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckJoinMapTerm.lean")
  let r := parseLiveHostModuleCheckJoinMapTermSource disk
  match r with
  ...
  | FrontResult.accept m =>
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckJoinMapTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckJoinMapTerm parse false")
    unless hostFrontLiveHostModuleCheckJoinMapTermReady do
      ...
      throw (IO.userError "hostFrontLiveHostModuleCheckJoinMapTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckJoinMapTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckJoinMapTerm source was accepted")
...
end SystemsLean.HostFrontLiveHostModuleCheckJoinMapTermSource
