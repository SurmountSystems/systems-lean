/-
  ...
  src/systems/SystemsLean/HostModuleCheckHostFrontTerm.lean with the
  ...
  and Bool defs, and end. The closed run printed cmds=11.
  ...
  SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKHOSTFRONTTERM_V0,
  PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM, liveRel,
  kernelCheckLiveHostModuleCheckHostFrontTermSource,
  hostFrontLiveHostModuleCheckHostFrontTermReady.
  Module: SystemsLean.HostFrontLiveHostModuleCheckHostFrontTerm
  Module must stay ASCII.
-/

import SystemsLean.HostFrontLiveHostTerm
import SystemsLean.HostFrontLiveHostModuleCheckHostFrontTermSource
...
def hostId : String := "HOST-FRONT-LIVE-HOSTMODULECHECKHOSTFRONTTERM"

/-- Greppable parse id. -/
def parseId : String := "PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM"

/-- Live file basename. -/
def liveRel : String := "HostModuleCheckHostFrontTerm.lean"
...
def hostFrontLiveHostModuleCheckHostFrontTermFullHost : Bool := false

/-- Honesty: this wrap is not an LLVM backend. FullBackend stays false. -/
def hostFrontLiveHostModuleCheckHostFrontTermFullBackend : Bool := false

/-- Honesty: not product residual free. -/
def hostFrontLiveHostModuleCheckHostFrontTermResidualFreeClaimed : Bool :=
  false

/-- Honesty: this wrap is not PROVABLY. The provably flag stays false. -/
def hostFrontLiveHostModuleCheckHostFrontTermProvablyUnlocked : Bool :=
  false
...
/-- Kept-command count printed by the closed run (cmds=11).
    From HostModuleCheckHostFrontTerm.lean. -/
...
/-- Parse live HostModuleCheckHostFrontTerm.lean text.
    Greppable: parseLiveHostModuleCheckHostFrontTermSource,
    PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM. -/
def parseLiveHostModuleCheckHostFrontTermSource (src : String) :
    FrontResult :=
  ...
  if toks.isEmpty then FrontResult.reject reasonEmptyModule
    ...
    | some cmds =>
      if cmds.isEmpty then FrontResult.reject reasonEmptyModule
        ...
        let m : Module :=
          { name := HostTerm.n "SystemsLean.HostModuleCheckHostFrontTerm"
            commands := cmds }
        if isWellFormed m then FrontResult.accept m
        else FrontResult.reject reasonNotWellFormed

/-- Kernel-check the live parse. Not a fixture. Not a hardcoded true.
    Greppable: kernelCheckLiveHostModuleCheckHostFrontTermSource,
    PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM. -/
def kernelCheckLiveHostModuleCheckHostFrontTermSource (src : String) :
    Bool :=
  match parseLiveHostModuleCheckHostFrontTermSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Accepted live module when parse succeeds. -/
def liveParsed? : Option Module :=
  match parseLiveHostModuleCheckHostFrontTermSource
      liveHostModuleCheckHostFrontTermSource with
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthHostFrontTermSurfaceBar"
          && s == "TERM-SURFACE"
      | _ => false

/-- Scope literal is HostFront-only. -/
def liveParseHasScope : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "checkDepthHostFrontTermSurfaceScope"
          && s == "HostFront-only"
      | _ => false
  ...
  | some m =>
    m.commands.any fun c =>
      ...
      | Cmd.def_ x (some HostType.bool) (Term.litBool b) =>
        x.raw == "hostModuleCheckHostFrontTermSurfaceDualOk" && b == true
      | _ => false
...
def liveParseHasSuffix : Bool :=
  ...
  | some m =>
    m.commands.any fun c =>
      match c with
      | Cmd.def_ x (some HostType.string) (Term.litString s) =>
        x.raw == "hostModuleCheckHostFrontTermSkeletonSuffix"
          && s == "end SystemsLean.HostFront\\n"
      | _ => false

/-- Wrap module lastSeg is HostModuleCheckHostFrontTerm
    (no module line in the live file). -/
def liveParseHasModule : Bool :=
  match liveParsed? with
  | none => false
  | some m =>
    m.name.raw == "SystemsLean.HostModuleCheckHostFrontTerm"
      && lastSeg m.name.raw == "HostModuleCheckHostFrontTerm"
...
/-- Collapsed hostFrontReady body text. `++` is not a kernel atom. -/
def needleEmptyStruct : String :=
  "hostFrontGoldensStructuralOk && hostFrontIdsOk && hostFrontMultGradesOk"
...
def liveNeedlesOk : Bool :=
  let src := liveHostModuleCheckHostFrontTermSource
...
/-- End-to-end ready: live text parse kernel-checks.
    Greppable: hostFrontLiveHostModuleCheckHostFrontTermReady,
    PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM,
    HOST-FRONT-LIVE-HOSTMODULECHECKHOSTFRONTTERM.
    Real conjunction: parse plus kernelCheck plus this file's commands.
    Not a hardcoded true. -/
def hostFrontLiveHostModuleCheckHostFrontTermReady : Bool :=
  (stageId ==
      "SLAKE_HOST_FRONT_LIVE_HOSTMODULECHECKHOSTFRONTTERM_V0")
    && (hostId == "HOST-FRONT-LIVE-HOSTMODULECHECKHOSTFRONTTERM")
    && (parseId == "PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM")
    && (liveRel == "HostModuleCheckHostFrontTerm.lean")
    && (liveHostModuleCheckHostFrontTermRel ==
        "src/systems/SystemsLean/HostModuleCheckHostFrontTerm.lean")
    && liveParseDoesNotUseMultFixture
    && !hostFrontLiveHostModuleCheckHostFrontTermFullHost
    && !hostFrontLiveHostModuleCheckHostFrontTermFullBackend
    && !hostFrontLiveHostModuleCheckHostFrontTermResidualFreeClaimed
    && !hostFrontLiveHostModuleCheckHostFrontTermProvablyUnlocked
    && kernelCheckLiveHostModuleCheckHostFrontTermSource
        liveHostModuleCheckHostFrontTermSource
...
def liveParseRejectsEmpty : Bool :=
  match parseLiveHostModuleCheckHostFrontTermSource "" with
...
def runLive (root : System.FilePath) : IO Unit := do
  IO.println s!"== {stageId}: PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM =="
  IO.println s!"  host={hostId} file={liveHostModuleCheckHostFrontTermRel}"
  IO.println s!"liveRel={liveRel}"
  let path := root / liveHostModuleCheckHostFrontTermRel
  unless (<- path.pathExists) do
    IO.eprintln s!"error: missing {liveHostModuleCheckHostFrontTermRel}"
    throw (IO.userError s!"missing {liveHostModuleCheckHostFrontTermRel}")
  let disk <- IO.FS.readFile path
  if disk != liveHostModuleCheckHostFrontTermSource then
    IO.eprintln "error: dual-pin mismatch: on-disk HostModuleCheckHostFrontTerm.lean != liveHostModuleCheckHostFrontTermSource"
    throw (IO.userError "dual-pin mismatch live HostModuleCheckHostFrontTerm.lean")
  let r := parseLiveHostModuleCheckHostFrontTermSource disk
  match r with
  | FrontResult.reject reason =>
    IO.eprintln s!"error: PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM reject {reason}"
    throw (IO.userError s!"PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM reject {reason}")
  | FrontResult.accept m =>
    let k := HostKernel.kernelCheck m
    IO.println s!"PASS PARSE-LIVE-HOSTMODULECHECKHOSTFRONTTERM ACCEPT cmds={m.commands.length} kernelCheck={k}"
    ...
    unless k do
      IO.eprintln "error: kernelCheck live HostModuleCheckHostFrontTerm parse false"
      throw (IO.userError "kernelCheck live HostModuleCheckHostFrontTerm parse false")
    unless hostFrontLiveHostModuleCheckHostFrontTermReady do
      IO.eprintln "error: hostFrontLiveHostModuleCheckHostFrontTermReady false"
      throw (IO.userError "hostFrontLiveHostModuleCheckHostFrontTermReady false")
    unless liveParseRejectsEmpty do
      IO.eprintln "error: empty HostModuleCheckHostFrontTerm source was accepted"
      throw (IO.userError "empty HostModuleCheckHostFrontTerm source was accepted")
