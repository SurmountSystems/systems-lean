/-
  SYSTEMS_LEAN_HOST partial -- live HostModuleCheckIrGraphTerm.lean.
  ...
  liveRel is HostModuleCheckIrGraphTerm.lean. Not a path.
  Greppable: liveRel, kernelCheckLiveHostModuleCheckIrGraphTermSource,
  PARSE-LIVE-HOST-MODULE-CHECK-IR-GRAPH-TERM.
  Module: SystemsLean.HostFrontLiveHostModuleCheckIrGraphTermSource
...
namespace SystemsLean.HostFrontLiveHostModuleCheckIrGraphTermSource
...
def liveRel : String := "HostModuleCheckIrGraphTerm.lean"

/-- Pinned live HostModuleCheckIrGraphTerm.lean bytes. -/
def liveHostModuleCheckIrGraphTermSource : String := r#"/-
...
def kernelCheckLiveHostModuleCheckIrGraphTermSource (src : String) : Bool :=
  match parseLiveLlvmComposeTextMainSource src with
  | FrontResult.accept m => HostKernel.kernelCheck m
  | FrontResult.reject _ => false

/-- Ready is parse plus kernelCheck of the pinned text. -/
def hostFrontLiveHostModuleCheckIrGraphTermSourceReady : Bool :=
  liveRel == "HostModuleCheckIrGraphTerm.lean"
    && kernelCheckLiveHostModuleCheckIrGraphTermSource liveHostModuleCheckIrGraphTermSource
...
def runLive (root : System.FilePath) : IO Unit := do
  ...
  if disk != liveHostModuleCheckIrGraphTermSource then
    throw (IO.userError "dual-pin mismatch HostModuleCheckIrGraphTerm.lean")
  let k := kernelCheckLiveHostModuleCheckIrGraphTermSource disk
  IO.println s!"kernelCheck={k}"
  unless k do
    throw (IO.userError "kernelCheck live HostModuleCheckIrGraphTerm parse false")
  unless hostFrontLiveHostModuleCheckIrGraphTermSourceReady do
...
end SystemsLean.HostFrontLiveHostModuleCheckIrGraphTermSource
