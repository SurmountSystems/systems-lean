...
| Slake skeleton under `src/systems/` | Layout stubs: `types.md`/`Types.slake`, `mult.md`/`Mult.slake`, `linear.md`/`Linear.slake`, `erasure.md`/`Erasure.slake`, `extract.md`/`Extract.slake`; pure Nix presence + process glue `src/systems/check.sh`; product path is `just build` (freestanding wire); unit markers (SKELETON + UNIT_SURFACE) pure Nix. **Not freestanding residual free.** |
...
| Real freestanding emit path V0 + out/freestanding-c populate | Official product path is `just build` (B38 freestanding dual-eq WRITE via freestanding-capable-regenerate; former `out-freestanding-c` recipe retired; no alias). Writes emit + installs `out/freestanding-c/` (preserves README). `just check` runs build first and sets `SYSTEMS_PRODUCT_WIRE_FRESH=1` so systems check does not double-regenerate. Still **not residual free**; **not PROVABLY**. |
...
|----------|------|----------------|
| P0 | Shell/C paydown | **done (bulk + stamp delete):** plan `.agents/plans/plan-paydown-shell-c-surfaces.md` waves 0-E + Thin process glue + compile-path stamp delete. Dual pure Nix presence; emit bash deleted; `script/slake-compile-path.sh` deleted (static pure Nix + host CompilePath). Remaining shell: process glue only (`check.sh` dual/systems + optional `script/git-hooks/pre-commit`). Product residual free is a separate claim (true as of 2026-08-01; host residual remains). |
